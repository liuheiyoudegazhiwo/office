...
import { searchbyno, userQueryList, addSysUser, updateSysUser, userremove, resetPassword, updateUserstatus } from '@/api/sysapi'
import { trans_data_fun, deepCopy } from '@/utils/trans'
import {
	store,
	mutations
	...
	methods: {
		...
		saveuser(e) {
			...
			addSysUser(objadd).then(res => {
				objadd.id = res
				uni.setStorageSync('userinfo_user',JSON.stringify(objadd) )
				this.loginSuccess(e)
			}).catch(error => {
				console.log(error)
			})
