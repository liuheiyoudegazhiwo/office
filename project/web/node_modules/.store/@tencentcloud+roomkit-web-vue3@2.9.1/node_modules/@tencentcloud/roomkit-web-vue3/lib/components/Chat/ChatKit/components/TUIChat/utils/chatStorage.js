...
const _ChatStorage = class _ChatStorage {
  ...
  setChatStorage(key, value) {
    ...
    try {
      if (env.isUniFrameWork) {
        universalApi.TUIGlobal.setStorageSync(_ChatStorage.CHAT_STORAGE_KEY, JSON.stringify(this.chatStorage));
      } else {
  ...
  getChatStorageFromLocalStorage() {
    ...
    if (env.isUniFrameWork) {
      chatStorageString = universalApi.TUIGlobal.getStorageSync(_ChatStorage.CHAT_STORAGE_KEY) || "";
    } else {
