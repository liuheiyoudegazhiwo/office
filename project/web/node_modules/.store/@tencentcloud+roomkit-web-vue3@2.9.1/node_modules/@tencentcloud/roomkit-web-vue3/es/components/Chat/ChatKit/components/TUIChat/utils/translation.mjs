...
import TUIChatEngine, { TUIStore, TUIChatService, TUITranslateService, TUIUserService } from "@tencentcloud/chat-uikit-engine";
const _Translator = class _Translator {
  ...
  async get(message) {
    ...
    const currentMessage = TUIStore.getMessageModel(message.ID);
    if (!currentMessage) {
      return [];
    }
