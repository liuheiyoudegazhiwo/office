...
const _OfflinePushInfoManager = class _OfflinePushInfoManager {
  constructor() {
    ...
    this.offlinePushInfo = {
      [_const.PUSH_SCENE.CHAT]: info.chatOfflinePushInfo,
      [_const.PUSH_SCENE.CALL]: info.callOfflinePushInfo
    };
  }
  static getInstance() {
  ...
  getOfflinePushInfo(scene) {
    if (!Object.values(_const.PUSH_SCENE).includes(scene)) {
  ...
  genDesc(messageType, payload) {
    ...
    if (messageType === TUIChatEngine.TYPES.MSG_TEXT) {
      desc = index.transformTextWithKeysToEmojiNames(payload.text);
    }
    if (messageType === TUIChatEngine.TYPES.MSG_CUSTOM) {
      desc = payload.description;
    }
    return desc || TUIChatEngine.TUITranslateService.t(`TUIChat.${_const.DEFAULT_DESC[messageType]}`);
  }
  create(options) {
    var _a;
    const { conversation, messageType = "", payload = {} } = options || {};
    const userInfo = TUIChatEngine.TUIStore.getData(TUIChatEngine.StoreName.USER, "userProfile");
    const entity = {
    ...
    return {
      ...
      ...this.offlinePushInfo[_const.PUSH_SCENE.CHAT]
    };
  }
};
