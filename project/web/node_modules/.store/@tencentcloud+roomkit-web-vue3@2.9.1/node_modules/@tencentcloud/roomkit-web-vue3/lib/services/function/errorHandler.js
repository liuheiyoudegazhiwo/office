...
class ErrorHandler {
  ...
  handleEnterRoomError(error) {
    if (error.code === TUIRoomEngine.TUIErrorCode.ERR_NEED_PASSWORD) {
      this.service.emit(types.EventType.ROOM_NEED_PASSWORD, error.code);
      return;
    }
    if (error.code === TUIRoomEngine.TUIErrorCode.ERR_WRONG_PASSWORD) {
    ...
    let message = "";
    switch (error.code) {
      case TUIRoomEngine.TUIErrorCode.ERR_ROOM_ID_NOT_EXIST:
    ...
    this.service.emit(types.EventType.ROOM_NOTICE_MESSAGE_BOX, {
      ...
      callback: async () => {
        this.service.emit(types.EventType.ROOM_ERROR, {
          code: error.code,
...
class MediaDeviceErrorHandler {
  ...
  handleError(error) {
    ...
    let callback = null;
    switch (error.code) {
      case TUIRoomEngine.TUIErrorCode.ERR_CAMERA_START_FAILED:
      case TUIRoomEngine.TUIErrorCode.ERR_MICROPHONE_START_FAILED:
        mediaType = error.code === TUIRoomEngine.TUIErrorCode.ERR_CAMERA_START_FAILED ? "camera" : "microphone";
        mediaErrorTitle = this.getMediaDeviceErrorTitle(mediaType);
        mediaErrorMsg = this.getMediaDeviceErrorMsg(mediaType);
        break;
      case TUIRoomEngine.TUIErrorCode.ERR_CAMERA_NOT_AUTHORIZED:
      case TUIRoomEngine.TUIErrorCode.ERR_MICROPHONE_NOT_AUTHORIZED:
        mediaType = error.code === TUIRoomEngine.TUIErrorCode.ERR_CAMERA_NOT_AUTHORIZED ? "camera" : "microphone";
        ...
        break;
      case TUIRoomEngine.TUIErrorCode.ERR_CAMERA_OCCUPIED:
      case TUIRoomEngine.TUIErrorCode.ERR_MICROPHONE_OCCUPIED:
        mediaType = error.code === TUIRoomEngine.TUIErrorCode.ERR_CAMERA_OCCUPIED ? "camera" : "microphone";
        mediaErrorTitle = this.getMediaDeviceErrorTitle(mediaType);
