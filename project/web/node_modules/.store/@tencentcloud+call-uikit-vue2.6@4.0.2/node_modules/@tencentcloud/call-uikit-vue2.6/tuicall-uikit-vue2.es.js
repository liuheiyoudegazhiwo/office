(function(){"use strict";try{if(typeof document<"u"){var t=document.createElement("style");t.appendChild(document.createTextNode(".tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-row{display:flex;flex-wrap:wrap;position:relative;box-sizing:border-box;width:100%}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-col{display:flex;flex-wrap:wrap;position:relative;box-sizing:border-box}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-loading_dot-container{display:flex;justify-content:space-between;align-items:center;height:100%}.tk-loading_dot-container .tk-loading_dot:nth-child(1){opacity:0;animation-duration:.8s;animation-delay:0s;animation-play-state:running}.tk-loading_dot-container .tk-loading_dot:nth-child(2){opacity:.083;animation-duration:.8s;animation-delay:.2666666667s;animation-play-state:running}.tk-loading_dot-container .tk-loading_dot:nth-child(3){opacity:.1667;animation-duration:.8s;animation-delay:.5333333333s;animation-play-state:running}.tk-loading_dot-container .tk-loading_dot{width:20%;height:20%;border-radius:50%;background-color:#fff;animation-duration:1.8s;animation-name:dotting;animation-timing-function:linear;animation-iteration-count:infinite;animation-fill-mode:both}@keyframes dotting{0%{opacity:.15}1%{opacity:.8}33%{opacity:.8}34%{opacity:.15}to{opacity:.15}}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-loading_circle-container{height:100%;border:2px solid;border-radius:50%;border-top-color:transparent;border-right-color:transparent;border-bottom-color:#fff;border-left-color:#fff;background:0 0;vertical-align:middle;box-sizing:border-box;animation:spin 1s linear infinite}@keyframes spin{0%{transform:rotate(0)}to{transform:rotate(360deg)}}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-loading{display:flex;align-items:center;flex-direction:column}.tk-loading .tk-loading_text{margin:10px}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-image{position:relative;display:flex;align-items:center;justify-content:center;overflow:hidden;vertical-align:middle;width:300px;height:225px}.tk-image .tk-image_inner{width:100%;height:100%}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-button{display:inline-flex;align-items:center;justify-content:center}.tk-button.tk-button--small{height:24px}.tk-button.tk-button--middle{height:32px}.tk-button.tk-button--large{height:40px}.tk-button .tk-button--content{color:#606266;font-size:14px;font-weight:500}.tk-button.tk-circle.tk-button--small{width:24px}.tk-button.tk-circle.tk-button--default{width:32px}.tk-button.tk-circle.tk-button--large{width:40px}.btn-content[data-v-71774dda],.btn-content[data-v-224de994]{display:flex;flex-direction:column;align-items:center}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-text{display:inline-block;overflow:hidden;text-align:center;line-height:normal;white-space:nowrap;font-weight:400;font-size:14px;color:#303133}.tk-text.tk-text--line-clamp{display:-webkit-inline-box;-webkit-box-orient:vertical;white-space:normal}.btn-content[data-v-037b2b4c]{display:flex;flex-direction:column;align-items:center}.top-bar-container[data-v-2bcaf4e1]{position:absolute;z-index:2;width:100%;height:5.8%;display:flex;align-items:center}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-overlay{position:fixed;left:0;top:0;bottom:0;width:100%}.tk-overlay .tk-overlay_mask-container{width:100%;height:100%;z-index:0;position:absolute}.tk-overlay .tk-overlay_mask{position:absolute;left:0;top:0;right:0;bottom:0;background-color:#00000080;z-index:1}.tk-overlay .tk-overlay_slot{position:absolute;left:0;right:0;top:0;bottom:0;z-index:1;display:flex;justify-content:center}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-avatar{display:inline-flex;justify-content:center;align-items:center;box-sizing:border-box;text-align:center;overflow:hidden;color:#fff;background:#c0c4cc;width:40px;height:40px;font-size:14px}.tk-avatar.tk-avatar--square{border-radius:4px}.tk-avatar.tk-avatar--circle{border-radius:100%}.tk-avatar .tk-avatar_img,.tk-avatar .tk-avatar_image{width:100%;height:100%}.mic-container[data-v-3cbc030b]{position:relative;width:24px;height:24px}.mic-container .mic-level-container[data-v-3cbc030b]{position:absolute;left:6px;width:8px;height:14px;display:flex;flex-wrap:wrap;border-radius:4px;overflow:hidden;flex-direction:column-reverse;justify-content:space-between}.mic-container .mic-level-container .mic-level[data-v-3cbc030b]{width:100%;background-color:#27c39f;transition:height .2s}.overlay-stream-container.mobile .overlay-stream-content-container[data-v-01ddfaf4]{width:100%;top:26%;position:absolute;display:flex;justify-content:center;flex-direction:column}.overlay-stream-container.mobile .overlay-stream-content-container .overlay-stream-content[data-v-01ddfaf4]{display:flex;flex-direction:column;align-items:center}.overlay-stream-container.mobile .overlay-stream-content-container .overlay-stream-content .overlay-stream-avatar[data-v-01ddfaf4]{margin-bottom:12px}.overlay-stream-container.mobile .overlay-stream-content-container .overlay-stream-content .overlay-stream-info[data-v-01ddfaf4]{display:flex;align-items:center}.overlay-stream-container.mobile .overlay-stream-content-container .overlay-stream-content .overlay-stream-tip[data-v-01ddfaf4]{margin-top:12px;color:#fff}.overlay-stream-container.pc .overlay-stream-content-container[data-v-01ddfaf4]{display:flex;height:100%;align-items:center;justify-content:center}.overlay-stream-container.pc .overlay-stream-content-container .overlay-stream-content[data-v-01ddfaf4]{display:flex;flex-direction:column;align-items:center}.overlay-stream-container.pc .overlay-stream-content-container .overlay-stream-content .overlay-stream-info[data-v-01ddfaf4]{display:flex;align-items:center}.overlay-stream-container[data-v-01ddfaf4]{position:absolute;display:flex;height:100%;width:100%;flex-direction:column;justify-content:center;align-items:center}.waiting-container[data-v-4bab4e9f]{position:absolute;top:0;left:0;bottom:0;right:0;z-index:1}.groupcall-info[data-v-4bab4e9f]{display:flex;flex-direction:column;align-items:center;margin-top:24px;z-index:1;color:#fff;width:100%}.groupcall-info .avatar-group[data-v-4bab4e9f]{display:flex;flex-wrap:wrap;margin-top:12px;align-items:center;justify-content:center;max-width:70%}.groupcall-info .avatar-group .avatar-item[data-v-4bab4e9f]{width:10vw;height:10vw;margin-left:10px;margin-top:10px}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-toggle-window{height:100%;position:relative}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-toggle-window-item{height:100%;position:absolute}.tk-toggle-window-item.tk-toggle-window-item--big{width:100%;height:100%;z-index:0}.tk-toggle-window-item.mobile.tk-toggle-window-item--small{z-index:1;width:24.3%!important;height:19.7%!important;border-radius:5px;left:98%;top:8%;transform:translate(-100%);overflow:hidden}.tk-toggle-window-item.pc.tk-toggle-window-item--small{top:2%;left:2%;width:22%;height:21%;border-radius:12px;overflow:hidden;z-index:1}.stream-userInfo[data-v-3bd02630]{padding:2px 5px;display:flex;align-items:center;background-color:#000000a6;color:#fff}.stream-userInfo .nickname[data-v-3bd02630]{display:flex}.stream-icon[data-v-411d981d]{width:30px;height:30px;background:rgba(34,38,46,.5019607843);border-radius:50%;display:flex;align-items:center;justify-content:center}.switch-camera[data-v-411d981d]{margin:0 12px}.pusher-container[data-v-71047565]{width:100%;height:100%;position:relative;background-color:#4c515a}.pusher-container .audio-stream-container[data-v-71047565]{position:absolute;z-index:3;width:100%;height:100%}.pusher-container .stream-info-container[data-v-71047565]{position:absolute;bottom:0;z-index:3;width:100%}.pusher-container .stream-info-container.mobile[data-v-71047565]{margin-bottom:8px}.player-container[data-v-7884ead8]{width:100%;height:100%;position:relative;background-color:#4c515a}.player-container .audio-stream-container[data-v-7884ead8]{position:absolute;z-index:1;width:100%;height:100%}.player-container .stream-info-container[data-v-7884ead8]{position:absolute;bottom:0;z-index:1;width:100%}.player-container .stream-info-container.mobile[data-v-7884ead8]{margin-bottom:8px}.float-control-panel[data-v-4df5fc21]{width:168px;height:56px;background:white;z-index:13;display:flex;flex-wrap:nowrap;justify-content:center;border-radius:40px;box-shadow:#00000029 0 3px 6px,#0000003b 0 3px 6px}.float-control-item-icon[data-v-4df5fc21]{display:flex;flex-direction:row;justify-content:center;align-items:center;width:56px;height:56px;position:relative}.float-control-item-icon-container[data-v-4df5fc21]{border-radius:40px;width:40px;height:40px;margin:.5rem;cursor:pointer;display:flex;flex-direction:row;justify-content:center;align-items:center}.float-control-item-icon-container[data-v-4df5fc21]:hover{background:rgba(218,218,218,.3)}.singlecall-video-float[data-v-05f43520]{width:110px;height:196px;display:flex;z-index:99;flex-direction:column;align-items:center;background:#000;border-radius:12px;box-shadow:0 0 10px #35394166;overflow:hidden}.singlecall-video-float .singlecall-video-float-content[data-v-05f43520]{width:100%;height:100%;display:flex;justify-content:center;flex-direction:column;align-items:center}.singlecall-video-float .float-window-tip-container[data-v-05f43520]{position:absolute;bottom:8px}.singlecall-audio-float[data-v-05f43520]{width:72px;height:72px;display:flex;z-index:99;flex-direction:column;align-items:center;background:#FFF;border-radius:12px;box-shadow:0 0 10px #35394166}.singlecall-audio-float .singlecall-audio-float-content[data-v-05f43520]{width:100%;height:100%;display:flex;justify-content:center;flex-direction:column;align-items:center}.click-container[data-v-76fc7036]{position:absolute;width:100%;height:100%;z-index:2}.groupcall-video-float.float[data-v-76fc7036]{width:72px;height:90px;display:flex;z-index:99;flex-direction:column;align-items:center;border-radius:12px;background:#FFF;box-shadow:0 0 10px #35394166;box-sizing:border-box;overflow:hidden}.groupcall-video-float.float .stream-container[data-v-76fc7036]{position:relative;width:72px;height:70px}.groupcall-video-float.float .video[data-v-76fc7036]{width:72px;height:70px;position:absolute}.groupcall-video-float.float .audio[data-v-76fc7036]{position:absolute;width:100%;height:70px;padding-top:10px;background-color:#fff;display:flex;flex-direction:column;align-items:center}.groupcall-video-float.float .device-status[data-v-76fc7036]{width:100%;height:20px;background-color:#f9f6f4;display:flex;position:relative;justify-content:space-around}.groupcall-video-float.not-float[data-v-76fc7036]{width:100%;height:100%}.groupcall-video-float.not-float .stream-container[data-v-76fc7036],.groupcall-video-float.not-float .stream-container .video[data-v-76fc7036]{height:100%}.float-window-container.not-float[data-v-59a0c3c2]{width:100%;height:100%}.float-window-container.float[data-v-59a0c3c2]{position:absolute}.float-window-container.float.pc[data-v-59a0c3c2]{top:50px;left:50%;transform:translate(-50%)}.float-window-container.float.mobile[data-v-59a0c3c2]{top:150px;right:0}.singlecall-media-container[data-v-5ca72ad0]{width:100%;height:100%;position:absolute;z-index:0}.singlecall-media-container.float[data-v-5ca72ad0]{position:relative}.singlecall-media-container.pc[data-v-5ca72ad0]{border-radius:12px;overflow:hidden}.roggle-btn[data-v-5ca72ad0]{position:absolute;left:100px;z-index:100;top:0}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-message{position:fixed;left:50%;transform:translate(-50%);padding:10px;border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:14px;z-index:9999;color:#000;border-color:#e9e9eb;background-color:#f4f4f5}.tk-message .tk-message_icon{margin-right:5px}.tk-message .tk-message_close{cursor:pointer;margin-left:5px}.tk-message--info{color:#909399;border-color:#e9e9eb;background-color:#f4f4f5}.tk-message--success{color:#67c23a;border-color:#e1f3d8;background-color:#f0f9eb}.tk-message--warning{color:#e6a23c;border-color:#faecd8;background-color:#fdf6ec}.tk-message--error{color:#f56c6c;border-color:#fde2e2;background-color:#fef0f0}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-grid-item{display:flex;justify-content:center}.tk-grid-item.h5{transition-property:width,height,left,top;transition-duration:.3s;transition-timing-function:ease-in}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-popover{position:relative}.tk-popover .tk-popover_content{display:inline-block;vertical-align:middle;min-width:120px;position:absolute;background:#FFFFFF;border-radius:4px;font-size:14px;box-shadow:0 0 12px #0000001f;overflow-wrap:break-word;box-sizing:border-box;text-align:center}.tk-popover .tk-popover_arrow{position:absolute;transform:translate(-50%);width:0;height:0}.tk-popover .tk-popover_trigger{display:inline-block;position:relative;vertical-align:middle}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-popover{position:relative}.tk-popover .tk-popover_content{display:inline-block;vertical-align:middle;min-width:120px;position:absolute;background:#FFFFFF;border-radius:4px;font-size:14px;box-shadow:0 0 12px #0000001f;overflow-wrap:break-word;box-sizing:border-box;text-align:center}.tk-popover .tk-popover_arrow{position:absolute;transform:translate(-50%);width:0;height:0}.tk-popover .tk-popover_trigger{display:inline-block;position:relative;vertical-align:middle}.device-selector-container[data-v-af70734e]{width:180px;display:flex;flex-direction:column;align-items:center;padding:6px;font-weight:500}.device-selector-container .device-item[data-v-af70734e]{overflow:hidden;padding:5px 3px;text-align:left;line-height:16px;cursor:pointer}.device-selector-container .device-item[data-v-af70734e]:hover{border-radius:5px;background:rgba(255,255,255,.3215686275)}.device-selector-container .device-item.select[data-v-af70734e]{background-color:#0f101433;border-radius:3px}.device-selector-container .control-item[data-v-af70734e]{width:100%;height:1px;background-color:#fff3}.scroll-container[data-v-af70734e]{width:100%;overflow:hidden}.scroll-container .scroll-content[data-v-af70734e]{max-height:60px;margin-right:-26px;overflow:hidden auto;padding-right:20px}.btn-content[data-v-fdb51114],.btn-content[data-v-348e1782],.btn-content[data-v-3da27647],.btn-content[data-v-76abffb6],.btn-content[data-v-4448bf22],.btn-content[data-v-9ce02c5e],.btn-content[data-v-32c39f09],.btn-content[data-v-7122a1b4],.btn-content[data-v-dd0c1858]{display:flex;flex-direction:column;align-items:center}.button-panel-container[data-v-26c8cb08]{position:absolute;z-index:1}.button-panel-container.pc[data-v-26c8cb08]{width:60%;height:63px;margin:0 auto;bottom:6%;left:50%;z-index:2;transform:translate(-50%)}.button-panel-container.mobile[data-v-26c8cb08]{display:flex;justify-content:center;height:27%;bottom:0;width:100%}.button-panel-container.mobile.h5[data-v-26c8cb08]{transition-property:width,height,left,top;transition-duration:.3s;transition-timing-function:ease-in}.button-panel-container.mobile.groupCall.showBackGround[data-v-26c8cb08]{background-color:#4f586b}.button-panel-container.mobile .button-group[data-v-26c8cb08]{position:absolute;width:72%;top:2vh;height:80%}.button-panel-container.mobile.close[data-v-26c8cb08]{height:14%;align-items:center}.button-panel-container.mobile.close .button-group[data-v-26c8cb08]{position:absolute;width:72%;right:6.2vw;height:40px;top:auto;bottom:auto}.button-panel-container .button-group[data-v-26c8cb08]{position:relative;height:100%}.button-panel-container .toggle-button-container[data-v-26c8cb08]{display:flex;align-items:center;position:absolute;left:8.2vw}.button-panel-container .toggle-button-container.h5[data-v-26c8cb08]{transition-property:width,height,left,top;transition-duration:.3s;transition-timing-function:ease-in}.button-panel-container.open .toggle-button-container[data-v-26c8cb08]{bottom:6vh}.singlecall-container[data-v-221760b8]{height:100%}.stream-loading-container[data-v-b3ff0e0c]{width:100%;height:100%;position:absolute;z-index:1;display:flex;align-items:center;justify-content:center}.groupcall-media-container[data-v-4c7859ad]{width:100%;height:100%;position:absolute;z-index:1}.groupcall-media-container.pc[data-v-4c7859ad]{border-radius:12px;overflow:hidden}.groupcall-media-container.pc.two-layout[data-v-4c7859ad]{margin-top:20%}.groupcall-media-container.pc .tk-toggle-window-item--small[data-v-4c7859ad]{top:2%;left:2%;width:22%;height:21%;border-radius:12px;overflow:hidden}.groupcall-media-container.mobile[data-v-4c7859ad]{margin-top:5.5vh}.groupcall-media-container.mobile.float[data-v-4c7859ad]{margin-top:0}.groupcall-media-container.mobile.two-layout[data-v-4c7859ad]{margin-top:15vh}.groupcall-media-container.mobile.two-layout.float[data-v-4c7859ad]{margin-top:0}.dialog[data-v-4c125b40]{background:rgba(0,0,0,.3)}.dialog-main[data-v-4c125b40]{background:#FFFFFF}.dialog-main-header[data-v-4c125b40]{font-weight:500;color:#333}.dialog-main-title[data-v-4c125b40]{font-family:PingFangSC-Medium;font-weight:500;color:#333}.dialog-main-back[data-v-4c125b40]{background:none}.dialog-main-content[data-v-4c125b40]{font-weight:400;color:#333}.btn[data-v-4c125b40]{font-weight:400;color:#fff;letter-spacing:0}.btn-cancel[data-v-4c125b40]{border:1px solid #dddddd;color:#666}.btn-default[data-v-4c125b40]{background:#006EFF;border:1px solid #006EFF}.dialog[data-v-4c125b40]{position:absolute;width:100%;height:100%;left:0;top:0;z-index:6;display:flex;justify-content:center;align-items:center}.dialog-main[data-v-4c125b40]{min-width:368px;border-radius:10px;padding:20px 30px}.dialog-main-header[data-v-4c125b40]{display:flex;justify-content:space-between;align-items:center;font-size:16px;line-height:30px}.dialog-main-title[data-v-4c125b40]{font-size:16px;line-height:30px}.dialog-main-content[data-v-4c125b40]{font-size:14px}.dialog-main-footer[data-v-4c125b40]{display:flex;justify-content:flex-end}.btn[data-v-4c125b40]{padding:8px 20px;margin:0 6px;border-radius:4px;border:none;font-size:14px;text-align:center;line-height:20px}.btn[data-v-4c125b40]:disabled{opacity:.3}.btn[data-v-4c125b40]:last-child{margin-right:0}.dialog-h5[data-v-4c125b40]{height:100%;top:0;align-items:inherit}.dialog-h5 .dialog-main[data-v-4c125b40]{border-radius:0;padding:0;display:flex;flex-direction:column;overflow:hidden;width:100%;min-height:80px;min-width:120px}.dialog-h5 .dialog-main-content[data-v-4c125b40]{flex:1;min-width:0;min-height:0;text-align:center}.dialog-h5 .dialog-main-content-uniapp[data-v-4c125b40]{padding:40px 0}.dialog-h5 .dialog-main-footer[data-v-4c125b40]{border-top:1px solid #DDDDDD}.dialog-h5 .dialog-main-footer .btn[data-v-4c125b40]{flex:1;margin:0;background:none;border-right:1px solid #DDDDDD}.dialog-h5 .dialog-main-footer .btn-default[data-v-4c125b40]{color:#ff584c;border:none}.center[data-v-4c125b40]{align-items:center;padding:20px;box-sizing:border-box}.icon[data-v-c4142a3c]{display:inline-flex;justify-content:center;align-items:center;margin:0}.main[data-v-65942e88]{background:#FFFFFF;border:1px solid #E0E0E0;box-shadow:0 -4px 12px #0000000f}.main .left[data-v-65942e88]{border-right:1px solid #E8E8E9}.main .transfer-header[data-v-65942e88]{font-weight:500;color:#000;letter-spacing:0}.main .transfer-header input[data-v-65942e88]{background:#FFFFFF;border:1px solid #DEE0E3;font-weight:500;color:#8f959e;letter-spacing:0}.main .transfer-list .transfer-text[data-v-65942e88]{font-weight:500;color:#8f959e;letter-spacing:0}.main .transfer-list-item .disabled[data-v-65942e88]{background:#eeeeee}.btn[data-v-65942e88]{background:#3370FF;border:0 solid #2F80ED;font-weight:400;color:#fff}.btn-cancel[data-v-65942e88]{background:#FFFFFF;border:1px solid #DDDDDD;color:#828282}.btn-no[data-v-65942e88]{background:#e8e8e9;border:1px solid #DDDDDD;font-weight:400;color:#fff}.transfer-h5-header[data-v-65942e88]{background:#FFFFFF}.transfer-h5-header .title[data-v-65942e88]{font-family:PingFangSC-Medium;font-weight:500;color:#000;letter-spacing:0}.main[data-v-65942e88]{box-sizing:border-box;width:541px;height:390px;display:flex;border-radius:8px;padding:20px 0}.main .right[data-v-65942e88]{padding:0 20px;flex:1}.main .right .transfer-list[data-v-65942e88]{padding-right:20px}.main .left[data-v-65942e88]{flex:1;overflow-y:hidden;display:flex;flex-direction:column}.main .left .transfer-header[data-v-65942e88]{padding:0 20px}.main .left .transfer-left-main[data-v-65942e88]{flex:1;overflow-y:auto;padding:0 13px}.main .right[data-v-65942e88]{display:flex;flex-direction:column;text-align:left}.main .right .transfer-right-footer[data-v-65942e88]{align-self:flex-end}.main .right .transfer-right-footer .btn-cancel[data-v-65942e88]{margin-right:12px}.main .right .transfer-list[data-v-65942e88]{overflow-y:auto}.main .transfer-header[data-v-65942e88]{font-size:14px;line-height:14px;padding-bottom:20px}.main .transfer-header input[data-v-65942e88]{box-sizing:border-box;width:100%;border-radius:30px;font-size:10px;line-height:14px;padding:9px 12px}.main .transfer-list[data-v-65942e88]{flex:1;display:flex;flex-direction:column}.main .transfer-list .transfer-text[data-v-65942e88]{font-size:10px;line-height:14px}.main .transfer-list-item[data-v-65942e88]{padding:6px 0;display:flex;align-items:center;font-size:14px;text-align:left}.main .transfer-list-item-content[data-v-65942e88]{flex:1;display:flex;align-items:center}.main .transfer-list-item .avatar[data-v-65942e88]{margin:0 5px 0 8px;border-radius:50%}.main .transfer-list-item .name[data-v-65942e88]{width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1}.avatar[data-v-65942e88]{width:36px;height:36px;border-radius:5px;font-size:12px;display:flex;justify-content:center;align-items:center}.btn[data-v-65942e88],.btn-no[data-v-65942e88]{padding:4px 28px;font-size:12px;line-height:24px;border-radius:4px}.space-between[data-v-65942e88]{justify-content:space-between}.select-all[data-v-65942e88]{padding-left:8px;font-size:14px}.more[data-v-65942e88]{display:flex;justify-content:center;align-items:center;cursor:pointer;font-size:14px}.transfer-h5[data-v-65942e88]{width:100vw;height:100vh;display:flex;flex-direction:column}.transfer-h5-header[data-v-65942e88]{position:relative;display:flex;justify-content:space-between;align-items:center;font-size:18px;padding:16px 18px}.transfer-h5-header .space[data-v-65942e88],.transfer-h5-header .icon[data-v-65942e88]{width:18px;height:18px}.transfer-h5 .main[data-v-65942e88]{flex-direction:column;width:auto;height:550px;border-radius:0;border:none;box-shadow:none;max-height:calc(100% - 50px);padding:0}.transfer-h5 .main .avatar[data-v-65942e88]{border-radius:5px}.transfer-h5 .main .left[data-v-65942e88]{padding:0;flex:1;border:none;display:flex;flex-direction:column}.transfer-h5 .main .left .transfer-header[data-v-65942e88]{position:sticky;top:0;padding:0 18px}.transfer-h5 .main .left .transfer-header input[data-v-65942e88]{border-radius:5px;font-size:14px}.transfer-h5 .main .left-uniapp-input[data-v-65942e88]{height:36px}.transfer-h5 .main .right[data-v-65942e88]{flex:0;flex-direction:row;align-items:center;box-shadow:inset 0 1px #eee;padding:8px 18px}.transfer-h5 .main .right .transfer-list[data-v-65942e88]{flex-direction:row;width:0}.transfer-h5 .main .right .transfer-list-item-content[data-v-65942e88]{flex:none}.transfer-h5 .main .right .transfer-right-footer[data-v-65942e88]{padding:6px 0;display:flex;align-items:center}.transfer-h5 .main .right .transfer-right-footer .btn[data-v-65942e88]{font-size:14px}ul[data-v-65942e88],ol[data-v-65942e88],li[data-v-65942e88]{margin:0;padding:0}.icon-unselected[data-v-65942e88]{width:18px;height:18px;background:#FFFFFF;border:1px solid #DDDDDD;border-radius:11px;box-sizing:border-box}.groupcall-container[data-v-58680342]{height:100%}.card[data-v-631fa89e]{box-sizing:border-box;border-radius:4px;border:1px solid #EBEEF5;background-color:#fff;overflow:hidden;color:#303133;transition:.3s;box-shadow:0 2px 12px #0000001a}.card .card-body[data-v-631fa89e]{padding:20px}.card-wrapper[data-v-631fa89e]{display:flex;flex-direction:column;align-items:center;width:500px;position:relative}.card-wrapper .button[data-v-631fa89e]{box-sizing:border-box;background-color:#104ef5;color:#fff;font-size:12px;height:37px;border-radius:4px;padding:10px}.card-wrapper .image-wrapper[data-v-631fa89e]{box-sizing:border-box}.title[data-v-631fa89e]{padding:10px 0;font-size:16px;font-weight:500;line-height:22px}.desc[data-v-631fa89e]{font-size:12px;margin-bottom:10px}.button[data-v-631fa89e]{background-color:#104ef5;color:#fff;font-size:12px;height:37px;border-radius:4px;padding:10px}.allow[data-v-631fa89e]{font-size:12px;color:#666;padding:10px 0 20px}.image[data-v-631fa89e]{width:100%;border-radius:10px}.close[data-v-631fa89e]{position:absolute;z-index:999;display:flex;justify-content:center;align-items:center;font-size:20px;right:20px;cursor:pointer;width:20px;height:20px}.tips[data-v-ff1df20c]{width:100%;height:100%}.card[data-v-ff1df20c]{position:fixed;right:0;bottom:0;box-shadow:0 2px 12px #0000001a;border-radius:4px;border:1px solid #EBEEF5;background-color:#fff;overflow:hidden;color:#303133;transition:.3s}.tag[data-v-ff1df20c]{position:fixed;left:0;top:0;width:320px;text-align:left}.toast-container[data-v-1a36ce82]{position:fixed;display:flex;align-items:center;z-index:1002;justify-content:space-between;left:50%;transform:translate(-50%);padding:15px 15px 15px 20px;background-color:#000;color:#fff;box-sizing:border-box;border-radius:4px;border-width:1px;border-style:solid}.toast-container .toast-content[data-v-1a36ce82]{margin-right:16px}.toast-container .toast-close-icon[data-v-1a36ce82]{width:16px;line-height:16px;cursor:pointer}.toast-container .toast-close-icon[data-v-1a36ce82]:before{font-style:normal;content:url(data:image/svg+xml;base64,)}.info[data-v-1a36ce82]{border-color:#ebeef5;color:#909399;background-color:#edf2fc}.success[data-v-1a36ce82]{color:#67c23a;background-color:#f0f9eb;border-color:#e1f3d8}.waring[data-v-1a36ce82]{color:#e6a23c;background-color:#fdf6ec;border-color:#faecd8}.error[data-v-1a36ce82]{color:#f56c6c;background-color:#fef0f0;border-color:#fde2e2}.slide-up-enter-active[data-v-1a36ce82],.slide-up-leave-active[data-v-1a36ce82]{transition:all .5s ease-out}.slide-up-enter-from[data-v-1a36ce82]{opacity:0;transform:translate(-50%,20px)}.slide-up-leave-to[data-v-1a36ce82]{opacity:0;transform:translate(-50%,-20px)}.fade-enter-from[data-v-1a36ce82],.fade-leave-to[data-v-1a36ce82]{opacity:0;transform:translate(-50%,-100%)}.TUICallKit-mobile,.TUICallKit-mobile .singCall{width:100%;height:100%}.transition-animation{transform:translateY(-100%);animation:slideInDown .5s ease forwards}@keyframes slideInDown{0%{transform:translateY(-100%)}to{transform:translateY(0)}}.TUICallKit-desktop{margin:0 auto;position:relative;border-radius:inherit;width:100%;height:100%;color:#fff;display:flex;flex-direction:column;justify-content:center;overflow:hidden;z-index:12;border-radius:16px}.TUICallKit-desktop .singCall{width:100%;height:100%}.mobile-audio{background-color:#fff}.miniMized{width:168px!important;height:56px!important;overflow:visible!important}.miniMized-mobile-audio{width:72px;height:72px;position:fixed;top:40px;right:40px}.miniMized-mobile-video{width:40%;height:30%;position:fixed;top:40px;right:40px}.banner-pc{position:relative;cursor:pointer}.banner-pc .top{height:24px;padding-right:10px;border-radius:4px;background:#F4F5F9;display:inline-flex;align-items:center}.banner-pc .top .call-icon{margin:4px 4px 4px 8px}.banner-pc .top .text{font-weight:400;font-size:12px;line-height:16px;color:#666}.banner-pc .content{width:220px;margin-top:8px;padding:10px 18px 6px;box-shadow:0 0 24px #161e2733;background-color:#fff;border-radius:10px;position:absolute;z-index:10;display:flex;display:-webkit-flex;flex-direction:column;align-items:center}.banner-pc .content .content-arrows{width:12px;height:12px;border-radius:2px;background-color:#fff;transform:rotate(45deg);position:absolute;top:-5px;left:80px}.banner-pc .content .avatar-box{display:flex;justify-content:center;flex-wrap:wrap}.banner-pc .content .avatar{margin:6px;display:flex;border-radius:4px}.banner-pc .content .btn{margin:10px 0;width:97px;height:30px;background:#F2F5FC;border-radius:4px;display:flex;justify-content:center;align-items:center;font-size:12px;color:#1c66e5;cursor:pointer}.banner-h5{padding:0 16px;position:relative}.banner-h5 .top{height:36px;background:#FFFFFF;border-radius:4px;display:flex;display:-webkit-flex;justify-content:space-between;align-items:center}.banner-h5 .top .left{display:flex;align-items:center}.banner-h5 .top .left .text{font-weight:400;font-size:12px;color:#666}.banner-h5 .content-box{box-sizing:border-box;width:100%;padding:0 16px 10px;background:#FFFFFF;box-shadow:0 3px 3px 1px #b4b4b440;border-radius:0 0 8px 8px;position:absolute;left:0px;z-index:10}.banner-h5 .content-box .content{background:#EEF0F2;border-radius:6px}.banner-h5 .content-box .content .avatar-box{padding:30px 6px;border-bottom:1px solid rgba(112,112,112,.1058823529);display:flex;display:-webkit-flex;justify-content:center;flex-wrap:wrap}.banner-h5 .content-box .content .btn{padding:10px;font-weight:600;font-size:14px;text-align:center;color:#333;cursor:pointer}")),document.head.appendChild(t)}}catch(e){console.error("vite-plugin-css-injected-by-js",e)}})();
import { TUICallEvent as T, TUICallEngine as Bs } from "@tencentcloud/call-engine-js";
import { TUICore as ee, TUIConstants as $, TUILogin as Vs } from "@tencentcloud/tui-core";
import yn from "@tencentcloud/chat";
import Xt from "vue";
import { inject as dA, ref as I, onMounted as sA, onUnmounted as tA, toRefs as j, watch as Y, computed as C, provide as rA, watchEffect as $A, toRef as Ut, unref as Ps, nextTick as Fs, reactive as Xe } from "@vue/composition-api";
var r = /* @__PURE__ */ ((A) => (A.CALL = "call", A.CUSTOM = "custom", A))(r || {}), V = /* @__PURE__ */ ((A) => (A[A.UNKNOWN = 0] = "UNKNOWN", A[A.AUDIO = 1] = "AUDIO", A[A.VIDEO = 2] = "VIDEO", A))(V || {}), H = /* @__PURE__ */ ((A) => (A.UNKNOWN = "unknown", A.CALLEE = "callee", A.CALLER = "caller", A))(H || {}), w = /* @__PURE__ */ ((A) => (A.IDLE = "idle", A.CALLING = "calling", A.CONNECTED = "connected", A))(w || {}), ie = /* @__PURE__ */ ((A) => (A.CONTAIN = "contain", A.COVER = "cover", A.FILL = "fill", A))(ie || {}), le = /* @__PURE__ */ ((A) => (A.RESOLUTION_480P = "480p", A.RESOLUTION_720P = "720p", A.RESOLUTION_1080P = "1080p", A))(le || {}), oo = /* @__PURE__ */ ((A) => (A.EN = "en", A["ZH-CN"] = "zh-cn", A.JA_JP = "ja_JP", A))(oo || {});
const aA = {
  IDLE: "idle",
  BE_INVITED: "be-invited",
  DIALING_C2C: "dialing-c2c",
  DIALING_GROUP: "dialing-group",
  CALLING_C2C_AUDIO: "calling-c2c-audio",
  CALLING_C2C_VIDEO: "calling-c2c-video",
  CALLING_GROUP_AUDIO: "calling-group-audio",
  CALLING_GROUP_VIDEO: "calling-group-video"
}, Rs = {
  unknown: 0,
  audio: 1,
  video: 2
};
var oA = /* @__PURE__ */ ((A) => (A.MICROPHONE = "microphone", A.CAMERA = "camera", A.SPEAKER = "speaker", A))(oA || {}), FA = /* @__PURE__ */ ((A) => (A[A.FRONT = 0] = "FRONT", A[A.BACK = 1] = "BACK", A))(FA || {}), LA = /* @__PURE__ */ ((A) => (A.Camera = "camera", A.Microphone = "microphone", A.SwitchCamera = "switchCamera", A.InviteUser = "inviteUser", A))(LA || {}), GA = /* @__PURE__ */ ((A) => (A.Open = "open", A.Close = "close", A))(GA || {}), et = /* @__PURE__ */ ((A) => (A.LOCAL = "local", A.REMOTE = "remote", A))(et || {}), tt = /* @__PURE__ */ ((A) => (A.LocalInLargeView = "local", A.RemoteInLargeView = "remote", A))(tt || {}), KA = /* @__PURE__ */ ((A) => (A[A.INVITE = 1] = "INVITE", A[A.CANCEL_INVITE = 2] = "CANCEL_INVITE", A[A.ACCEPT_INVITE = 3] = "ACCEPT_INVITE", A[A.REJECT_INVITE = 4] = "REJECT_INVITE", A[A.INVITE_TIMEOUT = 5] = "INVITE_TIMEOUT", A))(KA || {});
const En = {
  SWITCH_TO_AUDIO_CALL_FAILED: 60001,
  SWITCH_TO_VIDEO_CALL_FAILED: 60002,
  MICROPHONE_UNAVAILABLE: 60003,
  CAMERA_UNAVAILABLE: 60004,
  BAN_DEVICE: 60005,
  NOT_SUPPORTED_WEBRTC: 60006,
  ERROR_BLACKLIST: 20007
}, Tn = {
  SWITCH_TO_AUDIO_CALL_FAILED: "switchToAudioCall-call-failed",
  SWITCH_TO_VIDEO_CALL_FAILED: "switchToVideoCall-call-failed",
  MICROPHONE_UNAVAILABLE: "microphone-unavailable",
  CAMERA_UNAVAILABLE: "camera-unavailable",
  BAN_DEVICE: "ban-device",
  NOT_SUPPORTED_WEBRTC: "not-supported-webrtc",
  ERROR_BLACKLIST: "blacklist-user-tips"
};
var so = /* @__PURE__ */ ((A) => (A[A.NORMAL = 0] = "NORMAL", A[A.RELEASE = 1] = "RELEASE", A[A.WARNING = 2] = "WARNING", A[A.ERROR = 3] = "ERROR", A[A.NONE = 4] = "NONE", A))(so || {});
const $e = {
  CALL_STATUS: "callStatus",
  CALL_ROLE: "callRole",
  CALL_MEDIA_TYPE: "callMediaType",
  LOCAL_USER_INFO: "localUserInfo",
  LOCAL_USER_INFO_EXCLUDE_VOLUMN: "localUserInfoExcludeVolume",
  REMOTE_USER_INFO_LIST: "remoteUserInfoList",
  REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST: "remoteUserInfoExcludeVolumeList",
  CALLER_USER_INFO: "callerUserInfo",
  IS_GROUP: "isGroup",
  CALL_DURATION: "callDuration",
  CALL_TIPS: "callTips",
  TOAST_INFO: "toastInfo",
  IS_MINIMIZED: "isMinimized",
  ENABLE_FLOAT_WINDOW: "enableFloatWindow",
  BIG_SCREEN_USER_ID: "bigScreenUserId",
  LANGUAGE: "language",
  IS_CLICKABLE: "isClickable",
  DISPLAY_MODE: "displayMode",
  VIDEO_RESOLUTION: "videoResolution",
  PUSHER: "pusher",
  PLAYER: "player",
  IS_EAR_PHONE: "isEarPhone",
  IS_MUTE_SPEAKER: "isMuteSpeaker",
  SHOW_PERMISSION_TIP: "SHOW_PERMISSION_TIP",
  NETWORK_STATUS: "NetWorkStatus",
  GROUP_ID: "groupID",
  ROOM_ID: "roomID",
  ROOM_ID_TYPE: "roomIdType",
  SHOW_SELECT_USER: "showSelectUser",
  IS_SHOW_ENABLE_VIRTUAL_BACKGROUND: "isShowEnableVirtualBackground",
  ENABLE_VIRTUAL_BACKGROUND: "enableVirtualBackground",
  GROUP_CALL_MEMBERS: "groupCallMembers",
  PUSHER_ID: "pusherId"
}, Gs = {
  INNER_ATTR_KIT_INFO: "inner_attr_kit_info"
}, ks = {
  INITIAL_PUSHER: "initialPusher",
  NEW_PUSHER: "newPusher"
}, o = {
  PREFIX: "\u3010CallService\u3011",
  AUDIO: "audio",
  VIDEO: "video",
  LOCAL_VIDEO: "localVideo",
  ERROR: "error",
  TIMEOUT: "timeout",
  RAF: "raf",
  INTERVAL: "interval",
  DEFAULT: "default",
  BOOLEAN: "boolean",
  STRING: "string",
  NUMBER: "number",
  OBJECT: "object",
  ARRAY: "array",
  FUNCTION: "function",
  UNDEFINED: "undefined",
  UNKNOWN: "unknown",
  ALL: "all",
  MYSELF: "myself",
  DEVICE_LIST: "deviceList",
  CAMERA_POSITION: "cameraPosition",
  CUSTOM_UI_CONFIG: "customUIConfig",
  TRANSLATE: "translate",
  ...ks,
  ...$e,
  ...Gs
}, _s = "https://web.sdk.qcloud.com/component/TUIKit/assets/call.png", Ws = "https://web.sdk.qcloud.com/component/TUIKit/assets/call-video-reverse.svg", wn = 2147483647, qs = 3, Xs = 4;
var Ft = /* @__PURE__ */ ((A) => (A.MAC = "mac", A.WIN = "win", A))(Ft || {}), Qt = /* @__PURE__ */ ((A) => (A[A.TUI_CALL_KIT = 14] = "TUI_CALL_KIT", A[A.TIM_CALL_KIT = 15] = "TIM_CALL_KIT", A))(Qt || {}), Rt = /* @__PURE__ */ ((A) => (A[A.NUMBER_ROOM_ID = 1] = "NUMBER_ROOM_ID", A[A.STRING_ROOM_ID = 2] = "STRING_ROOM_ID", A))(Rt || {});
async function Qs(A) {
  if (!A)
    return !1;
  try {
    const t = await new Promise((e, n) => {
      const a = new XMLHttpRequest();
      a.open("HEAD", A, !0), a.onload = () => e(a), a.onerror = () => n(a), a.send();
    });
    return t.status === 200 && t.getResponseHeader("Content-Type") === "audio/mpeg";
  } catch (t) {
    return console.warn(t), !1;
  }
}
function nt(A) {
  if (typeof A != "object" || A === null)
    return A;
  let t = Array.isArray(A) ? [] : {};
  for (let e in A)
    A.hasOwnProperty(e) && (t[e] = nt(A[e]));
  return t;
}
const Zs = {
  hangup: "\u6302\u65AD",
  reject: "\u62D2\u7EDD",
  accept: "\u63A5\u53D7",
  camera: "\u6444\u50CF\u5934",
  microphone: "\u9EA6\u514B\u98CE",
  speaker: "\u626C\u58F0\u5668",
  "open camera": "\u6253\u5F00\u6444\u50CF\u5934",
  "close camera": "\u5173\u95ED\u6444\u50CF\u5934",
  "open microphone": "\u6253\u5F00\u9EA6\u514B\u98CE",
  "close microphone": "\u5173\u95ED\u9EA6\u514B\u98CE",
  "video-to-audio": "\u8F6C\u8BED\u97F3\u901A\u8BDD",
  "virtual-background": "\u6A21\u7CCA\u80CC\u666F",
  "other side reject call": "\u5BF9\u65B9\u5DF2\u62D2\u7EDD",
  "reject call": "{{ userList }} \u62D2\u7EDD\u901A\u8BDD",
  cancel: "\u53D6\u6D88\u901A\u8BDD",
  "other side line busy": "\u5BF9\u65B9\u5FD9\u7EBF",
  "in busy": "{{ userList }} \u6B63\u5728\u5FD9",
  "call timeout": "\u547C\u53EB\u8D85\u65F6",
  "end call": "{{ userList }} \u7ED3\u675F\u901A\u8BDD",
  "caller calling message": "\u7B49\u5F85\u5BF9\u65B9\u63A5\u53D7\u9080\u8BF7",
  "callee calling video message": "\u9080\u8BF7\u4F60\u89C6\u9891\u901A\u8BDD",
  "callee calling audio message": "\u9080\u8BF7\u4F60\u8BED\u97F3\u901A\u8BDD",
  "no microphone access": "\u6CA1\u6709\u9EA6\u514B\u98CE\u6743\u9650",
  "no camera access": "\u6CA1\u6709\u6444\u50CF\u5934\u6743\u9650",
  "invite member": "\u9080\u8BF7\u6210\u5458",
  "Invited group call": "\u9080\u8BF7\u4F60\u52A0\u5165\u591A\u4EBA\u901A\u8BDD",
  "Those involved": "\u53C2\u4E0E\u901A\u8BDD\u7684\u6709\uFF1A",
  waiting: "\u7B49\u5F85\u63A5\u542C...",
  me: "(\u6211)",
  "browser-authorization": "\u6D4F\u89C8\u5668\u6388\u6743",
  "mac-privacy": "\u7CFB\u7EDF\u504F\u597D\u8BBE\u7F6E -> \u5B89\u5168\u4E0E\u9690\u79C1 -> \u9690\u79C1",
  "win-privacy": "\u8BBE\u7F6E -> \u9690\u79C1\u548C\u5B89\u5168\u6027 -> \u5E94\u7528\u6743\u9650",
  "mac-preferences": "\u6253\u5F00\u7CFB\u7EDF\u504F\u597D\u8BBE\u7F6E",
  "win-preferences": "\u6253\u5F00\u7CFB\u7EDF\u8BBE\u7F6E",
  "Please enter userID": "\u8BF7\u8F93\u5165 userID",
  "View more": "\u67E5\u770B\u66F4\u591A",
  "people selected": "\u4EBA\u5DF2\u9009\u4E2D",
  "Select all": "\u5168\u9009",
  Cancel: "\u53D6\u6D88",
  Done: "\u5B8C\u6210",
  "exist group call": "\u5F53\u524D\u7FA4\u7EC4\u4E2D\u5DF2\u7ECF\u5B58\u5728\u7FA4\u7EC4\u901A\u8BDD",
  "camera enabled": "\u6444\u50CF\u5934\u5DF2\u5F00",
  "camera disabled": "\u6444\u50CF\u5934\u5DF2\u5173",
  "microphone enabled": "\u9EA6\u514B\u98CE\u5DF2\u5F00",
  "microphone disabled": "\u9EA6\u514B\u98CE\u5DF2\u5173",
  "speaker enabled": "\u626C\u58F0\u5668\u5DF2\u5F00",
  "speaker disabled": "\u626C\u58F0\u5668\u5DF2\u5173",
  "open speaker": "\u5F00\u542F\u626C\u58F0\u5668",
  "close speaker": "\u5173\u95ED\u626C\u58F0\u5668",
  "wait to be called": "\u7B49\u5F85\u63A5\u542C",
  answered: "\u5DF2\u63A5\u901A",
  "people in the call": "\u4EBA\u53C2\u4E0E\u901A\u8BDD",
  "failed to obtain speakers": "\u65E0\u6CD5\u83B7\u53D6\u626C\u58F0\u5668",
  "you have a new call": "\u60A8\u6709\u4E00\u4E2A\u65B0\u7684\u901A\u8BDD",
  "switch camera": "\u7FFB\u8F6C",
  join: "\u52A0\u5165",
  "people on the call": "\u4EBA\u6B63\u5728\u901A\u8BDD",
  "Supports a maximum of 9 people for simultaneous calls": "\u6700\u591A\u652F\u63019\u4EBA\u540C\u65F6\u901A\u8BDD",
  you: "(\u4F60)",
  "The network is poor during your current call": "\u5F53\u524D\u901A\u8BDD\u4F60\u7684\u7F51\u7EDC\u4E0D\u4F73",
  "The other user network is poor during the current call": "\u5F53\u524D\u901A\u8BDD\u5BF9\u65B9\u7F51\u7EDC\u4E0D\u4F73",
  "TUICallKit init is not complete": "TUICallKit \u521D\u59CB\u5316\u767B\u5F55\u672A\u5B8C\u6210\uFF0C\u9700\u8981\u5728 init \u5B8C\u6210\u540E\u4F7F\u7528\u6B64 API",
  "Video call": "\u53D1\u8D77\u89C6\u9891\u901A\u8BDD",
  "Voice call": "\u53D1\u8D77\u8BED\u97F3\u901A\u8BDD",
  "Call End": "\u901A\u8BDD\u7ED3\u675F",
  "Switch voice call": "\u5207\u6362\u8BED\u97F3\u901A\u8BDD",
  "Switch video call": "\u5207\u6362\u89C6\u9891\u901A\u8BDD",
  "Call duration": "\u901A\u8BDD\u65F6\u957F",
  "Call Cancel": "\u5DF2\u53D6\u6D88",
  "Other Side Cancel": "\u5BF9\u65B9\u5DF2\u53D6\u6D88",
  Decline: "\u5DF2\u62D2\u7EDD",
  "Other Side Decline": "\u5BF9\u65B9\u5DF2\u62D2\u7EDD",
  "No answer": "\u8D85\u65F6\u65E0\u5E94\u7B54",
  "Other Side No Answer": "\u5BF9\u65B9\u65E0\u5E94\u7B54",
  Answered: "\u5DF2\u63A5\u542C",
  "Other Side Line Busy": "\u5BF9\u65B9\u5FD9\u7EBF\u4E2D",
  "Line Busy": "\u5FD9\u7EBF\u65E0\u5E94\u7B54",
  timeout: "{{ userList }} \u8D85\u65F6",
  "kick out": "\u88AB\u8E22",
  call: "\u901A\u8BDD",
  "video-call": "\u89C6\u9891\u901A\u8BDD",
  "audio-call": "\u97F3\u9891\u901A\u8BDD",
  search: "\u641C\u7D22",
  "search-result": "\u641C\u7D22\u7ED3\u679C",
  "Wechat scan right QR code": "\u5FAE\u4FE1\u626B\u53F3\u4E8C\u7EF4\u7801",
  "Use-phone-and-computer": "\u7528\u624B\u673A\u4E0E\u7535\u8111\u4E92\u6253\u4F53\u9A8C\u89C6\u9891\u901A\u8BDD",
  "Scan the QR code above": "\u626B\u63CF\u4E0A\u65B9\u4E8C\u7EF4\u7801",
  "no-user": "\u672A\u641C\u7D22\u5230\u7528\u6237",
  "member-not-added": "\u672A\u6DFB\u52A0\u6210\u5458",
  "not-login": "\u672A\u767B\u5F55",
  "login-status-expire": "\u767B\u5F55\u72B6\u6001\u5DF2\u5931\u6548\uFF0C\u8BF7\u5237\u65B0\u7F51\u9875\u91CD\u8BD5",
  "experience-multi-call": "\u4F53\u9A8C\u591A\u4EBA\u901A\u8BDD\u8BF7\u4E0B\u8F7D\u5168\u529F\u80FDdemo:",
  "not-support-multi-call": "\u591A\u4EBA\u901A\u8BDD\u63A5\u53E3\u672A\u5F00\u653E",
  "input-phone-userID": "\u8BF7\u8F93\u5165\u624B\u673A\u53F7/\u7528\u6237ID",
  userID: "\u7528\u6237ID",
  "already-enter": "\u5DF2\u7ECF\u8FDB\u5165\u5F53\u524D\u901A\u8BDD",
  "image-resolution": "\u5206\u8FA8\u7387",
  "default-image-resolution": "\u9ED8\u8BA4\u5206\u8FA8\u7387",
  "invited-person": "\u6DFB\u52A0\u6210\u5458",
  "be-rejected": "\u5BF9\u65B9\u5DF2\u62D2\u7EDD\uFF0C",
  "be-no-response": "\u5BF9\u65B9\u65E0\u5E94\u7B54\uFF0C",
  "be-line-busy": "\u5BF9\u65B9\u5FD9\u7EBF\u4E2D\uFF0C",
  "be-canceled": "\u5BF9\u65B9\u5DF2\u53D6\u6D88",
  "voice-call-end": "\u8BED\u97F3\u901A\u8BDD\u7ED3\u675F",
  "video-call-end": "\u89C6\u9891\u901A\u8BDD\u7ED3\u675F",
  "method-call-failed": "\u540C\u6B65\u64CD\u4F5C\u5931\u8D25",
  "failed-to-obtain-permission": "\u6743\u9650\u83B7\u53D6\u5931\u8D25",
  "environment-detection-failed": "\u73AF\u5883\u68C0\u6D4B\u5931\u8D25",
  "switchToAudioCall-call-failed": "\u5207\u8BED\u97F3\u8C03\u7528\u5931\u8D25",
  "switchToVideoCall-call-failed": "\u5207\u89C6\u9891\u8C03\u7528\u5931\u8D25",
  "microphone-unavailable": "\u6CA1\u6709\u53EF\u7528\u7684\u9EA6\u514B\u98CE\u8BBE\u5907",
  "camera-unavailable": "\u6CA1\u6709\u53EF\u7528\u7684\u6444\u50CF\u5934\u8BBE\u5907",
  "ban-device": "\u7528\u6237\u7981\u6B62\u4F7F\u7528\u8BBE\u5907",
  "not-supported-webrtc": "\u5F53\u524D\u73AF\u5883\u4E0D\u652F\u6301 WebRTC",
  "blacklist-user-tips": "\u53D1\u8D77\u901A\u8BDD\u5931\u8D25\uFF0C\u88AB\u5BF9\u65B9\u62C9\u5165\u9ED1\u540D\u5355\uFF0C\u7981\u6B62\u53D1\u8D77\uFF01",
  "is-already-calling": "TUICallKit \u5DF2\u5728\u901A\u8BDD\u72B6\u6001",
  "need-init": "TUICallKit \u53D1\u8D77\u901A\u8BDD\u524D\u9700\u4FDD\u8BC1 TUICallKitServer.init() \u65B9\u6CD5\u6267\u884C\u6210\u529F",
  "can't call yourself": "\u4E0D\u80FD\u547C\u53EB\u81EA\u5DF1",
  "accept-error": "\u63A5\u901A\u5931\u8D25",
  "accept-device-error": "\u63A5\u901A\u5931\u8D25\uFF0C\u901A\u8BDD\u8BBE\u5907\u83B7\u53D6\u5931\u8D25",
  "call-error": "\u53D1\u8D77\u901A\u8BDD\u5931\u8D25"
}, Ys = {
  hangup: "Hang up",
  reject: "Decline",
  accept: "Accept",
  camera: "Camera",
  microphone: "Microphone",
  speaker: "speaker",
  "open camera": "Open Camera",
  "close camera": "Close Camera",
  "open microphone": "Open Microphone",
  "close microphone": "Close Microphone",
  "video-to-audio": "Switch to audio",
  "virtual-background": "Blur Background",
  "other side reject call": "other side reject call",
  "reject call": "{{ userList }} Reject Call",
  cancel: "Cancel Call",
  "other side line busy": "other side line busy",
  "in busy": "{{ userList }} in busy",
  "call timeout": "call timeout",
  "no response from the other side": "no response from the other side",
  "end call": "{{ userList }} end call",
  "caller calling message": "Awaiting response",
  "callee calling video message": "invites you to a video call",
  "callee calling audio message": "invites you to a voice call",
  "no microphone access": "no microphone access",
  "no camera access": "no camera access",
  "invite member": "Invite Member",
  "Invited group call": "invites you to a group call",
  waiting: "Calling...",
  me: "(me)",
  "browser-authorization": "Browser authorization",
  "mac-privacy": "System Preferences -> Security and Privacy -> Privacy",
  "win-privacy": "Setting -> Privacy and Security -> App permissions",
  "mac-preferences": "Open System Preferences",
  "win-preferences": "Open Setting",
  "Please enter userID": "Please enter userID",
  "View more": "View more",
  "people selected": "people selected",
  "Select all": "Select all",
  Cancel: "Cancel",
  Done: "Done",
  "exist group call": "A group call already exists in the current group",
  "camera enabled": "Camera On",
  "camera disabled": "Camera Off",
  "microphone enabled": "Unmuted",
  "microphone disabled": "Muted",
  "speaker enabled": "Speaker On",
  "speaker disabled": "Speaker Off",
  "open speaker": "Turn on speaker",
  "close speaker": "Turn off speaker",
  "wait to be called": "Waiting",
  answered: "Connected",
  "people in the call": " other(s) in the call",
  "failed to obtain speakers": "failed to obtain speakers",
  "you have a new call": "You have a new call",
  "switch camera": "Switch",
  join: "Join",
  "people on the call": "people on the call",
  "Supports a maximum of 9 people for simultaneous calls": "Supports a maximum of 9 people for simultaneous calls",
  you: "(you)",
  "The network is poor during your current call": "The network is poor during your current call",
  "The other user network is poor during the current call": "The other party's network is poor during the current call",
  "TUICallKit init is not complete": "TUICallKit init is not complete. You need to use this API after the init API is finished.",
  "Video call": "Video call",
  "Voice call": "Voice call",
  "Call End": "Call End",
  "Switch voice call": "Switch voice call",
  "Switch video call": "Switch video call",
  "Call duration": "Duration",
  "Call Cancel": "Canceled",
  "Other Side Cancel": "Call canceled by caller",
  Decline: "Declined",
  "Other Side Decline": "Call declined by user",
  "No answer": "Call not answered",
  "Other Side No Answer": "Call wasn't answered",
  Answered: "Answered",
  "Other Side Line Busy": "Line busy",
  "Line Busy": "Line busy. Call not received.",
  "Those involved": "Those involved in the call are",
  call: "call",
  "video-call": "video call",
  "audio-call": "audio call",
  search: "search",
  "search-result": "search result",
  "no-user": "user not found",
  "member-not-added": "member not added",
  "input-phone-userID": "phone number or userID",
  "not-login": "not logged in",
  "login-status-expire": "login status is invalid, please refresh the page and try again",
  "experience-multi-call": "experience multi-person calls, please download the full-featured demo: ",
  "not-support-multi-call": "multi-person call interface is not open",
  userID: "userID",
  "already-enter": "entered the call",
  "camera-opened": "Camera on",
  "camera-closed": "Camera off",
  "microphone-opened": "Mic on",
  "microphone-closed": "Mic off",
  timeout: "{{ userList }} timeout",
  "kick out": "kick out",
  "image-resolution": "Resolution",
  "default-image-resolution": "Default",
  "invited-person": "Invite",
  "be-rejected": "Call declined, ",
  "be-no-response": "No response, ",
  "be-line-busy": "Line busy, ",
  "be-canceled": "The call is canceled, ",
  "voice-call-end": "Voice call ended",
  "video-call-end": "Video call ended",
  "method-call-failed": "Failed to sync the operation",
  "failed-to-obtain-permission": "Failed to obtain permissions",
  "environment-detection-failed": "Failed to check the environment",
  "switchToAudioCall-call-failed": "switch to audio call method failed",
  "switchToVideoCall-call-failed": "switch to video call method failed",
  "microphone-unavailable": "No mic found",
  "camera-unavailable": "No camera found",
  "ban-device": "Device access denied",
  "not-supported-webrtc": "Your current environment does not support WebRTC",
  "blacklist-user-tips": "The identifier is in blacklist. Failed to send this message!",
  "is-already-calling": "TUICallKit is already on a call",
  "need-init": "Before initiating a call with TUICallKit, ensure that the TUICallKitServer.init() method has executed successfully. ",
  "can't call yourself": "Can't call yourself",
  "Use-phone-and-computer": "Use your mobile phone and computer to experience video calls",
  "Wechat scan right QR code": "Wechat scan right QR code",
  "Scan the QR code above": "Scan the QR code above",
  "accept-error": "Accept failed",
  "accept-device-error": "Accept failed, unable to auth calling device",
  "call-error": "Start call failed"
}, Ks = {
  hangup: "\u901A\u8A71\u7D42\u4E86",
  reject: "\u62D2\u5426",
  accept: "\u5FDC\u7B54",
  camera: "\u30AB\u30E1\u30E9",
  microphone: "\u30DE\u30A4\u30AF",
  speaker: "\u30B9\u30D4\u30FC\u30AB\u30FC",
  "virtual-background": "\u30DC\u30B1\u80CC\u666F",
  "other side reject call": "\u901A\u8A71\u304C\u62D2\u5426\u3055\u308C\u307E\u3057\u305F",
  "reject call": "{{ userList }} \u901A\u8A71\u62D2\u5426",
  cancel: "\u901A\u8A71\u3092\u30AD\u30E3\u30F3\u30BB\u30EB",
  "other side line busy": "\u76F8\u624B\u304C\u901A\u8A71\u4E2D\u3067\u3059",
  "in busy": "{{ userList }} \u901A\u8A71\u4E2D",
  "call timeout": "\u547C\u3073\u51FA\u3057\u30BF\u30A4\u30E0\u30A2\u30A6\u30C8",
  "end call": "{{ userList }} \u901A\u8A71\u7D42\u4E86",
  "caller calling message": "\u5FDC\u7B54\u3092\u5F85\u3063\u3066\u3044\u307E\u3059",
  "callee calling video message": "\u30D3\u30C7\u30AA\u901A\u8A71\u306B\u62DB\u5F85\u3055\u308C\u307E\u3057\u305F",
  "callee calling audio message": "\u97F3\u58F0\u901A\u8A71\u306B\u62DB\u5F85\u3055\u308C\u307E\u3057\u305F",
  "no microphone access": "\u30DE\u30A4\u30AF\u306B\u30A2\u30AF\u30BB\u30B9\u3067\u304D\u307E\u305B\u3093",
  "no camera access": "\u30AB\u30E1\u30E9\u306B\u30A2\u30AF\u30BB\u30B9\u3067\u304D\u307E\u305B\u3093",
  "invite member": "\u30E1\u30F3\u30D0\u30FC\u3092\u62DB\u5F85\u3059\u308B",
  "browser-authorization": "\u30D6\u30E9\u30A6\u30B6\u8A8D\u8A3C",
  "mac-privacy": "\u30B7\u30B9\u30C6\u30E0\u74B0\u5883\u8A2D\u5B9A -> \u30BB\u30AD\u30E5\u30EA\u30C6\u30A3\u3068\u30D7\u30E9\u30A4\u30D0\u30B7\u30FC ->\u30D7\u30E9\u30A4\u30D0\u30B7\u30FC",
  "win-privacy": "\u8A2D\u5B9A -> \u30BB\u30AD\u30E5\u30EA\u30C6\u30A3\u3068\u30D7\u30E9\u30A4\u30D0\u30B7\u30FC ->\u30A2\u30D7\u30EA\u306E\u30A2\u30AF\u30BB\u30B9\u8A31\u53EF",
  "mac-preferences": "\u30B7\u30B9\u30C6\u30E0\u74B0\u5883\u8A2D\u5B9A\u3092\u958B\u304F",
  "win-preferences": "\u30B7\u30B9\u30C6\u30E0\u8A2D\u5B9A\u3092\u958B\u304F",
  "Please enter userID": "\u30E6\u30FC\u30B6\u30FCID\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044",
  "View more": "\u3082\u3063\u3068\u898B\u308B",
  "people selected": "\u4EBA\u304C\u9078\u629E\u3055\u308C\u307E\u3057\u305F",
  "Select all": "\u3059\u3079\u3066\u9078\u629E",
  Cancel: "\u30AD\u30E3\u30F3\u30BB\u30EB",
  Done: "\u5B8C\u4E86",
  "exist group call": "\u73FE\u5728\u306E\u30B0\u30EB\u30FC\u30D7\u306B\u306F\u65E2\u306B\u30B0\u30EB\u30FC\u30D7\u901A\u8A71\u304C\u5B58\u5728\u3057\u3066\u3044\u307E\u3059",
  "Video call": "\u30D3\u30C7\u30AA\u901A\u8A71\u3092\u958B\u59CB",
  "Voice call": "\u97F3\u58F0\u901A\u8A71\u3092\u958B\u59CB",
  "Call End": "\u901A\u8A71\u7D42\u4E86",
  "Switch audio call": "\u97F3\u58F0\u901A\u8A71\u306B\u5207\u308A\u66FF\u3048\u308B",
  "Switch video call": "\u30D3\u30C7\u30AA\u901A\u8A71\u306B\u5207\u308A\u66FF\u3048\u308B",
  "Call duration": "\u901A\u8A71\u6642\u9593",
  "Call Cancel": "\u901A\u8A71\u3092\u30AD\u30E3\u30F3\u30BB\u30EB\u3059\u308B",
  "Other Side Cancel": "\u76F8\u624B\u304C\u30AD\u30E3\u30F3\u30BB\u30EB\u3057\u307E\u3057\u305F",
  Decline: "\u901A\u8A71\u3092\u62D2\u5426\u3059\u308B",
  "Other Side Decline": "\u76F8\u624B\u304C\u62D2\u5426\u3057\u307E\u3057\u305F",
  "No answer": "\u7121\u5FDC\u7B54",
  "Other Side No Answer": "\u76F8\u624B\u304B\u3089\u306E\u8FD4\u7B54\u306F\u3042\u308A\u307E\u305B\u3093",
  Answered: "\u5FDC\u7B54\u3057\u307E\u3057\u305F",
  "Other Side Line Busy": "\u76F8\u624B\u306F\u73FE\u5728\u5FD9\u3057\u304F\u3066\u5BFE\u5FDC\u3067\u304D\u307E\u305B\u3093",
  "Line Busy": "\u5FD9\u3057\u3044\u306E\u3067\u5FDC\u7B54\u3067\u304D\u307E\u305B\u3093",
  "open camera": "\u30AA\u30FC\u30D7\u30F3\u30AB\u30E1\u30E9",
  "close camera": "\u30AB\u30E1\u30E9\u3092\u9589\u3058\u308B",
  "open microphone": "\u30AA\u30FC\u30D7\u30F3\u30DE\u30A4\u30AF",
  "close microphone": "\u30DE\u30A4\u30AF\u3092\u9589\u3058\u308B",
  "camera enabled": "\u30AB\u30E1\u30E9\u30AA\u30F3",
  "camera disabled": "\u30AB\u30E1\u30E9\u30AA\u30D5",
  "microphone enabled": "\u30DE\u30A4\u30AF\u30AA\u30F3",
  "microphone disabled": "\u30DE\u30A4\u30AF\u30AA\u30D5",
  "speaker enabled": "\u30B9\u30D4\u30FC\u30AB\u30FC\u30AA\u30F3",
  "speaker disabled": "\u30B9\u30D4\u30FC\u30AB\u30FC\u30AA\u30D5",
  "open speaker": "\u30B9\u30D4\u30FC\u30AB\u30FC\u3092\u30AA\u30F3\u306B\u3059\u308B",
  "close speaker": "\u30B9\u30D4\u30FC\u30AB\u30FC\u306E\u96FB\u6E90\u3092\u5207\u308A\u307E\u3059",
  "wait to be called": "\u5F85\u6A5F\u4E2D",
  answered: "\u63A5\u7D9A\u6E08\u307F",
  "people in the call": "\u901A\u8A71\u306B\u53C2\u52A0\u3057\u3066\u3044\u308B\u4EBA\u305F\u3061",
  "failed to obtain speakers": "\u30B9\u30D4\u30FC\u30AB\u30FC\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093",
  "you have a new call": "\u65B0\u3057\u3044\u901A\u8A71\u304C\u3042\u308A\u307E\u3059",
  "switch camera": "\u5207\u308A\u66FF\u3048",
  join: "\u53C2\u52A0\u3059\u308B",
  "people on the call": "\u4EBA\u304C\u901A\u8A71\u4E2D\u3067\u3059",
  "Supports a maximum of 9 people for simultaneous calls": "\u6700\u5927\u30679\u4EBA\u307E\u3067\u540C\u6642\u901A\u8A71\u304C\u53EF\u80FD\u3067\u3059",
  you: "(\u3042\u306A\u305F)",
  "The network is poor during your current call": "\u73FE\u5728\u306E\u901A\u8A71\u3067\u3001\u3042\u306A\u305F\u306E\u30CD\u30C3\u30C8\u30EF\u30FC\u30AF\u306F\u4E0D\u826F\u3067\u3059",
  "The other user network is poor during the current call": "\u73FE\u5728\u306E\u901A\u8A71\u3067\u3001\u76F8\u624B\u5074\u306E\u30CD\u30C3\u30C8\u30EF\u30FC\u30AF\u304C\u4E0D\u826F\u3067\u3059",
  "TUICallKit init is not complete": "TUICallKit\u306E\u521D\u671F\u5316\u30ED\u30B0\u30A4\u30F3\u304C\u5B8C\u4E86\u3057\u3066\u3044\u307E\u305B\u3093\u3002init \u304C\u5B8C\u4E86\u3057\u305F\u5F8C\u306B\u3053\u306EAPI\u3092\u4F7F\u7528\u3059\u308B\u5FC5\u8981\u304C\u3042\u308A\u307E\u3059\u3002",
  timeout: "{{ userList }} \u30BF\u30A4\u30E0\u30A2\u30A6\u30C8",
  "kick out": "\u30AD\u30C3\u30AF\u30A2\u30A6\u30C8\u3055\u308C\u307E\u3057\u305F",
  "Invited group call": "\u30B0\u30EB\u30FC\u30D7\u901A\u8A71\u306B\u62DB\u5F85\u3055\u308C\u307E\u3057\u305F\u3002",
  "Those involved": "\u53C2\u52A0\u8005\uFF1A",
  call: "\u901A\u8A71",
  "video-call": "\u30D3\u30C7\u30AA\u901A\u8A71",
  "audio-call": "\u97F3\u58F0\u901A\u8A71",
  search: "\u691C\u7D22",
  "search-result": "\u691C\u7D22\u7D50\u679C",
  "Wechat scan right QR code": "WeChat\u3067\u53F3\u5074\u306B\u3042\u308BQR\u30B3\u30FC\u30C9\u3092\u8AAD\u307F\u53D6\u308A\u307E\u3059\u3002",
  "Use-phone-and-computer": "\u643A\u5E2F\u96FB\u8A71\u3068\u30B3\u30F3\u30D4\u30E5\u30FC\u30BF\u3092\u4F7F\u7528\u3057\u3066\u30D3\u30C7\u30AA\u901A\u8A71\u3092\u4F53\u9A13\u3057\u3066\u304F\u3060\u3055\u3044",
  "Scan the QR code above": "\u4E0A\u306EQR\u30B3\u30FC\u30C9\u3092\u8AAD\u307F\u53D6\u308A\u307E\u3059\u3002",
  "no-user": "\u30E6\u30FC\u30B6\u30FC\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3067\u3057\u305F",
  "member-not-added": "\u30E1\u30F3\u30D0\u30FC\u304C\u8FFD\u52A0\u3055\u308C\u3066\u3044\u307E\u305B\u3093",
  "not-login": "\u30ED\u30B0\u30A4\u30F3\u3057\u3066\u3044\u307E\u305B\u3093",
  "login-status-expire": "\u30ED\u30B0\u30A4\u30F3\u306E\u6709\u52B9\u671F\u9650\u304C\u904E\u304E\u3066\u3044\u307E\u3059\u3002\u30DA\u30FC\u30B8\u3092\u66F4\u65B0\u3057\u3066\u3082\u3046\u4E00\u5EA6\u304A\u8A66\u3057\u304F\u3060\u3055\u3044",
  "experience-multi-call": "\u8907\u6570\u4EBA\u3067\u540C\u6642\u306B\u97F3\u58F0\u901A\u8A71\u3067\u304D\u308B\u30B0\u30EB\u30FC\u30D7\u901A\u8A71\u6A5F\u80FD\u3092\u4F53\u9A13\u3059\u308B\u306B\u306F\u3001\u5168\u6A5F\u80FD\u306E\u30C7\u30E2\u3092\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9\u3057\u3066\u304F\u3060\u3055\u3044",
  "not-support-multi-call": "\u30B0\u30EB\u30FC\u30D7\u901A\u8A71\u30A4\u30F3\u30BF\u30FC\u30D5\u30A7\u30A4\u30B9\u304C\u958B\u3044\u3066\u3044\u307E\u305B\u3093",
  "input-phone-userID": "\u643A\u5E2F\u96FB\u8A71\u756A\u53F7/\u30E6\u30FC\u30B6\u30FCID\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044",
  userID: "\u30E6\u30FC\u30B6\u30FCID",
  "already-enter": "\u3059\u3067\u306B\u901A\u8A71\u306B\u53C2\u52A0\u3057\u3066\u3044\u307E\u3059",
  waiting: "\u5FDC\u7B54\u3092\u5F85\u3063\u3066\u3044\u307E\u3059...",
  "camera-opened": "\u30AB\u30E1\u30E9\u304C\u30AA\u30F3\u306B\u306A\u3063\u3066\u3044\u307E\u3059",
  "camera-closed": "\u30AB\u30E1\u30E9\u304C\u30AA\u30D5\u306B\u306A\u3063\u3066\u3044\u307E\u3059",
  "microphone-opened": "\u30DE\u30A4\u30AF\u304C\u30AA\u30F3\u306B\u306A\u3063\u3066\u3044\u307E\u3059",
  "microphone-closed": "\u30DE\u30A4\u30AF\u304C\u30AA\u30D5\u306B\u306A\u3063\u3066\u3044\u307E\u3059",
  "image-resolution": "\u89E3\u50CF\u5EA6",
  "default-image-resolution": "\u30C7\u30D5\u30A9\u30EB\u30C8\u89E3\u50CF\u5EA6",
  "invited-person": "\u30E1\u30F3\u30D0\u30FC\u3092\u62DB\u5F85",
  "video-to-audio": "\u97F3\u58F0\u901A\u8A71\u306B\u5207\u308A\u66FF\u3048\u307E\u3059",
  me: "(\u81EA\u5206)",
  "be-rejected": "\u901A\u8A71\u304C\u62D2\u5426\u3055\u308C\u307E\u3057\u305F, ",
  "be-no-response": "\u5FDC\u7B54\u306A\u3057, ",
  "be-line-busy": "\u76F8\u624B\u304C\u901A\u8A71\u4E2D\u3067\u3059, ",
  "be-canceled": "\u76F8\u624B\u304C\u901A\u8A71\u3092\u30AD\u30E3\u30F3\u30BB\u30EB\u3057\u307E\u3057\u305F",
  "voice-call-end": "\u97F3\u58F0\u901A\u8A71\u304C\u7D42\u4E86\u3057\u307E\u3057\u305F",
  "video-call-end": "\u30D3\u30C7\u30AA\u901A\u8A71\u304C\u7D42\u4E86\u3057\u307E\u3057\u305F",
  "method-call-failed": "\u64CD\u4F5C\u306E\u540C\u671F\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
  "failed-to-obtain-permission": "\u6A29\u9650\u306E\u53D6\u5F97\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
  "environment-detection-failed": "\u74B0\u5883\u306E\u691C\u51FA\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
  "switchToAudioCall-call-failed": "\u97F3\u58F0\u901A\u8A71\u306B\u5207\u308A\u66FF\u3048\u308B\u3053\u3068\u306F\u3067\u304D\u307E\u305B\u3093",
  "switchToVideoCall-call-failed": "\u30D3\u30C7\u30AA\u901A\u8A71\u306B\u5207\u308A\u66FF\u3048\u308B\u3053\u3068\u306F\u3067\u304D\u307E\u305B\u3093",
  "microphone-unavailable": "\u4F7F\u7528\u3067\u304D\u308B\u30DE\u30A4\u30AF\u304C\u3042\u308A\u307E\u305B\u3093",
  "camera-unavailable": "\u4F7F\u7528\u3067\u304D\u308B\u30AB\u30E1\u30E9\u304C\u3042\u308A\u307E\u305B\u3093",
  "ban-device": "\u30C7\u30D0\u30A4\u30B9\u3078\u306E\u30A2\u30AF\u30BB\u30B9\u304C\u62D2\u5426\u3055\u308C\u307E\u3057\u305F",
  "not-supported-webrtc": "\u73FE\u5728\u306E\u74B0\u5883\u306FWebRTC\u3092\u30B5\u30DD\u30FC\u30C8\u3057\u3066\u3044\u307E\u305B\u3093",
  "blacklist-user-tips": "\u30E6\u30FC\u30B6\u30FC\u306F\u30D6\u30E9\u30C3\u30AF\u30EA\u30B9\u30C8\u306B\u767B\u9332\u3055\u308C\u3001\u901A\u8A71\u304C\u958B\u59CB\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F",
  "is-already-calling": "TUICallKit \u306F\u3059\u3067\u306B\u901A\u8A71\u4E2D\u3067\u3059",
  "need-init": "TUICallKit\u3067\u901A\u8A71\u3092\u958B\u59CB\u3059\u308B\u524D\u306B\u3001TUICallKitServer.init() \u30E1\u30BD\u30C3\u30C9\u304C\u6B63\u5E38\u306B\u5B9F\u884C\u3055\u308C\u305F\u3053\u3068\u3092\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002",
  "can't call yourself": "\u81EA\u5206\u306B\u96FB\u8A71\u3092\u304B\u3051\u308B\u3053\u3068\u304C\u3067\u304D\u307E\u305B\u3093",
  "accept-error": "\u63A5\u7D9A\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F",
  "accept-device-error": "\u63A5\u7D9A\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002\u767A\u4FE1\u5074\u30C7\u30D0\u30A4\u30B9\u3092\u8A8D\u8A3C\u3067\u304D\u307E\u305B\u3093",
  "call-error": "\u901A\u8A71\u304C\u958B\u59CB\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F"
}, at = typeof wx < "u" && typeof wx.getSystemInfoSync == "function" && Boolean(wx.getSystemInfoSync().fontSizeSetting), io = typeof uni < "u" && typeof uni > "u", jn = at || io, lo = typeof uni < "u", uo = function() {
  return (typeof uni < "u" || typeof window < "u") && !jn;
}(), Hs = function() {
  return at ? wx : lo ? uni : window;
}(), zA = uo && window && window.navigator && window.navigator.userAgent || "", Js = /Android/i.test(zA), $s = /(?:Windows Phone)/.test(zA), Ai = /(?:SymbianOS)/.test(zA), ei = /iPad/i.test(zA) || /iPhone/i.test(zA) || /iPod/i.test(zA), ro = Js || $s || Ai || ei, Zt = uo && !ro, ti = Zt && zA.includes("Windows NT"), ni = Zt && zA.includes("Mac");
class kA {
  constructor() {
    this.global = Hs, this.isPC = !1, this.isH5 = !1, this.isWeChat = !1, this.isApp = !1, this.isUniPlatform = !1, this.isOfficial = !1, this.isWIN = !1, this.isMAC = !1, this.initEnv();
  }
  static getInstance() {
    return kA.instance || (kA.instance = new kA()), kA.instance;
  }
  initEnv() {
    this.isPC = Zt, this.isH5 = ro, this.isWeChat = at, this.isApp = io && !at, this.isUniPlatform = lo, this.isWIN = ti, this.isMAC = ni;
  }
  initOfficial(t) {
    this.isOfficial = t === 1400187352 || t === 1400188366;
  }
}
const Dt = function(A) {
  return typeof A === o.UNDEFINED;
}, Yt = function(A) {
  if (typeof A !== o.OBJECT || A === null)
    return !1;
  const t = Object.getPrototypeOf(A);
  if (t === null)
    return !0;
  let e = t;
  for (; Object.getPrototypeOf(e) !== null; )
    e = Object.getPrototypeOf(e);
  return t === e;
}, co = function(A) {
  return typeof Array.isArray === o.FUNCTION ? Array.isArray(A) : Object.prototype.toString.call(A).match(/^\[object (.*)\]$/)[1].toLowerCase() === o.ARRAY;
}, ot = function(A) {
  return typeof A === o.STRING;
}, ai = function(A) {
  return typeof A === o.BOOLEAN;
}, go = function(A) {
  return A !== null && (typeof A === o.NUMBER && !isNaN(A - 0) || typeof A === o.OBJECT && A.constructor === Number);
};
function Io(A) {
  const t = Math.floor(A / 3600), e = Math.floor(A % 3600 / 60), n = Math.floor(A % 60);
  let a = t > 9 ? `${t}` : `0${t}`;
  return a += e > 9 ? `:${e}` : `:0${e}`, a += n > 9 ? `:${n}` : `:0${n}`, a;
}
function oi(A) {
  if (typeof A === o.STRING)
    try {
      return !!JSON.parse(A);
    } catch (t) {
      return console.debug(t), !1;
    }
  return !1;
}
const Qe = function(A) {
  return !A || !oi(A) ? A : JSON.parse(A);
};
function bn(A) {
  return (A == null ? void 0 : A.message.indexOf("is ongoing, please avoid repeated calls")) !== -1;
}
function si(A) {
  const { message: t } = A;
  return t.indexOf("NotAllowedError: Permission denied") !== -1;
}
function Ot() {
  return Date.now();
}
const At = function(A) {
  return typeof A === o.FUNCTION;
}, ii = () => {
  if (kA.getInstance().isWeChat)
    return "zh-cn";
  const A = ((navigator == null ? void 0 : navigator.language) || (navigator == null ? void 0 : navigator.userLanguage) || "").substr(0, 2);
  let t = "en";
  switch (A) {
    case "zh":
      t = "zh-cn";
      break;
    case "ja":
      t = "ja_JP";
      break;
    default:
      t = "en";
  }
  return t;
};
function li(A) {
}
const ui = function(A) {
  return Object.prototype.toString.call(A).match(/^\[object (.*)\]$/)[1].toLowerCase();
};
function ri(A, t, e) {
  if (!A.hasOwnProperty(t))
    return A;
  const n = {};
  return Object.keys(A).forEach((a) => {
    a === t ? n[e] = A[a] : n[a] = A[a];
  }), n;
}
function ci(A, t) {
  return A.replace(/{{\s*(\w+)(\s*,\s*[^}]+)?\s*}}/g, (e, n) => {
    const a = n.trim();
    return t[a] !== void 0 ? String(t[a]) : e;
  });
}
const lA = {
  OTHER_SIDE: "other side",
  CANCEL: "cancel",
  OTHER_SIDE_REJECT_CALL: "other side reject call",
  REJECT_CALL: "reject call",
  OTHER_SIDE_LINE_BUSY: "other side line busy",
  IN_BUSY: "in busy",
  CALL_TIMEOUT: "call timeout",
  END_CALL: "end call",
  TIMEOUT: "timeout",
  KICK_OUT: "kick out",
  CALLER_CALLING_MSG: "caller calling message",
  CALLER_GROUP_CALLING_MSG: "wait to be called",
  CALLEE_CALLING_VIDEO_MSG: "callee calling video message",
  CALLEE_CALLING_AUDIO_MSG: "callee calling audio message",
  NO_MICROPHONE_DEVICE_PERMISSION: "no microphone access",
  NO_CAMERA_DEVICE_PERMISSION: "no camera access",
  EXIST_GROUP_CALL: "exist group call",
  LOCAL_NETWORK_IS_POOR: "The network is poor during your current call",
  REMOTE_NETWORK_IS_POOR: "The other user network is poor during the current call"
}, YA = {
  "zh-cn": Zs,
  en: Ys,
  ja_JP: Ks
};
function WA(A) {
  var n, a;
  const t = d.getData(r.CALL, o.LANGUAGE);
  let e = "";
  if (ot(A))
    e = ((n = YA == null ? void 0 : YA[t]) == null ? void 0 : n[A]) || "";
  else if (Yt(A)) {
    const { key: s, options: i } = A;
    e = ((a = YA == null ? void 0 : YA[t]) == null ? void 0 : a[s]) || "", e = ci(e, i);
  }
  return e;
}
const di = "data:audio/mpeg;base64,", zn = "data:audio/mpeg;base64,";
class gi {
  constructor() {
    this._bellContext = null, this._isMuteBell = !1, this._calleeBellFilePath = zn, this._callRole = H.UNKNOWN, this._callStatus = w.IDLE, this._bellContext = new Audio(), this._bellContext.loop = !0;
  }
  setBellSrc() {
    try {
      let t = di;
      this._callRole === H.CALLEE && (t = this._calleeBellFilePath || zn), this._bellContext.src = t;
    } catch (t) {
      console.warn(`${o.PREFIX}Failed to setBellSrc, ${t}`);
    }
  }
  setBellProperties(t) {
    this._callRole = t.callRole || this._callRole, this._callStatus = t.callStatus || this._callStatus, this._calleeBellFilePath = t.calleeBellFilePath || this._calleeBellFilePath, this._isMuteBell = Dt(t.isMuteBell) ? this._isMuteBell : t.isMuteBell;
  }
  async play() {
    try {
      if (this._callStatus !== w.CALLING)
        return;
      this.setBellSrc(), this._callRole === H.CALLEE && !this._isMuteBell && await this._bellContext.play(), this._callRole === H.CALLER && await this._bellContext.play();
    } catch (t) {
      console.warn(`${o.PREFIX}Failed to play audio file, ${t}`);
    }
  }
  async stop() {
    try {
      await this._bellContext.pause();
    } catch (t) {
      console.warn(`${o.PREFIX}Failed to stop audio file, ${t}`);
    }
  }
  async setBellMute(t) {
    this._callStatus !== w.CALLING && this._callRole !== H.CALLEE || (t ? await this.stop() : await this.play());
  }
  destroy() {
    try {
      this._isMuteBell = !1, this._calleeBellFilePath = "", this._callRole = H.UNKNOWN, this._callStatus = w.IDLE, this._bellContext.pause(), this._bellContext = null;
    } catch (t) {
      console.warn(`${o.PREFIX}Failed to destroy, ${t}`);
    }
  }
}
function K() {
  return function(A, t, e) {
    const n = e.value, a = /* @__PURE__ */ new Set();
    return e.value = async function(...s) {
      var i, u;
      if (a.has(this)) {
        console.warn(`${o.PREFIX}previous ${t}() is ongoing, please avoid repeated calls`), (u = (i = this == null ? void 0 : this.getTUICallEngineInstance()) == null ? void 0 : i.reportLog) == null || u.call(i, {
          name: "TUICallKit.avoidRepeatedCall.fail",
          data: { name: t },
          error: `previous ${t}() is ongoing`
        });
        return;
      }
      try {
        a.add(this);
        const l = await n.apply(this, s);
        return a.delete(this), l;
      } catch (l) {
        throw a.delete(this), l;
      }
    }, e;
  };
}
const te = o.PREFIX + "API";
function gA(A) {
  return function(t, e, n) {
    let a = n.value;
    return n.value = function(...s) {
      return Ii.call(this, A, s, e), a.apply(this, s);
    }, n;
  };
}
function Ii(A, t, e) {
  try {
    if (t[0].SDKAppID || (A = ri(A, "SDKAppID", "sdkAppID")), co(A))
      for (let n = 0; n < A.length; n++)
        Un.call(this, {
          ...A[n],
          value: t[n],
          name: e
        });
    else
      for (const n in A)
        A.hasOwnProperty(n) && Un.call(this, {
          ...A[n],
          value: t[0][n],
          name: e,
          key: n
        });
  } catch (n) {
    throw console.error(n), n;
  }
}
function Un({ required: A, rules: t, range: e, value: n, allowEmpty: a, name: s, key: i }) {
  if (Dt(n)) {
    if (A)
      throw new Error(`${te}<${s}>: ${i} is required.`);
    return;
  }
  const u = t.some((c) => c === ui(n));
  let l = "";
  if (!u) {
    for (let c = 0; c < t.length; c++) {
      let g = t[c];
      g = g.replace(g[0], g[0].toUpperCase()), l += `${g}/`;
    }
    throw l = l.substring(0, l.length - 1), new Error(`${te}<${s}>: ${i} must be ${l}, current ${i} is ${typeof n}.`);
  }
  if (a === !1 && ot(n) && n.trim() === "")
    throw new Error(`${te}<${s}>: ${i} is blank.`);
  if (co(e) && e && e.indexOf(n) === -1)
    throw new Error(`${te}<${s}>: ${i} error, only be ${e}, current ${i} is ${n}.`);
  if (ot(e) && e.indexOf("~") !== -1) {
    const c = e.split("~");
    if (n < +c[0] || n > +c[1] || go(n) && Number.isNaN(n))
      throw new Error(`${te}<${s}>: ${i} error, only be ${e}, current ${i} is ${n}.`);
  }
}
const IA = {
  init: {
    SDKAppID: {
      required: !0,
      rules: [o.NUMBER],
      allowEmpty: !1
    },
    userID: {
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    },
    userSig: {
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    },
    tim: {
      required: !1,
      rules: [o.OBJECT]
    }
  },
  call: {
    userID: {
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    },
    type: {
      required: !0,
      rules: [o.NUMBER],
      range: [1, 2],
      allowEmpty: !1
    },
    roomID: {
      required: !1,
      rules: [o.NUMBER],
      range: `0~${wn}`,
      allowEmpty: !1
    },
    strRoomID: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !0
    },
    userData: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !1
    },
    timeout: {
      required: !1,
      rules: [o.NUMBER],
      allowEmpty: !1
    }
  },
  groupCall: {
    userIDList: {
      required: !0,
      rules: [o.ARRAY],
      allowEmpty: !1
    },
    type: {
      required: !0,
      rules: [o.NUMBER],
      range: [1, 2],
      allowEmpty: !1
    },
    groupID: {
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    },
    roomID: {
      required: !1,
      rules: [o.NUMBER],
      range: `0~${wn}`,
      allowEmpty: !1
    },
    strRoomID: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !0
    },
    timeout: {
      required: !1,
      rules: [o.NUMBER],
      allowEmpty: !1
    },
    userData: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !1
    },
    offlinePushInfo: {
      required: !1,
      rules: [o.OBJECT],
      allowEmpty: !1
    }
  },
  joinInGroupCall: {
    type: {
      required: !0,
      rules: [o.NUMBER],
      range: [1, 2],
      allowEmpty: !1
    },
    groupID: {
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    },
    roomID: {
      required: !0,
      rules: [o.NUMBER],
      allowEmpty: !1
    },
    strRoomID: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !0
    }
  },
  inviteUser: {
    userIDList: {
      required: !0,
      rules: [o.ARRAY],
      allowEmpty: !1
    }
  },
  setSelfInfo: {
    nickName: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !1
    },
    avatar: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !1
    }
  },
  enableFloatWindow: [
    {
      key: "enable",
      required: !1,
      rules: [o.BOOLEAN],
      allowEmpty: !1
    }
  ],
  enableAIVoice: [
    {
      key: "enable",
      required: !0,
      rules: [o.BOOLEAN],
      allowEmpty: !1
    }
  ],
  enableMuteMode: [
    {
      key: "enable",
      required: !0,
      rules: [o.BOOLEAN],
      allowEmpty: !1
    }
  ],
  setCallingBell: [
    {
      key: "filePath",
      required: !1,
      rules: [o.STRING],
      allowEmpty: !0
    }
  ],
  setLanguage: [
    {
      key: "language",
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    }
  ],
  setVideoDisplayMode: [
    {
      key: "displayMode",
      required: !0,
      rules: [o.STRING],
      range: [ie.CONTAIN, ie.COVER, ie.FILL],
      allowEmpty: !1
    }
  ],
  setVideoResolution: [
    {
      key: "resolution",
      required: !0,
      rules: [o.STRING],
      range: [le.RESOLUTION_1080P, le.RESOLUTION_480P, le.RESOLUTION_720P],
      allowEmpty: !1
    }
  ]
};
function Ae(A) {
  return function(t, e, n) {
    let a = n.value;
    return n.value = function(...s) {
      return pi.call(this, A, s, e), a.apply(this, s);
    }, n;
  };
}
function pi(A) {
  if ((A == null ? void 0 : A.engineInstance) && !this._tuiCallEngine) {
    const t = `${o.PREFIX} ${WA("TUICallKit init is not complete")}`;
    throw console.error(t), t;
  }
}
class Mi {
  constructor() {
    this.defaultStore = {
      callStatus: w.IDLE,
      callRole: H.UNKNOWN,
      callMediaType: V.UNKNOWN,
      localUserInfo: { userId: "" },
      localUserInfoExcludeVolume: { userId: "" },
      remoteUserInfoList: [],
      remoteUserInfoExcludeVolumeList: [],
      callerUserInfo: { userId: "" },
      isGroup: !1,
      callDuration: "00:00:00",
      callTips: "",
      toastInfo: { text: "" },
      isMinimized: !1,
      enableFloatWindow: !1,
      bigScreenUserId: "",
      language: ii(),
      isClickable: !1,
      deviceList: { cameraList: [], microphoneList: [], currentCamera: {}, currentMicrophone: {} },
      showPermissionTip: !1,
      netWorkQualityList: [],
      isMuteSpeaker: !1,
      groupID: "",
      roomID: 0,
      roomIdType: 0,
      cameraPosition: FA.FRONT,
      groupCallMembers: [],
      displayMode: ie.COVER,
      videoResolution: le.RESOLUTION_480P,
      showSelectUser: !1,
      pusher: {},
      player: [],
      isEarPhone: !1,
      pusherId: o.INITIAL_PUSHER,
      isShowEnableVirtualBackground: !1,
      enableVirtualBackground: !1,
      customUIConfig: {
        button: {},
        viewBackground: {},
        layoutMode: tt.RemoteInLargeView
      },
      translate: WA
    }, this.store = nt(this.defaultStore), this.prevStore = nt(this.defaultStore);
  }
  update(t, e) {
    switch (t) {
      case o.CALL_TIPS:
        const n = this.getData(t);
        this.prevStore[t] = n;
      default:
        this.store[t] = e;
    }
  }
  getPrevData(t) {
    return t ? this.prevStore[t] : this.prevStore;
  }
  getData(t) {
    return t ? this.store[t] : this.store;
  }
  reset(t = []) {
    t.length === 0 && (t = Object.keys(this.store));
    const e = t.reduce((n, a) => ({ ...n, [a]: this.defaultStore[a] }), {});
    this.store = {
      ...this.defaultStore,
      ...this.store,
      ...e
    };
  }
}
class hA {
  constructor() {
    this.timerId = -1, this.storeMap = {
      [r.CALL]: new Mi()
    }, this.task = {};
  }
  static getInstance() {
    return hA.instance || (hA.instance = new hA()), hA.instance;
  }
  watch(t, e, n) {
    this.task[t] || (this.task[t] = {});
    const a = this.task[t];
    Object.keys(e).forEach((s) => {
      const i = e[s];
      a[s] || (a[s] = /* @__PURE__ */ new Map()), a[s].set(i, 1);
      const { notifyRangeWhenWatch: u } = n || {};
      if (u === o.ALL && this.notify(t, s), u === o.MYSELF) {
        const l = this.getData(t, s);
        i.call(this, l);
      }
    });
  }
  unwatch(t, e) {
    if (!this.task[t])
      return;
    const n = this.task[t];
    Object.keys(e).forEach((a) => {
      n[a].delete(e[a]);
    });
  }
  update(t, e, n) {
    var a;
    (ot(n) || go(n) || ai(n)) && this.storeMap[t].store[e] === n || ((a = this.storeMap[t]) == null || a.update(e, n), this.notify(t, e));
  }
  getPrevData(t, e) {
    var n;
    return (n = this.storeMap[t]) == null ? void 0 : n.getPrevData(e);
  }
  getData(t, e) {
    var n;
    return (n = this.storeMap[t]) == null ? void 0 : n.getData(e);
  }
  notify(t, e) {
    if (!this.task[t])
      return;
    const n = this.task[t];
    if (n[e]) {
      const a = n[e], s = this.getData(t, e);
      for (const [i] of a.entries())
        i.call(this, s);
    }
  }
  reset(t, e = [], n = !1) {
    if (t in this.storeMap) {
      const a = this.storeMap[t];
      e.length === 0 && (e = Object.keys(a == null ? void 0 : a.store)), a.reset(e), n && e.forEach((s) => {
        this.notify(t, s);
      });
    }
  }
  updateStore(t, e) {
    const n = e || r.CALL;
    Object.keys(t).forEach((a) => {
      this.update(n, a, t[a]);
    });
  }
}
const k = hA.getInstance();
function Gt(A, t) {
  const e = {
    userId: A,
    nick: "",
    avatar: "",
    remark: "",
    displayUserInfo: "",
    isAudioAvailable: !1,
    isVideoAvailable: !1,
    isEnter: !1,
    domId: t || A
  };
  return t ? e : { ...e, isEnter: !1 };
}
async function Ci(A, t) {
  var n, a, s, i, u;
  let e = Gt(A, o.LOCAL_VIDEO);
  try {
    if (!t)
      return e;
    const l = await t.getMyProfile(), c = k == null ? void 0 : k.getData(r.CALL, o.LOCAL_USER_INFO);
    return (l == null ? void 0 : l.code) === 0 && (e = {
      ...e,
      ...c,
      userId: (n = l == null ? void 0 : l.data) == null ? void 0 : n.userID,
      nick: (a = l == null ? void 0 : l.data) == null ? void 0 : a.nick,
      avatar: (s = l == null ? void 0 : l.data) == null ? void 0 : s.avatar,
      displayUserInfo: ((i = l == null ? void 0 : l.data) == null ? void 0 : i.nick) || ((u = l == null ? void 0 : l.data) == null ? void 0 : u.userID)
    }), e;
  } catch (l) {
    return console.error(`${o.PREFIX}getMyProfile failed, error: ${l}.`), e;
  }
}
async function ue(A, t) {
  let e = A.map((n) => Gt(n));
  try {
    if (!t)
      return e;
    const n = await t.getFriendProfile({ userIDList: A });
    if (n.code === 0) {
      const { friendList: a = [], failureUserIDList: s = [] } = n.data;
      let i = s.map((g) => g.userID);
      if (s.length > 0) {
        const g = await t.getUserProfile({ userIDList: s.map((N) => N.userID) });
        (g == null ? void 0 : g.code) === 0 && (i = (g == null ? void 0 : g.data) || []);
      }
      const u = k == null ? void 0 : k.getData(r.CALL, o.REMOTE_USER_INFO_LIST), l = a.map((g) => g.userID), c = i.map((g) => g.userID);
      e = A.map((g) => {
        var x, m, P, W, eA, b, _;
        const N = Gt(g), p = l.indexOf(g), M = c.indexOf(g);
        let f = "", v = "", S = "", h = "";
        p !== -1 && (f = ((x = a[p]) == null ? void 0 : x.remark) || "", v = ((P = (m = a[p]) == null ? void 0 : m.profile) == null ? void 0 : P.nick) || "", S = f || v || N.userId || "", h = ((eA = (W = a[p]) == null ? void 0 : W.profile) == null ? void 0 : eA.avatar) || ""), M !== -1 && (v = ((b = i[M]) == null ? void 0 : b.nick) || "", S = v || N.userId || "", h = ((_ = i[M]) == null ? void 0 : _.avatar) || "");
        const E = u.find((Q) => Q.userId === g) || {};
        return { ...N, ...E, remark: f, nick: v, displayUserInfo: S, avatar: h };
      });
    }
    return e;
  } catch (n) {
    return console.error(`${o.PREFIX}getRemoteUserProfile failed, error: ${n}.`), e;
  }
}
function Ze() {
  const A = k.getData(r.CALL, o.CALL_STATUS);
  if (A === w.IDLE)
    return aA.IDLE;
  const t = k.getData(r.CALL, o.IS_GROUP);
  if (A === w.CALLING)
    return t ? aA.DIALING_GROUP : aA.DIALING_C2C;
  const e = k.getData(r.CALL, o.CALL_MEDIA_TYPE);
  return t ? e === V.AUDIO ? aA.CALLING_GROUP_AUDIO : aA.CALLING_GROUP_VIDEO : e === V.AUDIO ? aA.CALLING_C2C_AUDIO : aA.CALLING_C2C_VIDEO;
}
async function Ni(A, t, e, n) {
  let a = [];
  try {
    const s = await t.getGroupMemberList({ groupID: A, count: e, offset: n });
    if (s.code === 0)
      return s.data.memberList || a;
  } catch (s) {
    return console.error(`${o.PREFIX}getGroupMember failed, error: ${s}.`), a;
  }
}
async function fi(A, t) {
  let e = {};
  try {
    return (await t.getGroupProfile({ groupID: A })).data.group || e;
  } catch (n) {
    return console.warn(`${o.PREFIX}getGroupProfile failed, error: ${n}.`), e;
  }
}
function po(A, t) {
  A === 0 && t ? (k.update(r.CALL, o.ROOM_ID, t), k.update(r.CALL, o.ROOM_ID_TYPE, Rt.STRING_ROOM_ID)) : (k.update(r.CALL, o.ROOM_ID, A), k.update(r.CALL, o.ROOM_ID_TYPE, Rt.NUMBER_ROOM_ID));
}
function NA(A) {
  return A || {};
}
function xt(A) {
  if (A.length === 0)
    return;
  let t = k.getData(r.CALL, o.REMOTE_USER_INFO_LIST);
  A.forEach((e) => {
    t = t.filter((n) => n.userId !== e);
  }), k.update(r.CALL, o.REMOTE_USER_INFO_LIST, t), k.update(r.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, t);
}
function Ye(A) {
  A == null || A.getDeviceList("speaker").then((e) => {
    const n = k.getData(r.CALL, o.DEVICE_LIST), a = (e == null ? void 0 : e[0]) || {};
    k.update(
      r.CALL,
      o.DEVICE_LIST,
      { ...n, speakerList: e, currentSpeaker: a }
    );
  }).catch((e) => {
    console.error(`${o.PREFIX}updateSpeakerList failed, error: ${JSON.stringify(e)}.`);
  }), k.getData(r.CALL, o.CALL_MEDIA_TYPE) === V.VIDEO && (A == null || A.getDeviceList("camera").then((e) => {
    const n = k.getData(r.CALL, o.DEVICE_LIST), a = (e == null ? void 0 : e[0]) || {};
    k.update(
      r.CALL,
      o.DEVICE_LIST,
      { ...n, cameraList: e, currentCamera: a }
    );
  }).catch((e) => {
    console.error(`${o.PREFIX}updateCameraList failed, error: ${e}.`);
  })), A == null || A.getDeviceList("microphones").then((e) => {
    const n = k.getData(r.CALL, o.DEVICE_LIST), a = (e == null ? void 0 : e[0]) || {};
    k.update(
      r.CALL,
      o.DEVICE_LIST,
      { ...n, microphoneList: e, currentMicrophone: a }
    );
  }).catch((e) => {
    console.error(`${o.PREFIX}updateMicrophoneList failed, error: ${e}.`);
  });
}
function Bt(A, t, e) {
  let n = "";
  if (si(A)) {
    t === V.AUDIO && (n = lA.NO_MICROPHONE_DEVICE_PERMISSION), t === V.VIDEO && (n = lA.NO_CAMERA_DEVICE_PERMISSION);
    const a = e == null ? void 0 : e.getDevicePermission();
    a != null && a.audio || k.update(r.CALL, o.SHOW_PERMISSION_TIP, !0), n && k.update(r.CALL, o.TOAST_INFO, { content: n, type: o.ERROR }), console.error(`${o.PREFIX}call failed, error: ${A.message}.`);
  }
}
function DA(A, t) {
  let e = k.getData(r.CALL, o.LOCAL_USER_INFO);
  t === o.AUDIO && (e = { ...e, isAudioAvailable: A }), t === o.VIDEO && (e = { ...e, isVideoAvailable: A }), k.update(r.CALL, o.LOCAL_USER_INFO, e), k.update(r.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, e);
}
class st {
  static generateTaskID() {
    return this.currentTaskID++;
  }
  static run(t = o.TIMEOUT, e, n) {
    t === o.INTERVAL ? n = { delay: 2e3, count: 0, backgroundTask: !0, ...n } : n = { delay: 2e3, count: 0, backgroundTask: !0, ...n }, Yt(e) && (n = { ...n, ...e }), At(t) && (e = t, t = o.TIMEOUT);
    const a = {
      taskID: this.generateTaskID(),
      loopCount: 0,
      intervalID: null,
      timeoutID: null,
      taskName: t,
      callback: e,
      ...n
    };
    return this.taskMap.set(a.taskID, a), t === o.INTERNAL ? this.interval(a) : this.timeout(a), a.taskID;
  }
  static interval(t) {
    const e = () => {
      t.callback(), t.loopCount += 1, this.isBreakLoop(t);
    };
    return t.intervalID = setInterval(e, t.delay);
  }
  static timeout(t) {
    const e = () => {
      if (t.callback(), t.loopCount += 1, !this.isBreakLoop(t))
        return t.timeoutID = setTimeout(e, t.delay);
    };
    return t.timeoutID = setTimeout(e, t.delay);
  }
  static hasTask(t) {
    return this.taskMap.has(t);
  }
  static clearTask(t) {
    if (!this.taskMap.has(t))
      return !0;
    const { intervalID: e, timeoutID: n, onVisibilitychange: a } = this.taskMap.get(t);
    return e && clearInterval(e), n && clearTimeout(n), a && document.removeEventListener("visibilitychange", a), this.taskMap.delete(t), !0;
  }
  static isBreakLoop(t) {
    return this.taskMap.has(t.taskID) ? t.count !== 0 && t.loopCount >= t.count ? (this.clearTask(t.taskID), !0) : !1 : !0;
  }
}
st.taskMap = /* @__PURE__ */ new Map();
st.currentTaskID = 1;
const vi = function(A) {
  if (A === null || typeof A > "u")
    return !0;
  if (typeof A == "boolean")
    return !1;
  if (typeof A == "number")
    return A === 0;
  if (typeof A == "string" || typeof A == "function" || Array.isArray(A))
    return A.length === 0;
  if (A instanceof Error)
    return A.message === "";
  if (Yt(A)) {
    for (const t in A)
      if (Object.prototype.hasOwnProperty.call(A, t))
        return !1;
    return !0;
  }
  return !1;
}, Vt = "_local_user_id";
class _A {
  constructor() {
    this._viewConfig = {
      viewBackground: {
        local: {},
        remote: {}
      }
    }, this._isSetViewBackgroundConfig = { remote: !1, local: !1 }, this._tuiCallEngine = null, this._tuiStore = null;
  }
  static getInstance() {
    return _A.instance || (_A.instance = new _A()), _A.instance;
  }
  _updateViewBackground() {
    var n, a, s;
    const t = (n = this._tuiStore) == null ? void 0 : n.getData(r.CALL, o.CUSTOM_UI_CONFIG), { userId: e } = (a = this._tuiStore) == null ? void 0 : a.getData(r.CALL, o.LOCAL_USER_INFO);
    Object.keys(this._viewConfig.viewBackground.remote).includes(e) && delete this._viewConfig.viewBackground.remote[e], (s = this._tuiStore) == null || s.update(
      r.CALL,
      o.CUSTOM_UI_CONFIG,
      {
        ...t,
        viewBackground: {
          ...this._viewConfig.viewBackground.remote,
          ...this._viewConfig.viewBackground.local
        }
      }
    );
  }
  setEngineInstance(t) {
    this._tuiCallEngine = t;
  }
  setTUIStore(t) {
    this._tuiStore = t;
  }
  updateViewBackgroundUserId(t) {
    var e, n;
    if (t === "local") {
      const { userId: a } = (e = this._tuiStore) == null ? void 0 : e.getData(r.CALL, o.LOCAL_USER_INFO);
      if (Object.keys(this._viewConfig.viewBackground.remote).includes(a) && (delete this._viewConfig.viewBackground.remote[a], this._updateViewBackground()), !this._isSetViewBackgroundConfig.local)
        return;
      const s = this._viewConfig.viewBackground.local, i = s[a] || s[Vt];
      s[a] = s[Vt], this._viewConfig.viewBackground.local = { [a]: i }, this._updateViewBackground();
    } else {
      let a = this._viewConfig.viewBackground.remote;
      this._isSetViewBackgroundConfig.remote && Object.keys(a).includes("*") && (((n = this._tuiStore) == null ? void 0 : n.getData(r.CALL, o.REMOTE_USER_INFO_LIST)).map((u) => u.userId).forEach((u) => {
        Object.keys(a).includes(u) || (a[u] = a["*"]);
      }), this._viewConfig.viewBackground.remote = a, this._updateViewBackground());
    }
  }
  hideFeatureButton(t) {
    var n, a, s, i, u;
    (a = (n = this._tuiCallEngine) == null ? void 0 : n.reportLog) == null || a.call(n, {
      name: "TUICallKit.hideFeatureButton.start",
      data: { buttonName: t }
    });
    const e = (s = this._tuiStore) == null ? void 0 : s.getData(r.CALL, o.CUSTOM_UI_CONFIG);
    (u = this._tuiStore) == null || u.update(
      r.CALL,
      o.CUSTOM_UI_CONFIG,
      {
        ...e,
        button: {
          ...e.button,
          [t]: { ...((i = e.button) == null ? void 0 : i[t]) || {}, show: !1 }
        }
      }
    );
  }
  setLocalViewBackgroundImage(t) {
    var n, a, s;
    (a = (n = this._tuiCallEngine) == null ? void 0 : n.reportLog) == null || a.call(n, {
      name: "TUICallKit.setLocalViewBackgroundImage.start",
      data: { url: t }
    }), this._isSetViewBackgroundConfig.local = !0;
    let { userId: e } = (s = this._tuiStore) == null ? void 0 : s.getData(r.CALL, o.LOCAL_USER_INFO);
    vi(e) && (e = Vt), this._viewConfig.viewBackground.local = { [e]: t }, this._updateViewBackground();
  }
  setRemoteViewBackgroundImage(t, e) {
    var n, a;
    (a = (n = this._tuiCallEngine) == null ? void 0 : n.reportLog) == null || a.call(n, {
      name: "TUICallKit.setRemoteViewBackgroundImage.start",
      data: { userId: t, url: e }
    }), this._isSetViewBackgroundConfig.remote = !0, t === "*" && (this._viewConfig.viewBackground.remote = {}), this._viewConfig.viewBackground.remote[t] = e, this._updateViewBackground();
  }
  setLayoutMode(t) {
    var n, a;
    (a = (n = this._tuiCallEngine) == null ? void 0 : n.reportLog) == null || a.call(n, {
      name: "TUICallKit.setLayoutMode.start",
      data: { layoutMode: t }
    });
    const e = this._tuiStore.getData(r.CALL, o.CUSTOM_UI_CONFIG);
    this._tuiStore.update(
      r.CALL,
      o.CUSTOM_UI_CONFIG,
      {
        ...e,
        layoutMode: t
      }
    );
  }
  setCameraDefaultState(t) {
    var n, a;
    (a = (n = this._tuiCallEngine) == null ? void 0 : n.reportLog) == null || a.call(n, {
      name: "TUICallKit.setCameraDefaultState.start",
      data: { isOpen: t }
    });
    const e = nt(this._tuiStore.getData(r.CALL, o.CUSTOM_UI_CONFIG));
    Object.keys(e.button).includes(LA.Camera) || (e.button[LA.Camera] = {}), e.button[LA.Camera].state = t ? GA.Open : GA.Close, this._tuiStore.update(r.CALL, o.CUSTOM_UI_CONFIG, e);
  }
}
const Ke = hA.getInstance(), fA = {
  audioCall: () => "Voice call",
  videoCall: () => "Video call",
  switchToAudio: () => "Switch audio call",
  switchToVideo: () => "Switch video call",
  hangup: ({ callDuration: A }) => `${WA("Call duration")}\uFF1A${A}`
};
class JA {
  constructor(t) {
    var e, n, a;
    this._callService = t.callService, ee.registerEvent($.TUILogin.EVENT.LOGIN_STATE_CHANGED, $.TUILogin.EVENT_SUB_KEY.USER_LOGIN_SUCCESS, this), (e = $.TUIChat) != null && e.EVENT && ee.registerEvent((n = $.TUIChat.EVENT) == null ? void 0 : n.CHAT_STATE_CHANGED, (a = $.TUIChat.EVENT_SUB_KEY) == null ? void 0 : a.CHAT_OPENED, this), ee.registerService($.TUICalling.SERVICE.NAME, this), ee.registerExtension($.TUIChat.EXTENSION.INPUT_MORE.EXT_ID, this);
  }
  static getInstance(t) {
    return JA.instance || (JA.instance = new JA(t)), JA.instance;
  }
  callTUIService(t) {
    const { message: e } = t || {};
    ee.callService({
      serviceName: $.TUIChat.SERVICE.NAME,
      method: $.TUIChat.SERVICE.METHOD.UPDATE_MESSAGE_LIST,
      params: { message: e }
    });
  }
  onGetExtension(t, e) {
    var n, a;
    if (t === $.TUIChat.EXTENSION.INPUT_MORE.EXT_ID) {
      if ((a = (n = this._callService.getTUICallEngineInstance()) == null ? void 0 : n.reportLog) == null || a.call(n, { name: "TUICallKit.onGetExtension", data: { extensionID: t, params: e } }), Dt(e))
        return [];
      if ([$.TUIChat.TYPE.ROOM, $.TUIChat.TYPE.CUSTOMER_SERVICE].includes(e.chatType))
        return [];
      let s = [];
      const i = {
        weight: 1e3,
        text: "\u8BED\u97F3\u901A\u8BDD",
        icon: _s,
        data: {
          name: "voiceCall"
        },
        listener: {
          onClicked: async (l) => await this._handleTUICoreOnClick(l, l.type || V.AUDIO)
        }
      }, u = {
        weight: 900,
        text: "\u89C6\u9891\u901A\u8BDD",
        icon: Ws,
        data: {
          name: "videoCall"
        },
        listener: {
          onClicked: async (l) => await this._handleTUICoreOnClick(l, l.type || V.VIDEO)
        }
      };
      return e != null && e.chatType ? s = [i, u] : (!(e != null && e.filterVoice) && s.push(i), !(e != null && e.filterVideo) && s.push(u)), s;
    }
  }
  async onCall(t, e) {
    t === $.TUICalling.SERVICE.METHOD.START_CALL && await this._handleTUICoreOnClick(e, e.type);
  }
  async onNotifyEvent(t, e, n) {
    var a, s, i, u, l, c, g, N;
    try {
      if (t === $.TUILogin.EVENT.LOGIN_STATE_CHANGED)
        if (e === $.TUILogin.EVENT_SUB_KEY.USER_LOGIN_SUCCESS) {
          const { chat: p, userID: M, userSig: f, SDKAppID: v } = Vs.getContext();
          await ((a = this._callService) == null ? void 0 : a.init({ tim: p, userID: M, userSig: f, sdkAppID: v, isFromChat: !0, component: Qt.TIM_CALL_KIT })), (s = this._callService) == null || s.setIsFromChat(!0), (i = this._callService) == null || i.setLogLevel(so.NORMAL), this._addListenChatEvent();
        } else
          e === $.TUILogin.EVENT_SUB_KEY.USER_LOGOUT_SUCCESS && (this._removeListenChatEvent(), await ((u = this._callService) == null ? void 0 : u.destroyed()));
      if (((l = $.TUIChat) == null ? void 0 : l.EVENT) && t === $.TUIChat.EVENT.CHAT_STATE_CHANGED && e === $.TUIChat.EVENT_SUB_KEY.CHAT_OPENED) {
        if ((c = this._callService) == null || c.setCurrentGroupId((n == null ? void 0 : n.groupID) || ""), Ke.getData(r.CALL, o.CALL_STATUS) !== w.IDLE)
          return;
        const p = (g = this._callService) == null ? void 0 : g.getCurrentGroupId(), M = p ? await this.getGroupAttributes((N = this._callService) == null ? void 0 : N.getTim(), p) : {};
        await this.updateStoreBasedOnGroupAttributes(M);
      }
    } catch (p) {
      console.error(`${o.PREFIX}TUICore onNotifyEvent failed, error: ${p}.`);
    }
  }
  async updateStoreBasedOnGroupAttributes(t) {
    var e, n, a, s;
    (a = (n = (e = this._callService) == null ? void 0 : e.getTUICallEngineInstance()) == null ? void 0 : n.reportLog) == null || a.call(n, {
      name: "TUICallKit.getJoinGroupCallInfo.success",
      data: { groupAttributes: t }
    });
    try {
      const {
        group_id: i = "",
        room_id: u = 0,
        room_id_type: l = 0,
        call_media_type: c = o.UNKNOWN,
        user_list: g
      } = t[o.INNER_ATTR_KIT_INFO] ? JSON.parse(t[o.INNER_ATTR_KIT_INFO]) : {};
      let N = (g || []).map((M) => M.userid);
      N = N.length && await ue(N, (s = this._callService) == null ? void 0 : s.getTim());
      const p = {
        [o.GROUP_ID]: i,
        [o.GROUP_CALL_MEMBERS]: N,
        [o.ROOM_ID]: u,
        [o.CALL_MEDIA_TYPE]: Rs[c],
        [o.ROOM_ID_TYPE]: l
      };
      Ke.updateStore(p, r.CALL);
    } catch (i) {
      console.warn(`${o.PREFIX}updateStoreBasedOnGroupAttributes fail, error: ${i}`);
    }
  }
  async getGroupAttributes(t, e) {
    if (!e)
      return {};
    try {
      const { data: n } = await t.getGroupAttributes({
        groupID: e,
        keyList: []
      });
      return (n == null ? void 0 : n.groupAttributes) || {};
    } catch (n) {
      return console.warn(`${o.PREFIX}getGroupAttributes fail: ${n}`), {};
    }
  }
  isLineBusy(t) {
    var a;
    const e = Qe(t.payload.data), n = Qe(e == null ? void 0 : e.data);
    return (n == null ? void 0 : n.line_busy) === "line_busy" || (n == null ? void 0 : n.line_busy) === "" || ((a = n == null ? void 0 : n.data) == null ? void 0 : a.message) === "lineBusy";
  }
  async getCallKitMessage(t, e) {
    var p, M, f, v;
    const n = Qe(t.payload.data);
    if ((n == null ? void 0 : n.businessID) !== 1)
      return {};
    let a = "";
    const s = Qe(n == null ? void 0 : n.data), i = s.call_type, u = n.inviteeList, l = (p = s == null ? void 0 : s.data) == null ? void 0 : p.inviter, c = Ke.getData(r.CALL, o.LOCAL_USER_INFO).userId, g = l === c, N = (M = s == null ? void 0 : s.data) == null ? void 0 : M.cmd;
    switch (n == null ? void 0 : n.actionType) {
      case KA.INVITE: {
        a = fA[N]({ callDuration: Io(s == null ? void 0 : s.call_end) });
        break;
      }
      case KA.CANCEL_INVITE:
        a = g ? "Call Cancel" : "Other Side Cancel";
        break;
      case KA.ACCEPT_INVITE:
        ["switchToAudio", "switchToVideo"].includes(N) ? a = (f = fA == null ? void 0 : fA[N]) == null ? void 0 : f.call(fA) : a = WA("Answered");
        break;
      case KA.REJECT_INVITE:
        this.isLineBusy(t) ? a = g ? "Line Busy" : "Other Side Line Busy" : a = g ? "Other Side Decline" : "Decline";
        break;
      case KA.INVITE_TIMEOUT:
        ["switchToAudio", "switchToVideo"].includes(N) ? a = (v = fA == null ? void 0 : fA[N]) == null ? void 0 : v.call(fA) : a = g ? "Other Side No Answer" : "No answer";
        break;
    }
    return { messageCardContent: a, callMediaType: i, inviteeList: u };
  }
  _addListenChatEvent() {
    var t, e;
    if (!((t = this._callService) != null && t.getTim())) {
      console.warn(`${o.PREFIX}add tim event listener failed, tim is empty.`);
      return;
    }
    (e = this._callService) == null || e.getTim().on(yn.EVENT.GROUP_ATTRIBUTES_UPDATED, this._handleGroupAttributesUpdated, this);
  }
  _removeListenChatEvent() {
    var t, e;
    if (!((t = this._callService) != null && t.getTim())) {
      console.warn(`${o.PREFIX}remove tim event listener failed, tim is empty.`);
      return;
    }
    (e = this._callService) == null || e.getTim().off(yn.EVENT.GROUP_ATTRIBUTES_UPDATED, this._handleGroupAttributesUpdated, this);
  }
  async _handleTUICoreOnClick(t, e) {
    var n, a;
    try {
      const { groupID: s, userIDList: i = [], ...u } = t;
      s ? await ((n = this._callService) == null ? void 0 : n.groupCall({ groupID: s, userIDList: i, type: e, ...u })) : i.length === 1 && await ((a = this._callService) == null ? void 0 : a.call({ userID: i[0], type: e, ...u }));
    } catch (s) {
      console.debug(s);
    }
  }
  async _handleGroupAttributesUpdated(t) {
    var s;
    if (Ke.getData(r.CALL, o.CALL_STATUS) !== w.IDLE)
      return;
    const e = (t == null ? void 0 : t.data) || {}, { groupID: n = "", groupAttributes: a = {} } = e;
    n === ((s = this._callService) == null ? void 0 : s.getCurrentGroupId()) && await this.updateStoreBasedOnGroupAttributes(a);
  }
}
const Li = 0, hi = 1, On = 2;
function mi({ retryFunction: A, settings: t, onError: e, onRetrying: n, onRetryFailed: a, context: s }) {
  return function(...i) {
    const u = t.retries || 5;
    let l = 0, c = -1, g = Li;
    const N = async (p, M) => {
      const f = s || this;
      try {
        const v = await A.apply(f, i);
        l = 0, p(v);
      } catch (v) {
        const S = () => {
          clearTimeout(c), l = 0, g = On, M(v);
        }, h = () => {
          g !== On && l < u ? (l++, g = hi, At(n) && n.call(f, l, S), c = setTimeout(
            () => {
              c = -1, N(p, M);
            },
            Dt(t.timeout) ? 1e3 : t.timeout
          )) : (S(), At(a) && a.call(f, v));
        };
        At(e) ? e.call(f, v, h, M, i) : h();
      }
    };
    return new Promise(N);
  };
}
function Si(A) {
  return function(t, e, n) {
    const { retries: a = 5, timeout: s = 2e3, onError: i, onRetrying: u, onRetryFailed: l } = A, c = mi({
      retryFunction: n.value,
      settings: { retries: a, timeout: s },
      onError: i,
      onRetrying: u,
      onRetryFailed: l,
      context: null
    });
    return n.value = function(...g) {
      return c.apply(this, g);
    }, n;
  };
}
var Di = Object.defineProperty, yi = Object.getOwnPropertyDescriptor, Ei = (A, t, e, n) => {
  for (var a = n > 1 ? void 0 : n ? yi(t, e) : t, s = A.length - 1, i; s >= 0; s--)
    (i = A[s]) && (a = (n ? i(t, e, a) : i(a)) || a);
  return n && a && Di(t, e, a), a;
};
const U = hA.getInstance(), Ti = _A.getInstance(), ae = class {
  constructor(A) {
    this._callService = A.callService;
  }
  static getInstance(A) {
    return ae.instance || (ae.instance = new ae(A)), ae.instance;
  }
  addListenTuiCallEngineEvent() {
    var t;
    const A = (t = this._callService) == null ? void 0 : t.getTUICallEngineInstance();
    if (!A) {
      console.warn(`${o.PREFIX}add engine event listener failed, engine is empty.`);
      return;
    }
    A.on(T.ERROR, this._handleError, this), A.on(T.INVITED, this._handleNewInvitationReceived, this), A.on(T.USER_ACCEPT, this._handleUserAccept, this), A.on(T.USER_ENTER, this._handleUserEnter, this), A.on(T.USER_LEAVE, this._handleUserLeave, this), A.on(T.REJECT, this._handleInviteeReject, this), A.on(T.NO_RESP, this._handleNoResponse, this), A.on(T.LINE_BUSY, this._handleLineBusy, this), A.on(T.CALLING_CANCEL, this._handleCallingCancel, this), A.on(T.SDK_READY, this._handleSDKReady, this), A.on(T.KICKED_OUT, this._handleKickedOut, this), A.on(T.MESSAGE_SENT_BY_ME, this._messageSentByMe, this), T.CALL_MESSAGE && A.on(T.CALL_MESSAGE, this._handleCallMessage, this), T.ON_USER_NETWORK_QUALITY_CHANGED && A.on(T.ON_USER_NETWORK_QUALITY_CHANGED, this._handleNetworkQuality, this), A.on(T.CALLING_END, this._handleCallingEnd, this), A.on(T.CALL_TYPE_CHANGED, this._handleCallTypeChange, this), A.on(T.USER_VOICE_VOLUME, this._handleUserVoiceVolume, this), A.on(T.DEVICED_UPDATED, this._handleDeviceUpdate, this), A.on(T.USER_VIDEO_AVAILABLE, this._handleUserVideoAvailable, this), A.on(T.USER_AUDIO_AVAILABLE, this._handleUserAudioAvailable, this);
  }
  removeListenTuiCallEngineEvent() {
    var t;
    const A = (t = this._callService) == null ? void 0 : t.getTUICallEngineInstance();
    A.off(T.ERROR, this._handleError, this), A.off(T.INVITED, this._handleNewInvitationReceived, this), A.off(T.USER_ACCEPT, this._handleUserAccept, this), A.off(T.USER_ENTER, this._handleUserEnter, this), A.off(T.USER_LEAVE, this._handleUserLeave, this), A.off(T.REJECT, this._handleInviteeReject, this), A.off(T.NO_RESP, this._handleNoResponse, this), A.off(T.LINE_BUSY, this._handleLineBusy, this), A.off(T.CALLING_CANCEL, this._handleCallingCancel, this), A.off(T.SDK_READY, this._handleSDKReady, this), A.off(T.KICKED_OUT, this._handleKickedOut, this), A.off(T.MESSAGE_SENT_BY_ME, this._messageSentByMe, this), T.ON_USER_NETWORK_QUALITY_CHANGED && A.off(T.ON_USER_NETWORK_QUALITY_CHANGED, this._handleNetworkQuality, this), A.off(T.CALLING_END, this._handleCallingEnd, this), A.off(T.CALL_TYPE_CHANGED, this._handleCallTypeChange, this), A.off(T.USER_VOICE_VOLUME, this._handleUserVoiceVolume, this), A.off(T.DEVICED_UPDATED, this._handleDeviceUpdate, this), A.off(T.USER_VIDEO_AVAILABLE, this._handleUserVideoAvailable, this), A.off(T.USER_AUDIO_AVAILABLE, this._handleUserAudioAvailable, this);
  }
  _callerChangeToConnected() {
    var e;
    const A = U.getData(r.CALL, o.CALL_ROLE);
    U.getData(r.CALL, o.CALL_STATUS) === w.CALLING && A === H.CALLER && (U.update(r.CALL, o.CALL_STATUS, w.CONNECTED), (e = this._callService) == null || e.startTimer());
  }
  _unNormalEventsManager(A, t) {
    var a;
    console.log(`${o.PREFIX}${t} event data: ${JSON.stringify(A)}.`);
    const e = U.getData(r.CALL, o.IS_GROUP), n = U.getData(r.CALL, o.REMOTE_USER_INFO_LIST);
    switch (t) {
      case T.REJECT:
      case T.LINE_BUSY: {
        const { userID: s } = NA(A);
        let i = t === T.REJECT ? lA.OTHER_SIDE_REJECT_CALL : lA.OTHER_SIDE_LINE_BUSY, u = "";
        e && (u = (n.find((l) => l.userId === s) || {}).displayUserInfo || s, i = t === T.REJECT ? lA.REJECT_CALL : lA.IN_BUSY), U.update(r.CALL, o.TOAST_INFO, { content: { key: i, options: { userList: u } } }), s && xt([s]);
        break;
      }
      case T.NO_RESP: {
        const { userIDList: s = [] } = NA(A), i = e ? lA.TIMEOUT : lA.CALL_TIMEOUT, u = s.map((l) => (n.find((g) => g.userId === l) || {}).displayUserInfo || l);
        U.update(r.CALL, o.TOAST_INFO, { content: { key: i, options: { userList: u.join() } } }), s.length > 0 && xt(s);
        break;
      }
      case T.CALLING_CANCEL: {
        (a = this._callService) == null || a._resetCallStore();
        break;
      }
    }
  }
  _handleError(A) {
    var s;
    const { code: t, message: e } = A || {}, n = Object.values(En).indexOf(t);
    let a = "";
    if (n !== -1) {
      const i = Object.keys(En)[n];
      a = WA(Tn[i]), a && U.update(r.CALL, o.TOAST_INFO, { content: Tn[i], type: o.ERROR });
    }
    (s = this._callService) == null || s.executeExternalAfterCalling(), console.error(`${o.PREFIX}_handleError, errorCode: ${t}; errorMessage: ${a || e}.`);
  }
  async _handleNewInvitationReceived(A) {
    var v, S, h, E;
    console.log(`${o.PREFIX}onCallReceived event data: ${JSON.stringify(A)}.`);
    const { callerId: t = "", callMediaType: e, inviteData: n = {}, calleeIdList: a = [], groupID: s = "", roomID: i, strRoomID: u } = NA(A), l = U.getData(r.CALL, o.LOCAL_USER_INFO), c = [t, ...a.filter((x) => x !== l.userId)], g = e || n.callType, N = g === V.AUDIO ? lA.CALLEE_CALLING_AUDIO_MSG : lA.CALLEE_CALLING_VIDEO_MSG;
    let p = {
      [o.CALL_ROLE]: H.CALLEE,
      [o.IS_GROUP]: !!s || a.length > 1,
      [o.CALL_STATUS]: w.CALLING,
      [o.CALL_MEDIA_TYPE]: g,
      [o.CALL_TIPS]: N,
      [o.CALLER_USER_INFO]: { userId: t },
      [o.GROUP_ID]: s
    };
    po(i, u), U.updateStore(p, r.CALL), (v = this._callService) == null || v.executeExternalBeforeCalling(), (S = this._callService) != null && S.statusChanged && ((h = this._callService) == null || h.statusChanged({ oldStatus: aA.IDLE, newStatus: aA.BE_INVITED }));
    const M = await ue(c, (E = this._callService) == null ? void 0 : E.getTim()), [f] = M.filter((x) => x.userId === t);
    M.length > 0 && U.updateStore({
      [o.REMOTE_USER_INFO_LIST]: M,
      [o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST]: M,
      [o.CALLER_USER_INFO]: {
        userId: t,
        nick: (f == null ? void 0 : f.nick) || "",
        avatar: (f == null ? void 0 : f.avatar) || "",
        displayUserInfo: (f == null ? void 0 : f.remark) || (f == null ? void 0 : f.nick) || t
      }
    }, r.CALL);
  }
  _handleUserAccept(A) {
    this._callerChangeToConnected(), U.update(r.CALL, o.CALL_TIPS, { text: "answered", duration: 2e3 }), console.log(`${o.PREFIX}accept event data: ${JSON.stringify(A)}.`);
  }
  async _handleUserEnter(A) {
    var s;
    this._callerChangeToConnected();
    const { userID: t, data: e } = NA(A);
    let n = U.getData(r.CALL, o.REMOTE_USER_INFO_LIST);
    if (!n.find((i) => (i == null ? void 0 : i.userId) === t)) {
      n.push({ userId: t }), n.length > 0 && (U.update(r.CALL, o.REMOTE_USER_INFO_LIST, n), U.update(r.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, n));
      const [i] = await ue([t], (s = this._callService) == null ? void 0 : s.getTim());
      n = U.getData(r.CALL, o.REMOTE_USER_INFO_LIST), n.forEach((u) => {
        (u == null ? void 0 : u.userId) === t && (u = Object.assign(u, i));
      });
    }
    n = n.map((i) => (i.userId === t && (i.isEnter = !0), i)), n.length > 0 && (U.update(r.CALL, o.REMOTE_USER_INFO_LIST, n), U.update(r.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, n), Ti.updateViewBackgroundUserId("remote")), console.log(`${o.PREFIX}userEnter event data: ${JSON.stringify(A)}.`);
  }
  _handleUserLeave(A) {
    console.log(`${o.PREFIX}userLeave event data: ${JSON.stringify(A)}.`);
    const { data: t, userID: e } = NA(A);
    if (U.getData(r.CALL, o.IS_GROUP)) {
      const a = (U.getData(r.CALL, o.REMOTE_USER_INFO_LIST).find((s) => s.userId === e) || {}).displayUserInfo || e;
      U.update(r.CALL, o.TOAST_INFO, { content: { key: lA.END_CALL, options: { userList: a } } });
    }
    e && xt([e]);
  }
  _handleInviteeReject(A) {
    this._unNormalEventsManager(A, T.REJECT);
  }
  _handleNoResponse(A) {
    this._unNormalEventsManager(A, T.NO_RESP);
  }
  _handleLineBusy(A) {
    this._unNormalEventsManager(A, T.LINE_BUSY);
  }
  _handleCallingCancel(A) {
    var t;
    (t = this._callService) == null || t.executeExternalAfterCalling(), this._unNormalEventsManager(A, T.CALLING_CANCEL);
  }
  _handleCallingEnd(A) {
    var t, e;
    console.log(`${o.PREFIX}callEnd event data: ${JSON.stringify(A)}.`), (t = this._callService) == null || t.executeExternalAfterCalling(), (e = this._callService) == null || e._resetCallStore();
  }
  async _handleSDKReady(A) {
    var e;
    let t = U.getData(r.CALL, o.LOCAL_USER_INFO);
    t = await Ci(t.userId, (e = this._callService) == null ? void 0 : e.getTim()), U.update(r.CALL, o.LOCAL_USER_INFO, t), U.update(r.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, t);
  }
  _handleKickedOut(A) {
    var t, e, n;
    console.log(`${o.PREFIX}kickOut event data: ${JSON.stringify(A)}.`), (t = this._callService) != null && t.kickedOut && ((e = this._callService) == null || e.kickedOut(A)), U.update(r.CALL, o.CALL_TIPS, lA.KICK_OUT), (n = this._callService) == null || n._resetCallStore();
  }
  _messageSentByMe(A) {
    var e, n;
    const t = A == null ? void 0 : A.data;
    (e = this._callService) != null && e.onMessageSentByMe && ((n = this._callService) == null || n.onMessageSentByMe(t));
  }
  _handleCallMessage(A) {
    const t = NA(A);
    this._callService._chatCombine.callTUIService({ message: t });
  }
  _handleCallTypeChange(A) {
    const { newCallType: t, type: e } = NA(A);
    U.update(r.CALL, o.CALL_MEDIA_TYPE, t || e);
  }
  _handleNetworkQuality(A) {
    const { networkQualityList: t = [] } = NA(A);
    U.update(r.CALL, o.NETWORK_STATUS, t);
    const e = U.getData(r.CALL, o.IS_GROUP), n = U.getData(r.CALL, o.LOCAL_USER_INFO);
    if (U.getData(r.CALL, o.REMOTE_USER_INFO_LIST), !e && t.find((s) => (n == null ? void 0 : n.userId) === (s == null ? void 0 : s.userId) && (s == null ? void 0 : s.quality) >= Xs)) {
      U.update(r.CALL, o.CALL_TIPS, lA.LOCAL_NETWORK_IS_POOR);
      return;
    }
  }
  async _startRemoteView(A) {
    var t;
    if (!A) {
      console.warn(`${o.PREFIX}_startRemoteView userID is empty`);
      return;
    }
    if (!document.getElementById(A))
      return console.warn(`${o.PREFIX}_startRemoteView can't find HTMLElement sid: ${A}`), Promise.reject();
    try {
      const e = U.getData(r.CALL, o.DISPLAY_MODE);
      await ((t = this._callService) == null ? void 0 : t.getTUICallEngineInstance().startRemoteView({ userID: A, videoViewDomID: A, options: { objectFit: e } }));
    } catch (e) {
      return console.error(`${o.PREFIX}_startRemoteView error: ${e}.`), Promise.reject(e);
    }
  }
  _setRemoteUserInfoAudioVideoAvailable(A, t, e) {
    let n = U.getData(r.CALL, o.REMOTE_USER_INFO_LIST);
    n = n.map((a) => {
      if (a.userId === e) {
        if (t === o.AUDIO)
          return { ...a, isAudioAvailable: A };
        if (t === o.VIDEO)
          return { ...a, isVideoAvailable: A };
      }
      return a;
    }), n.length > 0 && (U.update(r.CALL, o.REMOTE_USER_INFO_LIST, n), U.update(r.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, n));
  }
  async _handleUserVideoAvailable(A) {
    const { userID: t, isVideoAvailable: e } = NA(A);
    console.log(`${o.PREFIX}_handleUserVideoAvailable event data: ${JSON.stringify(A)}.`);
    try {
      e && await this._startRemoteView(t);
    } catch (n) {
      console.error(`${o.PREFIX}_startRemoteView failed, error: ${n}.`);
    }
    this._setRemoteUserInfoAudioVideoAvailable(e, o.VIDEO, t);
  }
  _handleUserAudioAvailable(A) {
    const { userID: t, isAudioAvailable: e } = NA(A);
    console.log(`${o.PREFIX}_handleUserAudioAvailable event data: ${JSON.stringify(A)}.`), this._setRemoteUserInfoAudioVideoAvailable(e, o.AUDIO, t);
  }
  _handleUserVoiceVolume(A) {
    try {
      const { volumeMap: t } = NA(A);
      if ((t || []).length === 0)
        return;
      const e = U.getData(r.CALL, o.LOCAL_USER_INFO);
      let n = U.getData(r.CALL, o.REMOTE_USER_INFO_LIST);
      const [a] = t.filter((u) => u.userId === e.userId), s = t.reduce((u, l) => l.userId !== e.userId ? { ...u, [l.userId]: l.audioVolume } : u, {});
      e.volume = a.audioVolume, n = n.map((u) => ({ ...u, volume: s[u.userId] }));
      const i = {
        [o.LOCAL_USER_INFO]: e,
        [o.REMOTE_USER_INFO_LIST]: n
      };
      U.updateStore(i, r.CALL);
    } catch (t) {
      console.debug(t);
    }
  }
  _handleDeviceUpdate(A) {
    const { cameraList: t, microphoneList: e, speakerList: n, currentCamera: a, currentMicrophone: s, currentSpeaker: i } = A;
    U.update(r.CALL, o.DEVICE_LIST, { cameraList: t, microphoneList: e, speakerList: n, currentCamera: a, currentMicrophone: s, currentSpeaker: i });
  }
};
let Mo = ae;
Ei([
  Si({
    retries: 5,
    timeout: 200,
    onRetrying(A) {
      console.warn(`${o.PREFIX}_startRemoteView, retrying [${A}]`);
    }
  })
], Mo.prototype, "_startRemoteView", 1);
var wi = Object.defineProperty, ji = Object.getOwnPropertyDescriptor, G = (A, t, e, n) => {
  for (var a = n > 1 ? void 0 : n ? ji(t, e) : t, s = A.length - 1, i; s >= 0; s--)
    (i = A[s]) && (a = (n ? i(t, e, a) : i(a)) || a);
  return n && a && wi(t, e, a), a;
};
const y = kA.getInstance(), d = hA.getInstance(), EA = _A.getInstance();
EA.setTUIStore(d);
const xn = "4.0.2", bi = "vue2.6", oe = class {
  constructor() {
    this._tim = null, this._TUICore = null, this._timerId = -1, this._startTimeStamp = Ot(), this._bellContext = null, this._isFromChat = !1, this._currentGroupId = "", this._offlinePushInfo = null, this._permissionCheckTimer = null, this._chatCombine = null, this._engineEventHandler = null, this._handleCallStatusChange = async (A) => {
      var t, e, n, a;
      try {
        const s = {
          callRole: d.getData(r.CALL, o.CALL_ROLE),
          callStatus: d.getData(r.CALL, o.CALL_STATUS)
        };
        if (this._bellContext.setBellProperties(s), A === w.CALLING)
          await ((t = this == null ? void 0 : this._bellContext) == null ? void 0 : t.play());
        else {
          if (A === w.CONNECTED) {
            const i = d.getData(r.CALL, o.IS_GROUP), u = d.getData(r.CALL, o.CALL_MEDIA_TYPE), l = d.getData(r.CALL, o.REMOTE_USER_INFO_LIST), c = i ? aA.DIALING_GROUP : aA.DIALING_C2C;
            d.update(r.CALL, o.CALL_TIPS, ""), this.statusChanged && this.statusChanged({ oldStatus: c, newStatus: Ze() }), !i && u === V.VIDEO && this.switchScreen(l[0].domId);
          }
          if (A === w.IDLE && this._isFromChat) {
            const i = this._currentGroupId ? await ((e = this._chatCombine) == null ? void 0 : e.getGroupAttributes(this._tim, this._currentGroupId)) : {};
            await ((n = this._chatCombine) == null ? void 0 : n.updateStoreBasedOnGroupAttributes(i, d, this));
          }
          await ((a = this == null ? void 0 : this._bellContext) == null ? void 0 : a.stop());
        }
      } catch (s) {
        console.warn(`${o.PREFIX}handleCallStatusChange, ${s}.`);
      }
    }, console.log(`${o.PREFIX}version: ${xn}`), this._watchTUIStore(), this._engineEventHandler = Mo.getInstance({ callService: this }), this._chatCombine = JA.getInstance({ callService: this });
  }
  static getInstance() {
    return oe.instance || (oe.instance = new oe()), oe.instance;
  }
  async init(A) {
    var t, e;
    try {
      if (this._tuiCallEngine)
        return;
      let { userID: n, tim: a, userSig: s, sdkAppID: i, SDKAppID: u, isFromChat: l, component: c = Qt.TUI_CALL_KIT } = A;
      this._TUICore && (i = this._TUICore.SDKAppID, a = this._TUICore.tim), this._tim = a, console.log(`${o.PREFIX}init sdkAppId: ${i || u}, userId: ${n}`), this._tuiCallEngine = Bs.createInstance({
        tim: a,
        SDKAppID: i || u,
        frameWork: bi,
        language: 5,
        callkitVersion: xn,
        isFromChat: l || !1,
        component: c
      }), EA.setEngineInstance(this._tuiCallEngine), this._addListenTuiCallEngineEvent(), this._bellContext = new gi(), d.update(r.CALL, o.LOCAL_USER_INFO, { userId: n }), d.update(r.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, { userId: n }), EA.updateViewBackgroundUserId("local"), await this._tuiCallEngine.login({ userID: n, userSig: s, assetsPath: "" });
      const g = d.getData(r.CALL, o.CUSTOM_UI_CONFIG);
      (e = (t = this._tuiCallEngine) == null ? void 0 : t.reportLog) == null || e.call(t, {
        name: "TUICallkit.init",
        data: {
          uiConfig: g
        }
      });
    } catch (n) {
      throw console.error(`${o.PREFIX}init failed, error: ${n}.`), n;
    }
  }
  async destroyed() {
    var A;
    try {
      const t = d.getData(r.CALL, o.CALL_STATUS);
      if (t !== w.IDLE)
        throw new Error(`please destroyed when status is idle, current status: ${t}`);
      this._tuiCallEngine && (this._removeListenTuiCallEngineEvent(), await this._tuiCallEngine.destroyInstance(), this._tuiCallEngine = null), (A = this._bellContext) == null || A.destroy(), this._bellContext = null;
    } catch (t) {
      throw console.error(`${o.PREFIX}destroyed failed, error: ${t}.`), t;
    }
  }
  async call(A) {
    if (d.getData(r.CALL, o.CALL_STATUS) === w.IDLE)
      try {
        const { type: t, userID: e, offlinePushInfo: n } = A;
        if (d.getData(r.CALL, o.CALL_STATUS) !== w.IDLE)
          return;
        await this._updateCallStoreBeforeCall(t, [{ userId: e }]), this.executeExternalBeforeCalling(), A.offlinePushInfo = { ...this.getDefaultOfflinePushInfo(), ...n };
        const a = await this._tuiCallEngine.call(A);
        await this._updateCallStoreAfterCall([e], a);
      } catch (t) {
        this._handleCallError(t, "call");
      }
  }
  async groupCall(A) {
    if (d.getData(r.CALL, o.CALL_STATUS) === w.IDLE)
      try {
        const { userIDList: t, type: e, groupID: n, offlinePushInfo: a } = A;
        if (d.getData(r.CALL, o.CALL_STATUS) !== w.IDLE)
          return;
        const s = t.map((u) => ({ userId: u }));
        await this._updateCallStoreBeforeCall(e, s, n), this.executeExternalBeforeCalling(), A.offlinePushInfo = { ...this.getDefaultOfflinePushInfo(), ...a };
        const i = await this._tuiCallEngine.groupCall(A);
        await this._updateCallStoreAfterCall(t, i);
      } catch (t) {
        this._handleCallError(t, "groupCall");
      }
  }
  async inviteUser(A) {
    if (d.getData(r.CALL, o.CALL_STATUS) !== w.IDLE)
      try {
        const { userIDList: t } = A;
        let e = await ue(t, this.getTim());
        const n = d.getData(r.CALL, o.REMOTE_USER_INFO_LIST);
        if (t.filter((s) => !n.some((i) => i.userId === s)).length === 0)
          return;
        d.update(r.CALL, o.REMOTE_USER_INFO_LIST, [...n, ...e]), d.update(r.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, [...n, ...e]), this._tuiCallEngine && await this._tuiCallEngine.inviteUser(A);
      } catch (t) {
        console.error(`${o.PREFIX}inviteUser failed, error: ${t}.`);
      }
  }
  async joinInGroupCall(A) {
    if (d.getData(r.CALL, o.CALL_STATUS) !== w.CONNECTED)
      try {
        const t = {
          [o.CALL_ROLE]: H.CALLEE,
          [o.IS_GROUP]: !0,
          [o.CALL_STATUS]: w.CONNECTED,
          [o.CALL_MEDIA_TYPE]: A.type,
          [o.GROUP_ID]: A.groupID,
          [o.ROOM_ID]: A.roomID
        };
        d.updateStore(t, r.CALL);
        const e = await this._tuiCallEngine.joinInGroupCall(A), n = this._getFeatureButtonDefaultState(LA.Camera) === GA.Close;
        A.type === V.VIDEO && !n && await this.openCamera(o.LOCAL_VIDEO), d.update(r.CALL, o.IS_CLICKABLE, !0), this.startTimer(), Ye(this._tuiCallEngine), await this._tuiCallEngine.setVideoQuality(d.getData(r.CALL, o.VIDEO_RESOLUTION));
        const a = d.getData(r.CALL, o.LOCAL_USER_INFO);
        d.update(r.CALL, o.LOCAL_USER_INFO, { ...a, isEnter: !0 }), d.update(r.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, { ...a, isEnter: !0 }), DA(!0, o.AUDIO);
      } catch (t) {
        this._handleCallError(t, "joinInGroupCall");
      }
  }
  async calls(A) {
    if (d.getData(r.CALL, o.CALL_STATUS) === w.IDLE)
      try {
        const { userIDList: t, type: e, chatGroupID: n, offlinePushInfo: a } = A;
        if (d.getData(r.CALL, o.CALL_STATUS) !== w.IDLE)
          return;
        const s = t.map((u) => ({ userId: u }));
        await this._updateCallStoreBeforeCall(e, s, n), this.executeExternalBeforeCalling(), A.offlinePushInfo = { ...this.getDefaultOfflinePushInfo(), ...a };
        const i = await this._tuiCallEngine.calls(A);
        await this._updateCallStoreAfterCall(t, i);
      } catch (t) {
        this._handleCallError(t, "calls");
      }
  }
  async join(A) {
    if (d.getData(r.CALL, o.CALL_STATUS) !== w.CONNECTED)
      try {
        const t = await this._tuiCallEngine.join(A), e = this._getFeatureButtonDefaultState(LA.Camera) === GA.Close;
        A.type === V.VIDEO && !e && await this.openCamera(o.LOCAL_VIDEO), d.update(r.CALL, o.IS_CLICKABLE, !0), this.startTimer();
        const n = {
          [o.CALL_ROLE]: H.CALLEE,
          [o.IS_GROUP]: !0,
          [o.CALL_STATUS]: w.CONNECTED,
          [o.CALL_MEDIA_TYPE]: A.type,
          [o.GROUP_ID]: A.groupID,
          [o.ROOM_ID]: A.roomID
        };
        d.updateStore(n, r.CALL), Ye(this._tuiCallEngine), await this._tuiCallEngine.setVideoQuality(d.getData(r.CALL, o.VIDEO_RESOLUTION));
        const a = d.getData(r.CALL, o.LOCAL_USER_INFO);
        d.update(r.CALL, o.LOCAL_USER_INFO, { ...a, isEnter: !0 }), d.update(r.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, { ...a, isEnter: !0 }), DA(!0, o.AUDIO);
      } catch (t) {
        this._handleCallError(t, "join");
      }
  }
  getTUICallEngineInstance() {
    return (this == null ? void 0 : this._tuiCallEngine) || null;
  }
  setLogLevel(A) {
    var t;
    (t = this == null ? void 0 : this._tuiCallEngine) == null || t.setLogLevel(A);
  }
  setLanguage(A) {
    A && Object.values(oo).includes(A) && (d.update(r.CALL, o.LANGUAGE, A), d.update(r.CALL, o.TRANSLATE, WA.bind(null)));
  }
  enableFloatWindow(A) {
    d.update(r.CALL, o.ENABLE_FLOAT_WINDOW, A);
  }
  async setSelfInfo(A) {
    const { nickName: t, avatar: e } = A;
    try {
      await this._tuiCallEngine.setSelfInfo({ nickName: t, avatar: e });
    } catch (n) {
      console.error(`${o.PREFIX}setSelfInfo failed, error: ${n}.`);
    }
  }
  async enableVirtualBackground(A) {
    d.update(r.CALL, o.IS_SHOW_ENABLE_VIRTUAL_BACKGROUND, A);
  }
  async enableAIVoice(A) {
    try {
      await this._tuiCallEngine.enableAIVoice(A), console.log(`${o.PREFIX}enableAIVoice: ${A}.`);
    } catch (t) {
      throw console.error(`${o.PREFIX}enableAIVoice failed, error: ${t}.`), t;
    }
  }
  async setCallingBell(A) {
    let t = !0;
    if (t = await Qs(A), !t) {
      console.warn(`${o.PREFIX}setCallingBell failed, filePath: ${A}.`);
      return;
    }
    const e = { calleeBellFilePath: A };
    this._bellContext.setBellProperties(e);
  }
  async enableMuteMode(A) {
    try {
      const t = { isMuteBell: A };
      this._bellContext.setBellProperties(t), await this._bellContext.setBellMute(A);
    } catch (t) {
      console.warn(`${o.PREFIX}enableMuteMode failed, error: ${t}.`);
    }
  }
  hideFeatureButton(A) {
    EA.hideFeatureButton(A);
  }
  setLocalViewBackgroundImage(A) {
    EA.setLocalViewBackgroundImage(A);
  }
  setRemoteViewBackgroundImage(A, t) {
    EA.setRemoteViewBackgroundImage(A, t);
  }
  setLayoutMode(A) {
    EA.setLayoutMode(A);
  }
  setCameraDefaultState(A) {
    EA.setCameraDefaultState(A);
  }
  async accept() {
    var t, e, n, a, s, i;
    const A = d.getData(r.CALL, o.CALL_STATUS);
    if ((e = (t = this._tuiCallEngine) == null ? void 0 : t.reportLog) == null || e.call(t, {
      name: "TUICallKit.accept.start",
      data: { callStatus: A }
    }), A !== w.CONNECTED)
      try {
        d.update(r.CALL, o.CALL_STATUS, w.CONNECTED), Ye(this._tuiCallEngine);
        const u = await this._tuiCallEngine.accept();
        if (u) {
          (a = this._chatCombine) == null || a.callTUIService({ message: (n = u == null ? void 0 : u.data) == null ? void 0 : n.message }), d.update(r.CALL, o.IS_CLICKABLE, !0), this.startTimer();
          const l = d.getData(r.CALL, o.CALL_MEDIA_TYPE), c = this._getFeatureButtonDefaultState(LA.Camera) === GA.Close;
          l === V.VIDEO && !c && await this.openCamera(o.LOCAL_VIDEO), await this._tuiCallEngine.setVideoQuality(d.getData(r.CALL, o.VIDEO_RESOLUTION));
          const g = d.getData(r.CALL, o.LOCAL_USER_INFO);
          d.update(r.CALL, o.LOCAL_USER_INFO, { ...g, isEnter: !0 }), d.update(r.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, { ...g, isEnter: !0 }), DA(!0, o.AUDIO);
        }
      } catch (u) {
        if ((i = (s = this._tuiCallEngine) == null ? void 0 : s.reportLog) == null || i.call(s, {
          name: "TUICallKit.accept.fail",
          level: "error",
          error: u
        }), bn(u))
          return;
        Bt(u, V.AUDIO, this._tuiCallEngine), this._resetCallStore();
      }
  }
  async hangup() {
    if (d.getData(r.CALL, o.CALL_STATUS) !== w.IDLE) {
      try {
        const A = await this._tuiCallEngine.hangup();
        A == null || A.forEach((t) => {
          var e, n;
          (t == null ? void 0 : t.code) === 0 && ((n = this._chatCombine) == null || n.callTUIService({ message: (e = t == null ? void 0 : t.data) == null ? void 0 : e.message }));
        });
      } catch (A) {
        console.debug(A);
      }
      this._resetCallStore();
    }
  }
  async reject() {
    var A, t;
    if (d.getData(r.CALL, o.CALL_STATUS) !== w.IDLE) {
      try {
        const e = await this._tuiCallEngine.reject();
        (e == null ? void 0 : e.code) === 0 && ((t = this._chatCombine) == null || t.callTUIService({ message: (A = e == null ? void 0 : e.data) == null ? void 0 : A.message }));
      } catch (e) {
        console.debug(e);
      }
      this._resetCallStore();
    }
  }
  async openCamera(A) {
    try {
      if (y.isH5 || y.isWeChat) {
        const e = d.getData(r.CALL, o.CAMERA_POSITION) === FA.FRONT;
        this._tuiCallEngine.openCamera(A, e);
      } else
        await this._tuiCallEngine.openCamera(A);
      DA(!0, o.VIDEO);
    } catch (t) {
      Bt(t, V.VIDEO, this._tuiCallEngine), console.error(`${o.PREFIX}openCamera error: ${t}.`);
    }
  }
  async closeCamera() {
    try {
      await this._tuiCallEngine.closeCamera(), DA(!1, o.VIDEO);
    } catch (A) {
      console.error(`${o.PREFIX}closeCamera error: ${A}.`);
    }
  }
  async openMicrophone() {
    try {
      await this._tuiCallEngine.openMicrophone(), DA(!0, o.AUDIO);
    } catch (A) {
      console.error(`${o.PREFIX}openMicrophone failed, error: ${A}.`);
    }
  }
  async closeMicrophone() {
    try {
      await this._tuiCallEngine.closeMicrophone(), DA(!1, o.AUDIO);
    } catch (A) {
      console.error(`${o.PREFIX}closeMicrophone failed, error: ${A}.`);
    }
  }
  unMuteSpeaker() {
    var A, t;
    try {
      const e = (t = (A = this._tuiCallEngine) == null ? void 0 : A.getTRTCCloudInstance) == null ? void 0 : t.call(A);
      e && (e.muteAllRemoteAudio(!1), d.update(r.CALL, o.IS_MUTE_SPEAKER, !1));
    } catch (e) {
      console.error(`${o.PREFIX}unMuteSpeaker failed, error: ${e}.`);
    }
  }
  muteSpeaker() {
    var A, t;
    try {
      const e = (t = (A = this._tuiCallEngine) == null ? void 0 : A.getTRTCCloudInstance) == null ? void 0 : t.call(A);
      e && (e.muteAllRemoteAudio(!0), d.update(r.CALL, o.IS_MUTE_SPEAKER, !0));
    } catch (e) {
      console.error(`${o.PREFIX}muteSpeaker failed, error: ${e}.`);
    }
  }
  switchScreen(A) {
    !A || d.update(r.CALL, o.BIG_SCREEN_USER_ID, A);
  }
  async switchCallMediaType() {
    var A, t;
    try {
      const e = d.getData(r.CALL, o.CALL_MEDIA_TYPE);
      if (e === V.AUDIO) {
        console.warn(`${o.PREFIX}switchCallMediaType failed, ${e} not support.`);
        return;
      }
      const n = await this._tuiCallEngine.switchCallMediaType(V.AUDIO);
      (n == null ? void 0 : n.code) === 0 && ((t = this._chatCombine) == null || t.callTUIService({ message: (A = n == null ? void 0 : n.data) == null ? void 0 : A.message })), d.update(r.CALL, o.CALL_MEDIA_TYPE, V.AUDIO);
      const s = d.getData(r.CALL, o.IS_GROUP) ? aA.CALLING_GROUP_VIDEO : aA.CALLING_C2C_VIDEO, i = Ze();
      this.statusChanged && this.statusChanged({ oldStatus: s, newStatus: i });
    } catch (e) {
      console.error(`${o.PREFIX}switchCallMediaType failed, error: ${e}.`);
    }
  }
  async switchCamera() {
    const t = d.getData(r.CALL, o.CAMERA_POSITION) === FA.BACK ? FA.FRONT : FA.BACK;
    try {
      await this._tuiCallEngine.switchCamera(t), d.update(r.CALL, o.CAMERA_POSITION, t);
    } catch (e) {
      console.error(`${o.PREFIX}_switchCamera failed, error: ${e}.`);
    }
  }
  async setBlurBackground(A) {
    try {
      await this._tuiCallEngine.setBlurBackground(A ? qs : 0), d.update(r.CALL, o.ENABLE_VIRTUAL_BACKGROUND, A);
    } catch (t) {
      console.error(`${o.PREFIX}_setBlurBackground failed, error: ${t}.`);
    }
  }
  async switchDevice(A) {
    try {
      await this._tuiCallEngine.switchDevice(A);
    } catch (t) {
      console.error(`${o.PREFIX}_switchDevice failed, error: ${t}.`);
    }
  }
  async getDeviceList(A) {
    try {
      return await this._tuiCallEngine.getDeviceList(A);
    } catch (t) {
      this._handleCallError(t, "call");
    }
  }
  _addListenTuiCallEngineEvent() {
    this._engineEventHandler.addListenTuiCallEngineEvent();
  }
  _removeListenTuiCallEngineEvent() {
    this._engineEventHandler.removeListenTuiCallEngineEvent();
  }
  setCallback(A) {
    const { beforeCalling: t, afterCalling: e, onMinimized: n, onMessageSentByMe: a, kickedOut: s, statusChanged: i } = A;
    t && (this.beforeCalling = t), e && (this.afterCalling = e), n && (this.onMinimized = n), a && (this.onMessageSentByMe = a), s && (this.kickedOut = s), i && (this.statusChanged = i);
  }
  toggleMinimize() {
    const A = d.getData(r.CALL, o.IS_MINIMIZED);
    d.update(r.CALL, o.IS_MINIMIZED, !A), console.log(`${o.PREFIX}toggleMinimize: ${A} -> ${!A}.`), this.onMinimized && this.onMinimized(A, !A);
  }
  executeExternalBeforeCalling() {
    this.beforeCalling && this.beforeCalling();
  }
  executeExternalAfterCalling() {
    this.afterCalling && this.afterCalling();
  }
  setVideoDisplayMode(A) {
    d.update(r.CALL, o.DISPLAY_MODE, A);
  }
  async setVideoResolution(A) {
    var t;
    try {
      if (!A)
        return;
      d.update(r.CALL, o.VIDEO_RESOLUTION, A), await ((t = this._tuiCallEngine) == null ? void 0 : t.setVideoQuality(A));
    } catch (e) {
      console.warn(`${o.PREFIX}setVideoResolution failed, error: ${e}.`);
    }
  }
  startTimer() {
    this._timerId === -1 && (this._startTimeStamp = Ot(), this._timerId = st.run(o.TIMEOUT, this._updateCallDuration.bind(this), { delay: 1e3 }));
  }
  _handleCallError(A, t) {
    if (this._permissionCheckTimer && clearInterval(this._permissionCheckTimer), !bn(A))
      throw Bt(A, V.AUDIO, this._tuiCallEngine), console.error(`${o.PREFIX}${t} failed, error: ${A}.`), this._resetCallStore(), A;
  }
  async _updateCallStoreBeforeCall(A, t, e) {
    let n = lA.CALLER_CALLING_MSG;
    (e || d.getData(r.CALL, o.IS_MINIMIZED) || t.length > 1) && (n = lA.CALLER_GROUP_CALLING_MSG);
    let a = {
      [o.CALL_MEDIA_TYPE]: A,
      [o.CALL_ROLE]: H.CALLER,
      [o.REMOTE_USER_INFO_LIST]: t,
      [o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST]: t,
      [o.IS_GROUP]: !!e || t.length > 1,
      [o.CALL_TIPS]: n,
      [o.GROUP_ID]: e
    };
    d.updateStore({ ...a, [o.CALL_STATUS]: w.CALLING }, r.CALL), this.statusChanged && this.statusChanged({
      oldStatus: aA.IDLE,
      newStatus: e || t.length > 1 ? aA.DIALING_GROUP : aA.DIALING_C2C
    }), Ye(this._tuiCallEngine);
    const s = await ue(t.map((i) => i.userId), this.getTim());
    s.length > 0 && (d.update(r.CALL, o.REMOTE_USER_INFO_LIST, s), d.update(r.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, s));
  }
  async _updateCallStoreAfterCall(A, t) {
    var e, n;
    if (t) {
      d.update(r.CALL, o.IS_CLICKABLE, !0), po(t == null ? void 0 : t.roomID, t == null ? void 0 : t.strRoomID);
      const a = d.getData(r.CALL, o.CALL_MEDIA_TYPE);
      if (t.code === 0) {
        (n = this._chatCombine) == null || n.callTUIService({ message: (e = t == null ? void 0 : t.data) == null ? void 0 : e.message });
        try {
          await this._tuiCallEngine.setVideoQuality(d.getData(r.CALL, o.VIDEO_RESOLUTION));
        } catch (u) {
          console.warn(`${o.PREFIX}setVideoQuality failed, error: ${u}.`);
        }
      } else {
        this._resetCallStore();
        return;
      }
      const s = this._getFeatureButtonDefaultState(LA.Camera) === GA.Close;
      a === V.VIDEO && !s && await this.openCamera(o.LOCAL_VIDEO);
      const i = d.getData(r.CALL, o.LOCAL_USER_INFO);
      d.update(r.CALL, o.LOCAL_USER_INFO, { ...i, isEnter: !0 }), d.update(r.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, { ...i, isEnter: !0 }), DA(!0, o.AUDIO);
    } else
      this._permissionCheckTimer && clearInterval(this._permissionCheckTimer), this._permissionCheckTimer = null, this._resetCallStore();
  }
  _getFeatureButtonDefaultState(A) {
    var e;
    const { button: t } = d.getData(r.CALL, o.CUSTOM_UI_CONFIG);
    return (e = t == null ? void 0 : t[A]) == null ? void 0 : e.state;
  }
  _updateCallDuration() {
    const A = Math.round((Ot() - this._startTimeStamp) / 1e3), t = Io(A);
    d.update(r.CALL, o.CALL_DURATION, t);
  }
  _stopTimer() {
    this._timerId !== -1 && (st.clearTask(this._timerId), this._timerId = -1);
  }
  _resetCallStore() {
    const A = Ze();
    this._stopTimer();
    let t = Object.keys($e).filter((a) => {
      switch ($e[a]) {
        case o.CALL_STATUS:
        case o.LANGUAGE:
        case o.IS_GROUP:
        case o.DISPLAY_MODE:
        case o.VIDEO_RESOLUTION:
        case o.ENABLE_FLOAT_WINDOW:
        case o.LOCAL_USER_INFO:
        case o.IS_SHOW_ENABLE_VIRTUAL_BACKGROUND:
        case o.LOCAL_USER_INFO_EXCLUDE_VOLUMN:
          return !1;
        default:
          return !0;
      }
    });
    t = t.map((a) => $e[a]), d.reset(r.CALL, t), d.getData(r.CALL, o.CALL_STATUS) !== w.IDLE && d.reset(r.CALL, [o.CALL_STATUS], !0), d.reset(r.CALL, [o.IS_MINIMIZED], !0), d.reset(r.CALL, [o.IS_EAR_PHONE], !0), d.reset(r.CALL, [o.ENABLE_VIRTUAL_BACKGROUND], !0), d.reset(r.CALL, [o.IS_MUTE_SPEAKER], !0), d.update(r.CALL, o.LOCAL_USER_INFO, {
      ...d.getData(r.CALL, o.LOCAL_USER_INFO),
      isVideoAvailable: !1,
      isAudioAvailable: !1
    }), d.update(r.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, {
      ...d.getData(r.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN),
      isVideoAvailable: !1,
      isAudioAvailable: !1
    }), d.update(r.CALL, o.REMOTE_USER_INFO_LIST, []), d.update(r.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, []), d.update(r.CALL, o.CAMERA_POSITION, FA.FRONT);
    const n = Ze();
    A !== n && this.statusChanged && this.statusChanged({ oldStatus: A, newStatus: n });
  }
  async getGroupMemberList(A, t) {
    const e = d.getData(r.CALL, o.GROUP_ID);
    return await Ni(e, this.getTim(), A, t);
  }
  async getGroupProfile() {
    const A = d.getData(r.CALL, o.GROUP_ID);
    return await fi(A, this.getTim());
  }
  _watchTUIStore() {
    d == null || d.watch(r.CALL, {
      [o.CALL_STATUS]: this._handleCallStatusChange
    });
  }
  _unwatchTUIStore() {
    d == null || d.unwatch(r.CALL, {
      [o.CALL_STATUS]: this._handleCallStatusChange
    });
  }
  bindTUICore(A) {
    this._TUICore = A;
  }
  getTim() {
    var A, t;
    return this._tim ? this._tim : this._tuiCallEngine ? ((A = this._tuiCallEngine) == null ? void 0 : A.tim) || ((t = this._tuiCallEngine) == null ? void 0 : t.getTim()) : (console.warn(`${o.PREFIX}getTim warning: _tuiCallEngine Instance is not available.`), null);
  }
  setIsFromChat(A) {
    this._isFromChat = A;
  }
  setCurrentGroupId(A) {
    this._currentGroupId = A;
  }
  getCurrentGroupId() {
    return this._currentGroupId;
  }
  setDefaultOfflinePushInfo(A) {
    this._offlinePushInfo = A;
  }
  getDefaultOfflinePushInfo() {
    const A = d.getData(r.CALL, o.LOCAL_USER_INFO);
    return this._offlinePushInfo ? this._offlinePushInfo : {
      title: (A == null ? void 0 : A.displayUserInfo) || "",
      description: WA("you have a new call")
    };
  }
  async getCallMessage(A) {
    return await this._chatCombine.getCallKitMessage(A, this.getTim());
  }
};
let R = oe;
G([
  K(),
  gA(IA.init)
], R.prototype, "init", 1);
G([
  K(),
  gA(IA.call),
  Ae({ engineInstance: !0 })
], R.prototype, "call", 1);
G([
  K(),
  gA(IA.groupCall),
  Ae({ engineInstance: !0 })
], R.prototype, "groupCall", 1);
G([
  K(),
  gA(IA.inviteUser),
  Ae({ engineInstance: !0 })
], R.prototype, "inviteUser", 1);
G([
  K(),
  gA(IA.joinInGroupCall),
  Ae({ engineInstance: !0 })
], R.prototype, "joinInGroupCall", 1);
G([
  K(),
  Ae({ engineInstance: !0 })
], R.prototype, "calls", 1);
G([
  K(),
  Ae({ engineInstance: !0 })
], R.prototype, "join", 1);
G([
  gA(IA.setLanguage)
], R.prototype, "setLanguage", 1);
G([
  gA(IA.enableFloatWindow)
], R.prototype, "enableFloatWindow", 1);
G([
  gA(IA.setSelfInfo)
], R.prototype, "setSelfInfo", 1);
G([
  gA(IA.enableAIVoice)
], R.prototype, "enableAIVoice", 1);
G([
  gA(IA.setCallingBell)
], R.prototype, "setCallingBell", 1);
G([
  gA(IA.enableMuteMode)
], R.prototype, "enableMuteMode", 1);
G([
  K()
], R.prototype, "accept", 1);
G([
  K()
], R.prototype, "hangup", 1);
G([
  K()
], R.prototype, "reject", 1);
G([
  K()
], R.prototype, "openCamera", 1);
G([
  K()
], R.prototype, "closeCamera", 1);
G([
  K()
], R.prototype, "openMicrophone", 1);
G([
  K()
], R.prototype, "closeMicrophone", 1);
G([
  K()
], R.prototype, "unMuteSpeaker", 1);
G([
  K()
], R.prototype, "muteSpeaker", 1);
G([
  K()
], R.prototype, "switchScreen", 1);
G([
  K()
], R.prototype, "switchCallMediaType", 1);
G([
  K()
], R.prototype, "switchCamera", 1);
G([
  K()
], R.prototype, "setBlurBackground", 1);
G([
  K()
], R.prototype, "switchDevice", 1);
G([
  gA(IA.setVideoDisplayMode)
], R.prototype, "setVideoDisplayMode", 1);
G([
  gA(IA.setVideoResolution)
], R.prototype, "setVideoResolution", 1);
const O = R.getInstance(), Pt = Xt.version, Co = "CallInfoContextKey", No = "CallerUserInfoContextKey", fo = "UserInfoExcludeVolumeContextKey", vo = "FocusContextKey", Lo = "ButtonPanelContextKey", ho = "FloatWindowContextKey", mo = "IsClickableContextKey", So = "PopoverContextKey", Do = "CustomUIConfigContextKey", yo = "translateContextKey";
function zi() {
  return dA(No);
}
function Z() {
  return dA(Co);
}
function yt(A) {
  const t = I([]), e = I(""), n = (s) => {
    var i, u, l;
    switch (A) {
      case oA.CAMERA:
        t.value = (s == null ? void 0 : s.cameraList) || [], e.value = ((i = s == null ? void 0 : s.currentCamera) == null ? void 0 : i.deviceId) || "";
        break;
      case oA.MICROPHONE:
        t.value = (s == null ? void 0 : s.microphoneList) || [], e.value = ((u = s == null ? void 0 : s.currentMicrophone) == null ? void 0 : u.deviceId) || "";
        break;
      case oA.SPEAKER:
        t.value = (s == null ? void 0 : s.speakerList) || [], e.value = ((l = s == null ? void 0 : s.currentSpeaker) == null ? void 0 : l.deviceId) || "";
        break;
    }
  }, a = (s) => {
    e.value = s;
  };
  return sA(() => {
    d.watch(
      r.CALL,
      {
        [o.DEVICE_LIST]: n
      },
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), tA(() => {
    d.unwatch(r.CALL, {
      [o.DEVICE_LIST]: n
    });
  }), [{ deviceList: t, currentDeviceId: e }, { updateCurrentDeviceId: a }];
}
const Bn = (A) => typeof A == "object" && A !== null, Eo = (A, t) => {
  if (!Bn(A) || !Bn(t))
    return A === t;
  if (A === t)
    return !0;
  const e = Object.keys(A), n = Object.keys(t);
  if (e.length !== n.length)
    return !1;
  for (let a in A)
    if (!Eo(A[a], t[a]))
      return !1;
  return !0;
};
function Ui(A) {
  const t = document.getElementById(A);
  !document.fullscreenElement && t ? t.requestFullscreen().catch((e) => {
    console.error(`Error attempting to enable fullscreen mode: ${e.message} (${e.name})`);
  }) : document.exitFullscreen();
}
function Et(A) {
  if (typeof A != "object" || A === null)
    return A;
  let t = Array.isArray(A) ? [] : {};
  for (let e in A)
    A.hasOwnProperty(e) && (t[e] = Et(A[e]));
  return t;
}
const Oi = 500;
function Kt(A, t) {
  if (typeof A != "function" || t != null && typeof t != "function")
    throw new TypeError("Expected a function");
  const e = function(...n) {
    const a = t ? t.apply(this, n) : n[0], s = e.cache;
    if (s.has(a))
      return s.get(a);
    const i = A.apply(this, n);
    return e.cache = s.set(a, i) || s, i;
  };
  return e.cache = new (Kt.Cache || Map)(), e;
}
Kt.Cache = Map;
function xi(A) {
  const t = Kt(A, (e) => {
    const { cache: n } = t;
    return n.size === Oi && n.clear(), e;
  });
  return t;
}
const Bi = ".".charCodeAt(0), Vi = /\\(\\)?/g, Pi = RegExp(
  `[^.[\\]]+|\\[(?:([^"'][^[]*)|(["'])((?:(?!\\2)[^\\\\]|\\\\.)*?)\\2)\\]|(?=(?:\\.|\\[\\])(?:\\.|\\[\\]|$))`,
  "g"
), To = xi((A) => {
  const t = [];
  return A.charCodeAt(0) === Bi && t.push(""), A.replace(Pi, (e, n, a, s) => {
    let i = e;
    a ? i = s.replace(Vi, "$1") : n && (i = n.trim()), t.push(i);
  }), t;
});
function RA(A, t, e) {
  if (typeof A != "object" || !t)
    return;
  const n = To(t);
  let a = A;
  for (let s = 0; s < n.length; s++) {
    if (a == null)
      return;
    const i = n[s];
    s !== n.length - 1 ? a = a == null ? void 0 : a[i] : Object.assign(a, { [i]: e });
  }
}
function ne(A, t, e) {
  if (typeof A != "object")
    return;
  const n = To(t);
  let a = A;
  for (let s = 0; s < n.length; s++) {
    if (a == null)
      return;
    const i = n[s];
    s !== n.length - 1 ? a = a == null ? void 0 : a[i] : Array.isArray(a) && a.splice(i, 0, e);
  }
}
function wo(A) {
  return A == null || typeof A == "string" && A.trim().length === 0 ? !0 : Array.isArray(A) || typeof A == "object" ? Object.keys(A).length === 0 : !1;
}
function kt(A, t, e, n, a) {
  Object.keys(A).forEach((s) => {
    const i = A[s], u = wo(e) ? s : `${e}.${s}`;
    if (typeof i == "object")
      if (Array.isArray(i))
        for (let l = 0; l < i.length; l++) {
          const c = `${u}.${l}`;
          kt(i[l], t, c, n, a);
        }
      else
        kt(i, t, u, n, a);
    else if (t(i)) {
      const l = typeof a == "function" ? a({ key: i, value: u }) : u;
      n.push(l);
    }
  });
}
function jo() {
  const A = I(), t = I(d.getData(r.CALL, o.REMOTE_USER_INFO_LIST)), e = (s) => {
    (y.isWeChat ? s.enableMic : s.isAudioAvailable) && (A.value = { ...A.value, localVideo: s.volume });
  }, n = (s) => {
    const i = {};
    (s.length !== t.value.length || !Eo(s, t.value)) && (t.value = s, t.value.forEach((u) => {
      if (y.isWeChat ? u.hasAudio : u.isAudioAvailable) {
        const c = y.isWeChat ? u.userID : u.domId;
        i[c] = u.volume;
      }
    }), A.value = { ...A.value, ...i });
  };
  let a = {
    [o.LOCAL_USER_INFO]: e,
    [o.REMOTE_USER_INFO_LIST]: n
  };
  return y.isUniPlatform && (a = {
    [o.PUSHER]: e,
    [o.PLAYER]: n
  }), sA(() => {
    d.watch(
      r.CALL,
      a,
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), tA(() => {
    d.unwatch(r.CALL, a);
  }), A;
}
function Fi() {
  const A = I(""), t = I(!0), e = I(0), n = (a) => {
    typeof a == "object" ? (A.value = a.text, e.value = a.duration || 0) : A.value = a;
  };
  return sA(() => {
    d.watch(
      r.CALL,
      {
        [o.CALL_TIPS]: n
      },
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), tA(() => {
    d.unwatch(
      r.CALL,
      {
        [o.CALL_TIPS]: n
      }
    );
  }), { tip: A, show: t, duration: e };
}
function Ri() {
  const A = I(d.getData(r.CALL, o.NETWORK_STATUS)), t = (e) => {
    A.value = e;
  };
  return sA(() => {
    d.watch(
      r.CALL,
      {
        [o.NETWORK_STATUS]: t
      },
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), tA(() => {
    d.unwatch(r.CALL, {
      [o.NETWORK_STATUS]: t
    });
  }), { netWorkQualityList: A };
}
function bo() {
  const A = I(d.getData(r.CALL, o.PLAYER)), t = (n) => {
    A.value = n == null ? void 0 : n.map((a) => {
      const { userID: s, hasVideo: i, hasAudio: u } = a;
      return { userID: s, hasVideo: i, hasAudio: u };
    });
  }, e = {
    [o.PLAYER]: t
  };
  return sA(() => {
    d.watch(
      r.CALL,
      e,
      { notifyRangeWhenWatch: o.MYSELF }
    );
  }), tA(() => {
    d.unwatch(r.CALL, e);
  }), A;
}
function pA() {
  return dA(fo);
}
function Ht() {
  const A = I(d.getData(r.CALL, o.CALL_DURATION)), t = (e) => {
    A.value = e;
  };
  return sA(() => {
    d.watch(
      r.CALL,
      {
        [o.CALL_DURATION]: t
      },
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), tA(() => {
    d.unwatch(
      r.CALL,
      {
        [o.CALL_DURATION]: t
      }
    );
  }), { callDuration: A };
}
function _e() {
  return dA(Lo, { status: I("open") });
}
function zo() {
  return dA(vo, I("open"));
}
function vA() {
  return dA(ho);
}
function Gi() {
  return dA(mo);
}
function Tt() {
  return dA(So);
}
const ki = (A, t) => {
  const e = [{
    i: 0,
    x: 0,
    y: 0,
    w: t,
    h: t
  }];
  for (let n = 1; n < A; n++) {
    const a = e[n - 1].x + t === 12;
    e[n] = {
      i: n,
      x: e[n - 1].x + t === 12 ? 0 : e[n - 1].x + t,
      y: e[n - 1].y + (a ? t : 0),
      w: t,
      h: t
    };
  }
  return A === 3 && (e[A - 1].x += 3), A > 3 && y.isPC && A % 3 === 2 && (e[A - 1].x += 2, e[A - 2].x += 2), e;
};
function _i(A, t) {
  const { isFloatWindow: e } = j(vA()), n = I();
  return Y([A, t, e], () => {
    if (e.value) {
      const c = [];
      for (let g = 0; g < t.value; g++)
        c[g] = {
          i: g,
          x: 0,
          y: 0,
          w: 12,
          h: 12
        };
      n.value = c;
      return;
    }
    const a = Object.keys(Array.from({ length: t.value })), s = t.value <= 4 ? 6 : 4, i = ki(t.value, s);
    n.value = i;
    let u, l;
    if (A.value !== null) {
      if (a.length < 5) {
        const c = a.concat();
        c.splice(A.value, 1), c.unshift(A.value);
      } else
        l = A.value % 3, u = Math.floor(A.value / 3);
      if (a.length < 5) {
        const c = i.findIndex((g) => g.i === A.value);
        if (c !== -1) {
          const g = i[0];
          i[0] = i[c], i[c] = g;
          for (let N = 0; N < i.length; N++) {
            const p = i[N];
            N === 0 ? (p.w += 6, p.h += 6, p.x = 0, p.y = 0) : (p.x = (N - 1) * 4, p.y = 12, p.w = 4, p.h = 4);
          }
        }
      } else {
        let c;
        l === 0 ? (i[A.value + 1] && (i[A.value + 1].x += 4), i[A.value + 2] && (i[A.value + 2].y += 4), c = {
          i: A.value,
          x: 0,
          y: u * 4,
          w: 8,
          h: 8
        }) : l === 2 ? (c = {
          i: A.value,
          x: 4,
          y: u * 4,
          w: 8,
          h: 8
        }, i[A.value - 1].x = 0, i[A.value - 1].y += 4) : l === 1 && (c = {
          i: A.value,
          x: 4,
          y: u * 4,
          w: 8,
          h: 8
        }, i[A.value + 1] && (i[A.value + 1].x = 0, i[A.value + 1].y += 4));
        const g = 3 - l;
        for (let N = A.value + g; N < i.length; N++) {
          const p = i[N];
          p.y += 4;
        }
        i[A.value] = c;
      }
      n.value = i;
    }
  }, {
    immediate: !0
  }), n;
}
function We() {
  return dA(Do);
}
const Wi = {
  calling: [[
    { name: "switchCamera", customStyle: { justifyContent: "flex-start" }, props: {} },
    { name: "virtualBackground" },
    { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
  ], [
    {},
    { name: "hangup", customStyle: { paddingTop: "6vh" } },
    {}
  ]],
  accept: [[
    { name: "switchCamera", customStyle: { justifyContent: "flex-start" }, props: {} },
    { name: "virtualBackground" },
    { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
  ], [
    { name: "reject", customStyle: { paddingTop: "6vh", justifyContent: "center" } },
    {},
    { name: "accept", customStyle: { paddingTop: "6vh", justifyContent: "center" } }
  ]],
  connected: [[
    { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
    { name: "speaker" },
    { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
  ], [
    { name: "virtualBackground", customStyle: { justifyContent: "center", paddingTop: "6vh" }, props: {} },
    { name: "hangup", customStyle: { paddingTop: "6vh" } },
    { name: "switchCamera", customStyle: { justifyContent: "center", paddingTop: "6vh" }, props: {} }
  ]]
}, qi = {
  singleCall: {
    video: {
      calling: [
        [],
        [
          {},
          { name: "hangup" },
          {}
        ]
      ],
      accept: [
        [],
        [
          { name: "reject", customStyle: { justifyContent: "flex-end" } },
          {},
          { name: "accept", customStyle: { justifyContent: "flex-start" } }
        ]
      ],
      connected: [
        [
          { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
          { name: "speaker" },
          { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
        ],
        [
          {},
          { name: "hangup", customStyle: { paddingTop: "6vh" } },
          { name: "switchCamera", customStyle: { justifyContent: "center", paddingTop: "6vh" }, props: {} }
        ]
      ]
    },
    audio: {
      calling: [
        [],
        [
          {},
          { name: "hangup" },
          {}
        ]
      ],
      accept: [
        [],
        [
          { name: "reject", customStyle: { justifyContent: "flex-end" } },
          {},
          { name: "accept", customStyle: { justifyContent: "flex-start" } }
        ]
      ],
      connected: [
        [],
        [
          { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
          { name: "hangup" },
          { name: "speaker", customStyle: { justifyContent: "flex-end" } }
        ]
      ]
    }
  },
  groupCall: {
    video: {
      calling: [[
        { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
        { name: "speaker" },
        { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
      ], [
        {},
        { name: "hangup", customStyle: { paddingTop: "6vh" } },
        {}
      ]],
      accept: [
        [],
        [
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
          { name: "speaker" },
          { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
        ],
        [
          {},
          { name: "hangup", customStyle: { paddingTop: "6vh" } },
          {}
        ]
      ],
      close_calling: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "camera", props: {} },
          { name: "hangup" }
        ]
      ],
      close_connected: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "camera", props: {} },
          { name: "hangup" }
        ]
      ]
    },
    audio: {
      calling: [[
        { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
        { name: "speaker" },
        { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
      ], [
        {},
        { name: "hangup", customStyle: { paddingTop: "6vh" } },
        {}
      ]],
      accept: [
        [],
        [
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
          { name: "speaker" },
          { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
        ],
        [
          {},
          { name: "hangup", customStyle: { paddingTop: "6vh" } },
          {}
        ]
      ],
      close_calling: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "camera", props: {} },
          { name: "hangup" }
        ]
      ],
      close_connected: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "camera", props: {} },
          { name: "hangup" }
        ]
      ]
    }
  }
}, Xi = {
  singleCall: {
    video: {
      calling: [
        [
          { name: "camera", props: {} },
          { name: "microphone", props: {} },
          { name: "hangup" }
        ]
      ],
      accept: [
        [
          { name: "camera", props: {} },
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "camera", props: {} },
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "hangup" }
        ]
      ]
    },
    audio: {
      calling: [
        [
          { name: "microphone", props: {} },
          { name: "hangup" }
        ]
      ],
      accept: [
        [
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "hangup" }
        ]
      ]
    }
  },
  groupCall: {
    video: {
      calling: [[
        { name: "camera", props: {} },
        { name: "microphone", props: {} },
        { name: "inviteUser", props: {} },
        { name: "hangup" }
      ]],
      accept: [
        [
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "camera", props: {} },
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "inviteUser", props: {} },
          { name: "hangup" }
        ]
      ]
    },
    audio: {
      calling: [
        [
          { name: "microphone", props: {} },
          { name: "hangup" }
        ]
      ],
      accept: [
        [
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "inviteUser", props: {} },
          { name: "hangup" }
        ]
      ]
    }
  }
}, Vn = {
  pc: Xi,
  mobile: qi
};
function Uo(A) {
  const t = Et(A);
  return RA(t, "mobile.singleCall.video", Wi), ne(t, "pc.singleCall.video.calling[0][2]", { name: "virtualBackground", props: {} }), ne(t, "pc.singleCall.video.accept[0][1]", { name: "virtualBackground", props: {} }), ne(t, "pc.singleCall.video.connected[0][3]", { name: "virtualBackground", props: {} }), ne(t, "pc.groupCall.video.calling[0][3]", { name: "virtualBackground", props: {} }), ne(t, "pc.groupCall.video.connected[0][4]", { name: "virtualBackground", props: {} }), t;
}
function Qi(A, t, e) {
  let n = Et(A);
  return t ? (RA(n, "mobile.singleCall.video.connected[1][2].props.show", !0), e && Uo(n)) : (RA(n, "mobile.singleCall.video.connected[1][2].props.show", !1), e && (RA(n, "mobile.singleCall.video.connected[1][0].props.show", !1), RA(n, "pc.singleCall.video.connected[0][3].props.show", !1), RA(n, "pc.groupCall.video.connected[0][4].props.show", !1))), n;
}
function Zi() {
  const { isShowEnableVirtualBackground: A, callStatus: t } = j(Z()), e = We(), { localUserInfoExcludeVolume: n } = j(pA()), a = C(() => (n == null ? void 0 : n.value.isVideoAvailable) || !1), s = C(() => A.value && !y.isH5), i = I([]);
  return Y([e, A, a], () => {
    let u = Et(Vn);
    s.value && (u = Uo(Vn)), t.value === w.CONNECTED && (u = Qi(u, a.value, s.value));
    const { button: l } = e.value, c = [];
    function g(p) {
      return Object.keys(l).includes(p);
    }
    function N({ key: p, value: M }) {
      var E;
      const f = M.split(".");
      let v = f.slice(0, f.length - 1);
      const S = f.slice(0, f.length - 2);
      if (S[S.length - 1] === "0")
        for (let x = 0; x < 3; x++) {
          let m = S.slice();
          m.push(x), m.push("customStyle"), m.push("justifyContent"), m = m.join("."), c.push({ path: m, value: "center" });
        }
      return v.push("props"), v.push("show"), v = v.join("."), {
        path: v,
        value: (E = l == null ? void 0 : l[p]) == null ? void 0 : E.show
      };
    }
    kt(u, g, "", c, N), c == null || c.forEach((p) => {
      RA(u, p.path, p.value);
    }), i.value = u;
  }, {
    immediate: !0
  }), i;
}
function Oo() {
  const A = We(), t = I(A.value.viewBackground);
  return Y(A, () => {
    t.value = A.value.viewBackground;
  }), t;
}
function Yi() {
  const A = I(d.getData(r.CALL, o.ROOM_ID)), t = I(d.getData(r.CALL, o.ROOM_ID_TYPE)), e = I(d.getData(r.CALL, o.GROUP_ID)), n = I(d.getData(r.CALL, o.CALL_MEDIA_TYPE)), a = I(d.getData(r.CALL, o.GROUP_CALL_MEMBERS)), s = I(d.getData(r.CALL, o.CALL_STATUS)), i = (M) => {
    A.value = M;
  }, u = (M) => {
    e.value = M;
  }, l = (M) => {
    n.value = M;
  }, c = (M) => {
    a.value = M;
  }, g = (M) => {
    s.value = M;
  }, N = (M) => {
    t.value = M;
  }, p = {
    [o.ROOM_ID]: i,
    [o.GROUP_ID]: u,
    [o.CALL_MEDIA_TYPE]: l,
    [o.GROUP_CALL_MEMBERS]: c,
    [o.CALL_STATUS]: g,
    [o.ROOM_ID_TYPE]: N
  };
  return sA(() => {
    d.watch(
      r.CALL,
      p,
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), tA(() => {
    d.unwatch(r.CALL, p);
  }), {
    roomId: A,
    roomIdType: t,
    groupId: e,
    callMediaType: n,
    groupCallMembers: a,
    callStatus: s
  };
}
function J() {
  return dA(yo, I(li));
}
const Ki = {
  customStyle: {
    type: Object
  },
  customClass: {
    type: String
  }
}, Hi = ["start", "center", "end", "space-around", "space-between", "space-evenly"], Ji = ["top", "middle", "bottom"], $i = {
  gutter: {
    type: Number,
    default: 0
  },
  justify: {
    type: String,
    values: Hi,
    default: "start"
  },
  align: {
    type: String,
    values: Ji,
    default: "top"
  },
  customStyle: {
    type: Object,
    default: () => {
    }
  }
};
function X(...A) {
  const t = [], e = {}.hasOwnProperty;
  for (let n = 0; n < A.length; n++) {
    const a = A[n];
    if (!a)
      continue;
    const s = typeof a;
    if (s === "string" || s === "number")
      t.push(a);
    else if (Array.isArray(a)) {
      if (a.length) {
        const i = X.apply(null, a);
        i && t.push(i);
      }
    } else if (s === "object") {
      if (a.toString !== Object.prototype.toString && !a.toString.toString().includes("[native code]")) {
        t.push(a.toString());
        continue;
      }
      for (const i in a)
        e.call(a, i) && a[i] && t.push(i);
    }
  }
  return t.join(" ");
}
const re = typeof wx < "u" && typeof wx.getSystemInfoSync == "function" && Boolean(wx.getSystemInfoSync().fontSizeSetting), Al = typeof uni < "u" && typeof uni > "u", _t = re || Al, el = typeof uni < "u", xo = function() {
  return (typeof uni < "u" || typeof window < "u") && !_t;
}();
(function() {
  return re ? wx : el ? uni : window;
})();
const UA = xo && window && window.navigator && window.navigator.userAgent || "", tl = /Android/i.test(UA), nl = /(?:Windows Phone)/.test(UA), al = /(?:SymbianOS)/.test(UA), ol = /iPad/i.test(UA) || /iPhone/i.test(UA) || /iPod/i.test(UA), Bo = tl || nl || al || ol, qA = xo && !Bo;
qA && UA.includes("Windows NT");
qA && UA.includes("Mac");
function it(A) {
  const t = {};
  for (const e in A)
    Object.prototype.hasOwnProperty.call(A, e) && A[e] !== void 0 && (t[e] = A[e]);
  return t;
}
function sl() {
  let A = "unknow";
  return typeof Pt == "string" && (A = Pt.split(".")[0]), { version: Pt, majorVersion: A };
}
function Pn(A, t) {
  const { key: e, value: n } = t;
  return A.find((a) => a[e] === n);
}
const Vo = "RowContextKey", z = "tk", se = {
  SUCCESS: "success",
  INFO: "info",
  WARNING: "warning",
  ERROR: "error"
}, Jt = {
  options: {
    virtualHost: !0
  }
};
Jt.props = $i;
Jt.setup = (A, t) => {
  const e = A, n = C(() => e.gutter);
  return rA(Vo, {
    gutter: n
  }), {
    classname: X([`${z}-row`, `${z}-justify-${e.justify}`, `${z}-align-${e.align}`])
  };
};
var il = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.classname,
    style: [A.customStyle]
  }, [A._t("default")], 2);
}, ll = [];
function L(A, t, e, n, a, s, i, u) {
  var l = typeof A == "function" ? A.options : A;
  t && (l.render = t, l.staticRenderFns = e, l._compiled = !0), n && (l.functional = !0), s && (l._scopeId = "data-v-" + s);
  var c;
  if (i ? (c = function(p) {
    p = p || this.$vnode && this.$vnode.ssrContext || this.parent && this.parent.$vnode && this.parent.$vnode.ssrContext, !p && typeof __VUE_SSR_CONTEXT__ < "u" && (p = __VUE_SSR_CONTEXT__), a && a.call(this, p), p && p._registeredComponents && p._registeredComponents.add(i);
  }, l._ssrRegister = c) : a && (c = u ? function() {
    a.call(
      this,
      (l.functional ? this.parent : this).$root.$options.shadowRoot
    );
  } : a), c)
    if (l.functional) {
      l._injectStyles = c;
      var g = l.render;
      l.render = function(M, f) {
        return c.call(f), g(M, f);
      };
    } else {
      var N = l.beforeCreate;
      l.beforeCreate = N ? [].concat(N, c) : [c];
    }
  return {
    exports: A,
    options: l
  };
}
const Fn = {};
var ul = /* @__PURE__ */ L(
  Jt,
  il,
  ll,
  !1,
  rl,
  null,
  null,
  null
);
function rl(A) {
  for (let t in Fn)
    this[t] = Fn[t];
}
const $t = /* @__PURE__ */ function() {
  return ul.exports;
}(), cl = ["start", "center", "end", "space-around", "space-between", "space-evenly"], dl = ["top", "middle", "bottom"], gl = {
  span: {
    type: Number,
    default: 24
  },
  justify: {
    type: String,
    values: cl,
    default: "start"
  },
  align: {
    type: String,
    values: dl,
    default: "middle"
  },
  offset: {
    type: Number,
    default: 0
  }
}, An = {
  options: {
    virtualHost: !0
  }
};
An.props = gl;
An.setup = (A, t) => {
  const e = A, {
    gutter: n
  } = dA(Vo, {
    gutter: C(() => 0)
  }), a = X([`${z}-col`, `${z}-justify-${e.justify}`, `${z}-align-${e.align}`]), s = C(() => `${e.span / 24 * 100}%`), i = C(() => `${e.offset / 24 * 100}%`), u = C(() => `${n.value / 2}px`);
  return {
    classname: a,
    width: s,
    marginLeft: i,
    paddingLeft: u,
    paddingRight: u
  };
};
var Il = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.classname,
    style: {
      width: A.width,
      marginLeft: A.marginLeft,
      paddingLeft: A.paddingLeft,
      paddingRight: A.paddingRight
    }
  }, [A._t("default")], 2);
}, pl = [];
const Rn = {};
var Ml = /* @__PURE__ */ L(
  An,
  Il,
  pl,
  !1,
  Cl,
  null,
  null,
  null
);
function Cl(A) {
  for (let t in Rn)
    this[t] = Rn[t];
}
const en = /* @__PURE__ */ function() {
  return Ml.exports;
}(), Nl = {
  width: {
    type: String,
    default: "40px"
  },
  height: {
    type: String,
    default: "40px"
  },
  color: {
    type: String
  }
}, tn = {
  options: {
    virtualHost: !0
  }
};
tn.props = Nl;
tn.setup = (A, t) => {
  const e = A, n = `${z}-loading_dot-container`, a = `${z}-loading_dot`, s = C(() => ({
    width: e.width,
    height: e.height
  })), i = C(() => {
    const u = {};
    return e.color && (u.backgroundColor = e.color), u;
  });
  return {
    dotContainerClassName: n,
    dotClassName: a,
    dotContainerStyle: s,
    dotStyle: i
  };
};
var fl = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.dotContainerClassName,
    style: [A.dotContainerStyle]
  }, [e("div", {
    class: A.dotClassName,
    style: [A.dotStyle]
  }), e("div", {
    class: A.dotClassName,
    style: [A.dotStyle]
  }), e("div", {
    class: A.dotClassName,
    style: [A.dotStyle]
  })]);
}, vl = [];
const Gn = {};
var Ll = /* @__PURE__ */ L(
  tn,
  fl,
  vl,
  !1,
  hl,
  null,
  null,
  null
);
function hl(A) {
  for (let t in Gn)
    this[t] = Gn[t];
}
const ml = /* @__PURE__ */ function() {
  return Ll.exports;
}(), Sl = {
  width: {
    type: String,
    default: "40px"
  },
  height: {
    type: String,
    default: "40px"
  },
  color: {
    type: String
  }
}, nn = {
  options: {
    virtualHost: !0
  }
};
nn.props = Sl;
nn.setup = (A, t) => {
  const e = A, n = C(() => {
    const s = {
      width: e.width,
      height: e.height
    };
    return e.color && (s["--tk-loading-primary-color"] = e.color), s;
  }), a = `${z}-loading_circle-container`;
  return {
    circleContainerStyle: n,
    circleContainerClassName: a
  };
};
var Dl = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.circleContainerClassName,
    style: [A.circleContainerStyle]
  });
}, yl = [];
const kn = {};
var El = /* @__PURE__ */ L(
  nn,
  Dl,
  yl,
  !1,
  Tl,
  null,
  null,
  null
);
function Tl(A) {
  for (let t in kn)
    this[t] = kn[t];
}
const wl = /* @__PURE__ */ function() {
  return El.exports;
}(), jl = ["circle", "dot"], bl = ["row", "column"], zl = {
  mode: {
    type: String,
    values: jl,
    default: "circle"
  },
  loadingWidth: {
    type: String,
    default: "40px"
  },
  loadingHeight: {
    type: String,
    default: "40px"
  },
  color: {
    type: String
  },
  text: {
    type: String
  },
  layout: {
    type: String,
    values: bl,
    default: "column"
  }
}, ce = {
  options: {
    virtualHost: !0
  }
};
ce.props = zl;
ce.setup = (A, t) => {
  const e = A, n = `${z}-loading ${z}-loading--${e.mode}`, a = `${z}-loading_text`, s = C(() => ({
    flexDirection: e.layout
  }));
  return {
    loadingContainerClassName: n,
    loadingTextClassName: a,
    loadingContainerStyle: s
  };
};
ce.components = Object.assign({
  LoadingDot: ml,
  LoadingCircle: wl
}, ce.components);
var Ul = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.loadingContainerClassName,
    style: [A.loadingContainerStyle]
  }, [A.mode === "dot" ? e("LoadingDot", {
    attrs: {
      width: A.loadingWidth,
      height: A.loadingHeight,
      color: A.color
    }
  }) : e("LoadingCircle", {
    attrs: {
      width: A.loadingWidth,
      height: A.loadingHeight,
      color: A.color
    }
  }), A.text ? e("div", {
    class: A.loadingTextClassName
  }, [A._v(A._s(A.text))]) : A._e()], 1);
}, Ol = [];
const _n = {};
var xl = /* @__PURE__ */ L(
  ce,
  Ul,
  Ol,
  !1,
  Bl,
  null,
  null,
  null
);
function Bl(A) {
  for (let t in _n)
    this[t] = _n[t];
}
const an = /* @__PURE__ */ function() {
  return xl.exports;
}(), Vl = {
  src: {
    type: String
  },
  size: {
    type: Number,
    default: 20
  }
}, Pl = ["fill", "contain", "cover"], Wn = {
  fill: "scaleToFill",
  contain: "aspectFit",
  cover: "aspectFill"
}, Fl = {
  width: {
    type: String,
    default: "320px"
  },
  height: {
    type: String,
    default: "240px"
  },
  src: {
    type: String
  },
  fit: {
    type: String,
    values: Pl,
    default: "fill"
  },
  customStyle: {
    type: Object
  },
  defaultSrc: {
    type: String
  }
}, on = {
  options: {
    virtualHost: !0
  }
};
on.props = Fl;
on.setup = (A, t) => {
  const e = A, n = t.emit, a = I(!1), s = I(e.src);
  Y(() => e.src, () => {
    a.value = !1, s.value = e.src;
  });
  const i = C(() => X([`${z}-image`])), u = C(() => ({
    width: e.width,
    height: e.height,
    ...e.customStyle
  })), l = C(() => X([`${z}-image_inner`])), c = C(() => re ? Wn[e.fit] || Wn.cover : e.fit);
  function g(N) {
    a.value = !0, s.value = e.defaultSrc, n("error", N);
  }
  return {
    IN_WX_MINI_APP: re,
    hasLoadError: a,
    imgSrc: s,
    classname: i,
    imgStyle: u,
    imageClassName: l,
    objectFit: c,
    handleError: g
  };
};
var Rl = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.classname,
    style: [A.imgStyle]
  }, [A.hasLoadError && !A.defaultSrc ? [A._t("error")] : [A.IN_WX_MINI_APP ? A._e() : e("img", {
    class: A.imageClassName,
    style: {
      objectFit: A.objectFit
    },
    attrs: {
      src: A.imgSrc
    },
    on: {
      error: A.handleError
    }
  }), A.IN_WX_MINI_APP ? e("image", {
    class: A.imageClassName,
    attrs: {
      src: A.imgSrc,
      mode: A.objectFit
    },
    on: {
      error: A.handleError
    }
  }) : A._e()]], 2);
}, Gl = [];
const qn = {};
var kl = /* @__PURE__ */ L(
  on,
  Rl,
  Gl,
  !1,
  _l,
  null,
  null,
  null
);
function _l(A) {
  for (let t in qn)
    this[t] = qn[t];
}
const mA = /* @__PURE__ */ function() {
  return kl.exports;
}(), de = {
  options: {
    virtualHost: !0
  }
};
de.props = Vl;
de.setup = (A, t) => {
  const e = A, n = C(() => `${e.size}px`), a = C(() => `${e.size}px`);
  return {
    width: n,
    height: a
  };
};
de.components = Object.assign({
  TKImage: mA
}, de.components);
var Wl = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("TKImage", {
    attrs: {
      src: A.src,
      width: A.width,
      height: A.height
    }
  });
}, ql = [];
const Xn = {};
var Xl = /* @__PURE__ */ L(
  de,
  Wl,
  ql,
  !1,
  Ql,
  null,
  null,
  null
);
function Ql(A) {
  for (let t in Xn)
    this[t] = Xn[t];
}
const wt = /* @__PURE__ */ function() {
  return Xl.exports;
}(), Zl = ["small", "middle", "large"], Yl = ["row", "column"], Kl = ["circle", "round"], Hl = {
  iconSrc: {
    type: String
  },
  iconSize: {
    type: Number
  },
  text: {
    type: String
  },
  loading: {
    type: Boolean,
    default: !1
  },
  loadingColor: {
    type: String,
    default: "#fff"
  },
  loadingWidth: {
    type: String,
    default: "40px"
  },
  loadingHeight: {
    type: String,
    default: "40px"
  },
  size: {
    type: String,
    values: Zl,
    default: "middle"
  },
  width: {
    type: String
  },
  height: {
    type: String
  },
  color: {
    type: String
  },
  direction: {
    type: String,
    values: Yl,
    default: "row"
  },
  shape: {
    type: String,
    values: Kl
  },
  buttonStyle: {
    type: Object
  },
  buttonTextStyle: {
    type: Object
  }
}, ge = {
  options: {
    virtualHost: !0
  }
};
ge.props = Hl;
ge.setup = (A, t) => {
  const e = A, n = `${z}-button--content`, a = C(() => ({
    width: e.width,
    height: e.height,
    backgroundColor: e.color,
    flexDirection: e.direction,
    cursor: qA ? "pointer" : "auto",
    ...e.buttonStyle
  })), s = X([`${z}-button`, {
    [`${z}-${e.shape}`]: e.shape
  }, `${z}-button--${e.size}`]), i = t.emit;
  return {
    buttonTextClassName: n,
    style: a,
    buttonClassName: s,
    handleClick: (l) => {
      !e.loading && i("click", l);
    }
  };
};
ge.components = Object.assign({
  Loading: an,
  Icon: wt
}, ge.components);
var Jl = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.buttonClassName,
    style: [A.style],
    on: {
      click: A.handleClick
    }
  }, [A.loading ? e("Loading", {
    attrs: {
      loadingWidth: A.loadingWidth,
      loadingHeight: A.loadingHeight,
      color: A.loadingColor
    }
  }) : A._e(), A.iconSrc && !A.loading ? e("Icon", {
    attrs: {
      size: A.iconSize,
      src: A.iconSrc
    }
  }) : A._e(), A.text ? e("div", {
    class: A.buttonTextClassName,
    style: [A.buttonTextStyle]
  }, [A._v(" " + A._s(A.text) + " ")]) : A._e()], 1);
}, $l = [];
const Qn = {};
var Au = /* @__PURE__ */ L(
  ge,
  Jl,
  $l,
  !1,
  eu,
  null,
  null,
  null
);
function eu(A) {
  for (let t in Qn)
    this[t] = Qn[t];
}
const CA = /* @__PURE__ */ function() {
  return Au.exports;
}(), Wt = "data:image/svg+xml;base64,", Po = "data:image/svg+xml;base64,", Fo = "data:image/svg+xml;base64,", Ro = "data:image/svg+xml;base64,", Go = "data:image/svg+xml;base64,", ko = "data:image/svg+xml;base64,", _o = "data:image/svg+xml;base64,", tu = "data:image/svg+xml;base64,", nu = "data:image/svg+xml;base64,", bA = "data:image/svg+xml;base64,", qt = "data:image/svg+xml;base64,", au = "data:image/svg+xml;base64,", ou = "data:image/svg+xml;base64,", su = "data:image/svg+xml;base64,", iu = "data:image/svg+xml;base64,", wA = "data:image/svg+xml;base64,", lu = "data:image/svg+xml;base64,", uu = "data:image/svg+xml;base64,", AA = !y.isPC;
let Wo = iu, qo = lu;
AA || (Wo = ou, qo = su);
const F = {
  width: AA ? "60px" : "40px",
  height: AA ? "60px" : "40px",
  shape: "circle",
  iconSize: AA ? 30 : 20,
  showText: !0,
  textColor: "#D5E0F2",
  textSize: "12px",
  textStyle: {
    marginTop: "5px"
  }
}, D = {
  accept: {
    basicConfig: {
      ...F,
      color: "#51C271",
      iconSrc: nu
    },
    loadingConfig: {
      ...F,
      color: "#51C271",
      loadingWidth: AA ? "30px" : "20px",
      loadingHeight: AA ? "30px" : "20px"
    }
  },
  hangup: {
    basicConfig: {
      ...F,
      color: "#ED4651",
      iconSrc: Wt
    },
    loadingConfig: {
      ...F,
      color: "#ED4651",
      loadingWidth: AA ? "30px" : "20px",
      loadingHeight: AA ? "30px" : "20px"
    }
  },
  reject: {
    basicConfig: {
      ...F,
      color: "#ED4651",
      iconSrc: Wt
    }
  },
  camera: {
    basicConfig: {
      ...F,
      color: "#FFFFFF",
      iconSrc: Po,
      shape: "circle"
    },
    closedConfig: {
      ...F,
      color: "#6b758a4d",
      iconSrc: Fo
    },
    loadingConfig: {
      ...F,
      color: "#6b758a4d",
      loadingWidth: AA ? "30px" : "20px",
      loadingHeight: AA ? "30px" : "20px"
    }
  },
  microphone: {
    basicConfig: {
      ...F,
      color: "#FFFFFF",
      iconSrc: Ro
    },
    closedConfig: {
      ...F,
      color: "#6b758a4d",
      iconSrc: Go
    }
  },
  speaker: {
    basicConfig: {
      ...F,
      color: "#FFFFFF",
      iconSrc: ko
    },
    closedConfig: {
      ...F,
      color: "#6b758a4d",
      iconSrc: _o
    }
  },
  fullScreen: {
    basicConfig: {
      iconSize: 20,
      iconSrc: au
    }
  },
  minimize: {
    basicConfig: {
      iconSize: AA ? 24 : 20,
      iconSrc: Wo
    }
  },
  switchCamera: {
    basicConfig: {
      ...F,
      color: "transparent",
      iconSrc: wA,
      shape: "circle"
    }
  },
  inviteUser: {
    basicConfig: {
      ...F,
      color: AA ? "" : "#6b758a4d",
      width: AA ? "24px" : "40px",
      height: AA ? "24px" : "40px",
      shape: AA ? "" : "circle",
      iconSize: AA ? 24 : 20,
      iconSrc: qo
    }
  },
  toggleButtonPanel: {
    basicConfig: {
      color: "transparent",
      width: "40px",
      height: "40px",
      shape: "circle",
      iconSize: 40,
      iconSrc: uu
    }
  },
  virtualBackground: {
    basicConfig: {
      ...F,
      color: "#6b758a4d",
      iconSrc: bA,
      shape: "circle"
    },
    closedConfig: {
      ...F,
      color: "#FFFFFF",
      iconSrc: qt
    },
    loadingConfig: {
      ...F,
      color: "#6b758a4d",
      loadingWidth: AA ? "30px" : "20px",
      loadingHeight: AA ? "30px" : "20px"
    },
    disableConfig: {
      ...F,
      color: "#6b758a4d",
      iconSrc: bA,
      buttonStyle: {
        opacity: 0.6
      }
    }
  }
}, TA = {
  singleCall: {
    video: {
      calling: {
        ...D,
        switchCamera: {
          basicConfig: {
            ...F,
            color: "#6b758a4d",
            iconSrc: wA,
            shape: "circle",
            showText: !0
          },
          disableConfig: {
            ...F,
            color: "#6b758a4d",
            iconSrc: wA,
            shape: "circle",
            showText: !0,
            buttonStyle: {
              opacity: 0.6
            }
          }
        },
        hangup: {
          basicConfig: {
            ...D.hangup.basicConfig
          },
          loadingConfig: {
            ...D.hangup.loadingConfig
          }
        },
        reject: {
          basicConfig: {
            ...D.reject.basicConfig,
            showText: !1
          }
        }
      },
      accept: {
        ...D,
        switchCamera: {
          basicConfig: {
            ...F,
            color: "#6b758a4d",
            iconSrc: wA,
            shape: "circle"
          },
          disableConfig: {
            ...F,
            color: "#6b758a4d",
            iconSrc: wA,
            shape: "circle",
            showText: !0,
            buttonStyle: {
              opacity: 0.6
            }
          }
        },
        accept: {
          basicConfig: {
            ...D.accept.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.accept.loadingConfig,
            showText: !1
          }
        },
        reject: {
          basicConfig: {
            ...D.reject.basicConfig,
            showText: !1
          }
        }
      },
      connected: {
        ...D,
        virtualBackground: {
          basicConfig: {
            ...F,
            color: "transparent",
            iconSrc: bA,
            shape: "circle",
            showText: !1
          },
          closedConfig: {
            ...F,
            color: "transparent",
            iconSrc: bA,
            showText: !1
          },
          disableConfig: {
            ...F,
            color: "transparent",
            iconSrc: bA,
            showText: !1,
            buttonStyle: {
              opacity: 0.6
            }
          }
        },
        hangup: {
          basicConfig: {
            ...D.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.hangup.loadingConfig,
            showText: !1
          }
        },
        switchCamera: {
          basicConfig: {
            ...D.switchCamera.basicConfig,
            showText: !1
          },
          disableConfig: {
            ...D.switchCamera.basicConfig,
            showText: !1,
            buttonStyle: {
              opacity: 0.6
            }
          }
        }
      }
    },
    audio: {
      calling: D,
      accept: D,
      connected: D
    }
  },
  groupCall: {
    video: {
      calling: {
        ...D,
        switchCamera: {
          basicConfig: {
            ...F,
            color: "#6b758a4d",
            iconSrc: wA,
            shape: "circle"
          }
        },
        hangup: {
          basicConfig: {
            ...D.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.hangup.loadingConfig,
            showText: !1
          }
        },
        accept: {
          basicConfig: {
            ...D.accept.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.accept.loadingConfig,
            showText: !1
          }
        },
        reject: {
          basicConfig: {
            ...D.reject.basicConfig,
            showText: !1
          }
        }
      },
      accept: {
        ...D,
        hangup: {
          basicConfig: {
            ...D.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.hangup.loadingConfig,
            showText: !1
          }
        },
        accept: {
          basicConfig: {
            ...D.accept.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.accept.loadingConfig,
            showText: !1
          }
        },
        reject: {
          basicConfig: {
            ...D.reject.basicConfig,
            showText: !1
          }
        }
      },
      connected: {
        ...D,
        virtualBackground: {
          basicConfig: {
            ...F,
            color: "transparent",
            iconSrc: bA,
            shape: "circle",
            showText: !1
          },
          closedConfig: {
            ...F,
            color: "transparent",
            iconSrc: qt,
            showText: !1
          }
        },
        hangup: {
          basicConfig: {
            ...D.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.hangup.loadingConfig,
            showText: !1
          }
        }
      }
    },
    audio: {
      calling: {
        ...D,
        switchCamera: {
          basicConfig: {
            ...F,
            color: "#6b758a4d",
            iconSrc: wA,
            shape: "circle"
          }
        },
        hangup: {
          basicConfig: {
            ...D.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.hangup.loadingConfig,
            showText: !1
          }
        },
        accept: {
          basicConfig: {
            ...D.accept.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.accept.loadingConfig,
            showText: !1
          }
        },
        reject: {
          basicConfig: {
            ...D.reject.basicConfig,
            showText: !1
          }
        }
      },
      accept: {
        ...D,
        hangup: {
          basicConfig: {
            ...D.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.hangup.loadingConfig,
            showText: !1
          }
        },
        accept: {
          basicConfig: {
            ...D.accept.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.accept.loadingConfig,
            showText: !1
          }
        },
        reject: {
          basicConfig: {
            ...D.reject.basicConfig,
            showText: !1
          }
        }
      },
      connected: {
        ...D,
        hangup: {
          basicConfig: {
            ...D.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...D.hangup.loadingConfig,
            showText: !1
          }
        },
        virtualBackground: {
          basicConfig: {
            ...F,
            color: "transparent",
            iconSrc: bA,
            shape: "circle"
          },
          closedConfig: {
            ...F,
            color: "transparent",
            iconSrc: qt
          }
        }
      }
    }
  }
}, ru = {
  singleCall: {
    video: {
      calling: D,
      accept: D,
      connected: D
    },
    audio: {
      calling: D,
      accept: D,
      connected: D
    }
  },
  groupCall: {
    video: {
      calling: D,
      accept: D,
      connected: D
    },
    audio: {
      calling: D,
      accept: D,
      connected: D
    }
  }
}, cu = {
  pc: ru,
  mobile: TA
}, yA = {
  width: "40px",
  height: "40px",
  shape: "circle",
  iconSize: 20,
  textColor: "#D5E0F2"
}, He = {
  microphone: {
    basicConfig: {
      ...yA,
      color: "#FFFFFF",
      iconSrc: Ro
    },
    closedConfig: {
      ...yA,
      color: "#6b758a4d",
      iconSrc: Go
    }
  },
  speaker: {
    basicConfig: {
      ...yA,
      color: "#FFFFFF",
      iconSrc: ko
    },
    closedConfig: {
      ...yA,
      color: "#6b758a4d",
      iconSrc: _o
    }
  },
  camera: {
    basicConfig: {
      ...yA,
      color: "#FFFFFF",
      iconSrc: Po
    },
    closedConfig: {
      ...yA,
      color: "#6b758a4d",
      iconSrc: Fo
    }
  },
  hangup: {
    basicConfig: {
      ...yA,
      color: "#ED4651",
      iconSrc: Wt
    },
    loadingConfig: {
      ...yA,
      color: "#ED4651",
      loadingWidth: "20px",
      loadingHeight: "20px"
    }
  },
  toggleButtonPanel: {
    basicConfig: {
      color: "transparent",
      width: "40px",
      height: "40px",
      shape: "circle",
      iconSize: 40,
      iconSrc: tu
    }
  }
}, du = {
  mobile: {
    ...TA,
    groupCall: {
      video: {
        ...TA.groupCall.video,
        calling: {
          ...TA.groupCall.video.calling,
          ...He
        },
        connected: {
          ...TA.groupCall.video.connected,
          ...He
        }
      },
      audio: {
        ...TA.groupCall.audio,
        calling: {
          ...TA.groupCall.audio.calling,
          ...He
        },
        connected: {
          ...TA.groupCall.audio.connected,
          ...He
        }
      }
    }
  }
}, cA = (A, t) => {
  var c;
  const e = y.isPC ? "pc" : "mobile", { status: n } = _e(), { callStatus: a, callRole: s, isGroupCall: i, callType: u } = j(Z()), l = I((c = D == null ? void 0 : D[A]) == null ? void 0 : c[t.value]);
  return Y(
    [t, n, a, i, u],
    () => {
      var f, v, S, h, E;
      const g = i.value ? "groupCall" : "singleCall", N = u.value === V.AUDIO ? "audio" : "video";
      let p = "";
      a.value === w.CALLING ? p = s.value === H.CALLER ? "calling" : "accept" : a.value === w.CONNECTED && (p = "connected");
      let M = cu;
      n.value === "close" && (M = du), l.value = ((E = (h = (S = (v = (f = M == null ? void 0 : M[e]) == null ? void 0 : f[g]) == null ? void 0 : v[N]) == null ? void 0 : S[p]) == null ? void 0 : h[A]) == null ? void 0 : E[t.value]) || {};
    },
    { immediate: !0 }
  ), l;
}, lt = {
  options: {
    virtualHost: !0
  }
};
lt.setup = (A, t) => ({
  config: cA("minimize", I("basicConfig")),
  handleClick: async () => {
    if (document != null && document.fullscreenElement)
      try {
        document == null || document.exitFullscreen();
      } catch (a) {
        console.debug(a);
      }
    await O.toggleMinimize();
  }
});
lt.components = Object.assign({
  Button: CA
}, lt.components);
var gu = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("Button", {
    attrs: {
      iconSrc: A.config.iconSrc,
      iconSize: A.config.iconSize
    },
    on: {
      click: A.handleClick
    }
  });
}, Iu = [];
const Zn = {};
var pu = /* @__PURE__ */ L(
  lt,
  gu,
  Iu,
  !1,
  Mu,
  "71774dda",
  null,
  null
);
function Mu(A) {
  for (let t in Zn)
    this[t] = Zn[t];
}
const Cu = /* @__PURE__ */ function() {
  return pu.exports;
}(), ut = {
  options: {
    virtualHost: !0
  }
};
ut.setup = (A, t) => ({
  config: cA("fullScreen", I("basicConfig")),
  handleClick: async () => {
    await Ui("tuicallkit-id");
  }
});
ut.components = Object.assign({
  Button: CA
}, ut.components);
var Nu = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("Button", {
    attrs: {
      iconSrc: A.config.iconSrc,
      iconSize: A.config.iconSize
    },
    on: {
      click: A.handleClick
    }
  });
}, fu = [];
const Yn = {};
var vu = /* @__PURE__ */ L(
  ut,
  Nu,
  fu,
  !1,
  Lu,
  "224de994",
  null,
  null
);
function Lu(A) {
  for (let t in Yn)
    this[t] = Yn[t];
}
const hu = /* @__PURE__ */ function() {
  return vu.exports;
}(), mu = {
  width: {
    type: String
  },
  color: {
    type: String
  },
  size: {
    type: String
  },
  weight: {
    type: Number
  },
  truncated: {
    type: Boolean,
    default: !1
  },
  lineClamp: {
    type: Number
  },
  textStyle: {
    type: Object
  }
}, sn = {
  options: {
    virtualHost: !0
  }
};
sn.props = mu;
sn.setup = (A, t) => {
  const e = A, n = t.emit, a = X([`${z}-text`, {
    [`${z}-text--line-clamp`]: e.lineClamp
  }]), s = C(() => it({
    maxWidth: e.width,
    fontSize: e.size,
    fontWeight: e.weight,
    color: e.color,
    textOverflow: e.truncated ? "ellipsis" : "auto",
    "-webkit-line-clamp": e.lineClamp,
    ...e.textStyle
  }));
  return {
    textClassName: a,
    style: s,
    handleClick: () => {
      n("click");
    }
  };
};
var Su = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("span", {
    class: A.textClassName,
    style: [A.style],
    on: {
      click: A.handleClick
    }
  }, [A._t("default")], 2);
}, Du = [];
const Kn = {};
var yu = /* @__PURE__ */ L(
  sn,
  Su,
  Du,
  !1,
  Eu,
  null,
  null,
  null
);
function Eu(A) {
  for (let t in Kn)
    this[t] = Kn[t];
}
const iA = /* @__PURE__ */ function() {
  return yu.exports;
}(), SA = {
  width: {
    type: String
  },
  height: {
    type: String
  },
  showText: {
    type: Boolean,
    default: !0
  },
  iconSize: {
    type: String
  }
}, Ie = {
  options: {
    virtualHost: !0
  }
};
Ie.props = SA;
Ie.setup = (A, t) => {
  const e = A, n = cA("inviteUser", I("basicConfig")), a = J();
  return {
    props: e,
    config: n,
    t: a,
    handleClick: async () => {
      d.update(r.CALL, o.SHOW_SELECT_USER, !0);
    }
  };
};
Ie.components = Object.assign({
  Button: CA,
  TKText: iA
}, Ie.components);
var Tu = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "btn-content"
  }, [e("Button", {
    attrs: {
      iconSrc: A.config.iconSrc,
      color: A.config.color,
      iconSize: A.config.iconSize,
      width: A.props.width || A.config.width,
      height: A.props.height || A.config.height,
      shape: A.config.shape
    },
    on: {
      click: A.handleClick
    }
  }), A.showText ? e("TKText", {
    attrs: {
      textStyle: A.config.textStyle,
      color: A.config.textColor,
      size: A.config.textSize
    }
  }, [A._v(" " + A._s(A.t("invite member")) + " ")]) : A._e()], 1);
}, wu = [];
const Hn = {};
var ju = /* @__PURE__ */ L(
  Ie,
  Tu,
  wu,
  !1,
  bu,
  "037b2b4c",
  null,
  null
);
function bu(A) {
  for (let t in Hn)
    this[t] = Hn[t];
}
const Xo = /* @__PURE__ */ function() {
  return ju.exports;
}(), zu = {
  color: {
    type: String,
    default: "#FFF"
  },
  callDuration: {
    type: String,
    default: ""
  },
  fontSize: {
    type: String,
    default: "16px"
  }
}, ln = {
  options: {
    virtualHost: !0
  }
};
ln.props = zu;
ln.setup = (A, t) => {
  const e = A;
  return {
    timerStyle: C(() => ({
      color: e.color,
      fontSize: e.fontSize
    }))
  };
};
var Uu = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("span", {
    style: [A.timerStyle]
  }, [A._v(A._s(A.callDuration))]);
}, Ou = [];
const Jn = {};
var xu = /* @__PURE__ */ L(
  ln,
  Uu,
  Ou,
  !1,
  Bu,
  null,
  null,
  null
);
function Bu(A) {
  for (let t in Jn)
    this[t] = Jn[t];
}
const un = /* @__PURE__ */ function() {
  return xu.exports;
}(), pe = {
  options: {
    virtualHost: !0
  }
};
pe.props = Ki;
pe.setup = (A, t) => {
  const e = y.isPC, {
    callStatus: n,
    isGroupCall: a,
    callRole: s,
    allowedFullScreen: i
  } = j(Z()), {
    callDuration: u
  } = Ht(), l = We(), c = C(() => n.value === w.CONNECTED), g = I(d.getData(r.CALL, o.ENABLE_FLOAT_WINDOW)), N = C(() => {
    var f, v;
    return !a.value || ((v = (f = l.value.button) == null ? void 0 : f[LA.InviteUser]) == null ? void 0 : v.show) === !1 ? !1 : n.value === w.CALLING ? s.value === H.CALLER : !0;
  });
  function p(f) {
    g.value = f;
  }
  const M = {
    [o.ENABLE_FLOAT_WINDOW]: p
  };
  return sA(() => {
    d.watch(r.CALL, M, {
      notifyRangeWhenWatch: o.MYSELF
    });
  }), tA(() => {
    d.unwatch(r.CALL, {
      ...M
    });
  }), {
    isPC: e,
    allowedFullScreen: i,
    callDuration: u,
    showTimer: c,
    showMinimize: g,
    showInviteUser: N
  };
};
pe.components = Object.assign({
  Row: $t,
  Col: en,
  Timer: un,
  Minimize: Cu,
  FullScreen: hu,
  InviteUser: Xo
}, pe.components);
var Vu = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "top-bar-container",
    style: A.customStyle
  }, [A.isPC ? e("Row", [e("Col", {
    attrs: {
      span: 8
    }
  }), e("Col", {
    attrs: {
      span: 8,
      justify: "center",
      align: "center"
    }
  }, [A.showTimer ? e("Timer", {
    attrs: {
      "call-duration": A.callDuration
    }
  }) : A._e()], 1), e("Col", {
    attrs: {
      span: 8,
      justify: "end",
      align: "center"
    }
  }, [e("Row", [e("Col", {
    attrs: {
      span: 18
    }
  }), e("Col", {
    attrs: {
      span: 3,
      justify: "center"
    }
  }, [A.showMinimize ? e("Minimize") : A._e()], 1), e("Col", {
    attrs: {
      span: 3,
      justify: "center"
    }
  }, [A.allowedFullScreen ? e("FullScreen") : A._e()], 1)], 1)], 1)], 1) : A._e(), A.isPC ? A._e() : e("Row", [e("Col", {
    attrs: {
      span: 8,
      align: "center"
    }
  }, [e("Row", [e("Col", {
    attrs: {
      span: 8,
      justify: "center"
    }
  }, [!A.isPC && A.showMinimize ? e("Minimize") : A._e()], 1), e("Col", {
    attrs: {
      span: 16
    }
  })], 1)], 1), e("Col", {
    attrs: {
      span: 8,
      justify: "center",
      align: "center"
    }
  }, [A.showTimer ? e("Timer", {
    attrs: {
      "call-duration": A.callDuration
    }
  }) : A._e()], 1), e("Col", {
    attrs: {
      span: 8,
      justify: "end",
      align: "center"
    }
  }, [e("Row", [e("Col", {
    attrs: {
      span: 16
    }
  }), e("Col", {
    attrs: {
      span: 8,
      justify: "center"
    }
  }, [A.showInviteUser ? e("InviteUser", {
    attrs: {
      "show-text": A.isPC
    }
  }) : A._e()], 1)], 1)], 1)], 1)], 1);
}, Pu = [];
const $n = {};
var Fu = /* @__PURE__ */ L(
  pe,
  Vu,
  Pu,
  !1,
  Ru,
  "2bcaf4e1",
  null,
  null
);
function Ru(A) {
  for (let t in $n)
    this[t] = $n[t];
}
const Qo = /* @__PURE__ */ function() {
  return Fu.exports;
}(), Gu = {
  show: {
    type: Boolean,
    default: !0
  },
  showMask: {
    type: Boolean,
    default: !0
  },
  showBackgroundImage: {
    type: Boolean,
    default: !0
  },
  blur: {
    type: Boolean,
    default: !0
  },
  bgColor: {
    type: String
  },
  bgImage: {
    type: String
  },
  zIndex: {
    type: Number,
    default: 11e3
  },
  customClass: {
    type: String
  },
  customStyle: {
    type: Object
  },
  customMaskStyle: {
    type: Object
  },
  fit: {
    type: String,
    default: "cover"
  },
  defaultSrc: {
    type: String
  }
}, Me = {
  options: {
    virtualHost: !0
  }
};
Me.props = Gu;
Me.setup = (A, t) => {
  const e = A, n = t.emit, a = X([`${z}-overlay`, e.customClass]), s = `${z}-overlay_mask-container`, i = C(() => X([`${z}-overlay_mask`, {
    [`${z}-blur`]: e.blur
  }])), u = `${z}-overlay_slot`, l = C(() => ({
    zIndex: e.zIndex,
    ...e.customStyle
  })), c = C(() => ({
    backgroundColor: e.bgColor,
    ...e.customMaskStyle
  }));
  function g() {
    n("click");
  }
  return {
    overlayContainerClassName: a,
    maskContainerClassName: s,
    maskClassName: i,
    slotClassName: u,
    overlayStyle: l,
    maskStyle: c,
    handleClick: g,
    handleError: (p) => {
      n("error", p);
    }
  };
};
Me.components = Object.assign({
  TKImage: mA
}, Me.components);
var ku = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return A.show ? e("div", {
    class: A.overlayContainerClassName,
    style: [A.overlayStyle],
    on: {
      click: A.handleClick
    }
  }, [e("div", {
    class: A.maskContainerClassName
  }, [A.showMask ? e("div", {
    class: A.maskClassName,
    style: [A.maskStyle]
  }) : A._e(), A.showBackgroundImage ? e("TKImage", {
    attrs: {
      fit: A.fit,
      src: A.bgImage,
      width: "100%",
      height: "100%",
      defaultSrc: A.defaultSrc
    },
    on: {
      error: A.handleError
    }
  }) : A._e()], 1), e("div", {
    class: A.slotClassName
  }, [A._t("default")], 2)]) : A._e();
}, _u = [];
const Aa = {};
var Wu = /* @__PURE__ */ L(
  Me,
  ku,
  _u,
  !1,
  qu,
  null,
  null,
  null
);
function qu(A) {
  for (let t in Aa)
    this[t] = Aa[t];
}
const Zo = /* @__PURE__ */ function() {
  return Wu.exports;
}(), Xu = ["fill", "contain", "cover"], Qu = ["circle", "square"], Zu = {
  icon: {
    type: String
  },
  size: {
    type: [Number, String],
    default: 100
  },
  shape: {
    type: String,
    values: Qu,
    default: "square"
  },
  src: {
    type: String
  },
  defaultSrc: {
    type: String
  },
  text: {
    type: String
  },
  fit: {
    type: String,
    values: Xu,
    default: "cover"
  },
  customClass: {
    type: String
  }
}, Ce = {
  options: {
    virtualHost: !0
  }
};
Ce.props = Zu;
Ce.setup = (A, t) => {
  const e = A, n = I("");
  Y(() => e.src, () => {
    n.value = e.src;
  }, {
    immediate: !0
  });
  const a = C(() => X([`${z}-avatar`, `${z}-avatar--${e.shape}`, `${z}-avatar--${e.size}`, e.customClass])), s = C(() => typeof e.size == "number" ? `${e.size}px` : e.size), i = C(() => typeof e.size == "number" ? `${e.size}px` : e.size), u = C(() => ({
    width: s.value,
    height: i.value
  }));
  function l(c) {
    console.error(c), e.defaultSrc && (n.value = e.defaultSrc);
  }
  return {
    avatarSrc: n,
    classname: a,
    width: s,
    height: i,
    avatarStyle: u,
    handleError: l
  };
};
Ce.components = Object.assign({
  TKImage: mA
}, Ce.components);
var Yu = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.classname,
    style: [A.avatarStyle]
  }, [A.avatarSrc ? e("TKImage", {
    attrs: {
      fit: A.fit,
      width: A.width,
      height: A.height,
      src: A.avatarSrc
    },
    on: {
      error: A.handleError
    }
  }) : A._t("default")], 2);
}, Ku = [];
const ea = {};
var Hu = /* @__PURE__ */ L(
  Ce,
  Yu,
  Ku,
  !1,
  Ju,
  null,
  null,
  null
);
function Ju(A) {
  for (let t in ea)
    this[t] = ea[t];
}
const Yo = /* @__PURE__ */ function() {
  return Hu.exports;
}(), $u = "data:image/svg+xml;base64,", Ar = "data:image/svg+xml;base64,", er = {
  isMuted: {
    type: Boolean,
    default: !1
  },
  volume: {
    type: Number,
    default: 0
  }
}, Ne = {
  options: {
    virtualHost: !0
  }
};
Ne.props = er;
Ne.setup = (A, t) => {
  const e = A, n = C(() => e.isMuted ? Ar : $u), a = C(() => e.isMuted || !e.volume ? "" : `height: ${e.volume * 4}%`);
  return {
    iconSrc: n,
    volumeLevelStyle: a
  };
};
Ne.components = Object.assign({
  Icon: wt
}, Ne.components);
var tr = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "mic-container"
  }, [e("div", {
    staticClass: "mic-level-container"
  }, [e("div", {
    staticClass: "mic-level",
    style: A.volumeLevelStyle
  })]), e("Icon", {
    attrs: {
      src: A.iconSrc
    }
  })], 1);
}, nr = [];
const ta = {};
var ar = /* @__PURE__ */ L(
  Ne,
  tr,
  nr,
  !1,
  or,
  "3cbc030b",
  null,
  null
);
function or(A) {
  for (let t in ta)
    this[t] = ta[t];
}
const Ko = /* @__PURE__ */ function() {
  return ar.exports;
}(), XA = "data:image/svg+xml;base64,", sr = {
  showOverlayStream: {
    type: Boolean,
    default: !0
  },
  customClass: {
    type: String
  },
  customStyle: {
    type: Object
  },
  isSmallWindow: {
    type: Boolean,
    default: !1
  },
  tip: {
    type: String,
    default: null
  },
  showOverlay: {
    type: Boolean,
    default: !0
  },
  showMask: {
    type: Boolean,
    default: !0
  },
  showBackgroundImage: {
    type: Boolean,
    default: !0
  },
  blur: {
    type: Boolean,
    default: !0
  },
  bgColor: {
    type: String
  },
  bgImage: {
    type: String,
    default: XA
  },
  overlayZIndex: {
    type: Number
  },
  customOverlayClass: {
    type: String
  },
  fit: {
    type: String
  },
  showLoading: {
    type: Boolean,
    default: !1
  },
  showAvatar: {
    type: Boolean,
    default: !0
  },
  avatar: {
    type: String,
    default: XA
  },
  showUserName: {
    type: Boolean,
    default: !0
  },
  username: {
    type: String
  },
  color: {
    type: String,
    default: "#FFF"
  },
  showMicVolume: {
    type: Boolean,
    default: !1
  },
  isMuted: {
    type: Boolean,
    default: !1
  },
  volume: {
    type: Number,
    default: 0
  },
  showTip: {
    type: Boolean,
    default: !0
  }
}, fe = {
  options: {
    virtualHost: !0
  }
};
fe.props = sr;
fe.setup = (A, t) => {
  const e = t.emit, n = A, a = I(100), {
    isFloatWindow: s
  } = j(vA()), i = y.isPC ? "40px" : "20px";
  Y([() => n.isSmallWindow, s], () => {
    s.value ? a.value = n.isSmallWindow ? 20 : 40 : a.value = n.isSmallWindow ? 40 : 100;
  }, {
    immediate: !0
  });
  const u = C(() => X(["overlay-stream-container", {
    pc: y.isPC,
    mobile: !y.isPC,
    float: s.value
  }]));
  return {
    defaultAvatarSrc: XA,
    avatarSize: a,
    fontSize: i,
    overlayStreamClassName: u,
    handleError: (c) => {
      e("error", c);
    }
  };
};
fe.components = Object.assign({
  Overlay: Zo,
  Loading: an,
  Avatar: Yo,
  TKText: iA,
  MicrophoneVolume: Ko
}, fe.components);
var ir = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return A.showOverlayStream ? e("div", {
    class: A.overlayStreamClassName,
    style: [A.customStyle]
  }, [e("Overlay", {
    attrs: {
      show: A.showOverlay,
      "show-background-image": A.showBackgroundImage,
      "show-mask": A.showMask,
      blur: A.blur,
      zIndex: A.overlayZIndex,
      bgColor: A.bgColor,
      bgImage: A.bgImage || A.defaultAvatarSrc,
      fit: A.fit,
      defaultSrc: A.defaultAvatarSrc,
      customStyle: {
        position: "absolute",
        width: "100%",
        height: "100%"
      },
      "custom-mask-style": {
        "backdrop-filter": "blur(12px)",
        "-webkit-backdrop-filter": "blur(12px)"
      },
      customClass: A.customOverlayClass
    },
    on: {
      error: A.handleError
    }
  }, [e("div", {
    staticClass: "overlay-stream-content-container"
  }, [e("div", {
    staticClass: "overlay-stream-content"
  }, [A.showLoading ? e("Loading", {
    attrs: {
      mode: "dot"
    }
  }) : A._e(), e("div", {
    staticClass: "overlay-stream-avatar"
  }, [A.showAvatar ? e("Avatar", {
    attrs: {
      src: A.avatar || A.defaultAvatarSrc,
      size: A.avatarSize
    }
  }) : A._e()], 1), e("div", {
    staticClass: "overlay-stream-info"
  }, [A.showUserName ? e("TKText", {
    attrs: {
      truncated: !0,
      size: A.fontSize,
      color: A.color,
      width: "200px",
      weight: 500
    }
  }, [A._v(" " + A._s(A.username) + " ")]) : A._e(), A.showMicVolume ? e("MicrophoneVolume", {
    attrs: {
      isMuted: A.isMuted,
      volume: A.volume
    }
  }) : A._e()], 1), e("div", {
    staticClass: "overlay-stream-tip"
  }, [A.showTip && A.tip ? e("div", [A._v(A._s(A.tip))]) : A._e()])], 1), A._t("default")], 2)])], 1) : A._e();
}, lr = [];
const na = {};
var ur = /* @__PURE__ */ L(
  fe,
  ir,
  lr,
  !1,
  rr,
  "01ddfaf4",
  null,
  null
);
function rr(A) {
  for (let t in na)
    this[t] = na[t];
}
const Ho = /* @__PURE__ */ function() {
  return ur.exports;
}(), rt = {};
rt.setup = (A, t) => {
  const {
    localUserInfoExcludeVolume: e,
    remoteUserListExcludeVolume: n
  } = j(pA()), {
    callerUserInfo: a
  } = j(zi()), {
    isGroupCall: s
  } = j(Z()), i = Oo(), u = J(), l = C(() => {
    var h, E;
    return s.value ? a.value.displayUserInfo : (E = (h = n.value) == null ? void 0 : h[0]) == null ? void 0 : E.displayUserInfo;
  }), c = C(() => {
    var h, E;
    return s.value ? a.value.avatar : (E = (h = n.value) == null ? void 0 : h[0]) == null ? void 0 : E.avatar;
  }), g = C(() => s.value ? !0 : !e.value.isVideoAvailable), N = C(() => s.value ? u.value("Invited group call") : null), p = C(() => {
    var h, E;
    return s.value ? i.value[a.value.userId] || c.value : i.value[(E = (h = n.value) == null ? void 0 : h[0]) == null ? void 0 : E.userId] || c.value;
  }), M = I(!1), f = I("fill");
  let v = I(!1);
  return $A(() => {
    var h, E, x, m;
    if (v.value) {
      M.value = s.value ? !0 : !e.value.isVideoAvailable, f.value = "cover";
      return;
    }
    s.value ? (M.value = !i.value[a.value.userId], f.value = i.value[a.value.userId] ? "fill" : "cover") : (M.value = !e.value.isVideoAvailable && !i.value[(E = (h = n.value) == null ? void 0 : h[0]) == null ? void 0 : E.userId], f.value = i.value[(m = (x = n.value) == null ? void 0 : x[0]) == null ? void 0 : m.userId] ? "fill" : "cover");
  }), {
    TUIGlobal: y,
    defaultAvatarSrc: XA,
    remoteUserListExcludeVolume: n,
    isGroupCall: s,
    t: u,
    username: l,
    avatar: c,
    showBackgroundImage: g,
    tip: N,
    bgImage: p,
    showMask: M,
    fit: f,
    handleError: () => {
      v.value = !0;
    }
  };
};
rt.components = Object.assign({
  OverlayStream: Ho,
  TKText: iA,
  Avatar: Yo
}, rt.components);
var cr = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "waiting-container"
  }, [e("OverlayStream", {
    attrs: {
      username: A.username,
      avatar: A.avatar,
      "bg-image": A.bgImage,
      "show-avatar": !A.TUIGlobal.isPC,
      "show-loading": !1,
      "bg-color": "#22262ed9",
      "show-mask": A.showMask,
      "show-background-image": A.showBackgroundImage,
      tip: A.tip,
      fit: A.fit
    },
    on: {
      error: A.handleError
    }
  }, [!A.TUIGlobal.isPC && A.isGroupCall ? e("div", {
    staticClass: "groupcall-info"
  }, [e("div", {
    staticClass: "tip"
  }, [e("TKText", {
    attrs: {
      color: "#FFF"
    }
  }, [A._v(A._s(A.remoteUserListExcludeVolume.length) + A._s(A.t("people in the call")))])], 1), e("div", {
    staticClass: "avatar-group"
  }, A._l(A.remoteUserListExcludeVolume, function(n) {
    return e("div", {
      staticClass: "avatar-item"
    }, [e("Avatar", {
      attrs: {
        size: "100%",
        src: n.avatar || A.defaultAvatarSrc
      }
    })], 1);
  }), 0)]) : A._e()])], 1);
}, dr = [];
const aa = {};
var gr = /* @__PURE__ */ L(
  rt,
  cr,
  dr,
  !1,
  Ir,
  "4bab4e9f",
  null,
  null
);
function Ir(A) {
  for (let t in aa)
    this[t] = aa[t];
}
const Jo = /* @__PURE__ */ function() {
  return gr.exports;
}(), pr = {
  bigWindow: {
    type: String
  },
  showSmallWindow: {
    type: Boolean,
    default: !0
  },
  smallWindowWidth: {
    type: String,
    default: "30%"
  },
  smallWindowHeight: {
    type: String,
    default: "30%"
  }
}, $o = "ToggleWindowContextKey", rn = {
  options: {
    virtualHost: !0
  }
};
rn.props = pr;
rn.setup = (A, t) => {
  const e = A, n = t.emit, a = I(e.bigWindow), s = `${z}-toggle-window`, i = (u) => {
    a.value = u, n("toggle", u);
  };
  return Y(() => e.bigWindow, () => {
    a.value = e.bigWindow;
  }), rA($o, {
    bigWindow: a,
    toggleWindow: i,
    smallWindowWidth: Ut(e, "smallWindowWidth"),
    smallWindowHeight: Ut(e, "smallWindowHeight"),
    showSmallWindow: Ut(e, "showSmallWindow")
  }), {
    toggleWindowContainerClassName: s
  };
};
var Mr = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.toggleWindowContainerClassName
  }, [A._t("default")], 2);
}, Cr = [];
const oa = {};
var Nr = /* @__PURE__ */ L(
  rn,
  Mr,
  Cr,
  !1,
  fr,
  null,
  null,
  null
);
function fr(A) {
  for (let t in oa)
    this[t] = oa[t];
}
const vr = /* @__PURE__ */ function() {
  return Nr.exports;
}(), cn = {
  options: {
    virtualHost: !0
  }
};
cn.props = ["disabled"];
cn.setup = (A, t) => ({});
var Lr = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("root-portal", {
    attrs: {
      enable: !A.disabled
    }
  }, [A._t("default")], 2);
}, hr = [];
const sa = {};
var mr = /* @__PURE__ */ L(
  cn,
  Lr,
  hr,
  !1,
  Sr,
  null,
  null,
  null
);
function Sr(A) {
  for (let t in sa)
    this[t] = sa[t];
}
const Dr = /* @__PURE__ */ function() {
  return mr.exports;
}(), yr = {
  name: "teleport",
  props: {
    to: {
      type: String,
      required: !0
    },
    source: {
      type: String,
      required: !0
    },
    disabled: {
      type: Boolean,
      required: !0
    }
  },
  mounted() {
    if (this.disabled)
      return;
    const A = document.querySelector(this.to);
    A && A.appendChild(this.$el);
  },
  watch: {
    disabled: {
      immediate: !0,
      handler() {
        var A, t;
        this.disabled ? (t = document.querySelector(this.source)) == null || t.appendChild(this.$el) : (A = document.querySelector(this.to)) == null || A.appendChild(this.$el);
      }
    }
  },
  destroyed() {
    const A = document.querySelector(this.to);
    if (A)
      try {
        A.removeChild(this.$el);
      } catch (t) {
        console.debug(t);
      }
  }
};
var Er = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", [A._t("default")], 2);
}, Tr = [];
const ia = {};
var wr = /* @__PURE__ */ L(
  yr,
  Er,
  Tr,
  !1,
  jr,
  null,
  null,
  null
);
function jr(A) {
  for (let t in ia)
    this[t] = ia[t];
}
const br = /* @__PURE__ */ function() {
  return wr.exports;
}(), As = {
  options: {
    virtualHost: !0
  }
};
As.props = ["disabled", "to"];
var zr = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("Teleport", {
    attrs: {
      disabled: A.disabled,
      to: A.to
    }
  }, [A._t("default")], 2);
}, Ur = [];
const la = {};
var Or = /* @__PURE__ */ L(
  As,
  zr,
  Ur,
  !1,
  xr,
  null,
  null,
  null
);
function xr(A) {
  for (let t in la)
    this[t] = la[t];
}
const Br = /* @__PURE__ */ function() {
  return Or.exports;
}(), Vr = {
  disabled: {
    type: Boolean,
    default: !1
  },
  to: {
    type: String,
    default: "body"
  },
  source: {
    type: String,
    default: "body"
  }
}, ve = {};
ve.props = Vr;
ve.setup = (A, t) => {
  const {
    majorVersion: e
  } = sl(), n = C(() => e === "3"), a = C(() => e === "2");
  return {
    IN_MINI_APP: _t,
    isVue3: n,
    isVue2: a
  };
};
ve.components = Object.assign({
  PortalMini: Dr,
  PortalVue2: br,
  PortalVue3: Br
}, ve.components);
var Pr = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", [A.IN_MINI_APP ? e("PortalMini", {
    attrs: {
      disabled: A.disabled
    }
  }, [A._t("default")], 2) : A.isVue2 ? e("PortalVue2", {
    attrs: {
      disabled: A.disabled,
      to: A.to,
      source: A.source
    }
  }, [A._t("default")], 2) : e("PortalVue3", {
    attrs: {
      disabled: A.disabled,
      to: A.to
    }
  }, [A._t("default")], 2)], 1);
}, Fr = [];
const ua = {};
var Rr = /* @__PURE__ */ L(
  ve,
  Pr,
  Fr,
  !1,
  Gr,
  null,
  null,
  null
);
function Gr(A) {
  for (let t in ua)
    this[t] = ua[t];
}
const es = /* @__PURE__ */ function() {
  return Rr.exports;
}(), kr = {
  value: {
    type: String
  }
}, dn = {
  options: {
    virtualHost: !0
  }
};
dn.props = kr;
dn.setup = (A, t) => {
  const e = A, {
    bigWindow: n,
    toggleWindow: a,
    smallWindowWidth: s,
    smallWindowHeight: i,
    showSmallWindow: u
  } = dA($o), l = C(() => X([`${z}-toggle-window-item`, `${z}-toggle-window-item--${n.value === e.value ? "big" : "small"}`, {
    pc: qA,
    mobile: !qA
  }])), c = C(() => {
    let g = {};
    return n.value !== e.value && (g = {
      width: s.value,
      height: i.value
    }, u.value ? g.visibility = "" : g.visibility = "hidden"), g;
  });
  return {
    toggleWindow: a,
    classname: l,
    windowItemStyle: c
  };
};
var _r = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.classname,
    style: [A.windowItemStyle],
    on: {
      click: function() {
        return A.toggleWindow(A.value);
      }
    }
  }, [A._t("default")], 2);
}, Wr = [];
const ra = {};
var qr = /* @__PURE__ */ L(
  dn,
  _r,
  Wr,
  !1,
  Xr,
  null,
  null,
  null
);
function Xr(A) {
  for (let t in ra)
    this[t] = ra[t];
}
const Qr = /* @__PURE__ */ function() {
  return qr.exports;
}(), gn = {
  nickName: {
    type: String,
    default: ""
  },
  isSelf: {
    type: Boolean,
    default: !1
  },
  isMuted: {
    type: Boolean,
    default: !1
  },
  volume: {
    type: Number,
    default: 0
  },
  showNickName: {
    type: Boolean,
    default: !1
  },
  showSwitchCameraButton: {
    type: Boolean,
    default: !1
  },
  showVirtualBackgroundButton: {
    type: Boolean,
    default: !1
  },
  showNetWorkStatus: {
    type: Boolean,
    default: !1
  }
}, ts = "data:image/png;base64,", Le = {
  options: {
    virtualHost: !0
  }
};
Le.props = gn;
Le.setup = (A, t) => {
  const e = J();
  return {
    networkSrc: ts,
    t: e
  };
};
Le.components = Object.assign({
  Row: $t,
  Col: en,
  MicrophoneVolume: Ko,
  TKText: iA,
  TKImage: mA
}, Le.components);
var Zr = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("Row", [e("Col", {
    attrs: {
      span: 12,
      justify: "start",
      align: "center"
    }
  }, [e("div", {
    staticClass: "stream-userInfo"
  }, [e("MicrophoneVolume", {
    staticClass: "mic-volume",
    attrs: {
      "is-muted": A.isMuted,
      volume: A.volume
    }
  }), e("div", {
    staticClass: "nickname"
  }, [e("TKText", {
    attrs: {
      color: "#fff",
      truncated: !0,
      width: "80px",
      size: "14px"
    }
  }, [A._v(A._s(A.nickName))]), e("TKText", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isSelf,
      expression: "isSelf"
    }],
    attrs: {
      color: "#fff",
      size: "14px"
    }
  }, [A._v(A._s(A.t("me")))])], 1)], 1)]), e("Col", {
    attrs: {
      span: 12,
      justify: "center"
    }
  }, [A.showNetWorkStatus ? e("div", [e("TKImage", {
    attrs: {
      width: "24px",
      height: "24px",
      src: A.networkSrc
    }
  })], 1) : A._e()])], 1);
}, Yr = [];
const ca = {};
var Kr = /* @__PURE__ */ L(
  Le,
  Zr,
  Yr,
  !1,
  Hr,
  "3bd02630",
  null,
  null
);
function Hr(A) {
  for (let t in ca)
    this[t] = ca[t];
}
const Jr = /* @__PURE__ */ function() {
  return Kr.exports;
}(), $r = "data:image/svg+xml;base64,", Ac = "data:image/svg+xml;base64,", he = {
  options: {
    virtualHost: !0
  }
};
he.props = gn;
he.setup = (A, t) => {
  const {
    enableVirtualBackground: e
  } = j(Z());
  return {
    MicOnH5: $r,
    MicOffH5: Ac,
    SwitchCameraSrc: wA,
    VirtualBackgroundOpenSrc: bA,
    networkSrc: ts,
    switchCamera: async () => {
      await O.switchCamera();
    },
    setBlurBackground: async () => {
      await O.setBlurBackground(!e.value);
    }
  };
};
he.components = Object.assign({
  Row: $t,
  Col: en,
  TKText: iA,
  Icon: wt,
  TKImage: mA
}, he.components);
var ec = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("Row", {
    attrs: {
      "custom-style": {
        padding: "2px 5px"
      }
    }
  }, [e("Col", {
    attrs: {
      span: 12,
      justify: "start",
      align: "center"
    }
  }, [A.showNickName ? e("TKText", {
    attrs: {
      width: "100px",
      truncated: !0,
      color: "#FFF"
    }
  }, [A._v(" " + A._s(A.nickName) + " ")]) : A._e(), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: !A.isMuted && A.volume,
      expression: "!isMuted && volume"
    }]
  }, [e("Icon", {
    attrs: {
      size: 24,
      src: A.MicOnH5
    }
  })], 1), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isMuted,
      expression: "isMuted"
    }]
  }, [e("Icon", {
    attrs: {
      size: 24,
      src: A.MicOffH5
    }
  })], 1)], 1), e("Col", {
    attrs: {
      span: 12,
      justify: "end",
      align: "center"
    }
  }, [A.showNetWorkStatus ? e("div", [e("TKImage", {
    attrs: {
      width: "24px",
      height: "24px",
      src: A.networkSrc
    }
  })], 1) : A._e(), A.showSwitchCameraButton ? e("div", {
    staticClass: "switch-camera stream-icon",
    on: {
      click: function(n) {
        return n.stopPropagation(), A.switchCamera.apply(null, arguments);
      }
    }
  }, [e("TKImage", {
    attrs: {
      width: "15px",
      height: "15px",
      src: A.SwitchCameraSrc
    }
  })], 1) : A._e(), A.showVirtualBackgroundButton ? e("div", {
    staticClass: "stream-icon",
    on: {
      click: function(n) {
        return n.stopPropagation(), A.setBlurBackground.apply(null, arguments);
      }
    }
  }, [e("TKImage", {
    attrs: {
      width: "15px",
      height: "15px",
      src: A.VirtualBackgroundOpenSrc
    }
  })], 1) : A._e()])], 1);
}, tc = [];
const da = {};
var nc = /* @__PURE__ */ L(
  he,
  ec,
  tc,
  !1,
  ac,
  "411d981d",
  null,
  null
);
function ac(A) {
  for (let t in da)
    this[t] = da[t];
}
const oc = /* @__PURE__ */ function() {
  return nc.exports;
}(), me = {
  options: {
    virtualHost: !0
  }
};
me.props = gn;
me.setup = (A, t) => {
  const {
    isGroupCall: e
  } = Z(), n = y.isPC;
  return {
    isGroupCall: e,
    isPC: n
  };
};
me.components = Object.assign({
  StreamInfoPC: Jr,
  StreamInfoMobile: oc
}, me.components);
var sc = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", [A.isPC ? e("StreamInfoPC", {
    attrs: {
      nickName: A.nickName,
      isSelf: A.isSelf,
      isMuted: A.isMuted,
      volume: A.volume,
      showNetWorkStatus: A.showNetWorkStatus
    }
  }) : A._e(), A.isGroupCall && !A.isPC ? e("StreamInfoMobile", {
    attrs: {
      showSwitchCameraButton: A.showSwitchCameraButton,
      showVirtualBackgroundButton: A.showVirtualBackgroundButton,
      showNetWorkStatus: A.showNetWorkStatus,
      nickName: A.nickName,
      showNickName: A.showNickName,
      isSelf: A.isSelf,
      isMuted: A.isMuted,
      volume: A.volume
    }
  }) : A._e()], 1);
}, ic = [];
const ga = {};
var lc = /* @__PURE__ */ L(
  me,
  sc,
  ic,
  !1,
  uc,
  null,
  null,
  null
);
function uc(A) {
  for (let t in ga)
    this[t] = ga[t];
}
const ns = /* @__PURE__ */ function() {
  return lc.exports;
}(), rc = {
  show: {
    type: Boolean,
    default: !0
  },
  domId: {
    type: String
  },
  loading: {
    type: Boolean
  },
  showStreamInfo: {
    type: Boolean
  },
  showAudioStream: {
    type: Boolean
  }
}, In = {
  options: {
    virtualHost: !0
  }
};
In.props = rc;
In.setup = (A, t) => {
  const e = I(d.getData(r.CALL, o.PUSHER_ID)), n = X(["stream-info-container", {
    mobile: !y.isPC
  }]), a = (i) => {
    e.value = i;
  }, s = {
    [o.PUSHER_ID]: a
  };
  return sA(() => {
    d.watch(r.CALL, s, {
      notifyRangeWhenWatch: o.MYSELF
    });
  }), tA(() => {
    d.unwatch(r.CALL, s);
  }), {
    streamInfoContainerClassName: n
  };
};
var cc = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.show,
      expression: "show"
    }],
    staticClass: "pusher-container",
    attrs: {
      id: A.domId
    }
  }, [e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.showAudioStream,
      expression: "showAudioStream"
    }],
    staticClass: "audio-stream-container"
  }, [A._t("audio-stream")], 2), A._t("loading"), e("div", {
    class: A.streamInfoContainerClassName
  }, [A._t("stream-info")], 2)], 2);
}, dc = [];
const Ia = {};
var gc = /* @__PURE__ */ L(
  In,
  cc,
  dc,
  !1,
  Ic,
  "71047565",
  null,
  null
);
function Ic(A) {
  for (let t in Ia)
    this[t] = Ia[t];
}
const as = /* @__PURE__ */ function() {
  return gc.exports;
}(), pc = {
  show: {
    type: Boolean,
    default: !0
  },
  domId: {
    type: String
  },
  loading: {
    type: Boolean
  },
  showStreamInfo: {
    type: Boolean
  },
  showAudioStream: {
    type: Boolean
  }
}, pn = {
  options: {
    virtualHost: !0
  }
};
pn.props = pc;
pn.setup = (A, t) => ({
  streamInfoContainerClassName: X(["stream-info-container", {
    mobile: !y.isPC
  }])
});
var Mc = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.show,
      expression: "show"
    }],
    staticClass: "player-container",
    attrs: {
      id: A.domId
    }
  }, [e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.showAudioStream,
      expression: "showAudioStream"
    }],
    staticClass: "audio-stream-container"
  }, [A._t("audio-stream")], 2), A._t("loading"), e("div", {
    class: A.streamInfoContainerClassName
  }, [A._t("stream-info")], 2)], 2);
}, Cc = [];
const pa = {};
var Nc = /* @__PURE__ */ L(
  pn,
  Mc,
  Cc,
  !1,
  fc,
  "7884ead8",
  null,
  null
);
function fc(A) {
  for (let t in pa)
    this[t] = pa[t];
}
const os = /* @__PURE__ */ function() {
  return Nc.exports;
}(), vc = {
  avatar: {
    type: String
  },
  username: {
    type: String
  },
  isVideoAvailable: {
    type: Boolean
  },
  showStreamInfo: {
    type: Boolean
  },
  isSmallWindow: {
    type: Boolean,
    default: !1
  },
  isMuted: {
    type: Boolean,
    default: !0
  },
  volume: {
    type: Number,
    default: 0
  },
  userId: {
    type: String
  }
}, Se = {
  options: {
    virtualHost: !0
  }
};
Se.props = vc;
Se.setup = (A, t) => {
  const e = A, n = Oo(), {
    callType: a,
    isGroupCall: s
  } = j(Z()), {
    isFloatWindow: i
  } = j(vA()), u = C(() => !s.value && !y.isPC), l = C(() => s.value ? y.isPC : a.value === V.AUDIO), c = C(() => !s.value && y.isPC && a.value === V.AUDIO), g = C(() => ({
    zIndex: 1
  })), N = C(() => n.value[e.userId] || e.avatar), p = I(!n.value[e.userId]), M = I(n.value[e.userId] ? "fill" : "cover");
  return Y([() => e.userId, n], () => {
    n.value[e.userId] ? (M.value = "fill", p.value = !1) : (M.value = "cover", p.value = !0);
  }, {
    immediate: !0
  }), {
    isGroupCall: s,
    isFloatWindow: i,
    showAvatar: u,
    showUserName: l,
    showMicVolume: c,
    audioStreamStyle: g,
    bgImage: N,
    showViewMask: p,
    fit: M,
    handleError: () => {
      p.value = !0, M.value = "cover";
    }
  };
};
Se.components = Object.assign({
  OverlayStream: Ho
}, Se.components);
var Lc = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("OverlayStream", {
    attrs: {
      avatar: A.avatar,
      "bg-image": A.bgImage,
      "show-avatar": A.showAvatar,
      username: A.username,
      "show-user-name": A.showUserName,
      "show-mask": !A.isGroupCall && A.showViewMask,
      fit: A.fit,
      blur: !A.isGroupCall,
      "show-mic-volume": A.showMicVolume,
      "show-tip": !A.isGroupCall && !A.isFloatWindow,
      "custom-style": A.audioStreamStyle,
      "is-small-window": A.isSmallWindow,
      "is-muted": A.isMuted,
      volume: A.volume,
      "bg-color": "rgba(0, 0, 0, 0.5)"
    },
    on: {
      error: A.handleError
    }
  });
}, hc = [];
const Ma = {};
var mc = /* @__PURE__ */ L(
  Se,
  Lc,
  hc,
  !1,
  Sc,
  null,
  null,
  null
);
function Sc(A) {
  for (let t in Ma)
    this[t] = Ma[t];
}
const ss = /* @__PURE__ */ function() {
  return mc.exports;
}(), Dc = "data:image/svg+xml;base64,", yc = "data:image/svg+xml;base64,", Ec = "data:image/svg+xml;base64,", Tc = "data:image/svg+xml;base64,", is = {
  options: {
    virtualHost: !0
  }
};
is.setup = (A, t) => {
  const {
    localUserInfoExcludeVolume: e
  } = j(pA()), {
    callRole: n,
    callStatus: a
  } = j(Z()), {
    isFloatWindow: s
  } = j(vA());
  function i() {
    O.toggleMinimize();
  }
  async function u() {
    n.value === H.CALLEE && a.value === w.CALLING ? await O.reject() : await O.hangup();
  }
  async function l() {
    e.value.isAudioAvailable ? await O.closeMicrophone() : await O.openMicrophone();
  }
  return {
    FloatCallEnd: Dc,
    FloatMicrophoneSVG: yc,
    FloatMicrophoneClosedSVG: Ec,
    FloatFullScreenSVG: Tc,
    localUserInfo: e,
    isFloatWindow: s,
    toggleMinimize: i,
    hangup: u,
    toggleMicrophone: l
  };
};
var wc = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "float-window-container"
  }, [e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isFloatWindow,
      expression: "isFloatWindow"
    }],
    staticClass: "float-control-panel"
  }, [e("div", {
    staticClass: "float-control-item-icon"
  }, [e("div", {
    staticClass: "float-control-item-icon-container",
    on: {
      click: A.hangup
    }
  }, [e("img", {
    attrs: {
      src: A.FloatCallEnd
    }
  })])]), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isFloatWindow,
      expression: "isFloatWindow"
    }],
    staticClass: "float-control-item-icon"
  }, [e("div", {
    staticClass: "float-control-item-icon-container",
    on: {
      click: A.toggleMicrophone
    }
  }, [A.localUserInfo.isAudioAvailable ? e("img", {
    attrs: {
      src: A.FloatMicrophoneSVG
    }
  }) : e("img", {
    attrs: {
      src: A.FloatMicrophoneClosedSVG
    }
  })])]), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isFloatWindow,
      expression: "isFloatWindow"
    }],
    staticClass: "float-control-item-icon"
  }, [e("div", {
    staticClass: "float-control-item-icon-container",
    on: {
      click: A.toggleMinimize
    }
  }, [e("img", {
    attrs: {
      src: A.FloatFullScreenSVG
    }
  })])])]), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: !A.isFloatWindow,
      expression: "!isFloatWindow"
    }]
  }, [A._t("default")], 2)]);
}, jc = [];
const Ca = {};
var bc = /* @__PURE__ */ L(
  is,
  wc,
  jc,
  !1,
  zc,
  "4df5fc21",
  null,
  null
);
function zc(A) {
  for (let t in Ca)
    this[t] = Ca[t];
}
const Uc = /* @__PURE__ */ function() {
  return bc.exports;
}(), ls = "data:image/svg+xml;base64,", ct = {
  options: {
    virtualHost: !0
  }
};
ct.setup = (A, t) => {
  const {
    callType: e,
    callStatus: n
  } = j(Z()), {
    isFloatWindow: a
  } = j(vA()), {
    callDuration: s
  } = Ht(), i = J(), u = C(() => X(["float-window-container", {
    "singlecall-video-float": e.value === V.VIDEO && a.value,
    "singlecall-audio-float": e.value === V.AUDIO && a.value
  }]));
  function l() {
    a.value && O.toggleMinimize();
  }
  return {
    CallMediaType: V,
    CallStatus: w,
    earphoneSrc: ls,
    callType: e,
    callStatus: n,
    isFloatWindow: a,
    callDuration: s,
    t: i,
    floatWindowClassName: u,
    closeFloatWindow: l
  };
};
ct.components = Object.assign({
  TKText: iA,
  TKImage: mA,
  Timer: un
}, ct.components);
var Oc = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.floatWindowClassName
  }, [e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: !(A.isFloatWindow && A.callType === A.CallMediaType.AUDIO),
      expression: "!(isFloatWindow && callType === CallMediaType.AUDIO)"
    }],
    staticClass: "singlecall-video-float-content",
    on: {
      click: A.closeFloatWindow
    }
  }, [A._t("default"), e("div", {
    staticClass: "float-window-tip-container"
  }, [A.callStatus === A.CallStatus.CALLING && A.isFloatWindow ? e("TKText", {
    attrs: {
      color: "#FFF",
      size: "12px"
    }
  }, [A._v(A._s(A.t("wait to be called")))]) : A._e()], 1)], 2), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isFloatWindow && A.callType === A.CallMediaType.AUDIO,
      expression: "isFloatWindow && callType === CallMediaType.AUDIO"
    }],
    staticClass: "singlecall-audio-float-content",
    on: {
      click: A.closeFloatWindow
    }
  }, [e("TKImage", {
    attrs: {
      width: "36px",
      height: "36px",
      src: A.earphoneSrc
    }
  }), A.callStatus === A.CallStatus.CONNECTED ? e("Timer", {
    attrs: {
      fontSize: "12px",
      callDuration: A.callDuration,
      color: "#12b969"
    }
  }) : A._e(), A.callStatus === A.CallStatus.CALLING && A.isFloatWindow ? e("TKText", {
    attrs: {
      color: "#12b969",
      size: "12px"
    }
  }, [A._v(A._s(A.t("wait to be called")))]) : A._e()], 1)]);
}, xc = [];
const Na = {};
var Bc = /* @__PURE__ */ L(
  ct,
  Oc,
  xc,
  !1,
  Vc,
  "05f43520",
  null,
  null
);
function Vc(A) {
  for (let t in Na)
    this[t] = Na[t];
}
const Pc = /* @__PURE__ */ function() {
  return Bc.exports;
}(), Fc = "data:image/svg+xml;base64,", Rc = "data:image/svg+xml;base64,", Gc = "data:image/svg+xml;base64,", kc = "data:image/svg+xml;base64,", dt = {
  options: {
    virtualHost: !0
  }
};
dt.setup = (A, t) => {
  const {
    isFloatWindow: e
  } = j(vA()), {
    callDuration: n
  } = Ht(), {
    localUserInfoExcludeVolume: a
  } = j(pA()), {
    callStatus: s
  } = j(Z()), i = J(), u = C(() => a.value.isAudioAvailable ? Fc : Gc), l = C(() => a.value.isVideoAvailable ? Rc : kc), c = C(() => X(["groupcall-video-float", {
    "not-float": !e.value,
    float: e.value
  }]));
  function g() {
    e.value && O.toggleMinimize();
  }
  return {
    CallStatus: w,
    earphoneSrc: ls,
    isFloatWindow: e,
    callDuration: n,
    callStatus: s,
    t: i,
    microphoneSrc: u,
    cameraSrc: l,
    floatWindowClassName: c,
    closeFloatWindow: g
  };
};
dt.components = Object.assign({
  TKImage: mA,
  Timer: un,
  TKText: iA
}, dt.components);
var _c = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.floatWindowClassName
  }, [A.isFloatWindow ? e("div", {
    staticClass: "click-container",
    on: {
      "!click": function(n) {
        return n.stopPropagation(), A.closeFloatWindow.apply(null, arguments);
      }
    }
  }) : A._e(), e("div", {
    staticClass: "stream-container"
  }, [e("div", {
    staticClass: "video"
  }, [A._t("default")], 2), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isFloatWindow,
      expression: "isFloatWindow"
    }],
    staticClass: "audio"
  }, [e("TKImage", {
    attrs: {
      width: "36px",
      height: "36px",
      src: A.earphoneSrc
    }
  }), A.callStatus === A.CallStatus.CONNECTED ? e("Timer", {
    attrs: {
      fontSize: "12px",
      callDuration: A.callDuration,
      color: "#12b969"
    }
  }) : A._e(), A.callStatus === A.CallStatus.CALLING ? e("TKText", {
    attrs: {
      size: "12px",
      color: "#12b969"
    }
  }, [A._v(A._s(A.t("wait to be called")))]) : A._e()], 1)]), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isFloatWindow,
      expression: "isFloatWindow"
    }],
    staticClass: "device-status"
  }, [e("TKImage", {
    attrs: {
      width: "16px",
      height: "16px",
      src: A.microphoneSrc
    }
  }), e("TKImage", {
    attrs: {
      width: "16px",
      height: "16px",
      src: A.cameraSrc
    }
  })], 1)]);
}, Wc = [];
const fa = {};
var qc = /* @__PURE__ */ L(
  dt,
  _c,
  Wc,
  !1,
  Xc,
  "76fc7036",
  null,
  null
);
function Xc(A) {
  for (let t in fa)
    this[t] = fa[t];
}
const Qc = /* @__PURE__ */ function() {
  return qc.exports;
}(), gt = {
  options: {
    virtualHost: !0
  }
};
gt.setup = (A, t) => {
  const {
    isGroupCall: e
  } = j(Z());
  return {
    isGroupCall: e
  };
};
gt.components = Object.assign({
  FloatWindowSingleCall: Pc,
  FloatWindowGroupCall: Qc
}, gt.components);
var Zc = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    attrs: {
      id: "float-window-id"
    }
  }, [A.isGroupCall ? e("FloatWindowGroupCall", [A._t("default")], 2) : e("FloatWindowSingleCall", [A._t("default")], 2)], 1);
}, Yc = [];
const va = {};
var Kc = /* @__PURE__ */ L(
  gt,
  Zc,
  Yc,
  !1,
  Hc,
  null,
  null,
  null
);
function Hc(A) {
  for (let t in va)
    this[t] = va[t];
}
const Jc = /* @__PURE__ */ function() {
  return Kc.exports;
}(), It = {
  options: {
    virtualHost: !0
  }
};
It.setup = (A, t) => {
  const {
    isFloatWindow: e
  } = j(vA()), n = C(() => X(["float-window-container", {
    float: e.value,
    "not-float": !e.value,
    pc: y.isPC,
    mobile: !y.isPC
  }]));
  return {
    TUIGlobal: y,
    floatWindowContainerClassName: n
  };
};
It.components = Object.assign({
  FloatWindowPC: Uc,
  FloatWindowMobile: Jc
}, It.components);
var $c = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.floatWindowContainerClassName
  }, [A.TUIGlobal.isPC ? e("FloatWindowPC", [A._t("default")], 2) : A._e(), A.TUIGlobal.isPC ? A._e() : e("FloatWindowMobile", [A._t("default")], 2)], 1);
}, Ad = [];
const La = {};
var ed = /* @__PURE__ */ L(
  It,
  $c,
  Ad,
  !1,
  td,
  "59a0c3c2",
  null,
  null
);
function td(A) {
  for (let t in La)
    this[t] = La[t];
}
const us = /* @__PURE__ */ function() {
  return ed.exports;
}();
function nd() {
  const A = We(), { callStatus: t } = j(Z()), e = I(et.LOCAL), { remoteUserListExcludeVolume: n } = j(pA());
  return Y([n, A, t], () => {
    var i, u, l, c;
    if (t.value === w.CALLING)
      return;
    const a = [tt.RemoteInLargeView, tt.LocalInLargeView], s = (i = A.value) == null ? void 0 : i.layoutMode;
    if (a.includes(s)) {
      e.value = (u = A.value) == null ? void 0 : u.layoutMode;
      return;
    }
    (c = (l = n.value) == null ? void 0 : l[0]) != null && c.isEnter && (e.value = et.REMOTE);
  }, {
    immediate: !0
  }), e;
}
const pt = {
  options: {
    virtualHost: !0
  }
};
pt.setup = (A, t) => {
  const e = nd(), n = I(!0), {
    isFloatWindow: a
  } = j(vA()), {
    localUserInfoExcludeVolume: s,
    remoteUserListExcludeVolume: i
  } = j(pA()), u = jo(), {
    callType: l,
    callStatus: c
  } = j(Z()), g = bo(), N = C(() => {
    var S, h, E, x;
    return y.isWeChat ? (h = (S = g.value) == null ? void 0 : S.find((m) => {
      var P, W;
      return (m == null ? void 0 : m.userID) === ((W = (P = i.value) == null ? void 0 : P[0]) == null ? void 0 : W.userId);
    })) == null ? void 0 : h.hasVideo : (x = (E = i.value) == null ? void 0 : E[0]) == null ? void 0 : x.isVideoAvailable;
  }), p = C(() => {
    var S, h;
    return (h = (S = i.value) == null ? void 0 : S[0]) == null ? void 0 : h.domId;
  }), M = C(() => {
    var S, h, E;
    return (E = u.value) == null ? void 0 : E[(h = (S = i.value) == null ? void 0 : S[0]) == null ? void 0 : h.domId];
  });
  Y([l, c], () => {
    l.value === V.AUDIO || c.value === w.CALLING ? n.value = !1 : n.value = !0;
  }, {
    immediate: !0
  });
  const f = C(() => X(["singlecall-media-container", {
    mobile: !y.isPC,
    pc: y.isPC,
    float: a.value
  }]));
  function v(S) {
    e.value = S;
  }
  return {
    CallMediaType: V,
    ViewName: et,
    largeViewName: e,
    showSmallWindow: n,
    isFloatWindow: a,
    localUserInfoExcludeVolume: s,
    remoteUserListExcludeVolume: i,
    volumeMap: u,
    callType: l,
    isPusherVideoAvailable: N,
    playerDomId: p,
    remoteStreamVolume: M,
    singleMediaContainerClassName: f,
    handleToggle: v
  };
};
pt.components = Object.assign({
  Portal: es,
  FloatWindow: us,
  ToggleWindow: vr,
  ToggleWindowItem: Qr,
  Pusher: as,
  AudioStream: ss,
  TKStreamInfo: ns,
  Player: os
}, pt.components);
var ad = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("Portal", {
    attrs: {
      id: "source",
      disabled: !A.isFloatWindow,
      source: "#source",
      to: "body"
    }
  }, [e("FloatWindow", [e("div", {
    class: A.singleMediaContainerClassName
  }, [e("ToggleWindow", {
    attrs: {
      "big-window": A.largeViewName,
      "show-small-window": A.showSmallWindow
    },
    on: {
      toggle: A.handleToggle
    }
  }, [e("ToggleWindowItem", {
    key: A.ViewName.LOCAL,
    attrs: {
      value: A.ViewName.LOCAL
    }
  }, [e("Pusher", {
    attrs: {
      domId: A.localUserInfoExcludeVolume.domId,
      "show-audio-stream": !A.localUserInfoExcludeVolume.isVideoAvailable
    },
    scopedSlots: A._u([{
      key: "audio-stream",
      fn: function() {
        return [e("AudioStream", {
          attrs: {
            userId: A.localUserInfoExcludeVolume.userId,
            username: A.localUserInfoExcludeVolume.displayUserInfo,
            avatar: A.localUserInfoExcludeVolume.avatar,
            "is-video-available": A.localUserInfoExcludeVolume.isVideoAvailable,
            "is-small-window": A.largeViewName !== A.ViewName.LOCAL,
            "is-muted": !A.localUserInfoExcludeVolume.isAudioAvailable,
            volume: A.volumeMap && A.volumeMap[A.localUserInfoExcludeVolume.domId]
          }
        })];
      },
      proxy: !0
    }, {
      key: "stream-info",
      fn: function() {
        return [A.callType === A.CallMediaType.VIDEO ? e("TKStreamInfo", {
          attrs: {
            "nick-name": A.localUserInfoExcludeVolume.displayUserInfo,
            "is-self": !0,
            "is-muted": !A.localUserInfoExcludeVolume.isAudioAvailable,
            volume: A.volumeMap && A.volumeMap[A.localUserInfoExcludeVolume.domId]
          }
        }) : A._e()];
      },
      proxy: !0
    }])
  })], 1), e("ToggleWindowItem", {
    key: A.ViewName.REMOTE,
    attrs: {
      value: A.ViewName.REMOTE
    }
  }, [e("Player", {
    attrs: {
      "dom-id": A.playerDomId,
      "show-audio-stream": !A.isPusherVideoAvailable
    },
    scopedSlots: A._u([{
      key: "audio-stream",
      fn: function() {
        return [e("AudioStream", {
          attrs: {
            userId: A.remoteUserListExcludeVolume[0] && A.remoteUserListExcludeVolume[0].userId,
            username: A.remoteUserListExcludeVolume[0] && A.remoteUserListExcludeVolume[0].displayUserInfo,
            avatar: A.remoteUserListExcludeVolume[0] && A.remoteUserListExcludeVolume[0].avatar,
            "is-video-available": A.isPusherVideoAvailable,
            "is-small-window": A.largeViewName !== A.ViewName.REMOTE,
            "is-muted": A.remoteUserListExcludeVolume[0] && !A.remoteUserListExcludeVolume[0].isAudioAvailable,
            volume: A.remoteStreamVolume
          }
        })];
      },
      proxy: !0
    }, {
      key: "stream-info",
      fn: function() {
        return [A.callType === A.CallMediaType.VIDEO ? e("TKStreamInfo", {
          attrs: {
            "nick-name": A.remoteUserListExcludeVolume[0] && A.remoteUserListExcludeVolume[0].displayUserInfo,
            "is-muted": A.remoteUserListExcludeVolume[0] && !A.remoteUserListExcludeVolume[0].isAudioAvailable,
            volume: A.remoteStreamVolume
          }
        }) : A._e()];
      },
      proxy: !0
    }])
  })], 1)], 1)], 1)])], 1);
}, od = [];
const ha = {};
var sd = /* @__PURE__ */ L(
  pt,
  ad,
  od,
  !1,
  id,
  "5ca72ad0",
  null,
  null
);
function id(A) {
  for (let t in ha)
    this[t] = ha[t];
}
const ld = /* @__PURE__ */ function() {
  return sd.exports;
}(), ud = [se.SUCCESS, se.INFO, se.WARNING, se.ERROR], HA = {
  isShow: {
    type: Boolean,
    default: !1
  },
  message: {
    type: String,
    default: ""
  },
  type: {
    type: String,
    values: ud,
    default: se.INFO
  },
  duration: {
    type: Number,
    default: 3e3
  },
  offset: {
    type: Number,
    default: 16
  },
  showClose: {
    type: Boolean,
    default: !1
  },
  showIcon: {
    type: Boolean,
    default: !0
  },
  customClass: {
    type: String
  },
  customStyle: {
    type: Object
  }
};
function rd(A, t) {
  const e = I((A == null ? void 0 : A.message) || HA.message.default), n = I((A == null ? void 0 : A.duration) || HA.duration.default), a = I((A == null ? void 0 : A.type) || HA.type.default), s = I((A == null ? void 0 : A.offset) || HA.offset.default), i = I((A == null ? void 0 : A.showClose) || HA.showClose.default), u = I(!1);
  let l = -1;
  const c = (p) => {
    l > -1 && (clearTimeout(l), l = -1), u.value = !0, N(p || {}), n.value && (l = setTimeout(() => {
      g();
    }, n.value));
  }, g = () => {
    u.value = !1, l > -1 && (clearTimeout(l), l = -1);
  }, N = (p) => {
    const {
      message: M = e.value,
      type: f = a.value,
      offset: v = s.value,
      duration: S = n.value,
      showClose: h = i.value
    } = p;
    n.value = (A == null ? void 0 : A.duration) === 0 ? A == null ? void 0 : A.duration : S, e.value = M, a.value = f, s.value = v, i.value = h;
  };
  return Y(
    () => A == null ? void 0 : A.isShow,
    (p) => {
      p && c();
    },
    { immediate: !0 }
  ), Y(u, (p) => {
    p || t("onClose");
  }), {
    messageContent: e,
    messageDuration: n,
    messageType: a,
    messageOffset: s,
    isShowCloseIcon: i,
    visible: u,
    show: c,
    close: g
  };
}
const cd = "data:image/svg+xml;base64,", dd = "data:image/svg+xml;base64,", gd = "data:image/svg+xml;base64,", Id = "data:image/svg+xml;base64,", pd = "data:image/svg+xml;base64,", De = {
  options: {
    virtualHost: !0
  }
};
De.props = HA;
De.setup = (A, t) => {
  const e = A, n = t.emit, {
    messageContent: a,
    messageType: s,
    messageOffset: i,
    isShowCloseIcon: u,
    visible: l,
    show: c,
    close: g
  } = rd(e, n), N = C(() => X([`${z}-message`, `${z}-message--${s.value}`, e.customClass])), p = C(() => X([`${z}-message_icon`])), M = C(() => X([`${z}-message_close`])), f = C(() => `${i.value}px`), v = C(() => ({
    top: f.value,
    ...e.customStyle
  }));
  return Object.assign({
    CloseSrc: pd,
    messageContent: a,
    messageType: s,
    isShowCloseIcon: u,
    visible: l,
    close: g,
    messageClassName: N,
    typeIconClassName: p,
    closeIconClassName: M,
    messageStyle: v,
    IconSrcMap: {
      info: Id,
      waring: gd,
      success: dd,
      error: cd
    }
  }, {
    show: c,
    close: g
  });
};
De.components = Object.assign({
  Icon: wt
}, De.components);
var Md = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return A.visible ? e("div", {
    class: A.messageClassName,
    style: [A.messageStyle]
  }, [e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.showIcon,
      expression: "showIcon"
    }],
    class: A.typeIconClassName
  }, [e("Icon", {
    attrs: {
      src: A.IconSrcMap[A.messageType]
    }
  })], 1), e("span", [A._v(A._s(A.messageContent))]), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isShowCloseIcon,
      expression: "isShowCloseIcon"
    }],
    class: A.closeIconClassName,
    on: {
      click: A.close
    }
  }, [e("Icon", {
    attrs: {
      src: A.CloseSrc
    }
  })], 1)]) : A._e();
}, Cd = [];
const ma = {};
var Nd = /* @__PURE__ */ L(
  De,
  Md,
  Cd,
  !1,
  fd,
  null,
  null,
  null
);
function fd(A) {
  for (let t in ma)
    this[t] = ma[t];
}
const rs = /* @__PURE__ */ function() {
  return Nd.exports;
}(), vd = {
  customClass: {
    type: String
  }
}, ye = {
  options: {
    virtualHost: !0,
    styleIsolation: "shared"
  }
};
ye.props = vd;
ye.setup = (A, t) => {
  const {
    isFloatWindow: e
  } = j(vA()), {
    tip: n,
    duration: a
  } = Fi(), s = J(), {
    isGroupCall: i
  } = j(Z()), u = I(null), l = A, c = y.isPC, g = C(() => [l.customClass]), N = C(() => it({
    color: "white",
    background: "none",
    position: "absolute",
    "z-index": 4,
    "align-items": "center",
    left: "50%",
    top: `${c ? "60%" : "70%"}`,
    transform: "translate(-50%, -50%)",
    "font-size": e.value ? "12px" : void 0
  })), p = C(() => it({
    color: "white",
    background: "none",
    position: "static",
    transform: "none",
    "font-size": e.value ? "12px" : void 0
  })), M = C(() => i.value ? p.value : N.value);
  return Y([n, s, a], () => {
    var f;
    (f = u.value) == null || f.show({
      message: s.value(n.value),
      duration: a.value,
      offset: 0
    });
  }), {
    message: u,
    tipClassName: g,
    customStyle: M
  };
};
ye.components = Object.assign({
  Message: rs
}, ye.components);
var Ld = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", [e("Message", {
    ref: "message",
    class: A.tipClassName,
    attrs: {
      showIcon: !1,
      customStyle: A.customStyle
    }
  })], 1);
}, hd = [];
const Sa = {};
var md = /* @__PURE__ */ L(
  ye,
  Ld,
  hd,
  !1,
  Sd,
  null,
  null,
  null
);
function Sd(A) {
  for (let t in Sa)
    this[t] = Sa[t];
}
const cs = /* @__PURE__ */ function() {
  return md.exports;
}(), Dd = ["%", "vw"], yd = {
  length: {
    type: Number,
    default: 0
  },
  unit: {
    type: String,
    values: Dd,
    default: "%"
  },
  enableFocus: {
    type: Boolean,
    default: !1
  },
  layout: {
    type: Array
  },
  focus: {
    type: [String, Number]
  }
}, ds = "GridContextKey", Mn = {
  options: {
    virtualHost: !0
  }
};
Mn.props = yd;
Mn.setup = (A, t) => {
  const e = A, n = I(e.focus), a = I(e.layout), s = I(e.unit), i = t.emit;
  function u(l) {
    i("toggle", l);
  }
  return Y(() => e.focus, () => n.value = e.focus), Y(() => e.layout, () => a.value = e.layout), Y(() => e.unit, () => s.value = e.unit), rA(ds, {
    layout: a,
    enableFocus: e.enableFocus,
    handleFocusChange: u,
    focus: n,
    unit: s
  }), {};
};
var Ed = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    style: {
      height: "100%"
    }
  }, [A._t("default")], 2);
}, Td = [];
const Da = {};
var wd = /* @__PURE__ */ L(
  Mn,
  Ed,
  Td,
  !1,
  jd,
  null,
  null,
  null
);
function jd(A) {
  for (let t in Da)
    this[t] = Da[t];
}
const gs = /* @__PURE__ */ function() {
  return wd.exports;
}(), Cn = {
  options: {
    virtualHost: !0
  }
};
Cn.props = ["index", "height", "customStyle"];
Cn.setup = (A, t) => {
  const e = A, {
    layout: n,
    enableFocus: a,
    handleFocusChange: s,
    focus: i,
    unit: u
  } = dA(ds), l = 100 / 12, c = l, g = I({}), N = X([`${z}-grid-item`, {
    pc: qA,
    mobile: !qA,
    h5: Bo
  }]), p = () => {
    const f = String(e.index) === String(i.value) ? null : e.index;
    a && s(f);
  }, M = C(() => !!Pn(n.value, {
    key: "i",
    value: e.index
  }));
  return $A(() => {
    const f = Pn(n.value, {
      key: "i",
      value: e.index
    });
    if (!f)
      return;
    const {
      x: v,
      y: S,
      w: h,
      h: E,
      customStyle: x,
      customProps: m
    } = f;
    g.value = {
      width: h * l + u.value,
      height: e.height || E * c + u.value,
      left: v * l + u.value,
      top: S * c + u.value,
      position: "absolute",
      visibility: (m == null ? void 0 : m.show) === !1 ? "hidden" : "",
      ...e.customStyle,
      ...x
    };
  }), tA(() => {
    String(e.index) === String(i.value) && a && s(null);
  }), {
    style: g,
    gridItemClassName: N,
    handleClick: p,
    show: M
  };
};
var bd = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return A.show ? e("div", {
    class: A.gridItemClassName,
    style: [A.style],
    on: {
      click: A.handleClick
    }
  }, [A._t("default")], 2) : A._e();
}, zd = [];
const ya = {};
var Ud = /* @__PURE__ */ L(
  Cn,
  bd,
  zd,
  !1,
  Od,
  null,
  null,
  null
);
function Od(A) {
  for (let t in ya)
    this[t] = ya[t];
}
const Is = /* @__PURE__ */ function() {
  return Ud.exports;
}();
function xd() {
  const A = I([]), t = I([]), { callStatus: e, isGroupCall: n, callType: a, callRole: s } = j(Z()), { status: i } = _e() || {}, u = Zi();
  return Y(
    [e, n, a, s, i, u],
    () => {
      var h, E, x;
      let l = u.value;
      const c = y.isPC ? "pc" : "mobile", g = n.value ? "groupCall" : "singleCall", N = a.value === V.AUDIO ? "audio" : "video";
      let p = e.value === w.CALLING ? s.value === H.CALLER ? "calling" : "accept" : e.value;
      n && (i == null ? void 0 : i.value) === "close" && (p = "close_" + p);
      const M = ((x = (E = (h = l == null ? void 0 : l[c]) == null ? void 0 : h[g]) == null ? void 0 : E[N]) == null ? void 0 : x[p]) || [];
      t.value = M;
      const f = [];
      let v = 0;
      for (let m = 0; m < M.length; m++) {
        const P = m === 0 ? M[m].filter((b) => {
          var _;
          return ((_ = b == null ? void 0 : b.props) == null ? void 0 : _.show) !== !1;
        }) : M[m];
        M[m] = P;
        const W = 12 / P.length, eA = 3;
        for (let b = 0; b < P.length; b++)
          f[v++] = {
            i: P[b].name,
            x: b * W,
            y: m * W,
            w: W,
            h: eA,
            customStyle: P[b].customStyle,
            customProps: P[b].props
          };
      }
      v = 0;
      let S = [];
      for (let m = 0; m < M.flat().length; m++)
        S[m] = f[v++];
      S = S.filter((m) => m.i), A.value = S;
    },
    {
      immediate: !0
    }
  ), { layout: A, config: t };
}
function Bd(...A) {
  let t = window == null ? void 0 : window.document, e = "", n = () => {
  }, a = {};
  if (typeof A[0] == "string" ? [e, n, a] = A : [t, e, n, a] = A, !!t)
    return t == null || t.addEventListener(e, n, a), () => {
      t == null || t.removeEventListener(e, n, a);
    };
}
function Vd(A, t) {
  let e = () => {
  };
  sA(() => {
    e = Bd("click", (a) => {
      const s = A.map((l) => Ps(l)), i = a.composedPath(), u = [];
      for (let l = 0; l < s.length; l++)
        u[l] = i.includes(s[l]);
      u.every((l) => !l) && t();
    }, { passive: !0 });
  }), tA(() => {
    e == null || e();
  });
}
const Pd = (A, t, e, n, a) => {
  const s = e.isShowArrow ? e.arrowSize : 0, i = `${A.width / 2 - t.width / 2}px`, u = `${A.height / 2 - t.height / 2}px`, l = `${s}px solid ${e.color}`, c = { ...n }, g = { ...a };
  switch (e.placement) {
    case "top":
      c.left = i, c.top = `-${t.height + s + e.arrowDistance}px`, g.borderTop = l, g.left = `${A.width / 2 - s / 2}px`, g.top = `-${s + e.arrowDistance}px`;
      break;
    case "bottom":
      c.left = i, c.top = `${A.height + s + e.arrowDistance}px`, g.borderBottom = l, g.left = `${A.width / 2 - s / 2}px`, g.top = `${A.height + e.arrowDistance - s}px`;
      break;
    case "left":
      c.left = `-${t.width + s + e.arrowDistance}px`, c.top = u, g.borderLeft = l, g.left = `-${s * 2 - e.arrowDistance}px`, g.top = `${A.height / 2 - s / 2}px`;
      break;
    case "right":
      c.left = `${A.width + s + e.arrowDistance}px`, c.top = u, g.borderRight = l, g.left = `${A.width + s * 2 - e.arrowDistance}px`, g.top = `${A.height / 2 - s / 2}px`;
      break;
  }
  return {
    finalContentStyle: c,
    finalArrowStyle: g
  };
}, Fd = ["click", "hover"], Rd = ["top", "bottom", "left", "right"], ps = {
  trigger: {
    type: String,
    values: Fd,
    default: "click"
  },
  placement: {
    type: String,
    values: Rd,
    default: "top"
  },
  color: {
    type: String
  },
  isShowArrow: {
    type: Boolean,
    default: !0
  },
  arrowSize: {
    type: Number,
    default: 5
  },
  arrowDistance: {
    type: Number,
    default: 5
  },
  show: {
    type: Boolean
  },
  autoClose: {
    type: Number,
    default: 300
  }
}, Nn = {
  options: {
    virtualHost: !0
  }
};
Nn.props = ps;
Nn.setup = (A, t) => {
  const e = A, n = `${z}-popover`, a = `${z}-popover_content`, s = `${z}-popover_arrow`, i = `${z}-popover_trigger`, u = I(), l = I(), c = I(), g = I(!1), N = I(null), p = C(() => g.value && e.isShowArrow && e.show), M = it({
    left: "0px",
    top: "0px",
    backgroundColor: e.color
  }), f = I(M), v = `${e.arrowSize}px solid transparent`, S = {
    borderLeft: v,
    borderRight: v,
    borderTop: v,
    borderBottom: v,
    top: "0px",
    left: "0px"
  }, h = I(S), E = t.emit, x = () => {
    Fs(() => {
      var QA, xA, BA, VA;
      if (!g.value || !e.show)
        return;
      const b = (xA = (QA = l.value).getBoundingClientRect) == null ? void 0 : xA.call(QA), _ = (VA = (BA = c.value).getBoundingClientRect) == null ? void 0 : VA.call(BA), {
        finalContentStyle: Q,
        finalArrowStyle: OA
      } = Pd(b, _, e, M, S);
      f.value = {
        ...M,
        ...Q
      }, h.value = {
        ...S,
        ...OA
      };
    });
  }, m = () => {
    clearTimeout(N.value);
  }, P = () => {
    e.trigger === "click" && (g.value = !g.value, x());
  }, W = () => {
    e.trigger === "hover" && (E("hover"), m(), g.value = !0, x());
  }, eA = () => {
    e.trigger === "hover" && (N.value = setTimeout(() => {
      g.value = !1, x();
    }, 1e3));
  };
  return Vd([u], () => {
    g.value = !1;
  }), {
    popoverClassName: n,
    popoverContentClassName: a,
    popoverArrowClassName: s,
    popoverTriggerClassName: i,
    popoverRef: u,
    triggerRef: l,
    contentRef: c,
    visible: g,
    showArrow: p,
    contentStyle: f,
    arrowStyle: h,
    clearTimer: m,
    handleClick: P,
    handleMouseEnter: W,
    handleMouseLeave: eA
  };
};
var Gd = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    ref: "popoverRef",
    class: A.popoverClassName
  }, [A.visible && A.show ? e("div", {
    ref: "contentRef",
    class: A.popoverContentClassName,
    style: [A.contentStyle],
    on: {
      mouseenter: A.clearTimer,
      mouseleave: A.handleMouseLeave
    }
  }, [A._t("content")], 2) : A._e(), A.showArrow ? e("div", {
    class: A.popoverArrowClassName,
    style: [A.arrowStyle]
  }) : A._e(), e("div", {
    ref: "triggerRef",
    class: A.popoverTriggerClassName,
    on: {
      click: A.handleClick,
      mouseenter: A.handleMouseEnter,
      mouseleave: A.handleMouseLeave
    }
  }, [A._t("trigger")], 2)]);
}, kd = [];
const Ea = {};
var _d = /* @__PURE__ */ L(
  Nn,
  Gd,
  kd,
  !1,
  Wd,
  null,
  null,
  null
);
function Wd(A) {
  for (let t in Ea)
    this[t] = Ea[t];
}
const qd = /* @__PURE__ */ function() {
  return _d.exports;
}(), Xd = {
  options: {
    virtualHost: !0
  }
};
var Qd = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div");
}, Zd = [];
const Ta = {};
var Yd = /* @__PURE__ */ L(
  Xd,
  Qd,
  Zd,
  !1,
  Kd,
  null,
  null,
  null
);
function Kd(A) {
  for (let t in Ta)
    this[t] = Ta[t];
}
const Hd = /* @__PURE__ */ function() {
  return Yd.exports;
}(), Ee = {
  options: {
    virtualHost: !0
  }
};
Ee.props = ps;
Ee.setup = (A, t) => {
  const e = t.emit;
  return {
    IN_WX_MINI_APP: re,
    handleHover: () => {
      e("hover");
    }
  };
};
Ee.components = Object.assign({
  PopoverWeb: qd,
  PopoverWx: Hd
}, Ee.components);
var Jd = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return A.IN_WX_MINI_APP ? A._e() : e("PopoverWeb", A._b({
    on: {
      hover: A.handleHover
    },
    scopedSlots: A._u([{
      key: "trigger",
      fn: function() {
        return [A._t("trigger")];
      },
      proxy: !0
    }, {
      key: "content",
      fn: function() {
        return [A._t("content")];
      },
      proxy: !0
    }], null, !0)
  }, "PopoverWeb", A.$props, !1));
}, $d = [];
const wa = {};
var Ag = /* @__PURE__ */ L(
  Ee,
  Jd,
  $d,
  !1,
  eg,
  null,
  null,
  null
);
function eg(A) {
  for (let t in wa)
    this[t] = wa[t];
}
const fn = /* @__PURE__ */ function() {
  return Ag.exports;
}(), tg = {
  deviceType: {
    type: String
  },
  isShowControlBtn: {
    type: Boolean,
    default: !1
  }
}, Te = {
  options: {
    virtualHost: !0
  }
};
Te.props = tg;
Te.setup = (A, t) => {
  const e = A, [{
    deviceList: n,
    currentDeviceId: a
  }, {
    updateCurrentDeviceId: s
  }] = yt(e.deviceType), {
    localUserInfoExcludeVolume: i
  } = j(pA()), {
    isMuteSpeaker: u
  } = j(Z()), l = Tt(), c = J(), g = async (f) => {
    try {
      M(), await O.switchDevice({
        deviceType: e.deviceType,
        deviceId: f
      }), s(f);
    } catch (v) {
      console.debug(v);
    }
  }, N = async () => {
    var f, v;
    M(), e.deviceType === oA.CAMERA && ((f = i.value) != null && f.isVideoAvailable ? await O.closeCamera() : await O.openCamera("localVideo")), e.deviceType === oA.MICROPHONE && ((v = i.value) != null && v.isAudioAvailable ? await O.closeMicrophone() : await O.openMicrophone()), e.deviceType === oA.SPEAKER && (u.value ? await O.unMuteSpeaker() : await O.muteSpeaker());
  }, p = C(() => {
    var f;
    if (e.deviceType === oA.CAMERA)
      return i != null && i.value.isVideoAvailable ? c.value("close camera") : c.value("open camera");
    if (e.deviceType === oA.MICROPHONE)
      return (f = i == null ? void 0 : i.value) != null && f.isAudioAvailable ? c.value("close microphone") : c.value("open microphone");
    if (e.deviceType === oA.SPEAKER)
      return u.value ? c.value("open speaker") : c.value("close speaker");
  }), M = () => {
    l.value = "close";
  };
  return {
    deviceList: n,
    currentDeviceId: a,
    handleClickDeviceItem: g,
    switchStatus: N,
    deviceText: p
  };
};
Te.components = Object.assign({
  TKText: iA
}, Te.components);
var ng = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "device-selector-container"
  }, [e("div", {
    staticClass: "scroll-container"
  }, [e("div", {
    staticClass: "scroll-content"
  }, A._l(A.deviceList, function(n) {
    return e("div", {
      key: n.deviceId
    }, [e("TKText", {
      staticClass: "device-item",
      class: {
        select: A.currentDeviceId === n.deviceId
      },
      attrs: {
        width: "100%",
        color: "#ffff",
        truncated: !0,
        size: "14px"
      },
      on: {
        click: function() {
          return A.handleClickDeviceItem(n.deviceId);
        }
      }
    }, [A._v(" " + A._s(n.label) + " ")])], 1);
  }), 0)]), e("div", {
    staticClass: "control-item"
  }), A.isShowControlBtn ? e("TKText", {
    staticClass: "device-item",
    attrs: {
      width: "100%",
      color: "#ffff",
      truncated: !0,
      size: "14px"
    },
    on: {
      click: A.switchStatus
    }
  }, [A._v(" " + A._s(A.deviceText) + " ")]) : A._e()], 1);
}, ag = [];
const ja = {};
var og = /* @__PURE__ */ L(
  Te,
  ng,
  ag,
  !1,
  sg,
  "af70734e",
  null,
  null
);
function sg(A) {
  for (let t in ja)
    this[t] = ja[t];
}
const vn = /* @__PURE__ */ function() {
  return og.exports;
}(), we = {
  options: {
    virtualHost: !0
  }
};
we.props = SA;
we.setup = (A, t) => {
  const e = J(), n = A, a = I(!0), s = y.isPC, {
    localUserInfoExcludeVolume: i
  } = j(pA()), u = I(i == null ? void 0 : i.value.isVideoAvailable), l = C(() => i == null ? void 0 : i.value.isVideoAvailable), c = Tt(), [{
    deviceList: g
  }] = yt(oA.CAMERA), N = C(() => {
    var h;
    return ((h = g.value) == null ? void 0 : h.length) > 0 && c.value === "camera";
  }), p = C(() => a.value ? l.value ? "basicConfig" : "closedConfig" : "loadingConfig"), M = C(() => i != null && i.value.isVideoAvailable ? e.value("camera enabled") : e.value("camera disabled")), f = cA("camera", p);
  return {
    DeviceType: oA,
    props: n,
    clickAble: a,
    isPC: s,
    isShowPopover: N,
    btnText: M,
    config: f,
    handleClick: async () => {
      a.value = !1, u.value = !l.value, l.value ? await O.closeCamera() : await O.openCamera("localVideo"), a.value = !0;
    },
    handleMouseEnter: () => {
      c.value = "camera";
    }
  };
};
we.components = Object.assign({
  Popover: fn,
  DeviceSelect: vn,
  Button: CA,
  TKText: iA
}, we.components);
var ig = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "btn-content"
  }, [A.isPC ? e("Popover", {
    attrs: {
      color: "rgba(107, 117, 138, 0.5)",
      show: A.isShowPopover,
      trigger: "hover"
    },
    on: {
      hover: A.handleMouseEnter
    },
    scopedSlots: A._u([{
      key: "content",
      fn: function() {
        return [e("DeviceSelect", {
          attrs: {
            isShowControlBtn: !0,
            deviceType: A.DeviceType.CAMERA
          }
        })];
      },
      proxy: !0
    }, {
      key: "trigger",
      fn: function() {
        return [e("Button", {
          attrs: {
            loading: !A.clickAble,
            iconSrc: A.config.iconSrc,
            color: A.config.color,
            iconSize: A.config.iconSize,
            width: A.props.width || A.config.width,
            height: A.props.height || A.config.height,
            loadingWidth: A.config.loadingWidth,
            loadingHeight: A.config.loadingHeight,
            shape: A.config.shape
          },
          on: {
            click: A.handleClick
          }
        })];
      },
      proxy: !0
    }], null, !1, 2755011860)
  }) : A._e(), A.isPC ? A._e() : e("Button", {
    attrs: {
      loading: !A.clickAble,
      iconSrc: A.config.iconSrc,
      color: A.config.color,
      iconSize: A.props.iconSize || A.config.iconSize,
      width: A.props.width || A.config.width,
      height: A.props.height || A.config.height,
      loadingWidth: A.config.loadingWidth,
      loadingHeight: A.config.loadingHeight,
      shape: "circle"
    },
    on: {
      click: A.handleClick
    }
  }), A.config.showText ? e("TKText", {
    attrs: {
      textStyle: A.config.textStyle,
      color: A.config.textColor,
      size: A.config.textSize
    }
  }, [A._v(" " + A._s(A.btnText) + " ")]) : A._e()], 1);
}, lg = [];
const ba = {};
var ug = /* @__PURE__ */ L(
  we,
  ig,
  lg,
  !1,
  rg,
  "fdb51114",
  null,
  null
);
function rg(A) {
  for (let t in ba)
    this[t] = ba[t];
}
const cg = /* @__PURE__ */ function() {
  return ug.exports;
}(), je = {
  options: {
    virtualHost: !0
  }
};
je.props = SA;
je.setup = (A, t) => {
  const e = A, n = Gi(), a = C(() => n.value ? "basicConfig" : "loadingConfig"), s = cA("hangup", a), i = J();
  return {
    props: e,
    isClickable: n,
    config: s,
    t: i,
    handleClick: async () => {
      await O.hangup();
    }
  };
};
je.components = Object.assign({
  Button: CA,
  TKText: iA
}, je.components);
var dg = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "btn-content"
  }, [e("Button", {
    attrs: {
      loading: !A.isClickable,
      loadingWidth: A.config.loadingWidth,
      loadingHeight: A.config.loadingHeight,
      iconSrc: A.config.iconSrc,
      color: A.config.color,
      iconSize: A.props.iconSize || A.config.iconSize,
      width: A.props.width || A.config.width,
      height: A.props.height || A.config.height,
      shape: A.config.shape
    },
    on: {
      click: A.handleClick
    }
  }), A.config.showText ? e("TKText", {
    attrs: {
      textStyle: A.config.textStyle,
      color: A.config.textColor,
      size: A.config.textSize
    }
  }, [A._v(" " + A._s(A.t("hangup")) + " ")]) : A._e()], 1);
}, gg = [];
const za = {};
var Ig = /* @__PURE__ */ L(
  je,
  dg,
  gg,
  !1,
  pg,
  "348e1782",
  null,
  null
);
function pg(A) {
  for (let t in za)
    this[t] = za[t];
}
const Mg = /* @__PURE__ */ function() {
  return Ig.exports;
}(), be = {
  options: {
    virtualHost: !0
  }
};
be.props = SA;
be.setup = (A, t) => {
  const e = I(!0), n = A, a = C(() => e.value ? "basicConfig" : "loadingConfig"), s = cA("accept", a);
  return {
    t: J(),
    clickAble: e,
    props: n,
    config: s,
    handleClick: async () => {
      e.value = !1, await O.accept(), e.value = !0;
    }
  };
};
be.components = Object.assign({
  Button: CA,
  TKText: iA
}, be.components);
var Cg = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "btn-content"
  }, [e("Button", {
    attrs: {
      iconSrc: A.config.iconSrc,
      color: A.config.color,
      iconSize: A.props.iconSize || A.config.iconSize,
      width: A.props.width || A.config.width,
      height: A.props.height || A.config.height,
      shape: A.config.shape,
      loading: !A.clickAble,
      loadingWidth: A.config.loadingWidth,
      loadingHeight: A.config.loadingHeight
    },
    on: {
      click: A.handleClick
    }
  }), A.config.showText ? e("TKText", {
    attrs: {
      textStyle: A.config.textStyle,
      color: A.config.textColor,
      size: A.config.textSize
    }
  }, [A._v(" " + A._s(A.t("accept")) + " ")]) : A._e()], 1);
}, Ng = [];
const Ua = {};
var fg = /* @__PURE__ */ L(
  be,
  Cg,
  Ng,
  !1,
  vg,
  "3da27647",
  null,
  null
);
function vg(A) {
  for (let t in Ua)
    this[t] = Ua[t];
}
const Lg = /* @__PURE__ */ function() {
  return fg.exports;
}(), ze = {
  options: {
    virtualHost: !0
  }
};
ze.props = SA;
ze.setup = (A, t) => {
  const e = y.isPC, n = I(!0), {
    localUserInfoExcludeVolume: a
  } = j(pA()), s = Tt(), [{
    deviceList: i
  }] = yt(oA.MICROPHONE), u = C(() => {
    var f;
    return ((f = i.value) == null ? void 0 : f.length) > 0 && s.value === "microphone";
  }), l = C(() => n.value ? a != null && a.value.isAudioAvailable ? "basicConfig" : "closedConfig" : "loadingConfig"), c = cA("microphone", l), g = J(), N = C(() => a != null && a.value.isAudioAvailable ? g.value("microphone enabled") : g.value("microphone disabled"));
  return {
    DeviceType: oA,
    isPC: e,
    clickAble: n,
    isShowPopover: u,
    config: c,
    btnText: N,
    handleClick: async () => {
      a != null && a.value.isAudioAvailable ? await O.closeMicrophone() : await O.openMicrophone();
    },
    handleMouseEnter: () => {
      s.value = "microphone";
    }
  };
};
ze.components = Object.assign({
  Popover: fn,
  DeviceSelect: vn,
  Button: CA,
  TKText: iA
}, ze.components);
var hg = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "btn-content"
  }, [A.isPC ? e("Popover", {
    attrs: {
      color: "rgba(107, 117, 138, 0.5)",
      show: A.isShowPopover,
      trigger: "hover"
    },
    on: {
      hover: A.handleMouseEnter
    },
    scopedSlots: A._u([{
      key: "content",
      fn: function() {
        return [e("DeviceSelect", {
          attrs: {
            isShowControlBtn: !0,
            deviceType: A.DeviceType.MICROPHONE
          }
        })];
      },
      proxy: !0
    }, {
      key: "trigger",
      fn: function() {
        return [e("Button", {
          attrs: {
            loading: !A.clickAble,
            iconSrc: A.config.iconSrc,
            color: A.config.color,
            iconSize: A.config.iconSize,
            width: A.config.width,
            height: A.config.height,
            loadingWidth: A.config.loadingWidth,
            loadingHeight: A.config.loadingHeight,
            shape: A.config.shape
          },
          on: {
            click: A.handleClick
          }
        })];
      },
      proxy: !0
    }], null, !1, 922566162)
  }) : A._e(), A.isPC ? A._e() : e("Button", {
    attrs: {
      loading: !A.clickAble,
      iconSrc: A.config.iconSrc,
      color: A.config.color,
      iconSize: A.config.iconSize,
      width: A.config.width,
      height: A.config.height,
      loadingWidth: A.config.loadingWidth,
      loadingHeight: A.config.loadingHeight,
      shape: A.config.shape
    },
    on: {
      click: A.handleClick
    }
  }), A.config.showText ? e("TKText", {
    attrs: {
      textStyle: A.config.textStyle,
      color: A.config.textColor,
      size: A.config.textSize
    }
  }, [A._v(" " + A._s(A.btnText) + " ")]) : A._e()], 1);
}, mg = [];
const Oa = {};
var Sg = /* @__PURE__ */ L(
  ze,
  hg,
  mg,
  !1,
  Dg,
  "76abffb6",
  null,
  null
);
function Dg(A) {
  for (let t in Oa)
    this[t] = Oa[t];
}
const yg = /* @__PURE__ */ function() {
  return Sg.exports;
}(), Ue = {
  options: {
    virtualHost: !0
  }
};
Ue.props = SA;
Ue.setup = (A, t) => {
  const e = A, n = cA("reject", I("basicConfig")), a = J();
  return {
    props: e,
    config: n,
    t: a,
    handleClick: async () => {
      await O.reject();
    }
  };
};
Ue.components = Object.assign({
  Button: CA,
  TKText: iA
}, Ue.components);
var Eg = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "btn-content"
  }, [e("Button", {
    attrs: {
      iconSrc: A.config.iconSrc,
      color: A.config.color,
      iconSize: A.config.iconSize,
      width: A.props.width || A.config.width,
      height: A.props.height || A.config.height,
      shape: A.config.shape
    },
    on: {
      click: A.handleClick
    }
  }), A.config.showText ? e("TKText", {
    attrs: {
      textStyle: A.config.textStyle,
      color: A.config.textColor,
      size: A.config.textSize
    }
  }, [A._v(" " + A._s(A.t("reject")) + " ")]) : A._e()], 1);
}, Tg = [];
const xa = {};
var wg = /* @__PURE__ */ L(
  Ue,
  Eg,
  Tg,
  !1,
  jg,
  "4448bf22",
  null,
  null
);
function jg(A) {
  for (let t in xa)
    this[t] = xa[t];
}
const bg = /* @__PURE__ */ function() {
  return wg.exports;
}(), Oe = {
  options: {
    virtualHost: !0
  }
};
Oe.props = SA;
Oe.setup = (A, t) => {
  const e = y.isWeChat, n = y.isPC, a = A, {
    isEarPhone: s,
    isMuteSpeaker: i
  } = j(Z()), u = Tt(), l = J(), c = C(() => s.value ? "closedConfig" : "basicConfig"), g = C(() => i.value ? "closedConfig" : "basicConfig"), [{
    deviceList: N,
    currentDeviceId: p
  }] = yt(oA.SPEAKER), M = C(() => {
    var E;
    return ((E = N.value) == null ? void 0 : E.length) > 0 && u.value === "speaker" && n;
  }), f = e ? cA("speaker", c) : cA("speaker", g);
  return C(() => s.value ? l.value("speaker disabled") : l.value("speaker enabled")), {
    DeviceType: oA,
    props: a,
    isShowPopover: M,
    config: f,
    handleClick: async () => {
      i.value ? await O.unMuteSpeaker() : await O.muteSpeaker();
    },
    getButtonText: () => {
      if (n) {
        const E = N.value.find((x) => x.deviceId === p.value);
        return E ? E.label : l.value("speaker enabled");
      } else
        return i.value ? l.value("speaker disabled") : l.value("speaker enabled");
    },
    handleMouseEnter: () => {
      u.value = "speaker";
    }
  };
};
Oe.components = Object.assign({
  Popover: fn,
  DeviceSelect: vn,
  Button: CA,
  TKText: iA
}, Oe.components);
var zg = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "btn-content"
  }, [e("Popover", {
    attrs: {
      color: "rgba(107, 117, 138, 0.5)",
      show: A.isShowPopover,
      trigger: "hover"
    },
    on: {
      hover: A.handleMouseEnter
    },
    scopedSlots: A._u([{
      key: "content",
      fn: function() {
        return [e("DeviceSelect", {
          attrs: {
            isShowControlBtn: !0,
            deviceType: A.DeviceType.SPEAKER
          }
        })];
      },
      proxy: !0
    }, {
      key: "trigger",
      fn: function() {
        return [e("Button", {
          attrs: {
            iconSrc: A.config.iconSrc,
            color: A.config.color,
            iconSize: A.props.iconSize || A.config.iconSize,
            width: A.props.width || A.config.width,
            height: A.props.height || A.config.height,
            shape: A.config.shape
          },
          on: {
            click: A.handleClick
          }
        })];
      },
      proxy: !0
    }])
  }), A.config.showText ? e("TKText", {
    attrs: {
      width: "60px",
      truncated: !0,
      textStyle: A.config.textStyle,
      color: A.config.textColor,
      size: A.config.textSize,
      lineClamp: 2
    }
  }, [A._v(" " + A._s(A.getButtonText()) + " ")]) : A._e()], 1);
}, Ug = [];
const Ba = {};
var Og = /* @__PURE__ */ L(
  Oe,
  zg,
  Ug,
  !1,
  xg,
  "9ce02c5e",
  null,
  null
);
function xg(A) {
  for (let t in Ba)
    this[t] = Ba[t];
}
const Bg = /* @__PURE__ */ function() {
  return Og.exports;
}(), Mt = {
  options: {
    virtualHost: !0
  }
};
Mt.setup = (A, t) => {
  const {
    localUserInfoExcludeVolume: e
  } = j(pA()), n = C(() => e == null ? void 0 : e.value.isVideoAvailable), a = C(() => n.value ? "basicConfig" : "disableConfig"), s = cA("switchCamera", a), i = J();
  return {
    config: s,
    t: i,
    handleClick: async () => {
      !n.value || await O.switchCamera();
    }
  };
};
Mt.components = Object.assign({
  Button: CA,
  TKText: iA
}, Mt.components);
var Vg = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "btn-content"
  }, [e("Button", {
    attrs: {
      iconSrc: A.config.iconSrc,
      iconSize: A.config.iconSize,
      color: A.config.color,
      width: A.config.width,
      height: A.config.height,
      buttonStyle: A.config.buttonStyle,
      shape: "circle"
    },
    on: {
      click: A.handleClick
    }
  }), A.config.showText ? e("TKText", {
    attrs: {
      textStyle: A.config.textStyle,
      color: A.config.textColor,
      size: A.config.textSize
    }
  }, [A._v(" " + A._s(A.t("switch camera")) + " ")]) : A._e()], 1);
}, Pg = [];
const Va = {};
var Fg = /* @__PURE__ */ L(
  Mt,
  Vg,
  Pg,
  !1,
  Rg,
  "32c39f09",
  null,
  null
);
function Rg(A) {
  for (let t in Va)
    this[t] = Va[t];
}
const Gg = /* @__PURE__ */ function() {
  return Fg.exports;
}(), xe = {
  options: {
    virtualHost: !0
  }
};
xe.props = SA;
xe.setup = (A, t) => {
  const e = A, n = I(!0), {
    localUserInfoExcludeVolume: a
  } = j(pA()), s = C(() => a == null ? void 0 : a.value.isVideoAvailable), {
    enableVirtualBackground: i
  } = j(Z()), u = C(() => n.value ? i.value ? "closedConfig" : s.value ? "basicConfig" : "disableConfig" : "loadingConfig"), l = cA("virtualBackground", u), c = J();
  return {
    props: e,
    clickAble: n,
    config: l,
    t: c,
    handleClick: async () => {
      !s.value || (n.value = !1, await O.setBlurBackground(!i.value), n.value = !0);
    }
  };
};
xe.components = Object.assign({
  Button: CA,
  TKText: iA
}, xe.components);
var kg = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "btn-content"
  }, [e("Button", {
    attrs: {
      loading: !A.clickAble,
      iconSrc: A.config.iconSrc,
      color: A.config.color,
      iconSize: A.props.iconSize || A.config.iconSize,
      width: A.props.width || A.config.width,
      height: A.props.height || A.config.height,
      shape: A.config.shape,
      loadingWidth: A.config.loadingWidth,
      loadingHeight: A.config.loadingHeight,
      buttonStyle: A.config.buttonStyle
    },
    on: {
      click: A.handleClick
    }
  }), A.config.showText ? e("TKText", {
    attrs: {
      width: "70px",
      lineClamp: 2,
      textStyle: A.config.textStyle,
      color: A.config.textColor,
      size: A.config.textSize
    }
  }, [A._v(" " + A._s(A.t("virtual-background")) + " ")]) : A._e()], 1);
}, _g = [];
const Pa = {};
var Wg = /* @__PURE__ */ L(
  xe,
  kg,
  _g,
  !1,
  qg,
  "7122a1b4",
  null,
  null
);
function qg(A) {
  for (let t in Pa)
    this[t] = Pa[t];
}
const Xg = /* @__PURE__ */ function() {
  return Wg.exports;
}(), Be = {
  options: {
    virtualHost: !0
  }
};
Be.props = SA;
Be.setup = (A, t) => {
  const {
    status: e
  } = _e(), n = A, a = cA("toggleButtonPanel", I("basicConfig"));
  return {
    props: n,
    config: a,
    handleClick: async () => {
      e.value = e.value === "close" ? "open" : "close";
    }
  };
};
Be.components = Object.assign({
  Button: CA
}, Be.components);
var Qg = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "btn-content"
  }, [e("Button", {
    attrs: {
      iconSrc: A.config.iconSrc,
      color: A.config.color,
      iconSize: A.props.iconSize || A.config.iconSize,
      width: A.props.width || A.config.width,
      height: A.props.height || A.config.height,
      shape: A.config.shape
    },
    on: {
      click: A.handleClick
    }
  })], 1);
}, Zg = [];
const Fa = {};
var Yg = /* @__PURE__ */ L(
  Be,
  Qg,
  Zg,
  !1,
  Kg,
  "dd0c1858",
  null,
  null
);
function Kg(A) {
  for (let t in Fa)
    this[t] = Fa[t];
}
const Hg = /* @__PURE__ */ function() {
  return Yg.exports;
}(), Ct = {
  options: {
    virtualHost: !0
  }
};
Ct.setup = (A, t) => {
  const e = I(null), n = I({}), {
    isGroupCall: a
  } = j(Z()), {
    status: s
  } = _e() || {}, i = I(!1), u = I(!1), l = I(""), c = zo();
  $A(() => {
    i.value || (a.value && !y.isPC && c.value !== null ? i.value = !0 : i.value = !1);
  });
  const g = C(() => X(["button-panel-container", {
    pc: y.isPC,
    mobile: !y.isPC,
    h5: y.isH5,
    groupCall: a.value,
    singleCall: !a.value,
    close: (s == null ? void 0 : s.value) === "close",
    open: (s == null ? void 0 : s.value) === "open",
    showBackGround: i.value
  }])), N = X(["toggle-button-container", {
    h5: y.isH5
  }]), p = y.isH5 ? {
    transitionProperty: "width,height,left,top",
    transitionDuration: "0.3s",
    transitionTimingFunction: "ease-in"
  } : {};
  function M(E) {
    e.value = E;
  }
  const {
    layout: f,
    config: v
  } = xd();
  Y(v, () => {
    const E = {}, x = v.value.flat();
    for (let m of x) {
      const {
        name: P,
        props: W = {}
      } = m;
      W.showText !== !1 && (W.showText = !0), E[P] = W;
    }
    n.value = E;
  }, {
    immediate: !0
  });
  function S(E) {
    u.value = E;
  }
  const h = {
    [o.IS_CLICKABLE]: S
  };
  return sA(() => {
    d.watch(r.CALL, h, {
      notifyRangeWhenWatch: o.MYSELF
    });
  }), tA(() => {
    d.unwatch(r.CALL, h);
  }), rA(mo, u), rA(So, l), {
    focus: e,
    showButtonPanelBackground: i,
    buttonPanelClassName: g,
    toggleButtonContainerClassName: N,
    customStyle: p,
    handleToggle: M,
    layout: f
  };
};
Ct.components = Object.assign({
  Grid: gs,
  GridItem: Is,
  Accept: Lg,
  Reject: bg,
  Microphone: yg,
  Hangup: Mg,
  Speaker: Bg,
  TKCamera: cg,
  VirtualBackground: Xg,
  SwitchCamera: Gg,
  InviteUser: Xo,
  ToggleButtonPanel: Hg
}, Ct.components);
var Jg = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    class: A.buttonPanelClassName
  }, [e("div", {
    staticClass: "button-group"
  }, [e("Grid", {
    attrs: {
      unit: "%",
      layout: A.layout,
      focus: A.focus
    },
    on: {
      toggle: A.handleToggle
    }
  }, [e("GridItem", {
    attrs: {
      index: "accept",
      height: "auto",
      customStyle: A.customStyle
    }
  }, [e("Accept")], 1), e("GridItem", {
    attrs: {
      index: "reject",
      height: "auto",
      customStyle: A.customStyle
    }
  }, [e("Reject")], 1), e("GridItem", {
    attrs: {
      index: "microphone",
      height: "auto",
      customStyle: A.customStyle
    }
  }, [e("Microphone")], 1), e("GridItem", {
    attrs: {
      index: "hangup",
      height: "auto",
      customStyle: A.customStyle
    }
  }, [e("Hangup")], 1), e("GridItem", {
    attrs: {
      index: "speaker",
      height: "auto",
      customStyle: A.customStyle
    }
  }, [e("Speaker")], 1), e("GridItem", {
    attrs: {
      index: "camera",
      height: "auto",
      customStyle: A.customStyle
    }
  }, [e("TKCamera")], 1), e("GridItem", {
    attrs: {
      index: "virtualBackground",
      height: "auto",
      customStyle: A.customStyle
    }
  }, [e("VirtualBackground")], 1), e("GridItem", {
    attrs: {
      index: "switchCamera",
      height: "auto",
      customStyle: A.customStyle
    }
  }, [e("SwitchCamera")], 1), e("GridItem", {
    attrs: {
      index: "inviteUser",
      height: "auto",
      customStyle: A.customStyle
    }
  }, [e("InviteUser")], 1)], 1)], 1), A.showButtonPanelBackground ? e("div", {
    class: A.toggleButtonContainerClassName
  }, [e("ToggleButtonPanel")], 1) : A._e()]);
}, $g = [];
const Ra = {};
var AI = /* @__PURE__ */ L(
  Ct,
  Jg,
  $g,
  !1,
  eI,
  "26c8cb08",
  null,
  null
);
function eI(A) {
  for (let t in Ra)
    this[t] = Ra[t];
}
const Ms = /* @__PURE__ */ function() {
  return AI.exports;
}(), Nt = {};
Nt.setup = (A, t) => {
  const {
    callStatus: e
  } = j(Z());
  return {
    CallStatus: w,
    callStatus: e
  };
};
Nt.components = Object.assign({
  TopBar: Qo,
  Waiting: Jo,
  MediaContainer: ld,
  Tip: cs,
  ButtonPanel: Ms
}, Nt.components);
var tI = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "singlecall-container"
  }, [e("TopBar"), A.callStatus === A.CallStatus.CALLING ? e("Waiting") : A._e(), e("MediaContainer"), e("Tip"), e("ButtonPanel")], 1);
}, nI = [];
const Ga = {};
var aI = /* @__PURE__ */ L(
  Nt,
  tI,
  nI,
  !1,
  oI,
  "221760b8",
  null,
  null
);
function oI(A) {
  for (let t in Ga)
    this[t] = Ga[t];
}
const sI = /* @__PURE__ */ function() {
  return aI.exports;
}(), ft = {};
ft.setup = (A, t) => ({});
ft.components = Object.assign({
  Loading: an
}, ft.components);
var iI = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "stream-loading-container"
  }, [e("Loading", {
    attrs: {
      mode: "dot",
      color: "#FFF",
      loadingWidth: "50px",
      loadingHeight: "50px"
    }
  })], 1);
}, lI = [];
const ka = {};
var uI = /* @__PURE__ */ L(
  ft,
  iI,
  lI,
  !1,
  rI,
  "b3ff0e0c",
  null,
  null
);
function rI(A) {
  for (let t in ka)
    this[t] = ka[t];
}
const cI = /* @__PURE__ */ function() {
  return uI.exports;
}(), vt = {
  options: {
    virtualHost: !0
  }
};
vt.setup = (A, t) => {
  const e = I("local"), n = I(null), a = I(!1);
  We();
  const {
    localUserInfoExcludeVolume: s,
    remoteUserListExcludeVolume: i
  } = j(pA()), {
    callStatus: u,
    callType: l,
    isShowEnableVirtualBackground: c,
    callRole: g
  } = j(Z()), {
    netWorkQualityList: N
  } = Ri(), p = C(() => i.value.length + 1), M = bo(), f = _i(n, p), v = jo(), {
    isFloatWindow: S
  } = j(vA()), h = zo(), {
    status: E
  } = _e() || {}, x = !y.isPC, m = C(() => String(n.value) === "0"), P = C(() => !y.isPC && m.value && s.value.isVideoAvailable), W = C(() => P.value), eA = C(() => P.value && c.value && y.isWeChat), b = C(() => y.isPC || m.value), _ = C(() => y.isPC || S.value ? "%" : "vw"), Q = C(() => [s.value, ...i.value].map((q) => {
    var nA;
    if (S.value) {
      if (((nA = v.value) == null ? void 0 : nA[q.domId]) >= 10)
        return q.domId;
    } else
      return q.domId;
  })), OA = C(() => !(g.value === H.CALLEE && u.value === w.CALLING && !S.value)), QA = C(() => {
    let q = "";
    return g.value === H.CALLEE && u.value === w.CALLING && !S.value && (q = "hidden"), {
      visibility: q
    };
  });
  function xA(q) {
    n.value = q, h.value = q, E.value = q !== null ? "close" : "open";
  }
  function BA(q) {
    var nA, uA;
    return y.isWeChat ? !((uA = (nA = M.value) == null ? void 0 : nA.find((PA) => PA.userID === q.userId)) != null && uA.hasVideo) : !q.isVideoAvailable;
  }
  function VA(q) {
    var nA, uA;
    return y.isWeChat ? !((uA = (nA = M.value) == null ? void 0 : nA.find((PA) => PA.userID === q.userId)) != null && uA.hasAudio) : !q.isAudioAvailable;
  }
  function qe(q) {
    if (!N.value)
      return;
    const nA = q !== s.value.userId;
    if (!y.isWeChat && nA)
      return;
    const uA = N.value.find((PA) => PA.userId === q);
    return uA && (uA == null ? void 0 : uA.quality) >= 4;
  }
  Y([i, l], () => {
    var q, nA;
    (nA = (q = i.value) == null ? void 0 : q[0]) != null && nA.isEnter && (e.value = "remote"), l.value === V.AUDIO ? a.value = !1 : a.value = !0;
  });
  const bt = C(() => X(["groupcall-media-container", {
    mobile: !y.isPC,
    pc: y.isPC,
    "two-layout": p.value === 2 && n.value === null,
    float: S.value
  }])), zt = C(() => u.value === w.CALLING && (l.value === V.AUDIO && !s.value.isAudioAvailable || l.value === V.VIDEO && !s.value.isVideoAvailable));
  return {
    TUIGlobal: y,
    focus: n,
    localUserInfoExcludeVolume: s,
    remoteUserListExcludeVolume: i,
    streamLength: p,
    layout: f,
    volumeMap: v,
    isFloatWindow: S,
    enableFocus: x,
    showSwitchCameraButton: W,
    showVirtualBackgroundButton: eA,
    showNickName: b,
    unit: _,
    visibleStreamIdList: Q,
    showStream: OA,
    mediaContainerStyle: QA,
    changeFocus: xA,
    isShowAudioStream: BA,
    isMute: VA,
    isShowNetWork: qe,
    groupMediaContainerClassName: bt,
    isPusherLoading: zt
  };
};
vt.components = Object.assign({
  Portal: es,
  FloatWindow: us,
  Grid: gs,
  GridItem: Is,
  Pusher: as,
  AudioStream: ss,
  StreamLoading: cI,
  TKStreamInfo: ns,
  Player: os
}, vt.components);
var dI = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("Portal", {
    attrs: {
      id: "source",
      disabled: !A.isFloatWindow,
      source: "#source",
      to: "body"
    }
  }, [e("FloatWindow", [e("div", {
    class: A.groupMediaContainerClassName,
    style: [A.mediaContainerStyle]
  }, [e("Grid", {
    attrs: {
      unit: A.unit,
      "enable-focus": A.enableFocus,
      focus: A.focus,
      length: A.streamLength,
      layout: A.layout
    },
    on: {
      toggle: A.changeFocus
    }
  }, [e("GridItem", {
    key: A.localUserInfoExcludeVolume.userId,
    attrs: {
      index: 0
    }
  }, [e("Pusher", {
    attrs: {
      domId: A.localUserInfoExcludeVolume.domId,
      "show-audio-stream": !A.localUserInfoExcludeVolume.isVideoAvailable,
      show: A.visibleStreamIdList.includes(A.localUserInfoExcludeVolume.domId) && A.showStream
    },
    scopedSlots: A._u([{
      key: "audio-stream",
      fn: function() {
        return [e("AudioStream", {
          attrs: {
            "user-id": A.localUserInfoExcludeVolume.userId,
            username: A.localUserInfoExcludeVolume.displayUserInfo,
            avatar: A.localUserInfoExcludeVolume.avatar,
            "is-video-available": A.localUserInfoExcludeVolume.isVideoAvailable
          }
        })];
      },
      proxy: !0
    }, {
      key: "loading",
      fn: function() {
        return [A.isPusherLoading ? e("StreamLoading") : A._e()];
      },
      proxy: !0
    }, {
      key: "stream-info",
      fn: function() {
        return [A.isFloatWindow ? A._e() : e("TKStreamInfo", {
          attrs: {
            "is-self": !0,
            "show-nick-name": A.showNickName,
            showSwitchCameraButton: A.showSwitchCameraButton,
            showVirtualBackgroundButton: A.showVirtualBackgroundButton,
            showNetWorkStatus: A.isShowNetWork(A.localUserInfoExcludeVolume.userId),
            nickName: A.localUserInfoExcludeVolume.displayUserInfo,
            isMuted: !A.localUserInfoExcludeVolume.isAudioAvailable,
            volume: A.volumeMap && A.volumeMap[A.localUserInfoExcludeVolume.domId]
          }
        })];
      },
      proxy: !0
    }])
  })], 1), A._l(A.remoteUserListExcludeVolume, function(n, a) {
    return e("GridItem", {
      key: n.userId,
      attrs: {
        index: a + 1
      }
    }, [e("Player", {
      attrs: {
        domId: n.domId,
        "show-audio-stream": A.isShowAudioStream(n),
        show: A.visibleStreamIdList.includes(n.domId) && A.showStream
      },
      scopedSlots: A._u([{
        key: "audio-stream",
        fn: function() {
          return [e("AudioStream", {
            attrs: {
              "user-id": n.userId,
              username: n.displayUserInfo,
              avatar: n.avatar
            }
          })];
        },
        proxy: !0
      }, {
        key: "loading",
        fn: function() {
          return [n.isEnter ? A._e() : e("StreamLoading")];
        },
        proxy: !0
      }, {
        key: "stream-info",
        fn: function() {
          return [A.isFloatWindow ? A._e() : e("TKStreamInfo", {
            attrs: {
              "show-nick-name": A.TUIGlobal.isPC || String(A.focus) === String(a + 1),
              "show-control-button": !1,
              showNetWorkStatus: A.isShowNetWork(n.userId),
              nickName: n.displayUserInfo,
              "is-muted": A.isMute(n),
              volume: A.volumeMap && A.volumeMap[n.domId]
            }
          })];
        },
        proxy: !0
      }], null, !0)
    })], 1);
  })], 2)], 1)])], 1);
}, gI = [];
const _a = {};
var II = /* @__PURE__ */ L(
  vt,
  dI,
  gI,
  !1,
  pI,
  "4c7859ad",
  null,
  null
);
function pI(A) {
  for (let t in _a)
    this[t] = _a[t];
}
const MI = /* @__PURE__ */ function() {
  return II.exports;
}(), Ln = {};
Ln.props = {
  isHeaderShow: {
    type: Boolean,
    default: !0
  },
  isFooterShow: {
    type: Boolean,
    default: !0
  },
  background: {
    type: Boolean,
    default: !0
  },
  title: {
    type: String,
    default: ""
  },
  center: {
    type: Boolean,
    default: !1
  },
  isH5: {
    type: Boolean,
    default: !1
  }
};
Ln.setup = (A, t) => {
  const e = {
    OUTSIDE: "outside",
    INSIDE: "inside"
  }, n = A, a = I(!0), s = I(!0), i = I(!0), u = I(""), l = J();
  $A(() => {
    u.value = n.title, a.value = n.isHeaderShow, s.value = n.isFooterShow, i.value = n.background;
  });
  const c = t.emit, g = (M) => {
    M === e.OUTSIDE && N();
  }, N = () => {
    c("cancel");
  };
  return {
    clickType: e,
    props: n,
    isHeaderShowDialog: a,
    isFooterShowDialog: s,
    backgroundDialog: i,
    showTitle: u,
    t: l,
    toggleView: g,
    close: N,
    submit: () => {
      c("submit");
    }
  };
};
var CI = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "dialog",
    class: [A.props.isH5 ? "dialog-h5" : "", A.center ? "center" : ""],
    on: {
      click: function(n) {
        return n.stopPropagation(), n.preventDefault(), A.toggleView(A.clickType.OUTSIDE);
      }
    }
  }, [e("main", {
    staticClass: "dialog-main",
    class: [A.backgroundDialog ? "" : "dialog-main-back"],
    on: {
      click: function(n) {
        return n.stopPropagation(), n.preventDefault(), A.toggleView(A.clickType.INSIDE);
      }
    }
  }, [A.isHeaderShowDialog ? e("header", {
    staticClass: "dialog-main-header"
  }, [e("h1", {
    staticClass: "dialog-main-title"
  }, [A._v(A._s(A.showTitle))]), e("i", {
    staticClass: "icon icon-close",
    on: {
      click: A.close
    }
  })]) : A._e(), e("div", {
    staticClass: "dialog-main-content",
    class: [A.props.isH5 ? "dialog-main-content-uniapp" : ""]
  }, [A._t("default")], 2), A.isFooterShowDialog ? e("footer", {
    staticClass: "dialog-main-footer"
  }, [e("button", {
    staticClass: "btn btn-cancel",
    on: {
      click: A.close
    }
  }, [A._v(A._s(A.t("Cancel")))]), e("button", {
    staticClass: "btn btn-default",
    on: {
      click: A.submit
    }
  }, [A._v(A._s(A.t("Done")))])]) : A._e()])]);
}, NI = [];
const Wa = {};
var fI = /* @__PURE__ */ L(
  Ln,
  CI,
  NI,
  !1,
  vI,
  "4c125b40",
  null,
  null
);
function vI(A) {
  for (let t in Wa)
    this[t] = Wa[t];
}
const LI = /* @__PURE__ */ function() {
  return fI.exports;
}(), hn = {};
hn.props = {
  file: {
    type: String,
    default: ""
  },
  width: {
    type: String,
    default: "20px"
  },
  height: {
    type: String,
    default: "20px"
  },
  isApp: {
    type: Boolean,
    default: !1
  }
};
hn.setup = (A, t) => {
  const e = t.emit;
  return {
    props: A,
    handlerImgClick: () => {
      e("click");
    }
  };
};
var hI = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "icon",
    on: {
      click: A.handlerImgClick
    }
  }, [A.props.isApp ? e("image", {
    staticClass: "icon",
    style: {
      width: A.props.width,
      height: A.props.height
    },
    attrs: {
      src: A.props.file
    }
  }) : e("img", {
    staticClass: "icon",
    style: {
      width: A.props.width,
      height: A.props.height
    },
    attrs: {
      src: A.props.file
    }
  })]);
}, mI = [];
const qa = {};
var SI = /* @__PURE__ */ L(
  hn,
  hI,
  mI,
  !1,
  DI,
  "c4142a3c",
  null,
  null
);
function DI(A) {
  for (let t in qa)
    this[t] = qa[t];
}
const yI = /* @__PURE__ */ function() {
  return SI.exports;
}(), EI = "data:image/svg+xml;base64,", TI = "data:image/svg+xml;base64,", wI = "data:image/svg+xml;base64,", Ve = {};
Ve.props = {
  list: {
    type: Array,
    default: () => []
  },
  selectedList: {
    type: Array,
    default: () => []
  },
  isSearch: {
    type: Boolean,
    default: !0
  },
  isRadio: {
    type: Boolean,
    default: !1
  },
  isCustomItem: {
    type: Boolean,
    default: !1
  },
  title: {
    type: String,
    default: ""
  },
  type: {
    type: String,
    default: ""
  },
  resultShow: {
    type: Boolean,
    default: !0
  },
  total: {
    type: Number,
    default: 0
  },
  isH5: {
    type: Boolean,
    default: !1
  }
};
Ve.setup = (A, t) => {
  const e = A, n = t.emit, a = I(""), s = I([]), i = I(0), u = I([]), l = I(!0), c = I(!1), g = I(""), N = I(""), p = J();
  $A(() => {
    const {
      list: m,
      isCustomItem: P,
      isSearch: W,
      title: eA,
      total: b,
      selectedList: _
    } = e;
    if (P)
      for (let Q = 0; Q < m.length; Q++)
        m[Q].conversationID.indexOf("@TIM#SYSTEM") > -1 && m.splice(Q, 1), s.value = m;
    else
      s.value = m;
    i.value = b || m.length, u.value = _ && _.length > 0 ? _ : u.value, l.value = W, c.value = P, g.value = eA, a.value = e.type;
  });
  const M = C(() => s.value.filter((m) => !m.isDisabled));
  return {
    selectedIcon: EI,
    backIcon: TI,
    cancelIcon: wI,
    props: e,
    transferList: s,
    transferTotal: i,
    transferSelectedList: u,
    isTransferSearch: l,
    isTransferCustomItem: c,
    transferTitle: g,
    searchValue: N,
    t: p,
    optional: M,
    handleInput: (m) => {
      N.value = m.target.value, n("search", m.target.value);
    },
    selected: (m) => {
      if (m.isDisabled)
        return;
      let P = u.value;
      const W = P.indexOf(m);
      if (W > -1)
        return u.value.splice(W, 1);
      e.isRadio && (P = []), P.push(m), u.value = P;
    },
    selectedAll: () => {
      u.value.length === M.value.length ? u.value = [] : u.value = [...M.value];
    },
    submit: () => {
      n("submit", u.value), N.value = "", u.value = [];
    },
    cancel: () => {
      n("cancel"), N.value = "", u.value = [];
    },
    getMore: () => {
      n("getMore");
    }
  };
};
Ve.components = Object.assign({
  TIcon: yI
}, Ve.components);
var jI = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "transfer",
    class: [A.props.isH5 ? "transfer-h5" : ""]
  }, [A.props.isH5 ? e("header", {
    staticClass: "transfer-header transfer-h5-header",
    on: {
      click: A.cancel
    }
  }, [e("TIcon", {
    staticClass: "icon",
    attrs: {
      file: A.backIcon,
      width: "18px",
      height: "18px"
    }
  }), e("span", {
    staticClass: "title"
  }, [A._v(A._s(A.transferTitle))]), e("span", {
    staticClass: "space"
  })], 1) : A._e(), e("main", {
    staticClass: "main"
  }, [e("div", {
    staticClass: "left"
  }, [e("header", {
    staticClass: "transfer-header"
  }, [!A.props.isH5 && A.isTransferSearch ? e("input", {
    class: [A.props.isH5 ? "left-uniapp-input" : ""],
    attrs: {
      type: "text",
      placeholder: A.t("Please enter userID"),
      enterkeyhint: "search"
    },
    domProps: {
      value: A.searchValue
    },
    on: {
      keyup: function(n) {
        return !n.type.indexOf("key") && A._k(n.keyCode, "enter", 13, n.key, "Enter") ? null : A.handleInput.apply(null, arguments);
      }
    }
  }) : A._e(), A.props.isH5 && A.isTransferSearch ? e("input", {
    class: [A.props.isH5 ? "left-uniapp-input" : ""],
    attrs: {
      type: "text",
      placeholder: A.t("Please enter userID"),
      enterkeyhint: "search"
    },
    domProps: {
      value: A.searchValue
    },
    on: {
      blur: A.handleInput,
      confirm: A.handleInput
    }
  }) : A._e()]), e("main", {
    staticClass: "transfer-left-main"
  }, [e("ul", {
    staticClass: "transfer-list"
  }, [A.optional.length > 1 && !A.isRadio ? e("li", {
    staticClass: "transfer-list-item",
    on: {
      click: A.selectedAll
    }
  }, [A.transferSelectedList.length === A.optional.length ? e("TIcon", {
    attrs: {
      file: A.selectedIcon,
      width: "18px",
      height: "18px"
    }
  }) : e("i", {
    staticClass: "icon-unselected"
  }), e("span", {
    staticClass: "select-all"
  }, [A._v(A._s(A.t("Select all")))])], 1) : A._e(), A._l(A.transferList, function(n) {
    return e("li", {
      key: n.userID,
      staticClass: "transfer-list-item",
      on: {
        click: function(a) {
          return A.selected(n);
        }
      }
    }, [A.transferSelectedList.indexOf(n) > -1 ? e("TIcon", {
      class: [n.isDisabled && "disabled"],
      attrs: {
        file: A.selectedIcon,
        width: "18px",
        height: "18px"
      }
    }) : e("i", {
      class: [n.isDisabled && "disabled", "icon-unselected"]
    }), A.isTransferCustomItem ? [A._t("left", null, {
      data: n
    })] : [e("img", {
      staticClass: "avatar",
      attrs: {
        src: n.avatar || "https://web.sdk.qcloud.com/component/TUIKit/assets/avatar_21.png",
        onerror: "this.src='https://web.sdk.qcloud.com/component/TUIKit/assets/avatar_21.png'"
      }
    }), e("span", {
      staticClass: "name"
    }, [A._v(A._s(n.nick || n.userID))]), n.isDisabled ? e("span") : A._e()]], 2);
  }), A.transferTotal > A.transferList.length ? e("li", {
    staticClass: "transfer-list-item more",
    on: {
      click: A.getMore
    }
  }, [A._v(" " + A._s(A.t("View more")) + " ")]) : A._e()], 2)])]), e("div", {
    staticClass: "right"
  }, [A.props.isH5 ? A._e() : e("header", {
    staticClass: "transfer-header"
  }, [A._v(" " + A._s(A.transferTitle) + " ")]), A.resultShow ? e("ul", {
    staticClass: "transfer-list"
  }, [A.transferSelectedList.length > 0 && !A.props.isH5 ? e("p", {
    staticClass: "transfer-text"
  }, [A._v(" " + A._s(A.transferSelectedList.length) + " " + A._s(A.t("people selected")) + " ")]) : A._e(), A._l(A.transferSelectedList, function(n, a) {
    return e("li", {
      key: a,
      staticClass: "transfer-list-item space-between"
    }, [e("aside", {
      staticClass: "transfer-list-item-content"
    }, [A.isTransferCustomItem ? [A._t("right", null, {
      data: n
    })] : [e("img", {
      staticClass: "avatar",
      attrs: {
        src: n.avatar || "https://web.sdk.qcloud.com/component/TUIKit/assets/avatar_21.png",
        onerror: "this.src='https://web.sdk.qcloud.com/component/TUIKit/assets/avatar_21.png'"
      }
    }), A.props.isH5 ? A._e() : e("span", {
      staticClass: "name"
    }, [A._v(A._s(n.nick || n.userID))])]], 2), A.props.isH5 ? A._e() : e("span", {
      on: {
        click: function(s) {
          return A.selected(n);
        }
      }
    }, [e("TIcon", {
      attrs: {
        file: A.cancelIcon,
        width: "18px",
        height: "18px"
      }
    })], 1)]);
  })], 2) : A._e(), e("footer", {
    staticClass: "transfer-right-footer"
  }, [e("button", {
    staticClass: "btn btn-cancel",
    on: {
      click: A.cancel
    }
  }, [A._v(A._s(A.t("Cancel")))]), A.transferSelectedList.length > 0 ? e("button", {
    staticClass: "btn",
    on: {
      click: A.submit
    }
  }, [A._v(" " + A._s(A.t("Done")) + " ")]) : e("button", {
    staticClass: "btn btn-no",
    on: {
      click: A.submit
    }
  }, [A._v(A._s(A.t("Done")))])])])])]);
}, bI = [];
const Xa = {};
var zI = /* @__PURE__ */ L(
  Ve,
  jI,
  bI,
  !1,
  UI,
  "65942e88",
  null,
  null
);
function UI(A) {
  for (let t in Xa)
    this[t] = Xa[t];
}
const OI = /* @__PURE__ */ function() {
  return zI.exports;
}(), Pe = {};
Pe.props = {
  isRadio: {
    type: Boolean,
    default: !1
  },
  isNeedSearch: {
    type: Boolean,
    default: !1
  },
  title: {
    type: String,
    default: ""
  },
  userList: {
    type: Array,
    default: () => []
  },
  total: {
    type: Number,
    default: 0
  },
  isPC: {
    type: Boolean,
    default: !0
  }
};
Pe.setup = (A, t) => {
  const e = t.emit;
  return {
    props: A,
    handleSelectUserCancel: () => {
      e("cancel");
    },
    handleTransferSubmit: (l) => {
      e("confirm", l);
    },
    handleTransferSearchUser: (l) => {
      e("search", l);
    },
    handleTransferGetMore: () => {
      e("getMore");
    }
  };
};
Pe.components = Object.assign({
  TDialog: LI,
  TTransfer: OI
}, Pe.components);
var xI = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("TDialog", {
    attrs: {
      isH5: !A.isPC,
      isHeaderShow: !1,
      isFooterShow: !1,
      background: !1
    },
    on: {
      cancel: A.handleSelectUserCancel
    }
  }, [e("TTransfer", {
    attrs: {
      isSearch: A.props.isNeedSearch,
      title: A.props.title,
      list: A.props.userList,
      isH5: !A.isPC,
      isRadio: A.props.isRadio,
      total: A.props.total
    },
    on: {
      getMore: A.handleTransferGetMore,
      search: A.handleTransferSearchUser,
      submit: A.handleTransferSubmit,
      cancel: A.handleSelectUserCancel
    }
  })], 1);
}, BI = [];
const Qa = {};
var VI = /* @__PURE__ */ L(
  Pe,
  xI,
  BI,
  !1,
  PI,
  null,
  null,
  null
);
function PI(A) {
  for (let t in Qa)
    this[t] = Qa[t];
}
const FI = /* @__PURE__ */ function() {
  return VI.exports;
}(), Lt = {
  options: {
    virtualHost: !0
  }
};
Lt.setup = (A, t) => ({
  defaultAvatarSrc: XA
});
Lt.components = Object.assign({
  Overlay: Zo
}, Lt.components);
var RI = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("Overlay", {
    attrs: {
      bgColor: "#22262ed9",
      bgImage: A.defaultAvatarSrc,
      customStyle: {
        position: "absolute",
        zIndex: 0
      }
    }
  });
}, GI = [];
const Za = {};
var kI = /* @__PURE__ */ L(
  Lt,
  RI,
  GI,
  !1,
  _I,
  null,
  null,
  null
);
function _I(A) {
  for (let t in Za)
    this[t] = Za[t];
}
const WI = /* @__PURE__ */ function() {
  return kI.exports;
}(), ht = {};
ht.setup = (A, t) => {
  const e = I(null), n = I("open"), a = I(!1), s = I(0), i = I([]), u = I([]), l = I(0), c = 30, g = e, N = {
    status: n
  }, {
    callRole: p,
    callStatus: M
  } = j(Z()), {
    isFloatWindow: f
  } = j(vA()), {
    localUserInfoExcludeVolume: v,
    remoteUserListExcludeVolume: S
  } = j(pA()), h = async (b) => {
    if (a.value = b, a.value) {
      await m();
      const _ = await O.getGroupProfile();
      s.value = _.memberCount;
    }
  }, E = () => {
    a.value = !1, d.update(r.CALL, o.SHOW_SELECT_USER, !1), l.value = 0, i.value = [];
  }, x = async () => {
    l.value += c, await m();
  }, m = async () => {
    const b = await O.getGroupMemberList(c, l.value), _ = [...S.value, v.value].map((Q) => Q.userId);
    i.value.push(...b), i.value = i.value.map((Q) => (_.includes(Q.userID) && (Q = {
      ...Q,
      isDisabled: !0
    }), Q)), u.value = i.value;
  }, P = async (b) => {
    try {
      if (b.length <= 0)
        return;
      a.value = !1, d.update(r.CALL, o.SHOW_SELECT_USER, !1), l.value = 0;
      const _ = b.map((Q) => Q.userID);
      await O.inviteUser({
        userIDList: _
      }), i.value = [];
    } catch (_) {
      console.debug(_);
    }
  }, W = (b) => {
    b ? i.value = i.value.filter((_) => _.userID.includes(b) || _.nick.includes(b)) : i.value = u.value;
  }, eA = {
    [o.SHOW_SELECT_USER]: h
  };
  return sA(() => {
    d.watch(r.CALL, eA, {
      notifyRangeWhenWatch: o.MYSELF
    });
  }), tA(() => {
    d.unwatch(r.CALL, eA);
  }), N.status = I("open"), g.value = null, rA(Lo, N), rA(vo, g), {
    CallRole: H,
    CallStatus: w,
    TUIGlobal: y,
    showSelectUser: a,
    memberCount: s,
    groupMemberList: i,
    callRole: p,
    callStatus: M,
    isFloatWindow: f,
    handleCancel: E,
    getMoreMemberList: x,
    handleSelectedResult: P,
    handleSearch: W
  };
};
ht.components = Object.assign({
  TopBar: Qo,
  Waiting: Jo,
  MediaContainer: MI,
  Tip: cs,
  ButtonPanel: Ms,
  BackGround: WI,
  SelectUser: FI
}, ht.components);
var qI = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "groupcall-container"
  }, [e("TopBar"), A.callStatus === A.CallStatus.CALLING && A.callRole === A.CallRole.CALLEE && !A.isFloatWindow ? e("Waiting") : A._e(), e("MediaContainer"), e("Tip"), e("ButtonPanel"), e("BackGround"), A.showSelectUser ? e("SelectUser", {
    attrs: {
      isNeedSearch: !0,
      userList: A.groupMemberList,
      isPC: A.TUIGlobal.isPC,
      total: A.memberCount
    },
    on: {
      confirm: A.handleSelectedResult,
      cancel: A.handleCancel,
      getMore: A.getMoreMemberList,
      search: A.handleSearch
    }
  }) : A._e()], 1);
}, XI = [];
const Ya = {};
var QI = /* @__PURE__ */ L(
  ht,
  qI,
  XI,
  !1,
  ZI,
  "58680342",
  null,
  null
);
function ZI(A) {
  for (let t in Ya)
    this[t] = Ya[t];
}
const YI = /* @__PURE__ */ function() {
  return QI.exports;
}();
var KI = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("svg", {
    attrs: {
      viewBox: "0 0 1024 1024",
      xmlns: "http://www.w3.org/2000/svg",
      "data-v-ea893728": ""
    }
  }, [e("path", {
    attrs: {
      fill: "currentColor",
      d: "M764.288 214.592 512 466.88 259.712 214.592a31.936 31.936 0 0 0-45.12 45.12L466.752 512 214.528 764.224a31.936 31.936 0 1 0 45.12 45.184L512 557.184l252.288 252.288a31.936 31.936 0 0 0 45.12-45.12L557.12 512.064l252.288-252.352a31.936 31.936 0 1 0-45.12-45.184z"
    }
  })]);
}, HI = [];
const JI = {}, Ka = {};
var $I = /* @__PURE__ */ L(
  JI,
  KI,
  HI,
  !1,
  Ap,
  null,
  null,
  null
);
function Ap(A) {
  for (let t in Ka)
    this[t] = Ka[t];
}
const ep = /* @__PURE__ */ function() {
  return $I.exports;
}(), Ha = "data:image/jpeg;base64,", tp = "data:image/jpeg;base64,", np = "data:image/png;base64,", ap = "data:image/png;base64,", Fe = {};
Fe.props = {
  platform: {
    key: "platform",
    required: !0,
    type: null
  },
  language: {
    key: "language",
    required: !0,
    type: String
  }
};
Fe.setup = (A, t) => {
  const e = {
    mac: {
      "zh-cn": Ha,
      en: tp
    },
    win: {
      "zh-cn": np,
      en: ap
    }
  }, n = A, a = C(() => n.platform === "mac" ? "mac-privacy" : "win-privacy"), s = C(() => n.platform === "mac" ? "mac-preferences" : "win-preferences"), i = I(Ha), u = I(!0), l = J(), c = (N) => {
    N && y.isPC && (u.value = !0);
  }, g = () => {
    u.value = !1;
  };
  return Y(() => n.language, () => {
    i.value = e[n.platform][n.language];
  }, {
    immediate: !0
  }), sA(() => {
    d.watch(r.CALL, {
      [o.SHOW_PERMISSION_TIP]: c
    }, {
      notifyRangeWhenWatch: o.MYSELF
    });
  }), tA(() => {
    d.unwatch(r.CALL, {
      [o.SHOW_PERMISSION_TIP]: c
    });
  }), {
    privacy: a,
    preferences: s,
    permitPng: i,
    showModal: u,
    t: l,
    handleClose: g
  };
};
Fe.components = Object.assign({
  CloseIcon: ep
}, Fe.components);
var op = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return A.showModal ? e("div", {
    staticClass: "card"
  }, [e("div", {
    staticClass: "card-body"
  }, [e("div", {
    staticClass: "close",
    on: {
      click: A.handleClose
    }
  }, [e("CloseIcon")], 1), e("div", {
    staticClass: "card-wrapper"
  }, [e("div", {
    staticClass: "title"
  }, [A._v(A._s(A.t("browser-authorization")))]), e("div", {
    staticClass: "button"
  }, [A._v(A._s(A.t(A.preferences)))]), e("div", {
    staticClass: "allow"
  }, [A._v(A._s(A.t(A.privacy)))]), e("div", {
    staticClass: "image-wrapper"
  }, [e("img", {
    staticClass: "image",
    attrs: {
      src: A.permitPng,
      alt: "system permit"
    }
  })])])])]) : A._e();
}, sp = [];
const Ja = {};
var ip = /* @__PURE__ */ L(
  Fe,
  op,
  sp,
  !1,
  lp,
  "631fa89e",
  null,
  null
);
function lp(A) {
  for (let t in Ja)
    this[t] = Ja[t];
}
const up = /* @__PURE__ */ function() {
  return ip.exports;
}(), mt = {};
mt.setup = (A, t) => {
  const e = C(() => y.isMAC ? Ft.MAC : Ft.WIN), n = I(d.getData(r.CALL, o.LANGUAGE)), a = (s) => n.value = s;
  return sA(() => {
    d.watch(r.CALL, {
      [o.LANGUAGE]: a
    }, {
      notifyRangeWhenWatch: o.MYSELF
    });
  }), tA(() => {
    d.unwatch(r.CALL, {
      [o.LANGUAGE]: a
    });
  }), {
    platform: e,
    language: n
  };
};
mt.components = Object.assign({
  SystemPermitTip: up
}, mt.components);
var rp = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "tip"
  }, [e("div", {
    staticClass: "card"
  }, [e("SystemPermitTip", {
    attrs: {
      platform: A.platform,
      language: A.language
    }
  })], 1)]);
}, cp = [];
const $a = {};
var dp = /* @__PURE__ */ L(
  mt,
  rp,
  cp,
  !1,
  gp,
  "ff1df20c",
  null,
  null
);
function gp(A) {
  for (let t in $a)
    this[t] = $a[t];
}
const Ip = /* @__PURE__ */ function() {
  return dp.exports;
}(), Cs = {};
let Je = null, pp = 1;
const Mp = () => {
  const A = document.body;
  Je && (A.removeChild(Je.$el), Je.$destroy());
  const t = Xt.extend(Ip), e = new t({
    propsData: {
      index: `toast-item-${pp++}`,
      destroy() {
        A.removeChild(e.$el), e.$destroy();
      }
    }
  });
  e.$mount(), A.appendChild(e.$el), Je = e;
};
Cs.show = () => Mp();
const mn = {};
mn.props = {
  type: {
    key: "type",
    required: !0,
    type: null
  },
  duration: {
    key: "duration",
    required: !1,
    type: Number
  },
  message: {
    key: "message",
    required: !0,
    type: String
  },
  index: {
    key: "index",
    required: !0,
    type: String
  },
  showClose: {
    key: "showClose",
    required: !1,
    type: Boolean
  },
  destroy: {
    key: "destroy",
    required: !0,
    type: Function
  },
  close: {
    key: "close",
    required: !0,
    type: Function
  }
};
mn.setup = (A, t) => {
  const e = A, n = I(!1), a = I(null), s = C(() => `toast-container ${e.type}`), i = () => {
    e.destroy();
  }, u = () => {
    e.close(), n.value = !1;
  }, l = () => {
    a.value && clearTimeout(a.value), e.duration && (a.value = setTimeout(() => {
      u();
    }, e.duration));
  };
  return sA(() => {
    n.value = !0, l();
  }), tA(() => {
    a.value && clearTimeout(a.value);
  }), {
    isShow: n,
    toastClass: s,
    handleAfterLeave: i,
    handleClose: u
  };
};
var Cp = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("Transition", {
    attrs: {
      name: "slide-up"
    },
    on: {
      "after-leave": A.handleAfterLeave
    }
  }, [e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isShow,
      expression: "isShow"
    }],
    key: A.index,
    class: A.toastClass
  }, [e("div", {
    staticClass: "toast-content"
  }, [A._v(A._s(A.message))]), e("i", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.showClose,
      expression: "showClose"
    }],
    staticClass: "toast-close-icon",
    on: {
      click: A.handleClose
    }
  })])]);
}, Np = [];
const Ao = {};
var fp = /* @__PURE__ */ L(
  mn,
  Cp,
  Np,
  !1,
  vp,
  "1a36ce82",
  null,
  null
);
function vp(A) {
  for (let t in Ao)
    this[t] = Ao[t];
}
const Lp = /* @__PURE__ */ function() {
  return fp.exports;
}(), jA = {}, MA = [];
let hp = 1;
function mp() {
  var t, e, n;
  const A = MA.length;
  return A > 0 ? (n = (e = (t = MA[A - 1]) == null ? void 0 : t.$el) == null ? void 0 : e.style) == null ? void 0 : n.top : "30px";
}
function Sp() {
  var t, e;
  const A = MA.length;
  return A > 0 ? (e = (t = MA[A - 1]) == null ? void 0 : t.$el) == null ? void 0 : e.offsetHeight : 0;
}
const Ns = (A) => {
  const t = document.body, { type: e, duration: n = 3e3, message: a, showClose: s = !1, onClose: i } = A, u = Xt.extend(Lp), l = new u({
    propsData: {
      type: e,
      duration: n,
      message: a,
      showClose: s,
      index: `toast-item-${hp++}`,
      close() {
        var M, f, v, S;
        const N = MA.findIndex((h) => h === l);
        console.log((f = (M = MA[N]) == null ? void 0 : M.$el) == null ? void 0 : f.offsetHeight);
        const p = parseInt((S = (v = MA[N]) == null ? void 0 : v.$el) == null ? void 0 : S.offsetHeight) + 20;
        for (let h = 0; h < MA.length; h++)
          h < N || (MA[h].$el.style.top = parseInt(MA[h].$el.style.top) - p + "px");
        MA.splice(N, 1);
      },
      destroy() {
        t.removeChild(l.$el), l.$destroy();
      }
    }
  });
  l.$mount(), t.appendChild(l.$el);
  const c = mp(), g = Sp();
  l.$el.style.top = parseInt(c) + parseInt(g) + 20 + "px", MA.push(l);
};
["info", "waring", "error", "success"].forEach((A) => {
  let t = {};
  jA[A] = (e) => {
    typeof e == "string" ? t.message = e : t = e, Ns({ type: A, ...t });
  };
});
jA.show = (A) => {
  let t = {};
  return typeof A == "string" ? (t.message = A, t.type = "info") : t = A, Ns({ type: "info", ...t });
};
const Re = {};
Re.props = {
  beforeCalling: {
    key: "beforeCalling",
    required: !1,
    type: Function
  },
  afterCalling: {
    key: "afterCalling",
    required: !1,
    type: Function
  },
  onMinimized: {
    key: "onMinimized",
    required: !1,
    type: Function
  },
  onMessageSentByMe: {
    key: "onMessageSentByMe",
    required: !1,
    type: Function
  },
  kickedOut: {
    key: "kickedOut",
    required: !1,
    type: Function
  },
  statusChanged: {
    key: "statusChanged",
    required: !1,
    type: Function
  },
  allowedMinimized: {
    key: "allowedMinimized",
    required: !1,
    type: Boolean,
    default: !1
  },
  allowedFullScreen: {
    key: "allowedFullScreen",
    required: !1,
    type: Boolean,
    default: !0
  },
  videoDisplayMode: {
    key: "videoDisplayMode",
    required: !1,
    type: String,
    default: "cover"
  },
  videoResolution: {
    key: "videoResolution",
    required: !1,
    type: String,
    default: "480p"
  }
};
Re.setup = (A, t) => {
  const e = !y.isPC, n = e ? "TUICallKit-mobile transition-animation" : "TUICallKit-desktop", a = I(""), s = I(""), i = I(""), u = I(!1), l = A, c = I(d.getData(r.CALL, o.CALL_STATUS)), g = I(d.getData(r.CALL, o.CALL_ROLE)), N = I(d.getData(r.CALL, o.CALL_MEDIA_TYPE)), p = I(d.getData(r.CALL, o.IS_GROUP)), M = I(d.getData(r.CALL, o.IS_EAR_PHONE)), f = I(null), v = I(d.getData(r.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN)), S = I(d.getData(r.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST)), h = I(d.getData(r.CALL, o.CALLER_USER_INFO)), E = I(d.getData(r.CALL, o.IS_MINIMIZED)), x = I(d.getData(r.CALL, o.ENABLE_VIRTUAL_BACKGROUND)), m = I(d.getData(r.CALL, o.IS_SHOW_ENABLE_VIRTUAL_BACKGROUND)), P = I(d.getData(r.CALL, o.CUSTOM_UI_CONFIG)), W = I(d.getData(r.CALL, o.IS_MUTE_SPEAKER)), eA = I(d.getData(r.CALL, o.TRANSLATE)), b = Xe({
    callStatus: c,
    callRole: g,
    callType: N,
    isGroupCall: p,
    isEarPhone: M,
    focusElement: f,
    allowedFullScreen: l.allowedFullScreen,
    enableVirtualBackground: x,
    isShowEnableVirtualBackground: m,
    isMuteSpeaker: W
  }), _ = Xe({
    callerUserInfo: h
  }), Q = Xe({
    localUserInfoExcludeVolume: v,
    remoteUserListExcludeVolume: S
  }), OA = Xe({
    isFloatWindow: E
  }), QA = I(eA), {
    beforeCalling: xA,
    afterCalling: BA,
    onMinimized: VA,
    onMessageSentByMe: qe,
    videoDisplayMode: bt,
    videoResolution: zt,
    kickedOut: q,
    statusChanged: nA,
    allowedMinimized: uA
  } = j(l), PA = (B) => {
    b.callStatus = B;
  }, vs = (B) => {
    b.isGroupCall = B;
  }, Ls = (B) => {
    if (typeof B == "object") {
      const {
        content: ZA,
        type: xs = "info"
      } = B;
      !wo(ZA) && Os(eA.value(ZA), xs);
    }
  }, hs = (B) => {
    b.callType = B, e && u.value && (i.value = "miniMized-mobile-audio", s.value = ""), e && !u.value && (s.value = "mobile-audio");
  }, Sn = (B) => {
    OA.isFloatWindow = B, B ? e ? b.callType === V.AUDIO ? s.value = "miniMized-mobile-audio" : i.value = "miniMized-mobile-video" : a.value = "miniMized" : (s.value = "mobile-audio", a.value = "", i.value = "");
  }, ms = (B) => {
    if (B && y.isPC)
      try {
        Cs.show();
      } catch (ZA) {
        console.debug(ZA);
      }
  }, Ss = (B) => {
    b.callRole = B;
  }, Ds = (B) => {
    Q.localUserInfoExcludeVolume = B;
  }, ys = (B) => {
    Q.remoteUserListExcludeVolume = B;
  }, Es = (B) => {
    _.callerUserInfo = B;
  }, Ts = (B) => {
    b.isEarPhone = B;
  }, ws = (B) => {
    b.enableVirtualBackground = B;
  }, js = (B) => {
    b.isShowEnableVirtualBackground = B;
  }, bs = (B) => {
    P.value = B;
  }, zs = (B) => {
    b.isMuteSpeaker = B;
  }, Us = (B) => {
    eA.value = B;
  };
  $A(() => {
    O.setCallback({
      beforeCalling: xA && xA.value,
      afterCalling: BA && BA.value,
      onMinimized: VA && VA.value,
      onMessageSentByMe: qe && qe.value,
      kickedOut: q && q.value,
      statusChanged: nA && nA.value
    });
  });
  const Dn = {
    [o.CALL_STATUS]: PA,
    [o.IS_GROUP]: vs,
    [o.TOAST_INFO]: Ls,
    [o.CALL_MEDIA_TYPE]: hs,
    [o.SHOW_PERMISSION_TIP]: ms,
    [o.CALL_ROLE]: Ss,
    [o.LOCAL_USER_INFO_EXCLUDE_VOLUMN]: Ds,
    [o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST]: ys,
    [o.CALLER_USER_INFO]: Es,
    [o.IS_EAR_PHONE]: Ts,
    [o.ENABLE_VIRTUAL_BACKGROUND]: ws,
    [o.IS_SHOW_ENABLE_VIRTUAL_BACKGROUND]: js,
    [o.CUSTOM_UI_CONFIG]: bs,
    [o.IS_MUTE_SPEAKER]: zs,
    [o.TRANSLATE]: Us
  };
  sA(() => {
    uA.value && O.enableFloatWindow(uA.value), O.setVideoDisplayMode(bt.value), O.setVideoResolution(zt.value), d.watch(r.CALL, Dn, {
      notifyRangeWhenWatch: o.MYSELF
    }), d.watch(r.CALL, {
      [o.IS_MINIMIZED]: Sn
    });
  });
  function Os(B, ZA) {
    switch (ZA) {
      case "info":
        jA == null || jA.info(B);
        break;
      case "error":
        jA == null || jA.error(B);
        break;
    }
  }
  return tA(async () => {
    d.unwatch(r.CALL, {
      ...Dn,
      [o.IS_MINIMIZED]: Sn
    });
  }), rA(Co, b), rA(No, _), rA(fo, Q), rA(ho, OA), rA(Do, P), rA(yo, QA), {
    CallStatus: w,
    bodyStyle: n,
    miniMizedDeskStyle: a,
    mobileAudioStyle: s,
    mobileVideoStyle: i,
    callInfoContextValue: b,
    floatWindowContextValue: OA
  };
};
Re.components = Object.assign({
  SingleCall: sI,
  GroupCall: YI
}, Re.components);
var Dp = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return A.callInfoContextValue.callStatus !== A.CallStatus.IDLE ? e("div", {
    class: [A.bodyStyle, A.miniMizedDeskStyle, A.mobileVideoStyle, A.mobileAudioStyle],
    style: [{
      visibility: A.floatWindowContextValue.isFloatWindow ? "hidden" : ""
    }],
    attrs: {
      id: "tuicallkit-id"
    }
  }, [A.callInfoContextValue.isGroupCall ? e("GroupCall", {
    staticClass: "singCall"
  }) : e("SingleCall", {
    staticClass: "singCall"
  })], 1) : A._e();
}, yp = [];
const eo = {};
var Ep = /* @__PURE__ */ L(
  Re,
  Dp,
  yp,
  !1,
  Tp,
  null,
  null,
  null
);
function Tp(A) {
  for (let t in eo)
    this[t] = eo[t];
}
const jt = /* @__PURE__ */ function() {
  return Ep.exports;
}(), fs = "data:image/svg+xml;base64,", Ge = {};
Ge.props = {
  userListInfo: {
    type: Array,
    default: []
  }
};
Ge.setup = (A, t) => {
  const e = t.emit, n = I(!1), a = J();
  return {
    CallIcon: fs,
    defaultAvatarSrc: XA,
    isShowContent: n,
    t: a,
    handleJoinGroup: () => {
      e("joinGroupCall"), n.value = !1;
    },
    handleClick: () => {
      n.value = !n.value;
    }
  };
};
Ge.components = Object.assign({
  TKImage: mA
}, Ge.components);
var wp = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "banner-pc"
  }, [e("div", {
    staticClass: "top",
    on: {
      click: A.handleClick
    }
  }, [e("TKImage", {
    staticClass: "call-icon",
    attrs: {
      src: A.CallIcon,
      width: "16px",
      height: "16px"
    }
  }), e("span", {
    staticClass: "text"
  }, [A._v(" " + A._s(A.userListInfo.length) + " " + A._s(A.t("people on the call")) + " ")])], 1), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isShowContent,
      expression: "isShowContent"
    }],
    staticClass: "content"
  }, [e("div", {
    staticClass: "content-arrows"
  }), e("div", {
    staticClass: "avatar-box"
  }, A._l(A.userListInfo, function(n) {
    return e("TKImage", {
      staticClass: "avatar",
      attrs: {
        src: n == null ? void 0 : n.avatar,
        defaultSrc: A.defaultAvatarSrc,
        fit: "cover",
        width: "32px",
        height: "32px"
      }
    });
  }), 1), e("div", {
    staticClass: "btn",
    on: {
      click: A.handleJoinGroup
    }
  }, [A._v(A._s(A.t("join")))])])]);
}, jp = [];
const to = {};
var bp = /* @__PURE__ */ L(
  Ge,
  wp,
  jp,
  !1,
  zp,
  null,
  null,
  null
);
function zp(A) {
  for (let t in to)
    this[t] = to[t];
}
const Up = /* @__PURE__ */ function() {
  return bp.exports;
}(), Op = "data:image/svg+xml;base64,", ke = {};
ke.props = {
  userListInfo: {
    type: Array,
    default: []
  }
};
ke.setup = (A, t) => {
  const e = J(), n = I({
    margin: "8px 10px 8px 0px"
  }), a = I({
    margin: "6px",
    borderRadius: "4px"
  }), s = I(!1), i = t.emit;
  return {
    CallIcon: fs,
    ArrowsIcon: Op,
    defaultAvatarSrc: XA,
    t: e,
    CallIconStyle: n,
    avatarImgStyle: a,
    isShowContent: s,
    handleJoinGroup: () => {
      i("joinGroupCall"), s.value = !1;
    },
    handleClick: () => {
      s.value = !s.value;
    }
  };
};
ke.components = Object.assign({
  TKImage: mA
}, ke.components);
var xp = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    staticClass: "banner-h5"
  }, [e("div", {
    staticClass: "top",
    on: {
      click: A.handleClick
    }
  }, [e("div", {
    staticClass: "left"
  }, [e("TKImage", {
    staticClass: "call-icon",
    attrs: {
      src: A.CallIcon,
      width: "16px",
      height: "16px",
      customStyle: A.CallIconStyle
    }
  }), e("span", {
    staticClass: "text"
  }, [A._v(" " + A._s(A.userListInfo.length) + " " + A._s(A.t("people on the call")) + " ")])], 1), e("TKImage", {
    attrs: {
      src: A.ArrowsIcon,
      width: "9px",
      height: "9px"
    }
  })], 1), e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isShowContent,
      expression: "isShowContent"
    }],
    staticClass: "content-box"
  }, [e("div", {
    staticClass: "content"
  }, [e("div", {
    staticClass: "avatar-box"
  }, A._l(A.userListInfo, function(n) {
    return e("TKImage", {
      attrs: {
        src: n == null ? void 0 : n.avatar,
        customStyle: A.avatarImgStyle,
        defaultSrc: A.defaultAvatarSrc,
        fit: "cover",
        width: "50px",
        height: "50px"
      }
    });
  }), 1), e("div", {
    staticClass: "btn",
    on: {
      click: function(n) {
        return n.stopPropagation(), A.handleJoinGroup.apply(null, arguments);
      }
    }
  }, [A._v(A._s(A.t("join")))])])])]);
}, Bp = [];
const no = {};
var Vp = /* @__PURE__ */ L(
  ke,
  xp,
  Bp,
  !1,
  Pp,
  null,
  null,
  null
);
function Pp(A) {
  for (let t in no)
    this[t] = no[t];
}
const Fp = /* @__PURE__ */ function() {
  return Vp.exports;
}(), St = {};
St.setup = (A, t) => {
  const e = I(y.isPC), n = I(null), {
    groupId: a,
    roomId: s,
    roomIdType: i,
    callMediaType: u,
    groupCallMembers: l,
    callStatus: c
  } = j(Yi()), g = C(() => a && (c == null ? void 0 : c.value) === w.IDLE && l.value.length);
  return {
    isPC: e,
    message: n,
    groupCallMembers: l,
    isShow: g,
    handleClick: async () => {
      if (l.value.length > 9) {
        n.value.show({
          message: "Supports a maximum of 9 people for simultaneous calls",
          type: "success"
        });
        return;
      }
      const p = {
        type: u.value,
        groupID: a.value,
        roomID: i.value === 1 ? Number(s.value) : 0,
        strRoomID: String(s.value)
      };
      try {
        await O.joinInGroupCall(p);
      } catch (M) {
        console.error(`${o.PREFIX}joinInGroupCall fail: ${M}`);
      }
    }
  };
};
St.components = Object.assign({
  JoinGroupCardPC: Up,
  JoinGroupCardMobile: Fp,
  Message: rs
}, St.components);
var Rp = function() {
  var A = this, t = A.$createElement, e = A._self._c || t;
  return e("div", {
    directives: [{
      name: "show",
      rawName: "v-show",
      value: A.isShow,
      expression: "isShow"
    }]
  }, [A.isPC ? e("JoinGroupCardPC", {
    attrs: {
      userListInfo: A.groupCallMembers
    },
    on: {
      joinGroupCall: A.handleClick
    }
  }) : e("JoinGroupCardMobile", {
    attrs: {
      userListInfo: A.groupCallMembers
    },
    on: {
      joinGroupCall: A.handleClick
    }
  }), e("Message", {
    ref: "message"
  })], 1);
}, Gp = [];
const ao = {};
var kp = /* @__PURE__ */ L(
  St,
  Rp,
  Gp,
  !1,
  _p,
  null,
  null,
  null
);
function _p(A) {
  for (let t in ao)
    this[t] = ao[t];
}
const Wp = /* @__PURE__ */ function() {
  return kp.exports;
}(), qp = (A) => (A.component("TUICallKit", { server: O }), O.bindTUICore(A), jt), Xp = (A) => {
  A.component("TUICallKit", jt).component("JoinGroupCard", Wp), console.log("TUICallKit&mini installed", A);
};
jt.plugin = qp;
jt.install = Xp;
const Jp = {
  AUDIO_CALL: 1,
  VIDEO_CALL: 2
}, $p = "4.0.2";
export {
  V as CallMediaType,
  H as CallRole,
  LA as FeatureButton,
  Wp as JoinGroupCard,
  tt as LayoutMode,
  o as NAME,
  aA as STATUS,
  r as StoreName,
  jt as TUICallKit,
  O as TUICallKitServer,
  Jp as TUICallType,
  y as TUIGlobal,
  d as TUIStore,
  $p as Version,
  ie as VideoDisplayMode,
  le as VideoResolution,
  Xp as install,
  qp as plugin,
  WA as t,
  EA as uiDesign
};
