(function(){"use strict";try{if(typeof document<"u"){var t=document.createElement("style");t.appendChild(document.createTextNode(".tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-row{display:flex;flex-wrap:wrap;position:relative;box-sizing:border-box;width:100%}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-col{display:flex;flex-wrap:wrap;position:relative;box-sizing:border-box}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-loading_dot-container{display:flex;justify-content:space-between;align-items:center;height:100%}.tk-loading_dot-container .tk-loading_dot:nth-child(1){opacity:0;animation-duration:.8s;animation-delay:0s;animation-play-state:running}.tk-loading_dot-container .tk-loading_dot:nth-child(2){opacity:.083;animation-duration:.8s;animation-delay:.2666666667s;animation-play-state:running}.tk-loading_dot-container .tk-loading_dot:nth-child(3){opacity:.1667;animation-duration:.8s;animation-delay:.5333333333s;animation-play-state:running}.tk-loading_dot-container .tk-loading_dot{width:20%;height:20%;border-radius:50%;background-color:#fff;animation-duration:1.8s;animation-name:dotting;animation-timing-function:linear;animation-iteration-count:infinite;animation-fill-mode:both}@keyframes dotting{0%{opacity:.15}1%{opacity:.8}33%{opacity:.8}34%{opacity:.15}to{opacity:.15}}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-loading_circle-container{height:100%;border:2px solid;border-radius:50%;border-top-color:transparent;border-right-color:transparent;border-bottom-color:#fff;border-left-color:#fff;background:0 0;vertical-align:middle;box-sizing:border-box;animation:spin 1s linear infinite}@keyframes spin{0%{transform:rotate(0)}to{transform:rotate(360deg)}}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-loading{display:flex;align-items:center;flex-direction:column}.tk-loading .tk-loading_text{margin:10px}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-image{position:relative;display:flex;align-items:center;justify-content:center;overflow:hidden;vertical-align:middle;width:300px;height:225px}.tk-image .tk-image_inner{width:100%;height:100%}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-button{display:inline-flex;align-items:center;justify-content:center}.tk-button.tk-button--small{height:24px}.tk-button.tk-button--middle{height:32px}.tk-button.tk-button--large{height:40px}.tk-button .tk-button--content{color:#606266;font-size:14px;font-weight:500}.tk-button.tk-circle.tk-button--small{width:24px}.tk-button.tk-circle.tk-button--default{width:32px}.tk-button.tk-circle.tk-button--large{width:40px}.btn-content[data-v-135d060f],.btn-content[data-v-f92abf93]{display:flex;flex-direction:column;align-items:center}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-text{display:inline-block;overflow:hidden;text-align:center;line-height:normal;white-space:nowrap;font-weight:400;font-size:14px;color:#303133}.tk-text.tk-text--line-clamp{display:-webkit-inline-box;-webkit-box-orient:vertical;white-space:normal}.btn-content[data-v-7c062dcf]{display:flex;flex-direction:column;align-items:center}.top-bar-container[data-v-977481d5]{position:absolute;z-index:2;width:100%;height:5.8%;display:flex;align-items:center}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-overlay{position:fixed;left:0;top:0;bottom:0;width:100%}.tk-overlay .tk-overlay_mask-container{width:100%;height:100%;z-index:0;position:absolute}.tk-overlay .tk-overlay_mask{position:absolute;left:0;top:0;right:0;bottom:0;background-color:#00000080;z-index:1}.tk-overlay .tk-overlay_slot{position:absolute;left:0;right:0;top:0;bottom:0;z-index:1;display:flex;justify-content:center}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-avatar{display:inline-flex;justify-content:center;align-items:center;box-sizing:border-box;text-align:center;overflow:hidden;color:#fff;background:#c0c4cc;width:40px;height:40px;font-size:14px}.tk-avatar.tk-avatar--square{border-radius:4px}.tk-avatar.tk-avatar--circle{border-radius:100%}.tk-avatar .tk-avatar_img,.tk-avatar .tk-avatar_image{width:100%;height:100%}.mic-container[data-v-ea062939]{position:relative;width:24px;height:24px}.mic-container .mic-level-container[data-v-ea062939]{position:absolute;left:6px;width:8px;height:14px;display:flex;flex-wrap:wrap;border-radius:4px;overflow:hidden;flex-direction:column-reverse;justify-content:space-between}.mic-container .mic-level-container .mic-level[data-v-ea062939]{width:100%;background-color:#27c39f;transition:height .2s}.overlay-stream-container.mobile .overlay-stream-content-container[data-v-98c0998b]{width:100%;top:26%;position:absolute;display:flex;justify-content:center;flex-direction:column}.overlay-stream-container.mobile .overlay-stream-content-container .overlay-stream-content[data-v-98c0998b]{display:flex;flex-direction:column;align-items:center}.overlay-stream-container.mobile .overlay-stream-content-container .overlay-stream-content .overlay-stream-avatar[data-v-98c0998b]{margin-bottom:12px}.overlay-stream-container.mobile .overlay-stream-content-container .overlay-stream-content .overlay-stream-info[data-v-98c0998b]{display:flex;align-items:center}.overlay-stream-container.mobile .overlay-stream-content-container .overlay-stream-content .overlay-stream-tip[data-v-98c0998b]{margin-top:12px;color:#fff}.overlay-stream-container.pc .overlay-stream-content-container[data-v-98c0998b]{display:flex;height:100%;align-items:center;justify-content:center}.overlay-stream-container.pc .overlay-stream-content-container .overlay-stream-content[data-v-98c0998b]{display:flex;flex-direction:column;align-items:center}.overlay-stream-container.pc .overlay-stream-content-container .overlay-stream-content .overlay-stream-info[data-v-98c0998b]{display:flex;align-items:center}.overlay-stream-container[data-v-98c0998b]{position:absolute;display:flex;height:100%;width:100%;flex-direction:column;justify-content:center;align-items:center}.waiting-container[data-v-8dc3669f]{position:absolute;top:0;left:0;bottom:0;right:0;z-index:1}.groupcall-info[data-v-8dc3669f]{display:flex;flex-direction:column;align-items:center;margin-top:24px;z-index:1;color:#fff;width:100%}.groupcall-info .avatar-group[data-v-8dc3669f]{display:flex;flex-wrap:wrap;margin-top:12px;align-items:center;justify-content:center;max-width:70%}.groupcall-info .avatar-group .avatar-item[data-v-8dc3669f]{width:10vw;height:10vw;margin-left:10px;margin-top:10px}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-toggle-window{height:100%;position:relative}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-toggle-window-item{height:100%;position:absolute}.tk-toggle-window-item.tk-toggle-window-item--big{width:100%;height:100%;z-index:0}.tk-toggle-window-item.mobile.tk-toggle-window-item--small{z-index:1;width:24.3%!important;height:19.7%!important;border-radius:5px;left:98%;top:8%;transform:translate(-100%);overflow:hidden}.tk-toggle-window-item.pc.tk-toggle-window-item--small{top:2%;left:2%;width:22%;height:21%;border-radius:12px;overflow:hidden;z-index:1}.stream-userInfo[data-v-73beaeef]{padding:2px 5px;display:flex;align-items:center;background-color:#000000a6;color:#fff}.stream-userInfo .nickname[data-v-73beaeef]{display:flex}.stream-icon[data-v-b2b7ff75]{width:30px;height:30px;background:rgba(34,38,46,.5019607843);border-radius:50%;display:flex;align-items:center;justify-content:center}.switch-camera[data-v-b2b7ff75]{margin:0 12px}.pusher-container[data-v-8cf5d7b3]{width:100%;height:100%;position:relative;background-color:#4c515a}.pusher-container .audio-stream-container[data-v-8cf5d7b3]{position:absolute;z-index:3;width:100%;height:100%}.pusher-container .stream-info-container[data-v-8cf5d7b3]{position:absolute;bottom:0;z-index:3;width:100%}.pusher-container .stream-info-container.mobile[data-v-8cf5d7b3]{margin-bottom:8px}.player-container[data-v-351781b9]{width:100%;height:100%;position:relative;background-color:#4c515a}.player-container .audio-stream-container[data-v-351781b9]{position:absolute;z-index:1;width:100%;height:100%}.player-container .stream-info-container[data-v-351781b9]{position:absolute;bottom:0;z-index:1;width:100%}.player-container .stream-info-container.mobile[data-v-351781b9]{margin-bottom:8px}.float-control-panel[data-v-63373c5d]{width:168px;height:56px;background:white;z-index:13;display:flex;flex-wrap:nowrap;justify-content:center;border-radius:40px;box-shadow:#00000029 0 3px 6px,#0000003b 0 3px 6px}.float-control-item-icon[data-v-63373c5d]{display:flex;flex-direction:row;justify-content:center;align-items:center;width:56px;height:56px;position:relative}.float-control-item-icon-container[data-v-63373c5d]{border-radius:40px;width:40px;height:40px;margin:.5rem;cursor:pointer;display:flex;flex-direction:row;justify-content:center;align-items:center}.float-control-item-icon-container[data-v-63373c5d]:hover{background:rgba(218,218,218,.3)}.singlecall-video-float[data-v-ea53b7e0]{width:110px;height:196px;display:flex;z-index:99;flex-direction:column;align-items:center;background:#000;border-radius:12px;box-shadow:0 0 10px #35394166;overflow:hidden}.singlecall-video-float .singlecall-video-float-content[data-v-ea53b7e0]{width:100%;height:100%;display:flex;justify-content:center;flex-direction:column;align-items:center}.singlecall-video-float .float-window-tip-container[data-v-ea53b7e0]{position:absolute;bottom:8px}.singlecall-audio-float[data-v-ea53b7e0]{width:72px;height:72px;display:flex;z-index:99;flex-direction:column;align-items:center;background:#FFF;border-radius:12px;box-shadow:0 0 10px #35394166}.singlecall-audio-float .singlecall-audio-float-content[data-v-ea53b7e0]{width:100%;height:100%;display:flex;justify-content:center;flex-direction:column;align-items:center}.click-container[data-v-0dfe3ae0]{position:absolute;width:100%;height:100%;z-index:2}.groupcall-video-float.float[data-v-0dfe3ae0]{width:72px;height:90px;display:flex;z-index:99;flex-direction:column;align-items:center;border-radius:12px;background:#FFF;box-shadow:0 0 10px #35394166;box-sizing:border-box;overflow:hidden}.groupcall-video-float.float .stream-container[data-v-0dfe3ae0]{position:relative;width:72px;height:70px}.groupcall-video-float.float .video[data-v-0dfe3ae0]{width:72px;height:70px;position:absolute}.groupcall-video-float.float .audio[data-v-0dfe3ae0]{position:absolute;width:100%;height:70px;padding-top:10px;background-color:#fff;display:flex;flex-direction:column;align-items:center}.groupcall-video-float.float .device-status[data-v-0dfe3ae0]{width:100%;height:20px;background-color:#f9f6f4;display:flex;position:relative;justify-content:space-around}.groupcall-video-float.not-float[data-v-0dfe3ae0]{width:100%;height:100%}.groupcall-video-float.not-float .stream-container[data-v-0dfe3ae0],.groupcall-video-float.not-float .stream-container .video[data-v-0dfe3ae0]{height:100%}.float-window-container.not-float[data-v-0d17419d]{width:100%;height:100%}.float-window-container.float[data-v-0d17419d]{position:absolute}.float-window-container.float.pc[data-v-0d17419d]{top:50px;left:50%;transform:translate(-50%)}.float-window-container.float.mobile[data-v-0d17419d]{top:150px;right:0}.singlecall-media-container[data-v-670a78d8]{width:100%;height:100%;position:absolute;z-index:0}.singlecall-media-container.float[data-v-670a78d8]{position:relative}.singlecall-media-container.pc[data-v-670a78d8]{border-radius:12px;overflow:hidden}.roggle-btn[data-v-670a78d8]{position:absolute;left:100px;z-index:100;top:0}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-message{position:fixed;left:50%;transform:translate(-50%);padding:10px;border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:14px;z-index:9999;color:#000;border-color:#e9e9eb;background-color:#f4f4f5}.tk-message .tk-message_icon{margin-right:5px}.tk-message .tk-message_close{cursor:pointer;margin-left:5px}.tk-message--info{color:#909399;border-color:#e9e9eb;background-color:#f4f4f5}.tk-message--success{color:#67c23a;border-color:#e1f3d8;background-color:#f0f9eb}.tk-message--warning{color:#e6a23c;border-color:#faecd8;background-color:#fdf6ec}.tk-message--error{color:#f56c6c;border-color:#fde2e2;background-color:#fef0f0}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-grid-item{display:flex;justify-content:center}.tk-grid-item.h5{transition-property:width,height,left,top;transition-duration:.3s;transition-timing-function:ease-in}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-popover{position:relative}.tk-popover .tk-popover_content{display:inline-block;vertical-align:middle;min-width:120px;position:absolute;background:#FFFFFF;border-radius:4px;font-size:14px;box-shadow:0 0 12px #0000001f;overflow-wrap:break-word;box-sizing:border-box;text-align:center}.tk-popover .tk-popover_arrow{position:absolute;transform:translate(-50%);width:0;height:0}.tk-popover .tk-popover_trigger{display:inline-block;position:relative;vertical-align:middle}.tk-justify-start{justify-content:flex-start}.tk-justify-center{justify-content:center}.tk-justify-end{justify-content:flex-end}.tk-justify-space-between{justify-content:space-between}.tk-justify-space-around{justify-content:space-around}.tk-justify-space-evenly{justify-content:space-evenly}.tk-align-center{align-items:center}.tk-align-start{align-items:flex-start}.tk-align-end{align-items:flex-end}.tk-align-stretch{align-items:stretch}.tk-align-baseline{align-items:baseline}.tk-blur{backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}.tk-round{border-radius:20px}.tk-circle{border-radius:100%}.tk-popover{position:relative}.tk-popover .tk-popover_content{display:inline-block;vertical-align:middle;min-width:120px;position:absolute;background:#FFFFFF;border-radius:4px;font-size:14px;box-shadow:0 0 12px #0000001f;overflow-wrap:break-word;box-sizing:border-box;text-align:center}.tk-popover .tk-popover_arrow{position:absolute;transform:translate(-50%);width:0;height:0}.tk-popover .tk-popover_trigger{display:inline-block;position:relative;vertical-align:middle}.device-selector-container[data-v-406bc3d4]{width:180px;display:flex;flex-direction:column;align-items:center;padding:6px;font-weight:500}.device-selector-container .device-item[data-v-406bc3d4]{overflow:hidden;padding:5px 3px;text-align:left;line-height:16px;cursor:pointer}.device-selector-container .device-item[data-v-406bc3d4]:hover{border-radius:5px;background:rgba(255,255,255,.3215686275)}.device-selector-container .device-item.select[data-v-406bc3d4]{background-color:#0f101433;border-radius:3px}.device-selector-container .control-item[data-v-406bc3d4]{width:100%;height:1px;background-color:#fff3}.scroll-container[data-v-406bc3d4]{width:100%;overflow:hidden}.scroll-container .scroll-content[data-v-406bc3d4]{max-height:60px;margin-right:-26px;overflow:hidden auto;padding-right:20px}.btn-content[data-v-24aa342a],.btn-content[data-v-e75a102a],.btn-content[data-v-e1377967],.btn-content[data-v-1a5a5781],.btn-content[data-v-cc016de6],.btn-content[data-v-44a3def7],.btn-content[data-v-df69c038],.btn-content[data-v-f29c2e7a],.btn-content[data-v-f7839a20]{display:flex;flex-direction:column;align-items:center}.button-panel-container[data-v-6582c17d]{position:absolute;z-index:1}.button-panel-container.pc[data-v-6582c17d]{width:60%;height:63px;margin:0 auto;bottom:6%;left:50%;z-index:2;transform:translate(-50%)}.button-panel-container.mobile[data-v-6582c17d]{display:flex;justify-content:center;height:27%;bottom:0;width:100%}.button-panel-container.mobile.h5[data-v-6582c17d]{transition-property:width,height,left,top;transition-duration:.3s;transition-timing-function:ease-in}.button-panel-container.mobile.groupCall.showBackGround[data-v-6582c17d]{background-color:#4f586b}.button-panel-container.mobile .button-group[data-v-6582c17d]{position:absolute;width:72%;top:2vh;height:80%}.button-panel-container.mobile.close[data-v-6582c17d]{height:14%;align-items:center}.button-panel-container.mobile.close .button-group[data-v-6582c17d]{position:absolute;width:72%;right:6.2vw;height:40px;top:auto;bottom:auto}.button-panel-container .button-group[data-v-6582c17d]{position:relative;height:100%}.button-panel-container .toggle-button-container[data-v-6582c17d]{display:flex;align-items:center;position:absolute;left:8.2vw}.button-panel-container .toggle-button-container.h5[data-v-6582c17d]{transition-property:width,height,left,top;transition-duration:.3s;transition-timing-function:ease-in}.button-panel-container.open .toggle-button-container[data-v-6582c17d]{bottom:6vh}.singlecall-container[data-v-317c1d41]{height:100%}.stream-loading-container[data-v-56b8bc56]{width:100%;height:100%;position:absolute;z-index:1;display:flex;align-items:center;justify-content:center}.groupcall-media-container[data-v-0629f0d8]{width:100%;height:100%;position:absolute;z-index:1}.groupcall-media-container.pc[data-v-0629f0d8]{border-radius:12px;overflow:hidden}.groupcall-media-container.pc.two-layout[data-v-0629f0d8]{margin-top:20%}.groupcall-media-container.pc .tk-toggle-window-item--small[data-v-0629f0d8]{top:2%;left:2%;width:22%;height:21%;border-radius:12px;overflow:hidden}.groupcall-media-container.mobile[data-v-0629f0d8]{margin-top:5.5vh}.groupcall-media-container.mobile.float[data-v-0629f0d8]{margin-top:0}.groupcall-media-container.mobile.two-layout[data-v-0629f0d8]{margin-top:15vh}.groupcall-media-container.mobile.two-layout.float[data-v-0629f0d8]{margin-top:0}.dialog[data-v-26735e82]{background:rgba(0,0,0,.3)}.dialog-main[data-v-26735e82]{background:#FFFFFF}.dialog-main-header[data-v-26735e82]{font-weight:500;color:#333}.dialog-main-title[data-v-26735e82]{font-family:PingFangSC-Medium;font-weight:500;color:#333}.dialog-main-back[data-v-26735e82]{background:none}.dialog-main-content[data-v-26735e82]{font-weight:400;color:#333}.btn[data-v-26735e82]{font-weight:400;color:#fff;letter-spacing:0}.btn-cancel[data-v-26735e82]{border:1px solid #dddddd;color:#666}.btn-default[data-v-26735e82]{background:#006EFF;border:1px solid #006EFF}.dialog[data-v-26735e82]{position:absolute;width:100%;height:100%;left:0;top:0;z-index:6;display:flex;justify-content:center;align-items:center}.dialog-main[data-v-26735e82]{min-width:368px;border-radius:10px;padding:20px 30px}.dialog-main-header[data-v-26735e82]{display:flex;justify-content:space-between;align-items:center;font-size:16px;line-height:30px}.dialog-main-title[data-v-26735e82]{font-size:16px;line-height:30px}.dialog-main-content[data-v-26735e82]{font-size:14px}.dialog-main-footer[data-v-26735e82]{display:flex;justify-content:flex-end}.btn[data-v-26735e82]{padding:8px 20px;margin:0 6px;border-radius:4px;border:none;font-size:14px;text-align:center;line-height:20px}.btn[data-v-26735e82]:disabled{opacity:.3}.btn[data-v-26735e82]:last-child{margin-right:0}.dialog-h5[data-v-26735e82]{height:100%;top:0;align-items:inherit}.dialog-h5 .dialog-main[data-v-26735e82]{border-radius:0;padding:0;display:flex;flex-direction:column;overflow:hidden;width:100%;min-height:80px;min-width:120px}.dialog-h5 .dialog-main-content[data-v-26735e82]{flex:1;min-width:0;min-height:0;text-align:center}.dialog-h5 .dialog-main-content-uniapp[data-v-26735e82]{padding:40px 0}.dialog-h5 .dialog-main-footer[data-v-26735e82]{border-top:1px solid #DDDDDD}.dialog-h5 .dialog-main-footer .btn[data-v-26735e82]{flex:1;margin:0;background:none;border-right:1px solid #DDDDDD}.dialog-h5 .dialog-main-footer .btn-default[data-v-26735e82]{color:#ff584c;border:none}.center[data-v-26735e82]{align-items:center;padding:20px;box-sizing:border-box}.icon[data-v-b989a330]{display:inline-flex;justify-content:center;align-items:center;margin:0}.main[data-v-cc0325fc]{background:#FFFFFF;border:1px solid #E0E0E0;box-shadow:0 -4px 12px #0000000f}.main .left[data-v-cc0325fc]{border-right:1px solid #E8E8E9}.main .transfer-header[data-v-cc0325fc]{font-weight:500;color:#000;letter-spacing:0}.main .transfer-header input[data-v-cc0325fc]{background:#FFFFFF;border:1px solid #DEE0E3;font-weight:500;color:#8f959e;letter-spacing:0}.main .transfer-list .transfer-text[data-v-cc0325fc]{font-weight:500;color:#8f959e;letter-spacing:0}.main .transfer-list-item .disabled[data-v-cc0325fc]{background:#eeeeee}.btn[data-v-cc0325fc]{background:#3370FF;border:0 solid #2F80ED;font-weight:400;color:#fff}.btn-cancel[data-v-cc0325fc]{background:#FFFFFF;border:1px solid #DDDDDD;color:#828282}.btn-no[data-v-cc0325fc]{background:#e8e8e9;border:1px solid #DDDDDD;font-weight:400;color:#fff}.transfer-h5-header[data-v-cc0325fc]{background:#FFFFFF}.transfer-h5-header .title[data-v-cc0325fc]{font-family:PingFangSC-Medium;font-weight:500;color:#000;letter-spacing:0}.main[data-v-cc0325fc]{box-sizing:border-box;width:541px;height:390px;display:flex;border-radius:8px;padding:20px 0}.main .right[data-v-cc0325fc]{padding:0 20px;flex:1}.main .right .transfer-list[data-v-cc0325fc]{padding-right:20px}.main .left[data-v-cc0325fc]{flex:1;overflow-y:hidden;display:flex;flex-direction:column}.main .left .transfer-header[data-v-cc0325fc]{padding:0 20px}.main .left .transfer-left-main[data-v-cc0325fc]{flex:1;overflow-y:auto;padding:0 13px}.main .right[data-v-cc0325fc]{display:flex;flex-direction:column;text-align:left}.main .right .transfer-right-footer[data-v-cc0325fc]{align-self:flex-end}.main .right .transfer-right-footer .btn-cancel[data-v-cc0325fc]{margin-right:12px}.main .right .transfer-list[data-v-cc0325fc]{overflow-y:auto}.main .transfer-header[data-v-cc0325fc]{font-size:14px;line-height:14px;padding-bottom:20px}.main .transfer-header input[data-v-cc0325fc]{box-sizing:border-box;width:100%;border-radius:30px;font-size:10px;line-height:14px;padding:9px 12px}.main .transfer-list[data-v-cc0325fc]{flex:1;display:flex;flex-direction:column}.main .transfer-list .transfer-text[data-v-cc0325fc]{font-size:10px;line-height:14px}.main .transfer-list-item[data-v-cc0325fc]{padding:6px 0;display:flex;align-items:center;font-size:14px;text-align:left}.main .transfer-list-item-content[data-v-cc0325fc]{flex:1;display:flex;align-items:center}.main .transfer-list-item .avatar[data-v-cc0325fc]{margin:0 5px 0 8px;border-radius:50%}.main .transfer-list-item .name[data-v-cc0325fc]{width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1}.avatar[data-v-cc0325fc]{width:36px;height:36px;border-radius:5px;font-size:12px;display:flex;justify-content:center;align-items:center}.btn[data-v-cc0325fc],.btn-no[data-v-cc0325fc]{padding:4px 28px;font-size:12px;line-height:24px;border-radius:4px}.space-between[data-v-cc0325fc]{justify-content:space-between}.select-all[data-v-cc0325fc]{padding-left:8px;font-size:14px}.more[data-v-cc0325fc]{display:flex;justify-content:center;align-items:center;cursor:pointer;font-size:14px}.transfer-h5[data-v-cc0325fc]{width:100vw;height:100vh;display:flex;flex-direction:column}.transfer-h5-header[data-v-cc0325fc]{position:relative;display:flex;justify-content:space-between;align-items:center;font-size:18px;padding:16px 18px}.transfer-h5-header .space[data-v-cc0325fc],.transfer-h5-header .icon[data-v-cc0325fc]{width:18px;height:18px}.transfer-h5 .main[data-v-cc0325fc]{flex-direction:column;width:auto;height:550px;border-radius:0;border:none;box-shadow:none;max-height:calc(100% - 50px);padding:0}.transfer-h5 .main .avatar[data-v-cc0325fc]{border-radius:5px}.transfer-h5 .main .left[data-v-cc0325fc]{padding:0;flex:1;border:none;display:flex;flex-direction:column}.transfer-h5 .main .left .transfer-header[data-v-cc0325fc]{position:sticky;top:0;padding:0 18px}.transfer-h5 .main .left .transfer-header input[data-v-cc0325fc]{border-radius:5px;font-size:14px}.transfer-h5 .main .left-uniapp-input[data-v-cc0325fc]{height:36px}.transfer-h5 .main .right[data-v-cc0325fc]{flex:0;flex-direction:row;align-items:center;box-shadow:inset 0 1px #eee;padding:8px 18px}.transfer-h5 .main .right .transfer-list[data-v-cc0325fc]{flex-direction:row;width:0}.transfer-h5 .main .right .transfer-list-item-content[data-v-cc0325fc]{flex:none}.transfer-h5 .main .right .transfer-right-footer[data-v-cc0325fc]{padding:6px 0;display:flex;align-items:center}.transfer-h5 .main .right .transfer-right-footer .btn[data-v-cc0325fc]{font-size:14px}ul[data-v-cc0325fc],ol[data-v-cc0325fc],li[data-v-cc0325fc]{margin:0;padding:0}.icon-unselected[data-v-cc0325fc]{width:18px;height:18px;background:#FFFFFF;border:1px solid #DDDDDD;border-radius:11px;box-sizing:border-box}.groupcall-container[data-v-aa4e300f]{height:100%}.card[data-v-9982fa76]{box-sizing:border-box;border-radius:4px;border:1px solid #EBEEF5;background-color:#fff;overflow:hidden;color:#303133;transition:.3s;box-shadow:0 2px 12px #0000001a}.card .card-body[data-v-9982fa76]{padding:20px}.card-wrapper[data-v-9982fa76]{display:flex;flex-direction:column;align-items:center;width:500px;position:relative}.card-wrapper .button[data-v-9982fa76]{box-sizing:border-box;background-color:#104ef5;color:#fff;font-size:12px;height:37px;border-radius:4px;padding:10px}.card-wrapper .image-wrapper[data-v-9982fa76]{box-sizing:border-box}.title[data-v-9982fa76]{padding:10px 0;font-size:16px;font-weight:500;line-height:22px}.desc[data-v-9982fa76]{font-size:12px;margin-bottom:10px}.button[data-v-9982fa76]{background-color:#104ef5;color:#fff;font-size:12px;height:37px;border-radius:4px;padding:10px}.allow[data-v-9982fa76]{font-size:12px;color:#666;padding:10px 0 20px}.image[data-v-9982fa76]{width:100%;border-radius:10px}.close[data-v-9982fa76]{position:absolute;z-index:999;display:flex;justify-content:center;align-items:center;font-size:20px;right:20px;cursor:pointer;width:20px;height:20px}.tips[data-v-cd63c32e]{width:100%;height:100%}.card[data-v-cd63c32e]{position:fixed;right:0;bottom:0;box-shadow:0 2px 12px #0000001a;border-radius:4px;border:1px solid #EBEEF5;background-color:#fff;overflow:hidden;color:#303133;transition:.3s}.tag[data-v-cd63c32e]{position:fixed;left:0;top:0;width:320px;text-align:left}.toast-container[data-v-27fc22c5]{position:fixed;display:flex;align-items:center;z-index:1002;justify-content:space-between;left:50%;transform:translate(-50%);padding:15px 15px 15px 20px;background-color:#000;color:#fff;box-sizing:border-box;border-radius:4px;border-width:1px;border-style:solid}.toast-container .toast-content[data-v-27fc22c5]{margin-right:16px}.toast-container .toast-close-icon[data-v-27fc22c5]{width:16px;line-height:16px;cursor:pointer}.toast-container .toast-close-icon[data-v-27fc22c5]:before{font-style:normal;content:url(data:image/svg+xml;base64,)}.info[data-v-27fc22c5]{border-color:#ebeef5;color:#909399;background-color:#edf2fc}.success[data-v-27fc22c5]{color:#67c23a;background-color:#f0f9eb;border-color:#e1f3d8}.waring[data-v-27fc22c5]{color:#e6a23c;background-color:#fdf6ec;border-color:#faecd8}.error[data-v-27fc22c5]{color:#f56c6c;background-color:#fef0f0;border-color:#fde2e2}.slide-up-enter-active[data-v-27fc22c5],.slide-up-leave-active[data-v-27fc22c5]{transition:all .5s ease-out}.slide-up-enter-from[data-v-27fc22c5]{opacity:0;transform:translate(-50%,20px)}.slide-up-leave-to[data-v-27fc22c5]{opacity:0;transform:translate(-50%,-20px)}.fade-enter-from[data-v-27fc22c5],.fade-leave-to[data-v-27fc22c5]{opacity:0;transform:translate(-50%,-100%)}.TUICallKit-mobile,.TUICallKit-mobile .singCall{width:100%;height:100%}.transition-animation{transform:translateY(-100%);animation:slideInDown .5s ease forwards}@keyframes slideInDown{0%{transform:translateY(-100%)}to{transform:translateY(0)}}.TUICallKit-desktop{margin:0 auto;position:relative;border-radius:inherit;width:100%;height:100%;color:#fff;display:flex;flex-direction:column;justify-content:center;overflow:hidden;z-index:12;border-radius:16px}.TUICallKit-desktop .singCall{width:100%;height:100%}.mobile-audio{background-color:#fff}.miniMized{width:168px!important;height:56px!important;overflow:visible!important}.miniMized-mobile-audio{width:72px;height:72px;position:fixed;top:40px;right:40px}.miniMized-mobile-video{width:40%;height:30%;position:fixed;top:40px;right:40px}.banner-pc{position:relative;cursor:pointer}.banner-pc .top{height:24px;padding-right:10px;border-radius:4px;background:#F4F5F9;display:inline-flex;align-items:center}.banner-pc .top .call-icon{margin:4px 4px 4px 8px}.banner-pc .top .text{font-weight:400;font-size:12px;line-height:16px;color:#666}.banner-pc .content{width:220px;margin-top:8px;padding:10px 18px 6px;box-shadow:0 0 24px #161e2733;background-color:#fff;border-radius:10px;position:absolute;z-index:10;display:flex;display:-webkit-flex;flex-direction:column;align-items:center}.banner-pc .content .content-arrows{width:12px;height:12px;border-radius:2px;background-color:#fff;transform:rotate(45deg);position:absolute;top:-5px;left:80px}.banner-pc .content .avatar-box{display:flex;justify-content:center;flex-wrap:wrap}.banner-pc .content .avatar{margin:6px;display:flex;border-radius:4px}.banner-pc .content .btn{margin:10px 0;width:97px;height:30px;background:#F2F5FC;border-radius:4px;display:flex;justify-content:center;align-items:center;font-size:12px;color:#1c66e5;cursor:pointer}.banner-h5{padding:0 16px;position:relative}.banner-h5 .top{height:36px;background:#FFFFFF;border-radius:4px;display:flex;display:-webkit-flex;justify-content:space-between;align-items:center}.banner-h5 .top .left{display:flex;align-items:center}.banner-h5 .top .left .text{font-weight:400;font-size:12px;color:#666}.banner-h5 .content-box{box-sizing:border-box;width:100%;padding:0 16px 10px;background:#FFFFFF;box-shadow:0 3px 3px 1px #b4b4b440;border-radius:0 0 8px 8px;position:absolute;left:0px;z-index:10}.banner-h5 .content-box .content{background:#EEF0F2;border-radius:6px}.banner-h5 .content-box .content .avatar-box{padding:30px 6px;border-bottom:1px solid rgba(112,112,112,.1058823529);display:flex;display:-webkit-flex;justify-content:center;flex-wrap:wrap}.banner-h5 .content-box .content .btn{padding:10px;font-weight:600;font-size:14px;text-align:center;color:#333;cursor:pointer}")),document.head.appendChild(t)}}catch(e){console.error("vite-plugin-css-injected-by-js",e)}})();
import { TUICallEvent as E, TUICallEngine as en } from "@tencentcloud/call-engine-js";
import { TUICore as ae, TUIConstants as AA, TUILogin as tn } from "@tencentcloud/tui-core";
import Tt from "@tencentcloud/chat";
import At, { inject as dA, ref as I, onMounted as sA, onUnmounted as aA, toRefs as b, watch as K, computed as M, defineComponent as S, provide as rA, watchEffect as $A, toRef as Re, unref as an, nextTick as nn, reactive as Me } from "vue";
var u = /* @__PURE__ */ ((a) => (a.CALL = "call", a.CUSTOM = "custom", a))(u || {}), _ = /* @__PURE__ */ ((a) => (a[a.UNKNOWN = 0] = "UNKNOWN", a[a.AUDIO = 1] = "AUDIO", a[a.VIDEO = 2] = "VIDEO", a))(_ || {}), J = /* @__PURE__ */ ((a) => (a.UNKNOWN = "unknown", a.CALLEE = "callee", a.CALLER = "caller", a))(J || {}), w = /* @__PURE__ */ ((a) => (a.IDLE = "idle", a.CALLING = "calling", a.CONNECTED = "connected", a))(w || {}), ue = /* @__PURE__ */ ((a) => (a.CONTAIN = "contain", a.COVER = "cover", a.FILL = "fill", a))(ue || {}), re = /* @__PURE__ */ ((a) => (a.RESOLUTION_480P = "480p", a.RESOLUTION_720P = "720p", a.RESOLUTION_1080P = "1080p", a))(re || {}), Gt = /* @__PURE__ */ ((a) => (a.EN = "en", a["ZH-CN"] = "zh-cn", a.JA_JP = "ja_JP", a))(Gt || {});
const nA = {
  IDLE: "idle",
  BE_INVITED: "be-invited",
  DIALING_C2C: "dialing-c2c",
  DIALING_GROUP: "dialing-group",
  CALLING_C2C_AUDIO: "calling-c2c-audio",
  CALLING_C2C_VIDEO: "calling-c2c-video",
  CALLING_GROUP_AUDIO: "calling-group-audio",
  CALLING_GROUP_VIDEO: "calling-group-video"
}, on = {
  unknown: 0,
  audio: 1,
  video: 2
};
var oA = /* @__PURE__ */ ((a) => (a.MICROPHONE = "microphone", a.CAMERA = "camera", a.SPEAKER = "speaker", a))(oA || {}), FA = /* @__PURE__ */ ((a) => (a[a.FRONT = 0] = "FRONT", a[a.BACK = 1] = "BACK", a))(FA || {}), SA = /* @__PURE__ */ ((a) => (a.Camera = "camera", a.Microphone = "microphone", a.SwitchCamera = "switchCamera", a.InviteUser = "inviteUser", a))(SA || {}), GA = /* @__PURE__ */ ((a) => (a.Open = "open", a.Close = "close", a))(GA || {}), ye = /* @__PURE__ */ ((a) => (a.LOCAL = "local", a.REMOTE = "remote", a))(ye || {}), Te = /* @__PURE__ */ ((a) => (a.LocalInLargeView = "local", a.RemoteInLargeView = "remote", a))(Te || {}), KA = /* @__PURE__ */ ((a) => (a[a.INVITE = 1] = "INVITE", a[a.CANCEL_INVITE = 2] = "CANCEL_INVITE", a[a.ACCEPT_INVITE = 3] = "ACCEPT_INVITE", a[a.REJECT_INVITE = 4] = "REJECT_INVITE", a[a.INVITE_TIMEOUT = 5] = "INVITE_TIMEOUT", a))(KA || {});
const Et = {
  SWITCH_TO_AUDIO_CALL_FAILED: 60001,
  SWITCH_TO_VIDEO_CALL_FAILED: 60002,
  MICROPHONE_UNAVAILABLE: 60003,
  CAMERA_UNAVAILABLE: 60004,
  BAN_DEVICE: 60005,
  NOT_SUPPORTED_WEBRTC: 60006,
  ERROR_BLACKLIST: 20007
}, wt = {
  SWITCH_TO_AUDIO_CALL_FAILED: "switchToAudioCall-call-failed",
  SWITCH_TO_VIDEO_CALL_FAILED: "switchToVideoCall-call-failed",
  MICROPHONE_UNAVAILABLE: "microphone-unavailable",
  CAMERA_UNAVAILABLE: "camera-unavailable",
  BAN_DEVICE: "ban-device",
  NOT_SUPPORTED_WEBRTC: "not-supported-webrtc",
  ERROR_BLACKLIST: "blacklist-user-tips"
};
var kt = /* @__PURE__ */ ((a) => (a[a.NORMAL = 0] = "NORMAL", a[a.RELEASE = 1] = "RELEASE", a[a.WARNING = 2] = "WARNING", a[a.ERROR = 3] = "ERROR", a[a.NONE = 4] = "NONE", a))(kt || {});
const he = {
  CALL_STATUS: "callStatus",
  CALL_ROLE: "callRole",
  CALL_MEDIA_TYPE: "callMediaType",
  LOCAL_USER_INFO: "localUserInfo",
  LOCAL_USER_INFO_EXCLUDE_VOLUMN: "localUserInfoExcludeVolume",
  REMOTE_USER_INFO_LIST: "remoteUserInfoList",
  REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST: "remoteUserInfoExcludeVolumeList",
  CALLER_USER_INFO: "callerUserInfo",
  IS_GROUP: "isGroup",
  CALL_DURATION: "callDuration",
  CALL_TIPS: "callTips",
  TOAST_INFO: "toastInfo",
  IS_MINIMIZED: "isMinimized",
  ENABLE_FLOAT_WINDOW: "enableFloatWindow",
  BIG_SCREEN_USER_ID: "bigScreenUserId",
  LANGUAGE: "language",
  IS_CLICKABLE: "isClickable",
  DISPLAY_MODE: "displayMode",
  VIDEO_RESOLUTION: "videoResolution",
  PUSHER: "pusher",
  PLAYER: "player",
  IS_EAR_PHONE: "isEarPhone",
  IS_MUTE_SPEAKER: "isMuteSpeaker",
  SHOW_PERMISSION_TIP: "SHOW_PERMISSION_TIP",
  NETWORK_STATUS: "NetWorkStatus",
  GROUP_ID: "groupID",
  ROOM_ID: "roomID",
  ROOM_ID_TYPE: "roomIdType",
  SHOW_SELECT_USER: "showSelectUser",
  IS_SHOW_ENABLE_VIRTUAL_BACKGROUND: "isShowEnableVirtualBackground",
  ENABLE_VIRTUAL_BACKGROUND: "enableVirtualBackground",
  GROUP_CALL_MEMBERS: "groupCallMembers",
  PUSHER_ID: "pusherId"
}, sn = {
  INNER_ATTR_KIT_INFO: "inner_attr_kit_info"
}, ln = {
  INITIAL_PUSHER: "initialPusher",
  NEW_PUSHER: "newPusher"
}, o = {
  PREFIX: "\u3010CallService\u3011",
  AUDIO: "audio",
  VIDEO: "video",
  LOCAL_VIDEO: "localVideo",
  ERROR: "error",
  TIMEOUT: "timeout",
  RAF: "raf",
  INTERVAL: "interval",
  DEFAULT: "default",
  BOOLEAN: "boolean",
  STRING: "string",
  NUMBER: "number",
  OBJECT: "object",
  ARRAY: "array",
  FUNCTION: "function",
  UNDEFINED: "undefined",
  UNKNOWN: "unknown",
  ALL: "all",
  MYSELF: "myself",
  DEVICE_LIST: "deviceList",
  CAMERA_POSITION: "cameraPosition",
  CUSTOM_UI_CONFIG: "customUIConfig",
  TRANSLATE: "translate",
  ...ln,
  ...he,
  ...sn
}, un = "https://web.sdk.qcloud.com/component/TUIKit/assets/call.png", rn = "https://web.sdk.qcloud.com/component/TUIKit/assets/call-video-reverse.svg", jt = 2147483647, cn = 3, dn = 4;
var Qe = /* @__PURE__ */ ((a) => (a.MAC = "mac", a.WIN = "win", a))(Qe || {}), et = /* @__PURE__ */ ((a) => (a[a.TUI_CALL_KIT = 14] = "TUI_CALL_KIT", a[a.TIM_CALL_KIT = 15] = "TIM_CALL_KIT", a))(et || {}), Ze = /* @__PURE__ */ ((a) => (a[a.NUMBER_ROOM_ID = 1] = "NUMBER_ROOM_ID", a[a.STRING_ROOM_ID = 2] = "STRING_ROOM_ID", a))(Ze || {});
async function gn(a) {
  if (!a)
    return !1;
  try {
    const e = await new Promise((t, A) => {
      const n = new XMLHttpRequest();
      n.open("HEAD", a, !0), n.onload = () => t(n), n.onerror = () => A(n), n.send();
    });
    return e.status === 200 && e.getResponseHeader("Content-Type") === "audio/mpeg";
  } catch (e) {
    return console.warn(e), !1;
  }
}
function Ee(a) {
  if (typeof a != "object" || a === null)
    return a;
  let e = Array.isArray(a) ? [] : {};
  for (let t in a)
    a.hasOwnProperty(t) && (e[t] = Ee(a[t]));
  return e;
}
const In = {
  hangup: "\u6302\u65AD",
  reject: "\u62D2\u7EDD",
  accept: "\u63A5\u53D7",
  camera: "\u6444\u50CF\u5934",
  microphone: "\u9EA6\u514B\u98CE",
  speaker: "\u626C\u58F0\u5668",
  "open camera": "\u6253\u5F00\u6444\u50CF\u5934",
  "close camera": "\u5173\u95ED\u6444\u50CF\u5934",
  "open microphone": "\u6253\u5F00\u9EA6\u514B\u98CE",
  "close microphone": "\u5173\u95ED\u9EA6\u514B\u98CE",
  "video-to-audio": "\u8F6C\u8BED\u97F3\u901A\u8BDD",
  "virtual-background": "\u6A21\u7CCA\u80CC\u666F",
  "other side reject call": "\u5BF9\u65B9\u5DF2\u62D2\u7EDD",
  "reject call": "{{ userList }} \u62D2\u7EDD\u901A\u8BDD",
  cancel: "\u53D6\u6D88\u901A\u8BDD",
  "other side line busy": "\u5BF9\u65B9\u5FD9\u7EBF",
  "in busy": "{{ userList }} \u6B63\u5728\u5FD9",
  "call timeout": "\u547C\u53EB\u8D85\u65F6",
  "end call": "{{ userList }} \u7ED3\u675F\u901A\u8BDD",
  "caller calling message": "\u7B49\u5F85\u5BF9\u65B9\u63A5\u53D7\u9080\u8BF7",
  "callee calling video message": "\u9080\u8BF7\u4F60\u89C6\u9891\u901A\u8BDD",
  "callee calling audio message": "\u9080\u8BF7\u4F60\u8BED\u97F3\u901A\u8BDD",
  "no microphone access": "\u6CA1\u6709\u9EA6\u514B\u98CE\u6743\u9650",
  "no camera access": "\u6CA1\u6709\u6444\u50CF\u5934\u6743\u9650",
  "invite member": "\u9080\u8BF7\u6210\u5458",
  "Invited group call": "\u9080\u8BF7\u4F60\u52A0\u5165\u591A\u4EBA\u901A\u8BDD",
  "Those involved": "\u53C2\u4E0E\u901A\u8BDD\u7684\u6709\uFF1A",
  waiting: "\u7B49\u5F85\u63A5\u542C...",
  me: "(\u6211)",
  "browser-authorization": "\u6D4F\u89C8\u5668\u6388\u6743",
  "mac-privacy": "\u7CFB\u7EDF\u504F\u597D\u8BBE\u7F6E -> \u5B89\u5168\u4E0E\u9690\u79C1 -> \u9690\u79C1",
  "win-privacy": "\u8BBE\u7F6E -> \u9690\u79C1\u548C\u5B89\u5168\u6027 -> \u5E94\u7528\u6743\u9650",
  "mac-preferences": "\u6253\u5F00\u7CFB\u7EDF\u504F\u597D\u8BBE\u7F6E",
  "win-preferences": "\u6253\u5F00\u7CFB\u7EDF\u8BBE\u7F6E",
  "Please enter userID": "\u8BF7\u8F93\u5165 userID",
  "View more": "\u67E5\u770B\u66F4\u591A",
  "people selected": "\u4EBA\u5DF2\u9009\u4E2D",
  "Select all": "\u5168\u9009",
  Cancel: "\u53D6\u6D88",
  Done: "\u5B8C\u6210",
  "exist group call": "\u5F53\u524D\u7FA4\u7EC4\u4E2D\u5DF2\u7ECF\u5B58\u5728\u7FA4\u7EC4\u901A\u8BDD",
  "camera enabled": "\u6444\u50CF\u5934\u5DF2\u5F00",
  "camera disabled": "\u6444\u50CF\u5934\u5DF2\u5173",
  "microphone enabled": "\u9EA6\u514B\u98CE\u5DF2\u5F00",
  "microphone disabled": "\u9EA6\u514B\u98CE\u5DF2\u5173",
  "speaker enabled": "\u626C\u58F0\u5668\u5DF2\u5F00",
  "speaker disabled": "\u626C\u58F0\u5668\u5DF2\u5173",
  "open speaker": "\u5F00\u542F\u626C\u58F0\u5668",
  "close speaker": "\u5173\u95ED\u626C\u58F0\u5668",
  "wait to be called": "\u7B49\u5F85\u63A5\u542C",
  answered: "\u5DF2\u63A5\u901A",
  "people in the call": "\u4EBA\u53C2\u4E0E\u901A\u8BDD",
  "failed to obtain speakers": "\u65E0\u6CD5\u83B7\u53D6\u626C\u58F0\u5668",
  "you have a new call": "\u60A8\u6709\u4E00\u4E2A\u65B0\u7684\u901A\u8BDD",
  "switch camera": "\u7FFB\u8F6C",
  join: "\u52A0\u5165",
  "people on the call": "\u4EBA\u6B63\u5728\u901A\u8BDD",
  "Supports a maximum of 9 people for simultaneous calls": "\u6700\u591A\u652F\u63019\u4EBA\u540C\u65F6\u901A\u8BDD",
  you: "(\u4F60)",
  "The network is poor during your current call": "\u5F53\u524D\u901A\u8BDD\u4F60\u7684\u7F51\u7EDC\u4E0D\u4F73",
  "The other user network is poor during the current call": "\u5F53\u524D\u901A\u8BDD\u5BF9\u65B9\u7F51\u7EDC\u4E0D\u4F73",
  "TUICallKit init is not complete": "TUICallKit \u521D\u59CB\u5316\u767B\u5F55\u672A\u5B8C\u6210\uFF0C\u9700\u8981\u5728 init \u5B8C\u6210\u540E\u4F7F\u7528\u6B64 API",
  "Video call": "\u53D1\u8D77\u89C6\u9891\u901A\u8BDD",
  "Voice call": "\u53D1\u8D77\u8BED\u97F3\u901A\u8BDD",
  "Call End": "\u901A\u8BDD\u7ED3\u675F",
  "Switch voice call": "\u5207\u6362\u8BED\u97F3\u901A\u8BDD",
  "Switch video call": "\u5207\u6362\u89C6\u9891\u901A\u8BDD",
  "Call duration": "\u901A\u8BDD\u65F6\u957F",
  "Call Cancel": "\u5DF2\u53D6\u6D88",
  "Other Side Cancel": "\u5BF9\u65B9\u5DF2\u53D6\u6D88",
  Decline: "\u5DF2\u62D2\u7EDD",
  "Other Side Decline": "\u5BF9\u65B9\u5DF2\u62D2\u7EDD",
  "No answer": "\u8D85\u65F6\u65E0\u5E94\u7B54",
  "Other Side No Answer": "\u5BF9\u65B9\u65E0\u5E94\u7B54",
  Answered: "\u5DF2\u63A5\u542C",
  "Other Side Line Busy": "\u5BF9\u65B9\u5FD9\u7EBF\u4E2D",
  "Line Busy": "\u5FD9\u7EBF\u65E0\u5E94\u7B54",
  timeout: "{{ userList }} \u8D85\u65F6",
  "kick out": "\u88AB\u8E22",
  call: "\u901A\u8BDD",
  "video-call": "\u89C6\u9891\u901A\u8BDD",
  "audio-call": "\u97F3\u9891\u901A\u8BDD",
  search: "\u641C\u7D22",
  "search-result": "\u641C\u7D22\u7ED3\u679C",
  "Wechat scan right QR code": "\u5FAE\u4FE1\u626B\u53F3\u4E8C\u7EF4\u7801",
  "Use-phone-and-computer": "\u7528\u624B\u673A\u4E0E\u7535\u8111\u4E92\u6253\u4F53\u9A8C\u89C6\u9891\u901A\u8BDD",
  "Scan the QR code above": "\u626B\u63CF\u4E0A\u65B9\u4E8C\u7EF4\u7801",
  "no-user": "\u672A\u641C\u7D22\u5230\u7528\u6237",
  "member-not-added": "\u672A\u6DFB\u52A0\u6210\u5458",
  "not-login": "\u672A\u767B\u5F55",
  "login-status-expire": "\u767B\u5F55\u72B6\u6001\u5DF2\u5931\u6548\uFF0C\u8BF7\u5237\u65B0\u7F51\u9875\u91CD\u8BD5",
  "experience-multi-call": "\u4F53\u9A8C\u591A\u4EBA\u901A\u8BDD\u8BF7\u4E0B\u8F7D\u5168\u529F\u80FDdemo:",
  "not-support-multi-call": "\u591A\u4EBA\u901A\u8BDD\u63A5\u53E3\u672A\u5F00\u653E",
  "input-phone-userID": "\u8BF7\u8F93\u5165\u624B\u673A\u53F7/\u7528\u6237ID",
  userID: "\u7528\u6237ID",
  "already-enter": "\u5DF2\u7ECF\u8FDB\u5165\u5F53\u524D\u901A\u8BDD",
  "image-resolution": "\u5206\u8FA8\u7387",
  "default-image-resolution": "\u9ED8\u8BA4\u5206\u8FA8\u7387",
  "invited-person": "\u6DFB\u52A0\u6210\u5458",
  "be-rejected": "\u5BF9\u65B9\u5DF2\u62D2\u7EDD\uFF0C",
  "be-no-response": "\u5BF9\u65B9\u65E0\u5E94\u7B54\uFF0C",
  "be-line-busy": "\u5BF9\u65B9\u5FD9\u7EBF\u4E2D\uFF0C",
  "be-canceled": "\u5BF9\u65B9\u5DF2\u53D6\u6D88",
  "voice-call-end": "\u8BED\u97F3\u901A\u8BDD\u7ED3\u675F",
  "video-call-end": "\u89C6\u9891\u901A\u8BDD\u7ED3\u675F",
  "method-call-failed": "\u540C\u6B65\u64CD\u4F5C\u5931\u8D25",
  "failed-to-obtain-permission": "\u6743\u9650\u83B7\u53D6\u5931\u8D25",
  "environment-detection-failed": "\u73AF\u5883\u68C0\u6D4B\u5931\u8D25",
  "switchToAudioCall-call-failed": "\u5207\u8BED\u97F3\u8C03\u7528\u5931\u8D25",
  "switchToVideoCall-call-failed": "\u5207\u89C6\u9891\u8C03\u7528\u5931\u8D25",
  "microphone-unavailable": "\u6CA1\u6709\u53EF\u7528\u7684\u9EA6\u514B\u98CE\u8BBE\u5907",
  "camera-unavailable": "\u6CA1\u6709\u53EF\u7528\u7684\u6444\u50CF\u5934\u8BBE\u5907",
  "ban-device": "\u7528\u6237\u7981\u6B62\u4F7F\u7528\u8BBE\u5907",
  "not-supported-webrtc": "\u5F53\u524D\u73AF\u5883\u4E0D\u652F\u6301 WebRTC",
  "blacklist-user-tips": "\u53D1\u8D77\u901A\u8BDD\u5931\u8D25\uFF0C\u88AB\u5BF9\u65B9\u62C9\u5165\u9ED1\u540D\u5355\uFF0C\u7981\u6B62\u53D1\u8D77\uFF01",
  "is-already-calling": "TUICallKit \u5DF2\u5728\u901A\u8BDD\u72B6\u6001",
  "need-init": "TUICallKit \u53D1\u8D77\u901A\u8BDD\u524D\u9700\u4FDD\u8BC1 TUICallKitServer.init() \u65B9\u6CD5\u6267\u884C\u6210\u529F",
  "can't call yourself": "\u4E0D\u80FD\u547C\u53EB\u81EA\u5DF1",
  "accept-error": "\u63A5\u901A\u5931\u8D25",
  "accept-device-error": "\u63A5\u901A\u5931\u8D25\uFF0C\u901A\u8BDD\u8BBE\u5907\u83B7\u53D6\u5931\u8D25",
  "call-error": "\u53D1\u8D77\u901A\u8BDD\u5931\u8D25"
}, pn = {
  hangup: "Hang up",
  reject: "Decline",
  accept: "Accept",
  camera: "Camera",
  microphone: "Microphone",
  speaker: "speaker",
  "open camera": "Open Camera",
  "close camera": "Close Camera",
  "open microphone": "Open Microphone",
  "close microphone": "Close Microphone",
  "video-to-audio": "Switch to audio",
  "virtual-background": "Blur Background",
  "other side reject call": "other side reject call",
  "reject call": "{{ userList }} Reject Call",
  cancel: "Cancel Call",
  "other side line busy": "other side line busy",
  "in busy": "{{ userList }} in busy",
  "call timeout": "call timeout",
  "no response from the other side": "no response from the other side",
  "end call": "{{ userList }} end call",
  "caller calling message": "Awaiting response",
  "callee calling video message": "invites you to a video call",
  "callee calling audio message": "invites you to a voice call",
  "no microphone access": "no microphone access",
  "no camera access": "no camera access",
  "invite member": "Invite Member",
  "Invited group call": "invites you to a group call",
  waiting: "Calling...",
  me: "(me)",
  "browser-authorization": "Browser authorization",
  "mac-privacy": "System Preferences -> Security and Privacy -> Privacy",
  "win-privacy": "Setting -> Privacy and Security -> App permissions",
  "mac-preferences": "Open System Preferences",
  "win-preferences": "Open Setting",
  "Please enter userID": "Please enter userID",
  "View more": "View more",
  "people selected": "people selected",
  "Select all": "Select all",
  Cancel: "Cancel",
  Done: "Done",
  "exist group call": "A group call already exists in the current group",
  "camera enabled": "Camera On",
  "camera disabled": "Camera Off",
  "microphone enabled": "Unmuted",
  "microphone disabled": "Muted",
  "speaker enabled": "Speaker On",
  "speaker disabled": "Speaker Off",
  "open speaker": "Turn on speaker",
  "close speaker": "Turn off speaker",
  "wait to be called": "Waiting",
  answered: "Connected",
  "people in the call": " other(s) in the call",
  "failed to obtain speakers": "failed to obtain speakers",
  "you have a new call": "You have a new call",
  "switch camera": "Switch",
  join: "Join",
  "people on the call": "people on the call",
  "Supports a maximum of 9 people for simultaneous calls": "Supports a maximum of 9 people for simultaneous calls",
  you: "(you)",
  "The network is poor during your current call": "The network is poor during your current call",
  "The other user network is poor during the current call": "The other party's network is poor during the current call",
  "TUICallKit init is not complete": "TUICallKit init is not complete. You need to use this API after the init API is finished.",
  "Video call": "Video call",
  "Voice call": "Voice call",
  "Call End": "Call End",
  "Switch voice call": "Switch voice call",
  "Switch video call": "Switch video call",
  "Call duration": "Duration",
  "Call Cancel": "Canceled",
  "Other Side Cancel": "Call canceled by caller",
  Decline: "Declined",
  "Other Side Decline": "Call declined by user",
  "No answer": "Call not answered",
  "Other Side No Answer": "Call wasn't answered",
  Answered: "Answered",
  "Other Side Line Busy": "Line busy",
  "Line Busy": "Line busy. Call not received.",
  "Those involved": "Those involved in the call are",
  call: "call",
  "video-call": "video call",
  "audio-call": "audio call",
  search: "search",
  "search-result": "search result",
  "no-user": "user not found",
  "member-not-added": "member not added",
  "input-phone-userID": "phone number or userID",
  "not-login": "not logged in",
  "login-status-expire": "login status is invalid, please refresh the page and try again",
  "experience-multi-call": "experience multi-person calls, please download the full-featured demo: ",
  "not-support-multi-call": "multi-person call interface is not open",
  userID: "userID",
  "already-enter": "entered the call",
  "camera-opened": "Camera on",
  "camera-closed": "Camera off",
  "microphone-opened": "Mic on",
  "microphone-closed": "Mic off",
  timeout: "{{ userList }} timeout",
  "kick out": "kick out",
  "image-resolution": "Resolution",
  "default-image-resolution": "Default",
  "invited-person": "Invite",
  "be-rejected": "Call declined, ",
  "be-no-response": "No response, ",
  "be-line-busy": "Line busy, ",
  "be-canceled": "The call is canceled, ",
  "voice-call-end": "Voice call ended",
  "video-call-end": "Video call ended",
  "method-call-failed": "Failed to sync the operation",
  "failed-to-obtain-permission": "Failed to obtain permissions",
  "environment-detection-failed": "Failed to check the environment",
  "switchToAudioCall-call-failed": "switch to audio call method failed",
  "switchToVideoCall-call-failed": "switch to video call method failed",
  "microphone-unavailable": "No mic found",
  "camera-unavailable": "No camera found",
  "ban-device": "Device access denied",
  "not-supported-webrtc": "Your current environment does not support WebRTC",
  "blacklist-user-tips": "The identifier is in blacklist. Failed to send this message!",
  "is-already-calling": "TUICallKit is already on a call",
  "need-init": "Before initiating a call with TUICallKit, ensure that the TUICallKitServer.init() method has executed successfully. ",
  "can't call yourself": "Can't call yourself",
  "Use-phone-and-computer": "Use your mobile phone and computer to experience video calls",
  "Wechat scan right QR code": "Wechat scan right QR code",
  "Scan the QR code above": "Scan the QR code above",
  "accept-error": "Accept failed",
  "accept-device-error": "Accept failed, unable to auth calling device",
  "call-error": "Start call failed"
}, Cn = {
  hangup: "\u901A\u8A71\u7D42\u4E86",
  reject: "\u62D2\u5426",
  accept: "\u5FDC\u7B54",
  camera: "\u30AB\u30E1\u30E9",
  microphone: "\u30DE\u30A4\u30AF",
  speaker: "\u30B9\u30D4\u30FC\u30AB\u30FC",
  "virtual-background": "\u30DC\u30B1\u80CC\u666F",
  "other side reject call": "\u901A\u8A71\u304C\u62D2\u5426\u3055\u308C\u307E\u3057\u305F",
  "reject call": "{{ userList }} \u901A\u8A71\u62D2\u5426",
  cancel: "\u901A\u8A71\u3092\u30AD\u30E3\u30F3\u30BB\u30EB",
  "other side line busy": "\u76F8\u624B\u304C\u901A\u8A71\u4E2D\u3067\u3059",
  "in busy": "{{ userList }} \u901A\u8A71\u4E2D",
  "call timeout": "\u547C\u3073\u51FA\u3057\u30BF\u30A4\u30E0\u30A2\u30A6\u30C8",
  "end call": "{{ userList }} \u901A\u8A71\u7D42\u4E86",
  "caller calling message": "\u5FDC\u7B54\u3092\u5F85\u3063\u3066\u3044\u307E\u3059",
  "callee calling video message": "\u30D3\u30C7\u30AA\u901A\u8A71\u306B\u62DB\u5F85\u3055\u308C\u307E\u3057\u305F",
  "callee calling audio message": "\u97F3\u58F0\u901A\u8A71\u306B\u62DB\u5F85\u3055\u308C\u307E\u3057\u305F",
  "no microphone access": "\u30DE\u30A4\u30AF\u306B\u30A2\u30AF\u30BB\u30B9\u3067\u304D\u307E\u305B\u3093",
  "no camera access": "\u30AB\u30E1\u30E9\u306B\u30A2\u30AF\u30BB\u30B9\u3067\u304D\u307E\u305B\u3093",
  "invite member": "\u30E1\u30F3\u30D0\u30FC\u3092\u62DB\u5F85\u3059\u308B",
  "browser-authorization": "\u30D6\u30E9\u30A6\u30B6\u8A8D\u8A3C",
  "mac-privacy": "\u30B7\u30B9\u30C6\u30E0\u74B0\u5883\u8A2D\u5B9A -> \u30BB\u30AD\u30E5\u30EA\u30C6\u30A3\u3068\u30D7\u30E9\u30A4\u30D0\u30B7\u30FC ->\u30D7\u30E9\u30A4\u30D0\u30B7\u30FC",
  "win-privacy": "\u8A2D\u5B9A -> \u30BB\u30AD\u30E5\u30EA\u30C6\u30A3\u3068\u30D7\u30E9\u30A4\u30D0\u30B7\u30FC ->\u30A2\u30D7\u30EA\u306E\u30A2\u30AF\u30BB\u30B9\u8A31\u53EF",
  "mac-preferences": "\u30B7\u30B9\u30C6\u30E0\u74B0\u5883\u8A2D\u5B9A\u3092\u958B\u304F",
  "win-preferences": "\u30B7\u30B9\u30C6\u30E0\u8A2D\u5B9A\u3092\u958B\u304F",
  "Please enter userID": "\u30E6\u30FC\u30B6\u30FCID\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044",
  "View more": "\u3082\u3063\u3068\u898B\u308B",
  "people selected": "\u4EBA\u304C\u9078\u629E\u3055\u308C\u307E\u3057\u305F",
  "Select all": "\u3059\u3079\u3066\u9078\u629E",
  Cancel: "\u30AD\u30E3\u30F3\u30BB\u30EB",
  Done: "\u5B8C\u4E86",
  "exist group call": "\u73FE\u5728\u306E\u30B0\u30EB\u30FC\u30D7\u306B\u306F\u65E2\u306B\u30B0\u30EB\u30FC\u30D7\u901A\u8A71\u304C\u5B58\u5728\u3057\u3066\u3044\u307E\u3059",
  "Video call": "\u30D3\u30C7\u30AA\u901A\u8A71\u3092\u958B\u59CB",
  "Voice call": "\u97F3\u58F0\u901A\u8A71\u3092\u958B\u59CB",
  "Call End": "\u901A\u8A71\u7D42\u4E86",
  "Switch audio call": "\u97F3\u58F0\u901A\u8A71\u306B\u5207\u308A\u66FF\u3048\u308B",
  "Switch video call": "\u30D3\u30C7\u30AA\u901A\u8A71\u306B\u5207\u308A\u66FF\u3048\u308B",
  "Call duration": "\u901A\u8A71\u6642\u9593",
  "Call Cancel": "\u901A\u8A71\u3092\u30AD\u30E3\u30F3\u30BB\u30EB\u3059\u308B",
  "Other Side Cancel": "\u76F8\u624B\u304C\u30AD\u30E3\u30F3\u30BB\u30EB\u3057\u307E\u3057\u305F",
  Decline: "\u901A\u8A71\u3092\u62D2\u5426\u3059\u308B",
  "Other Side Decline": "\u76F8\u624B\u304C\u62D2\u5426\u3057\u307E\u3057\u305F",
  "No answer": "\u7121\u5FDC\u7B54",
  "Other Side No Answer": "\u76F8\u624B\u304B\u3089\u306E\u8FD4\u7B54\u306F\u3042\u308A\u307E\u305B\u3093",
  Answered: "\u5FDC\u7B54\u3057\u307E\u3057\u305F",
  "Other Side Line Busy": "\u76F8\u624B\u306F\u73FE\u5728\u5FD9\u3057\u304F\u3066\u5BFE\u5FDC\u3067\u304D\u307E\u305B\u3093",
  "Line Busy": "\u5FD9\u3057\u3044\u306E\u3067\u5FDC\u7B54\u3067\u304D\u307E\u305B\u3093",
  "open camera": "\u30AA\u30FC\u30D7\u30F3\u30AB\u30E1\u30E9",
  "close camera": "\u30AB\u30E1\u30E9\u3092\u9589\u3058\u308B",
  "open microphone": "\u30AA\u30FC\u30D7\u30F3\u30DE\u30A4\u30AF",
  "close microphone": "\u30DE\u30A4\u30AF\u3092\u9589\u3058\u308B",
  "camera enabled": "\u30AB\u30E1\u30E9\u30AA\u30F3",
  "camera disabled": "\u30AB\u30E1\u30E9\u30AA\u30D5",
  "microphone enabled": "\u30DE\u30A4\u30AF\u30AA\u30F3",
  "microphone disabled": "\u30DE\u30A4\u30AF\u30AA\u30D5",
  "speaker enabled": "\u30B9\u30D4\u30FC\u30AB\u30FC\u30AA\u30F3",
  "speaker disabled": "\u30B9\u30D4\u30FC\u30AB\u30FC\u30AA\u30D5",
  "open speaker": "\u30B9\u30D4\u30FC\u30AB\u30FC\u3092\u30AA\u30F3\u306B\u3059\u308B",
  "close speaker": "\u30B9\u30D4\u30FC\u30AB\u30FC\u306E\u96FB\u6E90\u3092\u5207\u308A\u307E\u3059",
  "wait to be called": "\u5F85\u6A5F\u4E2D",
  answered: "\u63A5\u7D9A\u6E08\u307F",
  "people in the call": "\u901A\u8A71\u306B\u53C2\u52A0\u3057\u3066\u3044\u308B\u4EBA\u305F\u3061",
  "failed to obtain speakers": "\u30B9\u30D4\u30FC\u30AB\u30FC\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093",
  "you have a new call": "\u65B0\u3057\u3044\u901A\u8A71\u304C\u3042\u308A\u307E\u3059",
  "switch camera": "\u5207\u308A\u66FF\u3048",
  join: "\u53C2\u52A0\u3059\u308B",
  "people on the call": "\u4EBA\u304C\u901A\u8A71\u4E2D\u3067\u3059",
  "Supports a maximum of 9 people for simultaneous calls": "\u6700\u5927\u30679\u4EBA\u307E\u3067\u540C\u6642\u901A\u8A71\u304C\u53EF\u80FD\u3067\u3059",
  you: "(\u3042\u306A\u305F)",
  "The network is poor during your current call": "\u73FE\u5728\u306E\u901A\u8A71\u3067\u3001\u3042\u306A\u305F\u306E\u30CD\u30C3\u30C8\u30EF\u30FC\u30AF\u306F\u4E0D\u826F\u3067\u3059",
  "The other user network is poor during the current call": "\u73FE\u5728\u306E\u901A\u8A71\u3067\u3001\u76F8\u624B\u5074\u306E\u30CD\u30C3\u30C8\u30EF\u30FC\u30AF\u304C\u4E0D\u826F\u3067\u3059",
  "TUICallKit init is not complete": "TUICallKit\u306E\u521D\u671F\u5316\u30ED\u30B0\u30A4\u30F3\u304C\u5B8C\u4E86\u3057\u3066\u3044\u307E\u305B\u3093\u3002init \u304C\u5B8C\u4E86\u3057\u305F\u5F8C\u306B\u3053\u306EAPI\u3092\u4F7F\u7528\u3059\u308B\u5FC5\u8981\u304C\u3042\u308A\u307E\u3059\u3002",
  timeout: "{{ userList }} \u30BF\u30A4\u30E0\u30A2\u30A6\u30C8",
  "kick out": "\u30AD\u30C3\u30AF\u30A2\u30A6\u30C8\u3055\u308C\u307E\u3057\u305F",
  "Invited group call": "\u30B0\u30EB\u30FC\u30D7\u901A\u8A71\u306B\u62DB\u5F85\u3055\u308C\u307E\u3057\u305F\u3002",
  "Those involved": "\u53C2\u52A0\u8005\uFF1A",
  call: "\u901A\u8A71",
  "video-call": "\u30D3\u30C7\u30AA\u901A\u8A71",
  "audio-call": "\u97F3\u58F0\u901A\u8A71",
  search: "\u691C\u7D22",
  "search-result": "\u691C\u7D22\u7D50\u679C",
  "Wechat scan right QR code": "WeChat\u3067\u53F3\u5074\u306B\u3042\u308BQR\u30B3\u30FC\u30C9\u3092\u8AAD\u307F\u53D6\u308A\u307E\u3059\u3002",
  "Use-phone-and-computer": "\u643A\u5E2F\u96FB\u8A71\u3068\u30B3\u30F3\u30D4\u30E5\u30FC\u30BF\u3092\u4F7F\u7528\u3057\u3066\u30D3\u30C7\u30AA\u901A\u8A71\u3092\u4F53\u9A13\u3057\u3066\u304F\u3060\u3055\u3044",
  "Scan the QR code above": "\u4E0A\u306EQR\u30B3\u30FC\u30C9\u3092\u8AAD\u307F\u53D6\u308A\u307E\u3059\u3002",
  "no-user": "\u30E6\u30FC\u30B6\u30FC\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3067\u3057\u305F",
  "member-not-added": "\u30E1\u30F3\u30D0\u30FC\u304C\u8FFD\u52A0\u3055\u308C\u3066\u3044\u307E\u305B\u3093",
  "not-login": "\u30ED\u30B0\u30A4\u30F3\u3057\u3066\u3044\u307E\u305B\u3093",
  "login-status-expire": "\u30ED\u30B0\u30A4\u30F3\u306E\u6709\u52B9\u671F\u9650\u304C\u904E\u304E\u3066\u3044\u307E\u3059\u3002\u30DA\u30FC\u30B8\u3092\u66F4\u65B0\u3057\u3066\u3082\u3046\u4E00\u5EA6\u304A\u8A66\u3057\u304F\u3060\u3055\u3044",
  "experience-multi-call": "\u8907\u6570\u4EBA\u3067\u540C\u6642\u306B\u97F3\u58F0\u901A\u8A71\u3067\u304D\u308B\u30B0\u30EB\u30FC\u30D7\u901A\u8A71\u6A5F\u80FD\u3092\u4F53\u9A13\u3059\u308B\u306B\u306F\u3001\u5168\u6A5F\u80FD\u306E\u30C7\u30E2\u3092\u30C0\u30A6\u30F3\u30ED\u30FC\u30C9\u3057\u3066\u304F\u3060\u3055\u3044",
  "not-support-multi-call": "\u30B0\u30EB\u30FC\u30D7\u901A\u8A71\u30A4\u30F3\u30BF\u30FC\u30D5\u30A7\u30A4\u30B9\u304C\u958B\u3044\u3066\u3044\u307E\u305B\u3093",
  "input-phone-userID": "\u643A\u5E2F\u96FB\u8A71\u756A\u53F7/\u30E6\u30FC\u30B6\u30FCID\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044",
  userID: "\u30E6\u30FC\u30B6\u30FCID",
  "already-enter": "\u3059\u3067\u306B\u901A\u8A71\u306B\u53C2\u52A0\u3057\u3066\u3044\u307E\u3059",
  waiting: "\u5FDC\u7B54\u3092\u5F85\u3063\u3066\u3044\u307E\u3059...",
  "camera-opened": "\u30AB\u30E1\u30E9\u304C\u30AA\u30F3\u306B\u306A\u3063\u3066\u3044\u307E\u3059",
  "camera-closed": "\u30AB\u30E1\u30E9\u304C\u30AA\u30D5\u306B\u306A\u3063\u3066\u3044\u307E\u3059",
  "microphone-opened": "\u30DE\u30A4\u30AF\u304C\u30AA\u30F3\u306B\u306A\u3063\u3066\u3044\u307E\u3059",
  "microphone-closed": "\u30DE\u30A4\u30AF\u304C\u30AA\u30D5\u306B\u306A\u3063\u3066\u3044\u307E\u3059",
  "image-resolution": "\u89E3\u50CF\u5EA6",
  "default-image-resolution": "\u30C7\u30D5\u30A9\u30EB\u30C8\u89E3\u50CF\u5EA6",
  "invited-person": "\u30E1\u30F3\u30D0\u30FC\u3092\u62DB\u5F85",
  "video-to-audio": "\u97F3\u58F0\u901A\u8A71\u306B\u5207\u308A\u66FF\u3048\u307E\u3059",
  me: "(\u81EA\u5206)",
  "be-rejected": "\u901A\u8A71\u304C\u62D2\u5426\u3055\u308C\u307E\u3057\u305F, ",
  "be-no-response": "\u5FDC\u7B54\u306A\u3057, ",
  "be-line-busy": "\u76F8\u624B\u304C\u901A\u8A71\u4E2D\u3067\u3059, ",
  "be-canceled": "\u76F8\u624B\u304C\u901A\u8A71\u3092\u30AD\u30E3\u30F3\u30BB\u30EB\u3057\u307E\u3057\u305F",
  "voice-call-end": "\u97F3\u58F0\u901A\u8A71\u304C\u7D42\u4E86\u3057\u307E\u3057\u305F",
  "video-call-end": "\u30D3\u30C7\u30AA\u901A\u8A71\u304C\u7D42\u4E86\u3057\u307E\u3057\u305F",
  "method-call-failed": "\u64CD\u4F5C\u306E\u540C\u671F\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
  "failed-to-obtain-permission": "\u6A29\u9650\u306E\u53D6\u5F97\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
  "environment-detection-failed": "\u74B0\u5883\u306E\u691C\u51FA\u306B\u5931\u6557\u3057\u307E\u3057\u305F",
  "switchToAudioCall-call-failed": "\u97F3\u58F0\u901A\u8A71\u306B\u5207\u308A\u66FF\u3048\u308B\u3053\u3068\u306F\u3067\u304D\u307E\u305B\u3093",
  "switchToVideoCall-call-failed": "\u30D3\u30C7\u30AA\u901A\u8A71\u306B\u5207\u308A\u66FF\u3048\u308B\u3053\u3068\u306F\u3067\u304D\u307E\u305B\u3093",
  "microphone-unavailable": "\u4F7F\u7528\u3067\u304D\u308B\u30DE\u30A4\u30AF\u304C\u3042\u308A\u307E\u305B\u3093",
  "camera-unavailable": "\u4F7F\u7528\u3067\u304D\u308B\u30AB\u30E1\u30E9\u304C\u3042\u308A\u307E\u305B\u3093",
  "ban-device": "\u30C7\u30D0\u30A4\u30B9\u3078\u306E\u30A2\u30AF\u30BB\u30B9\u304C\u62D2\u5426\u3055\u308C\u307E\u3057\u305F",
  "not-supported-webrtc": "\u73FE\u5728\u306E\u74B0\u5883\u306FWebRTC\u3092\u30B5\u30DD\u30FC\u30C8\u3057\u3066\u3044\u307E\u305B\u3093",
  "blacklist-user-tips": "\u30E6\u30FC\u30B6\u30FC\u306F\u30D6\u30E9\u30C3\u30AF\u30EA\u30B9\u30C8\u306B\u767B\u9332\u3055\u308C\u3001\u901A\u8A71\u304C\u958B\u59CB\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F",
  "is-already-calling": "TUICallKit \u306F\u3059\u3067\u306B\u901A\u8A71\u4E2D\u3067\u3059",
  "need-init": "TUICallKit\u3067\u901A\u8A71\u3092\u958B\u59CB\u3059\u308B\u524D\u306B\u3001TUICallKitServer.init() \u30E1\u30BD\u30C3\u30C9\u304C\u6B63\u5E38\u306B\u5B9F\u884C\u3055\u308C\u305F\u3053\u3068\u3092\u78BA\u8A8D\u3057\u3066\u304F\u3060\u3055\u3044\u3002",
  "can't call yourself": "\u81EA\u5206\u306B\u96FB\u8A71\u3092\u304B\u3051\u308B\u3053\u3068\u304C\u3067\u304D\u307E\u305B\u3093",
  "accept-error": "\u63A5\u7D9A\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F",
  "accept-device-error": "\u63A5\u7D9A\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F\u3002\u767A\u4FE1\u5074\u30C7\u30D0\u30A4\u30B9\u3092\u8A8D\u8A3C\u3067\u304D\u307E\u305B\u3093",
  "call-error": "\u901A\u8A71\u304C\u958B\u59CB\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F"
}, we = typeof wx < "u" && typeof wx.getSystemInfoSync == "function" && Boolean(wx.getSystemInfoSync().fontSizeSetting), Wt = typeof uni < "u" && typeof uni > "u", bt = we || Wt, qt = typeof uni < "u", Xt = function() {
  return (typeof uni < "u" || typeof window < "u") && !bt;
}(), Mn = function() {
  return we ? wx : qt ? uni : window;
}(), PA = Xt && window && window.navigator && window.navigator.userAgent || "", fn = /Android/i.test(PA), Nn = /(?:Windows Phone)/.test(PA), Ln = /(?:SymbianOS)/.test(PA), vn = /iPad/i.test(PA) || /iPhone/i.test(PA) || /iPod/i.test(PA), Qt = fn || Nn || Ln || vn, tt = Xt && !Qt, Sn = tt && PA.includes("Windows NT"), Dn = tt && PA.includes("Mac");
class kA {
  constructor() {
    this.global = Mn, this.isPC = !1, this.isH5 = !1, this.isWeChat = !1, this.isApp = !1, this.isUniPlatform = !1, this.isOfficial = !1, this.isWIN = !1, this.isMAC = !1, this.initEnv();
  }
  static getInstance() {
    return kA.instance || (kA.instance = new kA()), kA.instance;
  }
  initEnv() {
    this.isPC = tt, this.isH5 = Qt, this.isWeChat = we, this.isApp = Wt && !we, this.isUniPlatform = qt, this.isWIN = Sn, this.isMAC = Dn;
  }
  initOfficial(e) {
    this.isOfficial = e === 1400187352 || e === 1400188366;
  }
}
const Ue = function(a) {
  return typeof a === o.UNDEFINED;
}, at = function(a) {
  if (typeof a !== o.OBJECT || a === null)
    return !1;
  const e = Object.getPrototypeOf(a);
  if (e === null)
    return !0;
  let t = e;
  for (; Object.getPrototypeOf(t) !== null; )
    t = Object.getPrototypeOf(t);
  return e === t;
}, Zt = function(a) {
  return typeof Array.isArray === o.FUNCTION ? Array.isArray(a) : Object.prototype.toString.call(a).match(/^\[object (.*)\]$/)[1].toLowerCase() === o.ARRAY;
}, je = function(a) {
  return typeof a === o.STRING;
}, hn = function(a) {
  return typeof a === o.BOOLEAN;
}, Yt = function(a) {
  return a !== null && (typeof a === o.NUMBER && !isNaN(a - 0) || typeof a === o.OBJECT && a.constructor === Number);
};
function Kt(a) {
  const e = Math.floor(a / 3600), t = Math.floor(a % 3600 / 60), A = Math.floor(a % 60);
  let n = e > 9 ? `${e}` : `0${e}`;
  return n += t > 9 ? `:${t}` : `:0${t}`, n += A > 9 ? `:${A}` : `:0${A}`, n;
}
function mn(a) {
  if (typeof a === o.STRING)
    try {
      return !!JSON.parse(a);
    } catch (e) {
      return console.debug(e), !1;
    }
  return !1;
}
const fe = function(a) {
  return !a || !mn(a) ? a : JSON.parse(a);
};
function zt(a) {
  return (a == null ? void 0 : a.message.indexOf("is ongoing, please avoid repeated calls")) !== -1;
}
function yn(a) {
  const { message: e } = a;
  return e.indexOf("NotAllowedError: Permission denied") !== -1;
}
function Ge() {
  return Date.now();
}
const me = function(a) {
  return typeof a === o.FUNCTION;
}, Tn = () => {
  if (kA.getInstance().isWeChat)
    return "zh-cn";
  const a = ((navigator == null ? void 0 : navigator.language) || (navigator == null ? void 0 : navigator.userLanguage) || "").substr(0, 2);
  let e = "en";
  switch (a) {
    case "zh":
      e = "zh-cn";
      break;
    case "ja":
      e = "ja_JP";
      break;
    default:
      e = "en";
  }
  return e;
};
function En(a) {
}
const wn = function(a) {
  return Object.prototype.toString.call(a).match(/^\[object (.*)\]$/)[1].toLowerCase();
};
function jn(a, e, t) {
  if (!a.hasOwnProperty(e))
    return a;
  const A = {};
  return Object.keys(a).forEach((n) => {
    n === e ? A[t] = a[n] : A[n] = a[n];
  }), A;
}
function bn(a, e) {
  return a.replace(/{{\s*(\w+)(\s*,\s*[^}]+)?\s*}}/g, (t, A) => {
    const n = A.trim();
    return e[n] !== void 0 ? String(e[n]) : t;
  });
}
const iA = {
  OTHER_SIDE: "other side",
  CANCEL: "cancel",
  OTHER_SIDE_REJECT_CALL: "other side reject call",
  REJECT_CALL: "reject call",
  OTHER_SIDE_LINE_BUSY: "other side line busy",
  IN_BUSY: "in busy",
  CALL_TIMEOUT: "call timeout",
  END_CALL: "end call",
  TIMEOUT: "timeout",
  KICK_OUT: "kick out",
  CALLER_CALLING_MSG: "caller calling message",
  CALLER_GROUP_CALLING_MSG: "wait to be called",
  CALLEE_CALLING_VIDEO_MSG: "callee calling video message",
  CALLEE_CALLING_AUDIO_MSG: "callee calling audio message",
  NO_MICROPHONE_DEVICE_PERMISSION: "no microphone access",
  NO_CAMERA_DEVICE_PERMISSION: "no camera access",
  EXIST_GROUP_CALL: "exist group call",
  LOCAL_NETWORK_IS_POOR: "The network is poor during your current call",
  REMOTE_NETWORK_IS_POOR: "The other user network is poor during the current call"
}, YA = {
  "zh-cn": In,
  en: pn,
  ja_JP: Cn
};
function qA(a) {
  var A, n;
  const e = d.getData(u.CALL, o.LANGUAGE);
  let t = "";
  if (je(a))
    t = ((A = YA == null ? void 0 : YA[e]) == null ? void 0 : A[a]) || "";
  else if (at(a)) {
    const { key: s, options: l } = a;
    t = ((n = YA == null ? void 0 : YA[e]) == null ? void 0 : n[s]) || "", t = bn(t, l);
  }
  return t;
}
const zn = "data:audio/mpeg;base64,", Ut = "data:audio/mpeg;base64,";
class Un {
  constructor() {
    this._bellContext = null, this._isMuteBell = !1, this._calleeBellFilePath = Ut, this._callRole = J.UNKNOWN, this._callStatus = w.IDLE, this._bellContext = new Audio(), this._bellContext.loop = !0;
  }
  setBellSrc() {
    try {
      let e = zn;
      this._callRole === J.CALLEE && (e = this._calleeBellFilePath || Ut), this._bellContext.src = e;
    } catch (e) {
      console.warn(`${o.PREFIX}Failed to setBellSrc, ${e}`);
    }
  }
  setBellProperties(e) {
    this._callRole = e.callRole || this._callRole, this._callStatus = e.callStatus || this._callStatus, this._calleeBellFilePath = e.calleeBellFilePath || this._calleeBellFilePath, this._isMuteBell = Ue(e.isMuteBell) ? this._isMuteBell : e.isMuteBell;
  }
  async play() {
    try {
      if (this._callStatus !== w.CALLING)
        return;
      this.setBellSrc(), this._callRole === J.CALLEE && !this._isMuteBell && await this._bellContext.play(), this._callRole === J.CALLER && await this._bellContext.play();
    } catch (e) {
      console.warn(`${o.PREFIX}Failed to play audio file, ${e}`);
    }
  }
  async stop() {
    try {
      await this._bellContext.pause();
    } catch (e) {
      console.warn(`${o.PREFIX}Failed to stop audio file, ${e}`);
    }
  }
  async setBellMute(e) {
    this._callStatus !== w.CALLING && this._callRole !== J.CALLEE || (e ? await this.stop() : await this.play());
  }
  destroy() {
    try {
      this._isMuteBell = !1, this._calleeBellFilePath = "", this._callRole = J.UNKNOWN, this._callStatus = w.IDLE, this._bellContext.pause(), this._bellContext = null;
    } catch (e) {
      console.warn(`${o.PREFIX}Failed to destroy, ${e}`);
    }
  }
}
function H() {
  return function(a, e, t) {
    const A = t.value, n = /* @__PURE__ */ new Set();
    return t.value = async function(...s) {
      var l, r;
      if (n.has(this)) {
        console.warn(`${o.PREFIX}previous ${e}() is ongoing, please avoid repeated calls`), (r = (l = this == null ? void 0 : this.getTUICallEngineInstance()) == null ? void 0 : l.reportLog) == null || r.call(l, {
          name: "TUICallKit.avoidRepeatedCall.fail",
          data: { name: e },
          error: `previous ${e}() is ongoing`
        });
        return;
      }
      try {
        n.add(this);
        const i = await A.apply(this, s);
        return n.delete(this), i;
      } catch (i) {
        throw n.delete(this), i;
      }
    }, t;
  };
}
const ne = o.PREFIX + "API";
function gA(a) {
  return function(e, t, A) {
    let n = A.value;
    return A.value = function(...s) {
      return On.call(this, a, s, t), n.apply(this, s);
    }, A;
  };
}
function On(a, e, t) {
  try {
    if (e[0].SDKAppID || (a = jn(a, "SDKAppID", "sdkAppID")), Zt(a))
      for (let A = 0; A < a.length; A++)
        Ot.call(this, {
          ...a[A],
          value: e[A],
          name: t
        });
    else
      for (const A in a)
        a.hasOwnProperty(A) && Ot.call(this, {
          ...a[A],
          value: e[0][A],
          name: t,
          key: A
        });
  } catch (A) {
    throw console.error(A), A;
  }
}
function Ot({ required: a, rules: e, range: t, value: A, allowEmpty: n, name: s, key: l }) {
  if (Ue(A)) {
    if (a)
      throw new Error(`${ne}<${s}>: ${l} is required.`);
    return;
  }
  const r = e.some((c) => c === wn(A));
  let i = "";
  if (!r) {
    for (let c = 0; c < e.length; c++) {
      let g = e[c];
      g = g.replace(g[0], g[0].toUpperCase()), i += `${g}/`;
    }
    throw i = i.substring(0, i.length - 1), new Error(`${ne}<${s}>: ${l} must be ${i}, current ${l} is ${typeof A}.`);
  }
  if (n === !1 && je(A) && A.trim() === "")
    throw new Error(`${ne}<${s}>: ${l} is blank.`);
  if (Zt(t) && t && t.indexOf(A) === -1)
    throw new Error(`${ne}<${s}>: ${l} error, only be ${t}, current ${l} is ${A}.`);
  if (je(t) && t.indexOf("~") !== -1) {
    const c = t.split("~");
    if (A < +c[0] || A > +c[1] || Yt(A) && Number.isNaN(A))
      throw new Error(`${ne}<${s}>: ${l} error, only be ${t}, current ${l} is ${A}.`);
  }
}
const IA = {
  init: {
    SDKAppID: {
      required: !0,
      rules: [o.NUMBER],
      allowEmpty: !1
    },
    userID: {
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    },
    userSig: {
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    },
    tim: {
      required: !1,
      rules: [o.OBJECT]
    }
  },
  call: {
    userID: {
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    },
    type: {
      required: !0,
      rules: [o.NUMBER],
      range: [1, 2],
      allowEmpty: !1
    },
    roomID: {
      required: !1,
      rules: [o.NUMBER],
      range: `0~${jt}`,
      allowEmpty: !1
    },
    strRoomID: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !0
    },
    userData: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !1
    },
    timeout: {
      required: !1,
      rules: [o.NUMBER],
      allowEmpty: !1
    }
  },
  groupCall: {
    userIDList: {
      required: !0,
      rules: [o.ARRAY],
      allowEmpty: !1
    },
    type: {
      required: !0,
      rules: [o.NUMBER],
      range: [1, 2],
      allowEmpty: !1
    },
    groupID: {
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    },
    roomID: {
      required: !1,
      rules: [o.NUMBER],
      range: `0~${jt}`,
      allowEmpty: !1
    },
    strRoomID: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !0
    },
    timeout: {
      required: !1,
      rules: [o.NUMBER],
      allowEmpty: !1
    },
    userData: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !1
    },
    offlinePushInfo: {
      required: !1,
      rules: [o.OBJECT],
      allowEmpty: !1
    }
  },
  joinInGroupCall: {
    type: {
      required: !0,
      rules: [o.NUMBER],
      range: [1, 2],
      allowEmpty: !1
    },
    groupID: {
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    },
    roomID: {
      required: !0,
      rules: [o.NUMBER],
      allowEmpty: !1
    },
    strRoomID: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !0
    }
  },
  inviteUser: {
    userIDList: {
      required: !0,
      rules: [o.ARRAY],
      allowEmpty: !1
    }
  },
  setSelfInfo: {
    nickName: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !1
    },
    avatar: {
      required: !1,
      rules: [o.STRING],
      allowEmpty: !1
    }
  },
  enableFloatWindow: [
    {
      key: "enable",
      required: !1,
      rules: [o.BOOLEAN],
      allowEmpty: !1
    }
  ],
  enableAIVoice: [
    {
      key: "enable",
      required: !0,
      rules: [o.BOOLEAN],
      allowEmpty: !1
    }
  ],
  enableMuteMode: [
    {
      key: "enable",
      required: !0,
      rules: [o.BOOLEAN],
      allowEmpty: !1
    }
  ],
  setCallingBell: [
    {
      key: "filePath",
      required: !1,
      rules: [o.STRING],
      allowEmpty: !0
    }
  ],
  setLanguage: [
    {
      key: "language",
      required: !0,
      rules: [o.STRING],
      allowEmpty: !1
    }
  ],
  setVideoDisplayMode: [
    {
      key: "displayMode",
      required: !0,
      rules: [o.STRING],
      range: [ue.CONTAIN, ue.COVER, ue.FILL],
      allowEmpty: !1
    }
  ],
  setVideoResolution: [
    {
      key: "resolution",
      required: !0,
      rules: [o.STRING],
      range: [re.RESOLUTION_1080P, re.RESOLUTION_480P, re.RESOLUTION_720P],
      allowEmpty: !1
    }
  ]
};
function Ae(a) {
  return function(e, t, A) {
    let n = A.value;
    return A.value = function(...s) {
      return xn.call(this, a, s, t), n.apply(this, s);
    }, A;
  };
}
function xn(a) {
  if ((a == null ? void 0 : a.engineInstance) && !this._tuiCallEngine) {
    const e = `${o.PREFIX} ${qA("TUICallKit init is not complete")}`;
    throw console.error(e), e;
  }
}
class Bn {
  constructor() {
    this.defaultStore = {
      callStatus: w.IDLE,
      callRole: J.UNKNOWN,
      callMediaType: _.UNKNOWN,
      localUserInfo: { userId: "" },
      localUserInfoExcludeVolume: { userId: "" },
      remoteUserInfoList: [],
      remoteUserInfoExcludeVolumeList: [],
      callerUserInfo: { userId: "" },
      isGroup: !1,
      callDuration: "00:00:00",
      callTips: "",
      toastInfo: { text: "" },
      isMinimized: !1,
      enableFloatWindow: !1,
      bigScreenUserId: "",
      language: Tn(),
      isClickable: !1,
      deviceList: { cameraList: [], microphoneList: [], currentCamera: {}, currentMicrophone: {} },
      showPermissionTip: !1,
      netWorkQualityList: [],
      isMuteSpeaker: !1,
      groupID: "",
      roomID: 0,
      roomIdType: 0,
      cameraPosition: FA.FRONT,
      groupCallMembers: [],
      displayMode: ue.COVER,
      videoResolution: re.RESOLUTION_480P,
      showSelectUser: !1,
      pusher: {},
      player: [],
      isEarPhone: !1,
      pusherId: o.INITIAL_PUSHER,
      isShowEnableVirtualBackground: !1,
      enableVirtualBackground: !1,
      customUIConfig: {
        button: {},
        viewBackground: {},
        layoutMode: Te.RemoteInLargeView
      },
      translate: qA
    }, this.store = Ee(this.defaultStore), this.prevStore = Ee(this.defaultStore);
  }
  update(e, t) {
    switch (e) {
      case o.CALL_TIPS:
        const A = this.getData(e);
        this.prevStore[e] = A;
      default:
        this.store[e] = t;
    }
  }
  getPrevData(e) {
    return e ? this.prevStore[e] : this.prevStore;
  }
  getData(e) {
    return e ? this.store[e] : this.store;
  }
  reset(e = []) {
    e.length === 0 && (e = Object.keys(this.store));
    const t = e.reduce((A, n) => ({ ...A, [n]: this.defaultStore[n] }), {});
    this.store = {
      ...this.defaultStore,
      ...this.store,
      ...t
    };
  }
}
class DA {
  constructor() {
    this.timerId = -1, this.storeMap = {
      [u.CALL]: new Bn()
    }, this.task = {};
  }
  static getInstance() {
    return DA.instance || (DA.instance = new DA()), DA.instance;
  }
  watch(e, t, A) {
    this.task[e] || (this.task[e] = {});
    const n = this.task[e];
    Object.keys(t).forEach((s) => {
      const l = t[s];
      n[s] || (n[s] = /* @__PURE__ */ new Map()), n[s].set(l, 1);
      const { notifyRangeWhenWatch: r } = A || {};
      if (r === o.ALL && this.notify(e, s), r === o.MYSELF) {
        const i = this.getData(e, s);
        l.call(this, i);
      }
    });
  }
  unwatch(e, t) {
    if (!this.task[e])
      return;
    const A = this.task[e];
    Object.keys(t).forEach((n) => {
      A[n].delete(t[n]);
    });
  }
  update(e, t, A) {
    var n;
    (je(A) || Yt(A) || hn(A)) && this.storeMap[e].store[t] === A || ((n = this.storeMap[e]) == null || n.update(t, A), this.notify(e, t));
  }
  getPrevData(e, t) {
    var A;
    return (A = this.storeMap[e]) == null ? void 0 : A.getPrevData(t);
  }
  getData(e, t) {
    var A;
    return (A = this.storeMap[e]) == null ? void 0 : A.getData(t);
  }
  notify(e, t) {
    if (!this.task[e])
      return;
    const A = this.task[e];
    if (A[t]) {
      const n = A[t], s = this.getData(e, t);
      for (const [l] of n.entries())
        l.call(this, s);
    }
  }
  reset(e, t = [], A = !1) {
    if (e in this.storeMap) {
      const n = this.storeMap[e];
      t.length === 0 && (t = Object.keys(n == null ? void 0 : n.store)), n.reset(t), A && t.forEach((s) => {
        this.notify(e, s);
      });
    }
  }
  updateStore(e, t) {
    const A = t || u.CALL;
    Object.keys(e).forEach((n) => {
      this.update(A, n, e[n]);
    });
  }
}
const q = DA.getInstance();
function Ye(a, e) {
  const t = {
    userId: a,
    nick: "",
    avatar: "",
    remark: "",
    displayUserInfo: "",
    isAudioAvailable: !1,
    isVideoAvailable: !1,
    isEnter: !1,
    domId: e || a
  };
  return e ? t : { ...t, isEnter: !1 };
}
async function Pn(a, e) {
  var A, n, s, l, r;
  let t = Ye(a, o.LOCAL_VIDEO);
  try {
    if (!e)
      return t;
    const i = await e.getMyProfile(), c = q == null ? void 0 : q.getData(u.CALL, o.LOCAL_USER_INFO);
    return (i == null ? void 0 : i.code) === 0 && (t = {
      ...t,
      ...c,
      userId: (A = i == null ? void 0 : i.data) == null ? void 0 : A.userID,
      nick: (n = i == null ? void 0 : i.data) == null ? void 0 : n.nick,
      avatar: (s = i == null ? void 0 : i.data) == null ? void 0 : s.avatar,
      displayUserInfo: ((l = i == null ? void 0 : i.data) == null ? void 0 : l.nick) || ((r = i == null ? void 0 : i.data) == null ? void 0 : r.userID)
    }), t;
  } catch (i) {
    return console.error(`${o.PREFIX}getMyProfile failed, error: ${i}.`), t;
  }
}
async function ce(a, e) {
  let t = a.map((A) => Ye(A));
  try {
    if (!e)
      return t;
    const A = await e.getFriendProfile({ userIDList: a });
    if (A.code === 0) {
      const { friendList: n = [], failureUserIDList: s = [] } = A.data;
      let l = s.map((g) => g.userID);
      if (s.length > 0) {
        const g = await e.getUserProfile({ userIDList: s.map((f) => f.userID) });
        (g == null ? void 0 : g.code) === 0 && (l = (g == null ? void 0 : g.data) || []);
      }
      const r = q == null ? void 0 : q.getData(u.CALL, o.REMOTE_USER_INFO_LIST), i = n.map((g) => g.userID), c = l.map((g) => g.userID);
      t = a.map((g) => {
        var T, j, F, Z, V, B, W;
        const f = Ye(g), p = i.indexOf(g), C = c.indexOf(g);
        let N = "", h = "", L = "", D = "";
        p !== -1 && (N = ((T = n[p]) == null ? void 0 : T.remark) || "", h = ((F = (j = n[p]) == null ? void 0 : j.profile) == null ? void 0 : F.nick) || "", L = N || h || f.userId || "", D = ((V = (Z = n[p]) == null ? void 0 : Z.profile) == null ? void 0 : V.avatar) || ""), C !== -1 && (h = ((B = l[C]) == null ? void 0 : B.nick) || "", L = h || f.userId || "", D = ((W = l[C]) == null ? void 0 : W.avatar) || "");
        const z = r.find((vA) => vA.userId === g) || {};
        return { ...f, ...z, remark: N, nick: h, displayUserInfo: L, avatar: D };
      });
    }
    return t;
  } catch (A) {
    return console.error(`${o.PREFIX}getRemoteUserProfile failed, error: ${A}.`), t;
  }
}
function Ne() {
  const a = q.getData(u.CALL, o.CALL_STATUS);
  if (a === w.IDLE)
    return nA.IDLE;
  const e = q.getData(u.CALL, o.IS_GROUP);
  if (a === w.CALLING)
    return e ? nA.DIALING_GROUP : nA.DIALING_C2C;
  const t = q.getData(u.CALL, o.CALL_MEDIA_TYPE);
  return e ? t === _.AUDIO ? nA.CALLING_GROUP_AUDIO : nA.CALLING_GROUP_VIDEO : t === _.AUDIO ? nA.CALLING_C2C_AUDIO : nA.CALLING_C2C_VIDEO;
}
async function Vn(a, e, t, A) {
  let n = [];
  try {
    const s = await e.getGroupMemberList({ groupID: a, count: t, offset: A });
    if (s.code === 0)
      return s.data.memberList || n;
  } catch (s) {
    return console.error(`${o.PREFIX}getGroupMember failed, error: ${s}.`), n;
  }
}
async function _n(a, e) {
  let t = {};
  try {
    return (await e.getGroupProfile({ groupID: a })).data.group || t;
  } catch (A) {
    return console.warn(`${o.PREFIX}getGroupProfile failed, error: ${A}.`), t;
  }
}
function Ht(a, e) {
  a === 0 && e ? (q.update(u.CALL, o.ROOM_ID, e), q.update(u.CALL, o.ROOM_ID_TYPE, Ze.STRING_ROOM_ID)) : (q.update(u.CALL, o.ROOM_ID, a), q.update(u.CALL, o.ROOM_ID_TYPE, Ze.NUMBER_ROOM_ID));
}
function fA(a) {
  return a || {};
}
function ke(a) {
  if (a.length === 0)
    return;
  let e = q.getData(u.CALL, o.REMOTE_USER_INFO_LIST);
  a.forEach((t) => {
    e = e.filter((A) => A.userId !== t);
  }), q.update(u.CALL, o.REMOTE_USER_INFO_LIST, e), q.update(u.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, e);
}
function Le(a) {
  a == null || a.getDeviceList("speaker").then((t) => {
    const A = q.getData(u.CALL, o.DEVICE_LIST), n = (t == null ? void 0 : t[0]) || {};
    q.update(
      u.CALL,
      o.DEVICE_LIST,
      { ...A, speakerList: t, currentSpeaker: n }
    );
  }).catch((t) => {
    console.error(`${o.PREFIX}updateSpeakerList failed, error: ${JSON.stringify(t)}.`);
  }), q.getData(u.CALL, o.CALL_MEDIA_TYPE) === _.VIDEO && (a == null || a.getDeviceList("camera").then((t) => {
    const A = q.getData(u.CALL, o.DEVICE_LIST), n = (t == null ? void 0 : t[0]) || {};
    q.update(
      u.CALL,
      o.DEVICE_LIST,
      { ...A, cameraList: t, currentCamera: n }
    );
  }).catch((t) => {
    console.error(`${o.PREFIX}updateCameraList failed, error: ${t}.`);
  })), a == null || a.getDeviceList("microphones").then((t) => {
    const A = q.getData(u.CALL, o.DEVICE_LIST), n = (t == null ? void 0 : t[0]) || {};
    q.update(
      u.CALL,
      o.DEVICE_LIST,
      { ...A, microphoneList: t, currentMicrophone: n }
    );
  }).catch((t) => {
    console.error(`${o.PREFIX}updateMicrophoneList failed, error: ${t}.`);
  });
}
function We(a, e, t) {
  let A = "";
  if (yn(a)) {
    e === _.AUDIO && (A = iA.NO_MICROPHONE_DEVICE_PERMISSION), e === _.VIDEO && (A = iA.NO_CAMERA_DEVICE_PERMISSION);
    const n = t == null ? void 0 : t.getDevicePermission();
    n != null && n.audio || q.update(u.CALL, o.SHOW_PERMISSION_TIP, !0), A && q.update(u.CALL, o.TOAST_INFO, { content: A, type: o.ERROR }), console.error(`${o.PREFIX}call failed, error: ${a.message}.`);
  }
}
function jA(a, e) {
  let t = q.getData(u.CALL, o.LOCAL_USER_INFO);
  e === o.AUDIO && (t = { ...t, isAudioAvailable: a }), e === o.VIDEO && (t = { ...t, isVideoAvailable: a }), q.update(u.CALL, o.LOCAL_USER_INFO, t), q.update(u.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, t);
}
class be {
  static generateTaskID() {
    return this.currentTaskID++;
  }
  static run(e = o.TIMEOUT, t, A) {
    e === o.INTERVAL ? A = { delay: 2e3, count: 0, backgroundTask: !0, ...A } : A = { delay: 2e3, count: 0, backgroundTask: !0, ...A }, at(t) && (A = { ...A, ...t }), me(e) && (t = e, e = o.TIMEOUT);
    const n = {
      taskID: this.generateTaskID(),
      loopCount: 0,
      intervalID: null,
      timeoutID: null,
      taskName: e,
      callback: t,
      ...A
    };
    return this.taskMap.set(n.taskID, n), e === o.INTERNAL ? this.interval(n) : this.timeout(n), n.taskID;
  }
  static interval(e) {
    const t = () => {
      e.callback(), e.loopCount += 1, this.isBreakLoop(e);
    };
    return e.intervalID = setInterval(t, e.delay);
  }
  static timeout(e) {
    const t = () => {
      if (e.callback(), e.loopCount += 1, !this.isBreakLoop(e))
        return e.timeoutID = setTimeout(t, e.delay);
    };
    return e.timeoutID = setTimeout(t, e.delay);
  }
  static hasTask(e) {
    return this.taskMap.has(e);
  }
  static clearTask(e) {
    if (!this.taskMap.has(e))
      return !0;
    const { intervalID: t, timeoutID: A, onVisibilitychange: n } = this.taskMap.get(e);
    return t && clearInterval(t), A && clearTimeout(A), n && document.removeEventListener("visibilitychange", n), this.taskMap.delete(e), !0;
  }
  static isBreakLoop(e) {
    return this.taskMap.has(e.taskID) ? e.count !== 0 && e.loopCount >= e.count ? (this.clearTask(e.taskID), !0) : !1 : !0;
  }
}
be.taskMap = /* @__PURE__ */ new Map();
be.currentTaskID = 1;
const Fn = function(a) {
  if (a === null || typeof a > "u")
    return !0;
  if (typeof a == "boolean")
    return !1;
  if (typeof a == "number")
    return a === 0;
  if (typeof a == "string" || typeof a == "function" || Array.isArray(a))
    return a.length === 0;
  if (a instanceof Error)
    return a.message === "";
  if (at(a)) {
    for (const e in a)
      if (Object.prototype.hasOwnProperty.call(a, e))
        return !1;
    return !0;
  }
  return !1;
}, qe = "_local_user_id";
class WA {
  constructor() {
    this._viewConfig = {
      viewBackground: {
        local: {},
        remote: {}
      }
    }, this._isSetViewBackgroundConfig = { remote: !1, local: !1 }, this._tuiCallEngine = null, this._tuiStore = null;
  }
  static getInstance() {
    return WA.instance || (WA.instance = new WA()), WA.instance;
  }
  _updateViewBackground() {
    var A, n, s;
    const e = (A = this._tuiStore) == null ? void 0 : A.getData(u.CALL, o.CUSTOM_UI_CONFIG), { userId: t } = (n = this._tuiStore) == null ? void 0 : n.getData(u.CALL, o.LOCAL_USER_INFO);
    Object.keys(this._viewConfig.viewBackground.remote).includes(t) && delete this._viewConfig.viewBackground.remote[t], (s = this._tuiStore) == null || s.update(
      u.CALL,
      o.CUSTOM_UI_CONFIG,
      {
        ...e,
        viewBackground: {
          ...this._viewConfig.viewBackground.remote,
          ...this._viewConfig.viewBackground.local
        }
      }
    );
  }
  setEngineInstance(e) {
    this._tuiCallEngine = e;
  }
  setTUIStore(e) {
    this._tuiStore = e;
  }
  updateViewBackgroundUserId(e) {
    var t, A;
    if (e === "local") {
      const { userId: n } = (t = this._tuiStore) == null ? void 0 : t.getData(u.CALL, o.LOCAL_USER_INFO);
      if (Object.keys(this._viewConfig.viewBackground.remote).includes(n) && (delete this._viewConfig.viewBackground.remote[n], this._updateViewBackground()), !this._isSetViewBackgroundConfig.local)
        return;
      const s = this._viewConfig.viewBackground.local, l = s[n] || s[qe];
      s[n] = s[qe], this._viewConfig.viewBackground.local = { [n]: l }, this._updateViewBackground();
    } else {
      let n = this._viewConfig.viewBackground.remote;
      this._isSetViewBackgroundConfig.remote && Object.keys(n).includes("*") && (((A = this._tuiStore) == null ? void 0 : A.getData(u.CALL, o.REMOTE_USER_INFO_LIST)).map((r) => r.userId).forEach((r) => {
        Object.keys(n).includes(r) || (n[r] = n["*"]);
      }), this._viewConfig.viewBackground.remote = n, this._updateViewBackground());
    }
  }
  hideFeatureButton(e) {
    var A, n, s, l, r;
    (n = (A = this._tuiCallEngine) == null ? void 0 : A.reportLog) == null || n.call(A, {
      name: "TUICallKit.hideFeatureButton.start",
      data: { buttonName: e }
    });
    const t = (s = this._tuiStore) == null ? void 0 : s.getData(u.CALL, o.CUSTOM_UI_CONFIG);
    (r = this._tuiStore) == null || r.update(
      u.CALL,
      o.CUSTOM_UI_CONFIG,
      {
        ...t,
        button: {
          ...t.button,
          [e]: { ...((l = t.button) == null ? void 0 : l[e]) || {}, show: !1 }
        }
      }
    );
  }
  setLocalViewBackgroundImage(e) {
    var A, n, s;
    (n = (A = this._tuiCallEngine) == null ? void 0 : A.reportLog) == null || n.call(A, {
      name: "TUICallKit.setLocalViewBackgroundImage.start",
      data: { url: e }
    }), this._isSetViewBackgroundConfig.local = !0;
    let { userId: t } = (s = this._tuiStore) == null ? void 0 : s.getData(u.CALL, o.LOCAL_USER_INFO);
    Fn(t) && (t = qe), this._viewConfig.viewBackground.local = { [t]: e }, this._updateViewBackground();
  }
  setRemoteViewBackgroundImage(e, t) {
    var A, n;
    (n = (A = this._tuiCallEngine) == null ? void 0 : A.reportLog) == null || n.call(A, {
      name: "TUICallKit.setRemoteViewBackgroundImage.start",
      data: { userId: e, url: t }
    }), this._isSetViewBackgroundConfig.remote = !0, e === "*" && (this._viewConfig.viewBackground.remote = {}), this._viewConfig.viewBackground.remote[e] = t, this._updateViewBackground();
  }
  setLayoutMode(e) {
    var A, n;
    (n = (A = this._tuiCallEngine) == null ? void 0 : A.reportLog) == null || n.call(A, {
      name: "TUICallKit.setLayoutMode.start",
      data: { layoutMode: e }
    });
    const t = this._tuiStore.getData(u.CALL, o.CUSTOM_UI_CONFIG);
    this._tuiStore.update(
      u.CALL,
      o.CUSTOM_UI_CONFIG,
      {
        ...t,
        layoutMode: e
      }
    );
  }
  setCameraDefaultState(e) {
    var A, n;
    (n = (A = this._tuiCallEngine) == null ? void 0 : A.reportLog) == null || n.call(A, {
      name: "TUICallKit.setCameraDefaultState.start",
      data: { isOpen: e }
    });
    const t = Ee(this._tuiStore.getData(u.CALL, o.CUSTOM_UI_CONFIG));
    Object.keys(t.button).includes(SA.Camera) || (t.button[SA.Camera] = {}), t.button[SA.Camera].state = e ? GA.Open : GA.Close, this._tuiStore.update(u.CALL, o.CUSTOM_UI_CONFIG, t);
  }
}
const ve = DA.getInstance(), NA = {
  audioCall: () => "Voice call",
  videoCall: () => "Video call",
  switchToAudio: () => "Switch audio call",
  switchToVideo: () => "Switch video call",
  hangup: ({ callDuration: a }) => `${qA("Call duration")}\uFF1A${a}`
};
class JA {
  constructor(e) {
    var t, A, n;
    this._callService = e.callService, ae.registerEvent(AA.TUILogin.EVENT.LOGIN_STATE_CHANGED, AA.TUILogin.EVENT_SUB_KEY.USER_LOGIN_SUCCESS, this), (t = AA.TUIChat) != null && t.EVENT && ae.registerEvent((A = AA.TUIChat.EVENT) == null ? void 0 : A.CHAT_STATE_CHANGED, (n = AA.TUIChat.EVENT_SUB_KEY) == null ? void 0 : n.CHAT_OPENED, this), ae.registerService(AA.TUICalling.SERVICE.NAME, this), ae.registerExtension(AA.TUIChat.EXTENSION.INPUT_MORE.EXT_ID, this);
  }
  static getInstance(e) {
    return JA.instance || (JA.instance = new JA(e)), JA.instance;
  }
  callTUIService(e) {
    const { message: t } = e || {};
    ae.callService({
      serviceName: AA.TUIChat.SERVICE.NAME,
      method: AA.TUIChat.SERVICE.METHOD.UPDATE_MESSAGE_LIST,
      params: { message: t }
    });
  }
  onGetExtension(e, t) {
    var A, n;
    if (e === AA.TUIChat.EXTENSION.INPUT_MORE.EXT_ID) {
      if ((n = (A = this._callService.getTUICallEngineInstance()) == null ? void 0 : A.reportLog) == null || n.call(A, { name: "TUICallKit.onGetExtension", data: { extensionID: e, params: t } }), Ue(t))
        return [];
      if ([AA.TUIChat.TYPE.ROOM, AA.TUIChat.TYPE.CUSTOMER_SERVICE].includes(t.chatType))
        return [];
      let s = [];
      const l = {
        weight: 1e3,
        text: "\u8BED\u97F3\u901A\u8BDD",
        icon: un,
        data: {
          name: "voiceCall"
        },
        listener: {
          onClicked: async (i) => await this._handleTUICoreOnClick(i, i.type || _.AUDIO)
        }
      }, r = {
        weight: 900,
        text: "\u89C6\u9891\u901A\u8BDD",
        icon: rn,
        data: {
          name: "videoCall"
        },
        listener: {
          onClicked: async (i) => await this._handleTUICoreOnClick(i, i.type || _.VIDEO)
        }
      };
      return t != null && t.chatType ? s = [l, r] : (!(t != null && t.filterVoice) && s.push(l), !(t != null && t.filterVideo) && s.push(r)), s;
    }
  }
  async onCall(e, t) {
    e === AA.TUICalling.SERVICE.METHOD.START_CALL && await this._handleTUICoreOnClick(t, t.type);
  }
  async onNotifyEvent(e, t, A) {
    var n, s, l, r, i, c, g, f;
    try {
      if (e === AA.TUILogin.EVENT.LOGIN_STATE_CHANGED)
        if (t === AA.TUILogin.EVENT_SUB_KEY.USER_LOGIN_SUCCESS) {
          const { chat: p, userID: C, userSig: N, SDKAppID: h } = tn.getContext();
          await ((n = this._callService) == null ? void 0 : n.init({ tim: p, userID: C, userSig: N, sdkAppID: h, isFromChat: !0, component: et.TIM_CALL_KIT })), (s = this._callService) == null || s.setIsFromChat(!0), (l = this._callService) == null || l.setLogLevel(kt.NORMAL), this._addListenChatEvent();
        } else
          t === AA.TUILogin.EVENT_SUB_KEY.USER_LOGOUT_SUCCESS && (this._removeListenChatEvent(), await ((r = this._callService) == null ? void 0 : r.destroyed()));
      if (((i = AA.TUIChat) == null ? void 0 : i.EVENT) && e === AA.TUIChat.EVENT.CHAT_STATE_CHANGED && t === AA.TUIChat.EVENT_SUB_KEY.CHAT_OPENED) {
        if ((c = this._callService) == null || c.setCurrentGroupId((A == null ? void 0 : A.groupID) || ""), ve.getData(u.CALL, o.CALL_STATUS) !== w.IDLE)
          return;
        const p = (g = this._callService) == null ? void 0 : g.getCurrentGroupId(), C = p ? await this.getGroupAttributes((f = this._callService) == null ? void 0 : f.getTim(), p) : {};
        await this.updateStoreBasedOnGroupAttributes(C);
      }
    } catch (p) {
      console.error(`${o.PREFIX}TUICore onNotifyEvent failed, error: ${p}.`);
    }
  }
  async updateStoreBasedOnGroupAttributes(e) {
    var t, A, n, s;
    (n = (A = (t = this._callService) == null ? void 0 : t.getTUICallEngineInstance()) == null ? void 0 : A.reportLog) == null || n.call(A, {
      name: "TUICallKit.getJoinGroupCallInfo.success",
      data: { groupAttributes: e }
    });
    try {
      const {
        group_id: l = "",
        room_id: r = 0,
        room_id_type: i = 0,
        call_media_type: c = o.UNKNOWN,
        user_list: g
      } = e[o.INNER_ATTR_KIT_INFO] ? JSON.parse(e[o.INNER_ATTR_KIT_INFO]) : {};
      let f = (g || []).map((C) => C.userid);
      f = f.length && await ce(f, (s = this._callService) == null ? void 0 : s.getTim());
      const p = {
        [o.GROUP_ID]: l,
        [o.GROUP_CALL_MEMBERS]: f,
        [o.ROOM_ID]: r,
        [o.CALL_MEDIA_TYPE]: on[c],
        [o.ROOM_ID_TYPE]: i
      };
      ve.updateStore(p, u.CALL);
    } catch (l) {
      console.warn(`${o.PREFIX}updateStoreBasedOnGroupAttributes fail, error: ${l}`);
    }
  }
  async getGroupAttributes(e, t) {
    if (!t)
      return {};
    try {
      const { data: A } = await e.getGroupAttributes({
        groupID: t,
        keyList: []
      });
      return (A == null ? void 0 : A.groupAttributes) || {};
    } catch (A) {
      return console.warn(`${o.PREFIX}getGroupAttributes fail: ${A}`), {};
    }
  }
  isLineBusy(e) {
    var n;
    const t = fe(e.payload.data), A = fe(t == null ? void 0 : t.data);
    return (A == null ? void 0 : A.line_busy) === "line_busy" || (A == null ? void 0 : A.line_busy) === "" || ((n = A == null ? void 0 : A.data) == null ? void 0 : n.message) === "lineBusy";
  }
  async getCallKitMessage(e, t) {
    var p, C, N, h;
    const A = fe(e.payload.data);
    if ((A == null ? void 0 : A.businessID) !== 1)
      return {};
    let n = "";
    const s = fe(A == null ? void 0 : A.data), l = s.call_type, r = A.inviteeList, i = (p = s == null ? void 0 : s.data) == null ? void 0 : p.inviter, c = ve.getData(u.CALL, o.LOCAL_USER_INFO).userId, g = i === c, f = (C = s == null ? void 0 : s.data) == null ? void 0 : C.cmd;
    switch (A == null ? void 0 : A.actionType) {
      case KA.INVITE: {
        n = NA[f]({ callDuration: Kt(s == null ? void 0 : s.call_end) });
        break;
      }
      case KA.CANCEL_INVITE:
        n = g ? "Call Cancel" : "Other Side Cancel";
        break;
      case KA.ACCEPT_INVITE:
        ["switchToAudio", "switchToVideo"].includes(f) ? n = (N = NA == null ? void 0 : NA[f]) == null ? void 0 : N.call(NA) : n = qA("Answered");
        break;
      case KA.REJECT_INVITE:
        this.isLineBusy(e) ? n = g ? "Line Busy" : "Other Side Line Busy" : n = g ? "Other Side Decline" : "Decline";
        break;
      case KA.INVITE_TIMEOUT:
        ["switchToAudio", "switchToVideo"].includes(f) ? n = (h = NA == null ? void 0 : NA[f]) == null ? void 0 : h.call(NA) : n = g ? "Other Side No Answer" : "No answer";
        break;
    }
    return { messageCardContent: n, callMediaType: l, inviteeList: r };
  }
  _addListenChatEvent() {
    var e, t;
    if (!((e = this._callService) != null && e.getTim())) {
      console.warn(`${o.PREFIX}add tim event listener failed, tim is empty.`);
      return;
    }
    (t = this._callService) == null || t.getTim().on(Tt.EVENT.GROUP_ATTRIBUTES_UPDATED, this._handleGroupAttributesUpdated, this);
  }
  _removeListenChatEvent() {
    var e, t;
    if (!((e = this._callService) != null && e.getTim())) {
      console.warn(`${o.PREFIX}remove tim event listener failed, tim is empty.`);
      return;
    }
    (t = this._callService) == null || t.getTim().off(Tt.EVENT.GROUP_ATTRIBUTES_UPDATED, this._handleGroupAttributesUpdated, this);
  }
  async _handleTUICoreOnClick(e, t) {
    var A, n;
    try {
      const { groupID: s, userIDList: l = [], ...r } = e;
      s ? await ((A = this._callService) == null ? void 0 : A.groupCall({ groupID: s, userIDList: l, type: t, ...r })) : l.length === 1 && await ((n = this._callService) == null ? void 0 : n.call({ userID: l[0], type: t, ...r }));
    } catch (s) {
      console.debug(s);
    }
  }
  async _handleGroupAttributesUpdated(e) {
    var s;
    if (ve.getData(u.CALL, o.CALL_STATUS) !== w.IDLE)
      return;
    const t = (e == null ? void 0 : e.data) || {}, { groupID: A = "", groupAttributes: n = {} } = t;
    A === ((s = this._callService) == null ? void 0 : s.getCurrentGroupId()) && await this.updateStoreBasedOnGroupAttributes(n);
  }
}
const Rn = 0, Gn = 1, xt = 2;
function kn({ retryFunction: a, settings: e, onError: t, onRetrying: A, onRetryFailed: n, context: s }) {
  return function(...l) {
    const r = e.retries || 5;
    let i = 0, c = -1, g = Rn;
    const f = async (p, C) => {
      const N = s || this;
      try {
        const h = await a.apply(N, l);
        i = 0, p(h);
      } catch (h) {
        const L = () => {
          clearTimeout(c), i = 0, g = xt, C(h);
        }, D = () => {
          g !== xt && i < r ? (i++, g = Gn, me(A) && A.call(N, i, L), c = setTimeout(
            () => {
              c = -1, f(p, C);
            },
            Ue(e.timeout) ? 1e3 : e.timeout
          )) : (L(), me(n) && n.call(N, h));
        };
        me(t) ? t.call(N, h, D, C, l) : D();
      }
    };
    return new Promise(f);
  };
}
function Wn(a) {
  return function(e, t, A) {
    const { retries: n = 5, timeout: s = 2e3, onError: l, onRetrying: r, onRetryFailed: i } = a, c = kn({
      retryFunction: A.value,
      settings: { retries: n, timeout: s },
      onError: l,
      onRetrying: r,
      onRetryFailed: i,
      context: null
    });
    return A.value = function(...g) {
      return c.apply(this, g);
    }, A;
  };
}
var qn = Object.defineProperty, Xn = Object.getOwnPropertyDescriptor, Qn = (a, e, t, A) => {
  for (var n = A > 1 ? void 0 : A ? Xn(e, t) : e, s = a.length - 1, l; s >= 0; s--)
    (l = a[s]) && (n = (A ? l(e, t, n) : l(n)) || n);
  return A && n && qn(e, t, n), n;
};
const O = DA.getInstance(), Zn = WA.getInstance(), se = class {
  constructor(a) {
    this._callService = a.callService;
  }
  static getInstance(a) {
    return se.instance || (se.instance = new se(a)), se.instance;
  }
  addListenTuiCallEngineEvent() {
    var e;
    const a = (e = this._callService) == null ? void 0 : e.getTUICallEngineInstance();
    if (!a) {
      console.warn(`${o.PREFIX}add engine event listener failed, engine is empty.`);
      return;
    }
    a.on(E.ERROR, this._handleError, this), a.on(E.INVITED, this._handleNewInvitationReceived, this), a.on(E.USER_ACCEPT, this._handleUserAccept, this), a.on(E.USER_ENTER, this._handleUserEnter, this), a.on(E.USER_LEAVE, this._handleUserLeave, this), a.on(E.REJECT, this._handleInviteeReject, this), a.on(E.NO_RESP, this._handleNoResponse, this), a.on(E.LINE_BUSY, this._handleLineBusy, this), a.on(E.CALLING_CANCEL, this._handleCallingCancel, this), a.on(E.SDK_READY, this._handleSDKReady, this), a.on(E.KICKED_OUT, this._handleKickedOut, this), a.on(E.MESSAGE_SENT_BY_ME, this._messageSentByMe, this), E.CALL_MESSAGE && a.on(E.CALL_MESSAGE, this._handleCallMessage, this), E.ON_USER_NETWORK_QUALITY_CHANGED && a.on(E.ON_USER_NETWORK_QUALITY_CHANGED, this._handleNetworkQuality, this), a.on(E.CALLING_END, this._handleCallingEnd, this), a.on(E.CALL_TYPE_CHANGED, this._handleCallTypeChange, this), a.on(E.USER_VOICE_VOLUME, this._handleUserVoiceVolume, this), a.on(E.DEVICED_UPDATED, this._handleDeviceUpdate, this), a.on(E.USER_VIDEO_AVAILABLE, this._handleUserVideoAvailable, this), a.on(E.USER_AUDIO_AVAILABLE, this._handleUserAudioAvailable, this);
  }
  removeListenTuiCallEngineEvent() {
    var e;
    const a = (e = this._callService) == null ? void 0 : e.getTUICallEngineInstance();
    a.off(E.ERROR, this._handleError, this), a.off(E.INVITED, this._handleNewInvitationReceived, this), a.off(E.USER_ACCEPT, this._handleUserAccept, this), a.off(E.USER_ENTER, this._handleUserEnter, this), a.off(E.USER_LEAVE, this._handleUserLeave, this), a.off(E.REJECT, this._handleInviteeReject, this), a.off(E.NO_RESP, this._handleNoResponse, this), a.off(E.LINE_BUSY, this._handleLineBusy, this), a.off(E.CALLING_CANCEL, this._handleCallingCancel, this), a.off(E.SDK_READY, this._handleSDKReady, this), a.off(E.KICKED_OUT, this._handleKickedOut, this), a.off(E.MESSAGE_SENT_BY_ME, this._messageSentByMe, this), E.ON_USER_NETWORK_QUALITY_CHANGED && a.off(E.ON_USER_NETWORK_QUALITY_CHANGED, this._handleNetworkQuality, this), a.off(E.CALLING_END, this._handleCallingEnd, this), a.off(E.CALL_TYPE_CHANGED, this._handleCallTypeChange, this), a.off(E.USER_VOICE_VOLUME, this._handleUserVoiceVolume, this), a.off(E.DEVICED_UPDATED, this._handleDeviceUpdate, this), a.off(E.USER_VIDEO_AVAILABLE, this._handleUserVideoAvailable, this), a.off(E.USER_AUDIO_AVAILABLE, this._handleUserAudioAvailable, this);
  }
  _callerChangeToConnected() {
    var t;
    const a = O.getData(u.CALL, o.CALL_ROLE);
    O.getData(u.CALL, o.CALL_STATUS) === w.CALLING && a === J.CALLER && (O.update(u.CALL, o.CALL_STATUS, w.CONNECTED), (t = this._callService) == null || t.startTimer());
  }
  _unNormalEventsManager(a, e) {
    var n;
    console.log(`${o.PREFIX}${e} event data: ${JSON.stringify(a)}.`);
    const t = O.getData(u.CALL, o.IS_GROUP), A = O.getData(u.CALL, o.REMOTE_USER_INFO_LIST);
    switch (e) {
      case E.REJECT:
      case E.LINE_BUSY: {
        const { userID: s } = fA(a);
        let l = e === E.REJECT ? iA.OTHER_SIDE_REJECT_CALL : iA.OTHER_SIDE_LINE_BUSY, r = "";
        t && (r = (A.find((i) => i.userId === s) || {}).displayUserInfo || s, l = e === E.REJECT ? iA.REJECT_CALL : iA.IN_BUSY), O.update(u.CALL, o.TOAST_INFO, { content: { key: l, options: { userList: r } } }), s && ke([s]);
        break;
      }
      case E.NO_RESP: {
        const { userIDList: s = [] } = fA(a), l = t ? iA.TIMEOUT : iA.CALL_TIMEOUT, r = s.map((i) => (A.find((g) => g.userId === i) || {}).displayUserInfo || i);
        O.update(u.CALL, o.TOAST_INFO, { content: { key: l, options: { userList: r.join() } } }), s.length > 0 && ke(s);
        break;
      }
      case E.CALLING_CANCEL: {
        (n = this._callService) == null || n._resetCallStore();
        break;
      }
    }
  }
  _handleError(a) {
    var s;
    const { code: e, message: t } = a || {}, A = Object.values(Et).indexOf(e);
    let n = "";
    if (A !== -1) {
      const l = Object.keys(Et)[A];
      n = qA(wt[l]), n && O.update(u.CALL, o.TOAST_INFO, { content: wt[l], type: o.ERROR });
    }
    (s = this._callService) == null || s.executeExternalAfterCalling(), console.error(`${o.PREFIX}_handleError, errorCode: ${e}; errorMessage: ${n || t}.`);
  }
  async _handleNewInvitationReceived(a) {
    var h, L, D, z;
    console.log(`${o.PREFIX}onCallReceived event data: ${JSON.stringify(a)}.`);
    const { callerId: e = "", callMediaType: t, inviteData: A = {}, calleeIdList: n = [], groupID: s = "", roomID: l, strRoomID: r } = fA(a), i = O.getData(u.CALL, o.LOCAL_USER_INFO), c = [e, ...n.filter((T) => T !== i.userId)], g = t || A.callType, f = g === _.AUDIO ? iA.CALLEE_CALLING_AUDIO_MSG : iA.CALLEE_CALLING_VIDEO_MSG;
    let p = {
      [o.CALL_ROLE]: J.CALLEE,
      [o.IS_GROUP]: !!s || n.length > 1,
      [o.CALL_STATUS]: w.CALLING,
      [o.CALL_MEDIA_TYPE]: g,
      [o.CALL_TIPS]: f,
      [o.CALLER_USER_INFO]: { userId: e },
      [o.GROUP_ID]: s
    };
    Ht(l, r), O.updateStore(p, u.CALL), (h = this._callService) == null || h.executeExternalBeforeCalling(), (L = this._callService) != null && L.statusChanged && ((D = this._callService) == null || D.statusChanged({ oldStatus: nA.IDLE, newStatus: nA.BE_INVITED }));
    const C = await ce(c, (z = this._callService) == null ? void 0 : z.getTim()), [N] = C.filter((T) => T.userId === e);
    C.length > 0 && O.updateStore({
      [o.REMOTE_USER_INFO_LIST]: C,
      [o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST]: C,
      [o.CALLER_USER_INFO]: {
        userId: e,
        nick: (N == null ? void 0 : N.nick) || "",
        avatar: (N == null ? void 0 : N.avatar) || "",
        displayUserInfo: (N == null ? void 0 : N.remark) || (N == null ? void 0 : N.nick) || e
      }
    }, u.CALL);
  }
  _handleUserAccept(a) {
    this._callerChangeToConnected(), O.update(u.CALL, o.CALL_TIPS, { text: "answered", duration: 2e3 }), console.log(`${o.PREFIX}accept event data: ${JSON.stringify(a)}.`);
  }
  async _handleUserEnter(a) {
    var s;
    this._callerChangeToConnected();
    const { userID: e, data: t } = fA(a);
    let A = O.getData(u.CALL, o.REMOTE_USER_INFO_LIST);
    if (!A.find((l) => (l == null ? void 0 : l.userId) === e)) {
      A.push({ userId: e }), A.length > 0 && (O.update(u.CALL, o.REMOTE_USER_INFO_LIST, A), O.update(u.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, A));
      const [l] = await ce([e], (s = this._callService) == null ? void 0 : s.getTim());
      A = O.getData(u.CALL, o.REMOTE_USER_INFO_LIST), A.forEach((r) => {
        (r == null ? void 0 : r.userId) === e && (r = Object.assign(r, l));
      });
    }
    A = A.map((l) => (l.userId === e && (l.isEnter = !0), l)), A.length > 0 && (O.update(u.CALL, o.REMOTE_USER_INFO_LIST, A), O.update(u.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, A), Zn.updateViewBackgroundUserId("remote")), console.log(`${o.PREFIX}userEnter event data: ${JSON.stringify(a)}.`);
  }
  _handleUserLeave(a) {
    console.log(`${o.PREFIX}userLeave event data: ${JSON.stringify(a)}.`);
    const { data: e, userID: t } = fA(a);
    if (O.getData(u.CALL, o.IS_GROUP)) {
      const n = (O.getData(u.CALL, o.REMOTE_USER_INFO_LIST).find((s) => s.userId === t) || {}).displayUserInfo || t;
      O.update(u.CALL, o.TOAST_INFO, { content: { key: iA.END_CALL, options: { userList: n } } });
    }
    t && ke([t]);
  }
  _handleInviteeReject(a) {
    this._unNormalEventsManager(a, E.REJECT);
  }
  _handleNoResponse(a) {
    this._unNormalEventsManager(a, E.NO_RESP);
  }
  _handleLineBusy(a) {
    this._unNormalEventsManager(a, E.LINE_BUSY);
  }
  _handleCallingCancel(a) {
    var e;
    (e = this._callService) == null || e.executeExternalAfterCalling(), this._unNormalEventsManager(a, E.CALLING_CANCEL);
  }
  _handleCallingEnd(a) {
    var e, t;
    console.log(`${o.PREFIX}callEnd event data: ${JSON.stringify(a)}.`), (e = this._callService) == null || e.executeExternalAfterCalling(), (t = this._callService) == null || t._resetCallStore();
  }
  async _handleSDKReady(a) {
    var t;
    let e = O.getData(u.CALL, o.LOCAL_USER_INFO);
    e = await Pn(e.userId, (t = this._callService) == null ? void 0 : t.getTim()), O.update(u.CALL, o.LOCAL_USER_INFO, e), O.update(u.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, e);
  }
  _handleKickedOut(a) {
    var e, t, A;
    console.log(`${o.PREFIX}kickOut event data: ${JSON.stringify(a)}.`), (e = this._callService) != null && e.kickedOut && ((t = this._callService) == null || t.kickedOut(a)), O.update(u.CALL, o.CALL_TIPS, iA.KICK_OUT), (A = this._callService) == null || A._resetCallStore();
  }
  _messageSentByMe(a) {
    var t, A;
    const e = a == null ? void 0 : a.data;
    (t = this._callService) != null && t.onMessageSentByMe && ((A = this._callService) == null || A.onMessageSentByMe(e));
  }
  _handleCallMessage(a) {
    const e = fA(a);
    this._callService._chatCombine.callTUIService({ message: e });
  }
  _handleCallTypeChange(a) {
    const { newCallType: e, type: t } = fA(a);
    O.update(u.CALL, o.CALL_MEDIA_TYPE, e || t);
  }
  _handleNetworkQuality(a) {
    const { networkQualityList: e = [] } = fA(a);
    O.update(u.CALL, o.NETWORK_STATUS, e);
    const t = O.getData(u.CALL, o.IS_GROUP), A = O.getData(u.CALL, o.LOCAL_USER_INFO);
    if (O.getData(u.CALL, o.REMOTE_USER_INFO_LIST), !t && e.find((s) => (A == null ? void 0 : A.userId) === (s == null ? void 0 : s.userId) && (s == null ? void 0 : s.quality) >= dn)) {
      O.update(u.CALL, o.CALL_TIPS, iA.LOCAL_NETWORK_IS_POOR);
      return;
    }
  }
  async _startRemoteView(a) {
    var e;
    if (!a) {
      console.warn(`${o.PREFIX}_startRemoteView userID is empty`);
      return;
    }
    if (!document.getElementById(a))
      return console.warn(`${o.PREFIX}_startRemoteView can't find HTMLElement sid: ${a}`), Promise.reject();
    try {
      const t = O.getData(u.CALL, o.DISPLAY_MODE);
      await ((e = this._callService) == null ? void 0 : e.getTUICallEngineInstance().startRemoteView({ userID: a, videoViewDomID: a, options: { objectFit: t } }));
    } catch (t) {
      return console.error(`${o.PREFIX}_startRemoteView error: ${t}.`), Promise.reject(t);
    }
  }
  _setRemoteUserInfoAudioVideoAvailable(a, e, t) {
    let A = O.getData(u.CALL, o.REMOTE_USER_INFO_LIST);
    A = A.map((n) => {
      if (n.userId === t) {
        if (e === o.AUDIO)
          return { ...n, isAudioAvailable: a };
        if (e === o.VIDEO)
          return { ...n, isVideoAvailable: a };
      }
      return n;
    }), A.length > 0 && (O.update(u.CALL, o.REMOTE_USER_INFO_LIST, A), O.update(u.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, A));
  }
  async _handleUserVideoAvailable(a) {
    const { userID: e, isVideoAvailable: t } = fA(a);
    console.log(`${o.PREFIX}_handleUserVideoAvailable event data: ${JSON.stringify(a)}.`);
    try {
      t && await this._startRemoteView(e);
    } catch (A) {
      console.error(`${o.PREFIX}_startRemoteView failed, error: ${A}.`);
    }
    this._setRemoteUserInfoAudioVideoAvailable(t, o.VIDEO, e);
  }
  _handleUserAudioAvailable(a) {
    const { userID: e, isAudioAvailable: t } = fA(a);
    console.log(`${o.PREFIX}_handleUserAudioAvailable event data: ${JSON.stringify(a)}.`), this._setRemoteUserInfoAudioVideoAvailable(t, o.AUDIO, e);
  }
  _handleUserVoiceVolume(a) {
    try {
      const { volumeMap: e } = fA(a);
      if ((e || []).length === 0)
        return;
      const t = O.getData(u.CALL, o.LOCAL_USER_INFO);
      let A = O.getData(u.CALL, o.REMOTE_USER_INFO_LIST);
      const [n] = e.filter((r) => r.userId === t.userId), s = e.reduce((r, i) => i.userId !== t.userId ? { ...r, [i.userId]: i.audioVolume } : r, {});
      t.volume = n.audioVolume, A = A.map((r) => ({ ...r, volume: s[r.userId] }));
      const l = {
        [o.LOCAL_USER_INFO]: t,
        [o.REMOTE_USER_INFO_LIST]: A
      };
      O.updateStore(l, u.CALL);
    } catch (e) {
      console.debug(e);
    }
  }
  _handleDeviceUpdate(a) {
    const { cameraList: e, microphoneList: t, speakerList: A, currentCamera: n, currentMicrophone: s, currentSpeaker: l } = a;
    O.update(u.CALL, o.DEVICE_LIST, { cameraList: e, microphoneList: t, speakerList: A, currentCamera: n, currentMicrophone: s, currentSpeaker: l });
  }
};
let Jt = se;
Qn([
  Wn({
    retries: 5,
    timeout: 200,
    onRetrying(a) {
      console.warn(`${o.PREFIX}_startRemoteView, retrying [${a}]`);
    }
  })
], Jt.prototype, "_startRemoteView", 1);
var Yn = Object.defineProperty, Kn = Object.getOwnPropertyDescriptor, k = (a, e, t, A) => {
  for (var n = A > 1 ? void 0 : A ? Kn(e, t) : e, s = a.length - 1, l; s >= 0; s--)
    (l = a[s]) && (n = (A ? l(e, t, n) : l(n)) || n);
  return A && n && Yn(e, t, n), n;
};
const y = kA.getInstance(), d = DA.getInstance(), zA = WA.getInstance();
zA.setTUIStore(d);
const Bt = "4.0.2", Hn = "vue2.7", le = class {
  constructor() {
    this._tim = null, this._TUICore = null, this._timerId = -1, this._startTimeStamp = Ge(), this._bellContext = null, this._isFromChat = !1, this._currentGroupId = "", this._offlinePushInfo = null, this._permissionCheckTimer = null, this._chatCombine = null, this._engineEventHandler = null, this._handleCallStatusChange = async (a) => {
      var e, t, A, n;
      try {
        const s = {
          callRole: d.getData(u.CALL, o.CALL_ROLE),
          callStatus: d.getData(u.CALL, o.CALL_STATUS)
        };
        if (this._bellContext.setBellProperties(s), a === w.CALLING)
          await ((e = this == null ? void 0 : this._bellContext) == null ? void 0 : e.play());
        else {
          if (a === w.CONNECTED) {
            const l = d.getData(u.CALL, o.IS_GROUP), r = d.getData(u.CALL, o.CALL_MEDIA_TYPE), i = d.getData(u.CALL, o.REMOTE_USER_INFO_LIST), c = l ? nA.DIALING_GROUP : nA.DIALING_C2C;
            d.update(u.CALL, o.CALL_TIPS, ""), this.statusChanged && this.statusChanged({ oldStatus: c, newStatus: Ne() }), !l && r === _.VIDEO && this.switchScreen(i[0].domId);
          }
          if (a === w.IDLE && this._isFromChat) {
            const l = this._currentGroupId ? await ((t = this._chatCombine) == null ? void 0 : t.getGroupAttributes(this._tim, this._currentGroupId)) : {};
            await ((A = this._chatCombine) == null ? void 0 : A.updateStoreBasedOnGroupAttributes(l, d, this));
          }
          await ((n = this == null ? void 0 : this._bellContext) == null ? void 0 : n.stop());
        }
      } catch (s) {
        console.warn(`${o.PREFIX}handleCallStatusChange, ${s}.`);
      }
    }, console.log(`${o.PREFIX}version: ${Bt}`), this._watchTUIStore(), this._engineEventHandler = Jt.getInstance({ callService: this }), this._chatCombine = JA.getInstance({ callService: this });
  }
  static getInstance() {
    return le.instance || (le.instance = new le()), le.instance;
  }
  async init(a) {
    var e, t;
    try {
      if (this._tuiCallEngine)
        return;
      let { userID: A, tim: n, userSig: s, sdkAppID: l, SDKAppID: r, isFromChat: i, component: c = et.TUI_CALL_KIT } = a;
      this._TUICore && (l = this._TUICore.SDKAppID, n = this._TUICore.tim), this._tim = n, console.log(`${o.PREFIX}init sdkAppId: ${l || r}, userId: ${A}`), this._tuiCallEngine = en.createInstance({
        tim: n,
        SDKAppID: l || r,
        frameWork: Hn,
        language: 5,
        callkitVersion: Bt,
        isFromChat: i || !1,
        component: c
      }), zA.setEngineInstance(this._tuiCallEngine), this._addListenTuiCallEngineEvent(), this._bellContext = new Un(), d.update(u.CALL, o.LOCAL_USER_INFO, { userId: A }), d.update(u.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, { userId: A }), zA.updateViewBackgroundUserId("local"), await this._tuiCallEngine.login({ userID: A, userSig: s, assetsPath: "" });
      const g = d.getData(u.CALL, o.CUSTOM_UI_CONFIG);
      (t = (e = this._tuiCallEngine) == null ? void 0 : e.reportLog) == null || t.call(e, {
        name: "TUICallkit.init",
        data: {
          uiConfig: g
        }
      });
    } catch (A) {
      throw console.error(`${o.PREFIX}init failed, error: ${A}.`), A;
    }
  }
  async destroyed() {
    var a;
    try {
      const e = d.getData(u.CALL, o.CALL_STATUS);
      if (e !== w.IDLE)
        throw new Error(`please destroyed when status is idle, current status: ${e}`);
      this._tuiCallEngine && (this._removeListenTuiCallEngineEvent(), await this._tuiCallEngine.destroyInstance(), this._tuiCallEngine = null), (a = this._bellContext) == null || a.destroy(), this._bellContext = null;
    } catch (e) {
      throw console.error(`${o.PREFIX}destroyed failed, error: ${e}.`), e;
    }
  }
  async call(a) {
    if (d.getData(u.CALL, o.CALL_STATUS) === w.IDLE)
      try {
        const { type: e, userID: t, offlinePushInfo: A } = a;
        if (d.getData(u.CALL, o.CALL_STATUS) !== w.IDLE)
          return;
        await this._updateCallStoreBeforeCall(e, [{ userId: t }]), this.executeExternalBeforeCalling(), a.offlinePushInfo = { ...this.getDefaultOfflinePushInfo(), ...A };
        const n = await this._tuiCallEngine.call(a);
        await this._updateCallStoreAfterCall([t], n);
      } catch (e) {
        this._handleCallError(e, "call");
      }
  }
  async groupCall(a) {
    if (d.getData(u.CALL, o.CALL_STATUS) === w.IDLE)
      try {
        const { userIDList: e, type: t, groupID: A, offlinePushInfo: n } = a;
        if (d.getData(u.CALL, o.CALL_STATUS) !== w.IDLE)
          return;
        const s = e.map((r) => ({ userId: r }));
        await this._updateCallStoreBeforeCall(t, s, A), this.executeExternalBeforeCalling(), a.offlinePushInfo = { ...this.getDefaultOfflinePushInfo(), ...n };
        const l = await this._tuiCallEngine.groupCall(a);
        await this._updateCallStoreAfterCall(e, l);
      } catch (e) {
        this._handleCallError(e, "groupCall");
      }
  }
  async inviteUser(a) {
    if (d.getData(u.CALL, o.CALL_STATUS) !== w.IDLE)
      try {
        const { userIDList: e } = a;
        let t = await ce(e, this.getTim());
        const A = d.getData(u.CALL, o.REMOTE_USER_INFO_LIST);
        if (e.filter((s) => !A.some((l) => l.userId === s)).length === 0)
          return;
        d.update(u.CALL, o.REMOTE_USER_INFO_LIST, [...A, ...t]), d.update(u.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, [...A, ...t]), this._tuiCallEngine && await this._tuiCallEngine.inviteUser(a);
      } catch (e) {
        console.error(`${o.PREFIX}inviteUser failed, error: ${e}.`);
      }
  }
  async joinInGroupCall(a) {
    if (d.getData(u.CALL, o.CALL_STATUS) !== w.CONNECTED)
      try {
        const e = {
          [o.CALL_ROLE]: J.CALLEE,
          [o.IS_GROUP]: !0,
          [o.CALL_STATUS]: w.CONNECTED,
          [o.CALL_MEDIA_TYPE]: a.type,
          [o.GROUP_ID]: a.groupID,
          [o.ROOM_ID]: a.roomID
        };
        d.updateStore(e, u.CALL);
        const t = await this._tuiCallEngine.joinInGroupCall(a), A = this._getFeatureButtonDefaultState(SA.Camera) === GA.Close;
        a.type === _.VIDEO && !A && await this.openCamera(o.LOCAL_VIDEO), d.update(u.CALL, o.IS_CLICKABLE, !0), this.startTimer(), Le(this._tuiCallEngine), await this._tuiCallEngine.setVideoQuality(d.getData(u.CALL, o.VIDEO_RESOLUTION));
        const n = d.getData(u.CALL, o.LOCAL_USER_INFO);
        d.update(u.CALL, o.LOCAL_USER_INFO, { ...n, isEnter: !0 }), d.update(u.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, { ...n, isEnter: !0 }), jA(!0, o.AUDIO);
      } catch (e) {
        this._handleCallError(e, "joinInGroupCall");
      }
  }
  async calls(a) {
    if (d.getData(u.CALL, o.CALL_STATUS) === w.IDLE)
      try {
        const { userIDList: e, type: t, chatGroupID: A, offlinePushInfo: n } = a;
        if (d.getData(u.CALL, o.CALL_STATUS) !== w.IDLE)
          return;
        const s = e.map((r) => ({ userId: r }));
        await this._updateCallStoreBeforeCall(t, s, A), this.executeExternalBeforeCalling(), a.offlinePushInfo = { ...this.getDefaultOfflinePushInfo(), ...n };
        const l = await this._tuiCallEngine.calls(a);
        await this._updateCallStoreAfterCall(e, l);
      } catch (e) {
        this._handleCallError(e, "calls");
      }
  }
  async join(a) {
    if (d.getData(u.CALL, o.CALL_STATUS) !== w.CONNECTED)
      try {
        const e = await this._tuiCallEngine.join(a), t = this._getFeatureButtonDefaultState(SA.Camera) === GA.Close;
        a.type === _.VIDEO && !t && await this.openCamera(o.LOCAL_VIDEO), d.update(u.CALL, o.IS_CLICKABLE, !0), this.startTimer();
        const A = {
          [o.CALL_ROLE]: J.CALLEE,
          [o.IS_GROUP]: !0,
          [o.CALL_STATUS]: w.CONNECTED,
          [o.CALL_MEDIA_TYPE]: a.type,
          [o.GROUP_ID]: a.groupID,
          [o.ROOM_ID]: a.roomID
        };
        d.updateStore(A, u.CALL), Le(this._tuiCallEngine), await this._tuiCallEngine.setVideoQuality(d.getData(u.CALL, o.VIDEO_RESOLUTION));
        const n = d.getData(u.CALL, o.LOCAL_USER_INFO);
        d.update(u.CALL, o.LOCAL_USER_INFO, { ...n, isEnter: !0 }), d.update(u.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, { ...n, isEnter: !0 }), jA(!0, o.AUDIO);
      } catch (e) {
        this._handleCallError(e, "join");
      }
  }
  getTUICallEngineInstance() {
    return (this == null ? void 0 : this._tuiCallEngine) || null;
  }
  setLogLevel(a) {
    var e;
    (e = this == null ? void 0 : this._tuiCallEngine) == null || e.setLogLevel(a);
  }
  setLanguage(a) {
    a && Object.values(Gt).includes(a) && (d.update(u.CALL, o.LANGUAGE, a), d.update(u.CALL, o.TRANSLATE, qA.bind(null)));
  }
  enableFloatWindow(a) {
    d.update(u.CALL, o.ENABLE_FLOAT_WINDOW, a);
  }
  async setSelfInfo(a) {
    const { nickName: e, avatar: t } = a;
    try {
      await this._tuiCallEngine.setSelfInfo({ nickName: e, avatar: t });
    } catch (A) {
      console.error(`${o.PREFIX}setSelfInfo failed, error: ${A}.`);
    }
  }
  async enableVirtualBackground(a) {
    d.update(u.CALL, o.IS_SHOW_ENABLE_VIRTUAL_BACKGROUND, a);
  }
  async enableAIVoice(a) {
    try {
      await this._tuiCallEngine.enableAIVoice(a), console.log(`${o.PREFIX}enableAIVoice: ${a}.`);
    } catch (e) {
      throw console.error(`${o.PREFIX}enableAIVoice failed, error: ${e}.`), e;
    }
  }
  async setCallingBell(a) {
    let e = !0;
    if (e = await gn(a), !e) {
      console.warn(`${o.PREFIX}setCallingBell failed, filePath: ${a}.`);
      return;
    }
    const t = { calleeBellFilePath: a };
    this._bellContext.setBellProperties(t);
  }
  async enableMuteMode(a) {
    try {
      const e = { isMuteBell: a };
      this._bellContext.setBellProperties(e), await this._bellContext.setBellMute(a);
    } catch (e) {
      console.warn(`${o.PREFIX}enableMuteMode failed, error: ${e}.`);
    }
  }
  hideFeatureButton(a) {
    zA.hideFeatureButton(a);
  }
  setLocalViewBackgroundImage(a) {
    zA.setLocalViewBackgroundImage(a);
  }
  setRemoteViewBackgroundImage(a, e) {
    zA.setRemoteViewBackgroundImage(a, e);
  }
  setLayoutMode(a) {
    zA.setLayoutMode(a);
  }
  setCameraDefaultState(a) {
    zA.setCameraDefaultState(a);
  }
  async accept() {
    var e, t, A, n, s, l;
    const a = d.getData(u.CALL, o.CALL_STATUS);
    if ((t = (e = this._tuiCallEngine) == null ? void 0 : e.reportLog) == null || t.call(e, {
      name: "TUICallKit.accept.start",
      data: { callStatus: a }
    }), a !== w.CONNECTED)
      try {
        d.update(u.CALL, o.CALL_STATUS, w.CONNECTED), Le(this._tuiCallEngine);
        const r = await this._tuiCallEngine.accept();
        if (r) {
          (n = this._chatCombine) == null || n.callTUIService({ message: (A = r == null ? void 0 : r.data) == null ? void 0 : A.message }), d.update(u.CALL, o.IS_CLICKABLE, !0), this.startTimer();
          const i = d.getData(u.CALL, o.CALL_MEDIA_TYPE), c = this._getFeatureButtonDefaultState(SA.Camera) === GA.Close;
          i === _.VIDEO && !c && await this.openCamera(o.LOCAL_VIDEO), await this._tuiCallEngine.setVideoQuality(d.getData(u.CALL, o.VIDEO_RESOLUTION));
          const g = d.getData(u.CALL, o.LOCAL_USER_INFO);
          d.update(u.CALL, o.LOCAL_USER_INFO, { ...g, isEnter: !0 }), d.update(u.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, { ...g, isEnter: !0 }), jA(!0, o.AUDIO);
        }
      } catch (r) {
        if ((l = (s = this._tuiCallEngine) == null ? void 0 : s.reportLog) == null || l.call(s, {
          name: "TUICallKit.accept.fail",
          level: "error",
          error: r
        }), zt(r))
          return;
        We(r, _.AUDIO, this._tuiCallEngine), this._resetCallStore();
      }
  }
  async hangup() {
    if (d.getData(u.CALL, o.CALL_STATUS) !== w.IDLE) {
      try {
        const a = await this._tuiCallEngine.hangup();
        a == null || a.forEach((e) => {
          var t, A;
          (e == null ? void 0 : e.code) === 0 && ((A = this._chatCombine) == null || A.callTUIService({ message: (t = e == null ? void 0 : e.data) == null ? void 0 : t.message }));
        });
      } catch (a) {
        console.debug(a);
      }
      this._resetCallStore();
    }
  }
  async reject() {
    var a, e;
    if (d.getData(u.CALL, o.CALL_STATUS) !== w.IDLE) {
      try {
        const t = await this._tuiCallEngine.reject();
        (t == null ? void 0 : t.code) === 0 && ((e = this._chatCombine) == null || e.callTUIService({ message: (a = t == null ? void 0 : t.data) == null ? void 0 : a.message }));
      } catch (t) {
        console.debug(t);
      }
      this._resetCallStore();
    }
  }
  async openCamera(a) {
    try {
      if (y.isH5 || y.isWeChat) {
        const t = d.getData(u.CALL, o.CAMERA_POSITION) === FA.FRONT;
        this._tuiCallEngine.openCamera(a, t);
      } else
        await this._tuiCallEngine.openCamera(a);
      jA(!0, o.VIDEO);
    } catch (e) {
      We(e, _.VIDEO, this._tuiCallEngine), console.error(`${o.PREFIX}openCamera error: ${e}.`);
    }
  }
  async closeCamera() {
    try {
      await this._tuiCallEngine.closeCamera(), jA(!1, o.VIDEO);
    } catch (a) {
      console.error(`${o.PREFIX}closeCamera error: ${a}.`);
    }
  }
  async openMicrophone() {
    try {
      await this._tuiCallEngine.openMicrophone(), jA(!0, o.AUDIO);
    } catch (a) {
      console.error(`${o.PREFIX}openMicrophone failed, error: ${a}.`);
    }
  }
  async closeMicrophone() {
    try {
      await this._tuiCallEngine.closeMicrophone(), jA(!1, o.AUDIO);
    } catch (a) {
      console.error(`${o.PREFIX}closeMicrophone failed, error: ${a}.`);
    }
  }
  unMuteSpeaker() {
    var a, e;
    try {
      const t = (e = (a = this._tuiCallEngine) == null ? void 0 : a.getTRTCCloudInstance) == null ? void 0 : e.call(a);
      t && (t.muteAllRemoteAudio(!1), d.update(u.CALL, o.IS_MUTE_SPEAKER, !1));
    } catch (t) {
      console.error(`${o.PREFIX}unMuteSpeaker failed, error: ${t}.`);
    }
  }
  muteSpeaker() {
    var a, e;
    try {
      const t = (e = (a = this._tuiCallEngine) == null ? void 0 : a.getTRTCCloudInstance) == null ? void 0 : e.call(a);
      t && (t.muteAllRemoteAudio(!0), d.update(u.CALL, o.IS_MUTE_SPEAKER, !0));
    } catch (t) {
      console.error(`${o.PREFIX}muteSpeaker failed, error: ${t}.`);
    }
  }
  switchScreen(a) {
    !a || d.update(u.CALL, o.BIG_SCREEN_USER_ID, a);
  }
  async switchCallMediaType() {
    var a, e;
    try {
      const t = d.getData(u.CALL, o.CALL_MEDIA_TYPE);
      if (t === _.AUDIO) {
        console.warn(`${o.PREFIX}switchCallMediaType failed, ${t} not support.`);
        return;
      }
      const A = await this._tuiCallEngine.switchCallMediaType(_.AUDIO);
      (A == null ? void 0 : A.code) === 0 && ((e = this._chatCombine) == null || e.callTUIService({ message: (a = A == null ? void 0 : A.data) == null ? void 0 : a.message })), d.update(u.CALL, o.CALL_MEDIA_TYPE, _.AUDIO);
      const s = d.getData(u.CALL, o.IS_GROUP) ? nA.CALLING_GROUP_VIDEO : nA.CALLING_C2C_VIDEO, l = Ne();
      this.statusChanged && this.statusChanged({ oldStatus: s, newStatus: l });
    } catch (t) {
      console.error(`${o.PREFIX}switchCallMediaType failed, error: ${t}.`);
    }
  }
  async switchCamera() {
    const e = d.getData(u.CALL, o.CAMERA_POSITION) === FA.BACK ? FA.FRONT : FA.BACK;
    try {
      await this._tuiCallEngine.switchCamera(e), d.update(u.CALL, o.CAMERA_POSITION, e);
    } catch (t) {
      console.error(`${o.PREFIX}_switchCamera failed, error: ${t}.`);
    }
  }
  async setBlurBackground(a) {
    try {
      await this._tuiCallEngine.setBlurBackground(a ? cn : 0), d.update(u.CALL, o.ENABLE_VIRTUAL_BACKGROUND, a);
    } catch (e) {
      console.error(`${o.PREFIX}_setBlurBackground failed, error: ${e}.`);
    }
  }
  async switchDevice(a) {
    try {
      await this._tuiCallEngine.switchDevice(a);
    } catch (e) {
      console.error(`${o.PREFIX}_switchDevice failed, error: ${e}.`);
    }
  }
  async getDeviceList(a) {
    try {
      return await this._tuiCallEngine.getDeviceList(a);
    } catch (e) {
      this._handleCallError(e, "call");
    }
  }
  _addListenTuiCallEngineEvent() {
    this._engineEventHandler.addListenTuiCallEngineEvent();
  }
  _removeListenTuiCallEngineEvent() {
    this._engineEventHandler.removeListenTuiCallEngineEvent();
  }
  setCallback(a) {
    const { beforeCalling: e, afterCalling: t, onMinimized: A, onMessageSentByMe: n, kickedOut: s, statusChanged: l } = a;
    e && (this.beforeCalling = e), t && (this.afterCalling = t), A && (this.onMinimized = A), n && (this.onMessageSentByMe = n), s && (this.kickedOut = s), l && (this.statusChanged = l);
  }
  toggleMinimize() {
    const a = d.getData(u.CALL, o.IS_MINIMIZED);
    d.update(u.CALL, o.IS_MINIMIZED, !a), console.log(`${o.PREFIX}toggleMinimize: ${a} -> ${!a}.`), this.onMinimized && this.onMinimized(a, !a);
  }
  executeExternalBeforeCalling() {
    this.beforeCalling && this.beforeCalling();
  }
  executeExternalAfterCalling() {
    this.afterCalling && this.afterCalling();
  }
  setVideoDisplayMode(a) {
    d.update(u.CALL, o.DISPLAY_MODE, a);
  }
  async setVideoResolution(a) {
    var e;
    try {
      if (!a)
        return;
      d.update(u.CALL, o.VIDEO_RESOLUTION, a), await ((e = this._tuiCallEngine) == null ? void 0 : e.setVideoQuality(a));
    } catch (t) {
      console.warn(`${o.PREFIX}setVideoResolution failed, error: ${t}.`);
    }
  }
  startTimer() {
    this._timerId === -1 && (this._startTimeStamp = Ge(), this._timerId = be.run(o.TIMEOUT, this._updateCallDuration.bind(this), { delay: 1e3 }));
  }
  _handleCallError(a, e) {
    if (this._permissionCheckTimer && clearInterval(this._permissionCheckTimer), !zt(a))
      throw We(a, _.AUDIO, this._tuiCallEngine), console.error(`${o.PREFIX}${e} failed, error: ${a}.`), this._resetCallStore(), a;
  }
  async _updateCallStoreBeforeCall(a, e, t) {
    let A = iA.CALLER_CALLING_MSG;
    (t || d.getData(u.CALL, o.IS_MINIMIZED) || e.length > 1) && (A = iA.CALLER_GROUP_CALLING_MSG);
    let n = {
      [o.CALL_MEDIA_TYPE]: a,
      [o.CALL_ROLE]: J.CALLER,
      [o.REMOTE_USER_INFO_LIST]: e,
      [o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST]: e,
      [o.IS_GROUP]: !!t || e.length > 1,
      [o.CALL_TIPS]: A,
      [o.GROUP_ID]: t
    };
    d.updateStore({ ...n, [o.CALL_STATUS]: w.CALLING }, u.CALL), this.statusChanged && this.statusChanged({
      oldStatus: nA.IDLE,
      newStatus: t || e.length > 1 ? nA.DIALING_GROUP : nA.DIALING_C2C
    }), Le(this._tuiCallEngine);
    const s = await ce(e.map((l) => l.userId), this.getTim());
    s.length > 0 && (d.update(u.CALL, o.REMOTE_USER_INFO_LIST, s), d.update(u.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, s));
  }
  async _updateCallStoreAfterCall(a, e) {
    var t, A;
    if (e) {
      d.update(u.CALL, o.IS_CLICKABLE, !0), Ht(e == null ? void 0 : e.roomID, e == null ? void 0 : e.strRoomID);
      const n = d.getData(u.CALL, o.CALL_MEDIA_TYPE);
      if (e.code === 0) {
        (A = this._chatCombine) == null || A.callTUIService({ message: (t = e == null ? void 0 : e.data) == null ? void 0 : t.message });
        try {
          await this._tuiCallEngine.setVideoQuality(d.getData(u.CALL, o.VIDEO_RESOLUTION));
        } catch (r) {
          console.warn(`${o.PREFIX}setVideoQuality failed, error: ${r}.`);
        }
      } else {
        this._resetCallStore();
        return;
      }
      const s = this._getFeatureButtonDefaultState(SA.Camera) === GA.Close;
      n === _.VIDEO && !s && await this.openCamera(o.LOCAL_VIDEO);
      const l = d.getData(u.CALL, o.LOCAL_USER_INFO);
      d.update(u.CALL, o.LOCAL_USER_INFO, { ...l, isEnter: !0 }), d.update(u.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, { ...l, isEnter: !0 }), jA(!0, o.AUDIO);
    } else
      this._permissionCheckTimer && clearInterval(this._permissionCheckTimer), this._permissionCheckTimer = null, this._resetCallStore();
  }
  _getFeatureButtonDefaultState(a) {
    var t;
    const { button: e } = d.getData(u.CALL, o.CUSTOM_UI_CONFIG);
    return (t = e == null ? void 0 : e[a]) == null ? void 0 : t.state;
  }
  _updateCallDuration() {
    const a = Math.round((Ge() - this._startTimeStamp) / 1e3), e = Kt(a);
    d.update(u.CALL, o.CALL_DURATION, e);
  }
  _stopTimer() {
    this._timerId !== -1 && (be.clearTask(this._timerId), this._timerId = -1);
  }
  _resetCallStore() {
    const a = Ne();
    this._stopTimer();
    let e = Object.keys(he).filter((n) => {
      switch (he[n]) {
        case o.CALL_STATUS:
        case o.LANGUAGE:
        case o.IS_GROUP:
        case o.DISPLAY_MODE:
        case o.VIDEO_RESOLUTION:
        case o.ENABLE_FLOAT_WINDOW:
        case o.LOCAL_USER_INFO:
        case o.IS_SHOW_ENABLE_VIRTUAL_BACKGROUND:
        case o.LOCAL_USER_INFO_EXCLUDE_VOLUMN:
          return !1;
        default:
          return !0;
      }
    });
    e = e.map((n) => he[n]), d.reset(u.CALL, e), d.getData(u.CALL, o.CALL_STATUS) !== w.IDLE && d.reset(u.CALL, [o.CALL_STATUS], !0), d.reset(u.CALL, [o.IS_MINIMIZED], !0), d.reset(u.CALL, [o.IS_EAR_PHONE], !0), d.reset(u.CALL, [o.ENABLE_VIRTUAL_BACKGROUND], !0), d.reset(u.CALL, [o.IS_MUTE_SPEAKER], !0), d.update(u.CALL, o.LOCAL_USER_INFO, {
      ...d.getData(u.CALL, o.LOCAL_USER_INFO),
      isVideoAvailable: !1,
      isAudioAvailable: !1
    }), d.update(u.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN, {
      ...d.getData(u.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN),
      isVideoAvailable: !1,
      isAudioAvailable: !1
    }), d.update(u.CALL, o.REMOTE_USER_INFO_LIST, []), d.update(u.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST, []), d.update(u.CALL, o.CAMERA_POSITION, FA.FRONT);
    const A = Ne();
    a !== A && this.statusChanged && this.statusChanged({ oldStatus: a, newStatus: A });
  }
  async getGroupMemberList(a, e) {
    const t = d.getData(u.CALL, o.GROUP_ID);
    return await Vn(t, this.getTim(), a, e);
  }
  async getGroupProfile() {
    const a = d.getData(u.CALL, o.GROUP_ID);
    return await _n(a, this.getTim());
  }
  _watchTUIStore() {
    d == null || d.watch(u.CALL, {
      [o.CALL_STATUS]: this._handleCallStatusChange
    });
  }
  _unwatchTUIStore() {
    d == null || d.unwatch(u.CALL, {
      [o.CALL_STATUS]: this._handleCallStatusChange
    });
  }
  bindTUICore(a) {
    this._TUICore = a;
  }
  getTim() {
    var a, e;
    return this._tim ? this._tim : this._tuiCallEngine ? ((a = this._tuiCallEngine) == null ? void 0 : a.tim) || ((e = this._tuiCallEngine) == null ? void 0 : e.getTim()) : (console.warn(`${o.PREFIX}getTim warning: _tuiCallEngine Instance is not available.`), null);
  }
  setIsFromChat(a) {
    this._isFromChat = a;
  }
  setCurrentGroupId(a) {
    this._currentGroupId = a;
  }
  getCurrentGroupId() {
    return this._currentGroupId;
  }
  setDefaultOfflinePushInfo(a) {
    this._offlinePushInfo = a;
  }
  getDefaultOfflinePushInfo() {
    const a = d.getData(u.CALL, o.LOCAL_USER_INFO);
    return this._offlinePushInfo ? this._offlinePushInfo : {
      title: (a == null ? void 0 : a.displayUserInfo) || "",
      description: qA("you have a new call")
    };
  }
  async getCallMessage(a) {
    return await this._chatCombine.getCallKitMessage(a, this.getTim());
  }
};
let G = le;
k([
  H(),
  gA(IA.init)
], G.prototype, "init", 1);
k([
  H(),
  gA(IA.call),
  Ae({ engineInstance: !0 })
], G.prototype, "call", 1);
k([
  H(),
  gA(IA.groupCall),
  Ae({ engineInstance: !0 })
], G.prototype, "groupCall", 1);
k([
  H(),
  gA(IA.inviteUser),
  Ae({ engineInstance: !0 })
], G.prototype, "inviteUser", 1);
k([
  H(),
  gA(IA.joinInGroupCall),
  Ae({ engineInstance: !0 })
], G.prototype, "joinInGroupCall", 1);
k([
  H(),
  Ae({ engineInstance: !0 })
], G.prototype, "calls", 1);
k([
  H(),
  Ae({ engineInstance: !0 })
], G.prototype, "join", 1);
k([
  gA(IA.setLanguage)
], G.prototype, "setLanguage", 1);
k([
  gA(IA.enableFloatWindow)
], G.prototype, "enableFloatWindow", 1);
k([
  gA(IA.setSelfInfo)
], G.prototype, "setSelfInfo", 1);
k([
  gA(IA.enableAIVoice)
], G.prototype, "enableAIVoice", 1);
k([
  gA(IA.setCallingBell)
], G.prototype, "setCallingBell", 1);
k([
  gA(IA.enableMuteMode)
], G.prototype, "enableMuteMode", 1);
k([
  H()
], G.prototype, "accept", 1);
k([
  H()
], G.prototype, "hangup", 1);
k([
  H()
], G.prototype, "reject", 1);
k([
  H()
], G.prototype, "openCamera", 1);
k([
  H()
], G.prototype, "closeCamera", 1);
k([
  H()
], G.prototype, "openMicrophone", 1);
k([
  H()
], G.prototype, "closeMicrophone", 1);
k([
  H()
], G.prototype, "unMuteSpeaker", 1);
k([
  H()
], G.prototype, "muteSpeaker", 1);
k([
  H()
], G.prototype, "switchScreen", 1);
k([
  H()
], G.prototype, "switchCallMediaType", 1);
k([
  H()
], G.prototype, "switchCamera", 1);
k([
  H()
], G.prototype, "setBlurBackground", 1);
k([
  H()
], G.prototype, "switchDevice", 1);
k([
  gA(IA.setVideoDisplayMode)
], G.prototype, "setVideoDisplayMode", 1);
k([
  gA(IA.setVideoResolution)
], G.prototype, "setVideoResolution", 1);
const x = G.getInstance(), Xe = At.version, $t = "CallInfoContextKey", Aa = "CallerUserInfoContextKey", ea = "UserInfoExcludeVolumeContextKey", ta = "FocusContextKey", aa = "ButtonPanelContextKey", na = "FloatWindowContextKey", oa = "IsClickableContextKey", sa = "PopoverContextKey", la = "CustomUIConfigContextKey", ia = "translateContextKey";
function Jn() {
  return dA(Aa);
}
function Y() {
  return dA($t);
}
function Oe(a) {
  const e = I([]), t = I(""), A = (s) => {
    var l, r, i;
    switch (a) {
      case oA.CAMERA:
        e.value = (s == null ? void 0 : s.cameraList) || [], t.value = ((l = s == null ? void 0 : s.currentCamera) == null ? void 0 : l.deviceId) || "";
        break;
      case oA.MICROPHONE:
        e.value = (s == null ? void 0 : s.microphoneList) || [], t.value = ((r = s == null ? void 0 : s.currentMicrophone) == null ? void 0 : r.deviceId) || "";
        break;
      case oA.SPEAKER:
        e.value = (s == null ? void 0 : s.speakerList) || [], t.value = ((i = s == null ? void 0 : s.currentSpeaker) == null ? void 0 : i.deviceId) || "";
        break;
    }
  }, n = (s) => {
    t.value = s;
  };
  return sA(() => {
    d.watch(
      u.CALL,
      {
        [o.DEVICE_LIST]: A
      },
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), aA(() => {
    d.unwatch(u.CALL, {
      [o.DEVICE_LIST]: A
    });
  }), [{ deviceList: e, currentDeviceId: t }, { updateCurrentDeviceId: n }];
}
const Pt = (a) => typeof a == "object" && a !== null, ua = (a, e) => {
  if (!Pt(a) || !Pt(e))
    return a === e;
  if (a === e)
    return !0;
  const t = Object.keys(a), A = Object.keys(e);
  if (t.length !== A.length)
    return !1;
  for (let n in a)
    if (!ua(a[n], e[n]))
      return !1;
  return !0;
};
function $n(a) {
  const e = document.getElementById(a);
  !document.fullscreenElement && e ? e.requestFullscreen().catch((t) => {
    console.error(`Error attempting to enable fullscreen mode: ${t.message} (${t.name})`);
  }) : document.exitFullscreen();
}
function xe(a) {
  if (typeof a != "object" || a === null)
    return a;
  let e = Array.isArray(a) ? [] : {};
  for (let t in a)
    a.hasOwnProperty(t) && (e[t] = xe(a[t]));
  return e;
}
const Ao = 500;
function nt(a, e) {
  if (typeof a != "function" || e != null && typeof e != "function")
    throw new TypeError("Expected a function");
  const t = function(...A) {
    const n = e ? e.apply(this, A) : A[0], s = t.cache;
    if (s.has(n))
      return s.get(n);
    const l = a.apply(this, A);
    return t.cache = s.set(n, l) || s, l;
  };
  return t.cache = new (nt.Cache || Map)(), t;
}
nt.Cache = Map;
function eo(a) {
  const e = nt(a, (t) => {
    const { cache: A } = e;
    return A.size === Ao && A.clear(), t;
  });
  return e;
}
const to = ".".charCodeAt(0), ao = /\\(\\)?/g, no = RegExp(
  `[^.[\\]]+|\\[(?:([^"'][^[]*)|(["'])((?:(?!\\2)[^\\\\]|\\\\.)*?)\\2)\\]|(?=(?:\\.|\\[\\])(?:\\.|\\[\\]|$))`,
  "g"
), ra = eo((a) => {
  const e = [];
  return a.charCodeAt(0) === to && e.push(""), a.replace(no, (t, A, n, s) => {
    let l = t;
    n ? l = s.replace(ao, "$1") : A && (l = A.trim()), e.push(l);
  }), e;
});
function RA(a, e, t) {
  if (typeof a != "object" || !e)
    return;
  const A = ra(e);
  let n = a;
  for (let s = 0; s < A.length; s++) {
    if (n == null)
      return;
    const l = A[s];
    s !== A.length - 1 ? n = n == null ? void 0 : n[l] : Object.assign(n, { [l]: t });
  }
}
function oe(a, e, t) {
  if (typeof a != "object")
    return;
  const A = ra(e);
  let n = a;
  for (let s = 0; s < A.length; s++) {
    if (n == null)
      return;
    const l = A[s];
    s !== A.length - 1 ? n = n == null ? void 0 : n[l] : Array.isArray(n) && n.splice(l, 0, t);
  }
}
function ca(a) {
  return a == null || typeof a == "string" && a.trim().length === 0 ? !0 : Array.isArray(a) || typeof a == "object" ? Object.keys(a).length === 0 : !1;
}
function Ke(a, e, t, A, n) {
  Object.keys(a).forEach((s) => {
    const l = a[s], r = ca(t) ? s : `${t}.${s}`;
    if (typeof l == "object")
      if (Array.isArray(l))
        for (let i = 0; i < l.length; i++) {
          const c = `${r}.${i}`;
          Ke(l[i], e, c, A, n);
        }
      else
        Ke(l, e, r, A, n);
    else if (e(l)) {
      const i = typeof n == "function" ? n({ key: l, value: r }) : r;
      A.push(i);
    }
  });
}
function da() {
  const a = I(), e = I(d.getData(u.CALL, o.REMOTE_USER_INFO_LIST)), t = (s) => {
    (y.isWeChat ? s.enableMic : s.isAudioAvailable) && (a.value = { ...a.value, localVideo: s.volume });
  }, A = (s) => {
    const l = {};
    (s.length !== e.value.length || !ua(s, e.value)) && (e.value = s, e.value.forEach((r) => {
      if (y.isWeChat ? r.hasAudio : r.isAudioAvailable) {
        const c = y.isWeChat ? r.userID : r.domId;
        l[c] = r.volume;
      }
    }), a.value = { ...a.value, ...l });
  };
  let n = {
    [o.LOCAL_USER_INFO]: t,
    [o.REMOTE_USER_INFO_LIST]: A
  };
  return y.isUniPlatform && (n = {
    [o.PUSHER]: t,
    [o.PLAYER]: A
  }), sA(() => {
    d.watch(
      u.CALL,
      n,
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), aA(() => {
    d.unwatch(u.CALL, n);
  }), a;
}
function oo() {
  const a = I(""), e = I(!0), t = I(0), A = (n) => {
    typeof n == "object" ? (a.value = n.text, t.value = n.duration || 0) : a.value = n;
  };
  return sA(() => {
    d.watch(
      u.CALL,
      {
        [o.CALL_TIPS]: A
      },
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), aA(() => {
    d.unwatch(
      u.CALL,
      {
        [o.CALL_TIPS]: A
      }
    );
  }), { tip: a, show: e, duration: t };
}
function so() {
  const a = I(d.getData(u.CALL, o.NETWORK_STATUS)), e = (t) => {
    a.value = t;
  };
  return sA(() => {
    d.watch(
      u.CALL,
      {
        [o.NETWORK_STATUS]: e
      },
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), aA(() => {
    d.unwatch(u.CALL, {
      [o.NETWORK_STATUS]: e
    });
  }), { netWorkQualityList: a };
}
function ga() {
  const a = I(d.getData(u.CALL, o.PLAYER)), e = (A) => {
    a.value = A == null ? void 0 : A.map((n) => {
      const { userID: s, hasVideo: l, hasAudio: r } = n;
      return { userID: s, hasVideo: l, hasAudio: r };
    });
  }, t = {
    [o.PLAYER]: e
  };
  return sA(() => {
    d.watch(
      u.CALL,
      t,
      { notifyRangeWhenWatch: o.MYSELF }
    );
  }), aA(() => {
    d.unwatch(u.CALL, t);
  }), a;
}
function pA() {
  return dA(ea);
}
function ot() {
  const a = I(d.getData(u.CALL, o.CALL_DURATION)), e = (t) => {
    a.value = t;
  };
  return sA(() => {
    d.watch(
      u.CALL,
      {
        [o.CALL_DURATION]: e
      },
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), aA(() => {
    d.unwatch(
      u.CALL,
      {
        [o.CALL_DURATION]: e
      }
    );
  }), { callDuration: a };
}
function ge() {
  return dA(aa, { status: I("open") });
}
function Ia() {
  return dA(ta, I("open"));
}
function LA() {
  return dA(na);
}
function lo() {
  return dA(oa);
}
function Be() {
  return dA(sa);
}
const io = (a, e) => {
  const t = [{
    i: 0,
    x: 0,
    y: 0,
    w: e,
    h: e
  }];
  for (let A = 1; A < a; A++) {
    const n = t[A - 1].x + e === 12;
    t[A] = {
      i: A,
      x: t[A - 1].x + e === 12 ? 0 : t[A - 1].x + e,
      y: t[A - 1].y + (n ? e : 0),
      w: e,
      h: e
    };
  }
  return a === 3 && (t[a - 1].x += 3), a > 3 && y.isPC && a % 3 === 2 && (t[a - 1].x += 2, t[a - 2].x += 2), t;
};
function uo(a, e) {
  const { isFloatWindow: t } = b(LA()), A = I();
  return K([a, e, t], () => {
    if (t.value) {
      const c = [];
      for (let g = 0; g < e.value; g++)
        c[g] = {
          i: g,
          x: 0,
          y: 0,
          w: 12,
          h: 12
        };
      A.value = c;
      return;
    }
    const n = Object.keys(Array.from({ length: e.value })), s = e.value <= 4 ? 6 : 4, l = io(e.value, s);
    A.value = l;
    let r, i;
    if (a.value !== null) {
      if (n.length < 5) {
        const c = n.concat();
        c.splice(a.value, 1), c.unshift(a.value);
      } else
        i = a.value % 3, r = Math.floor(a.value / 3);
      if (n.length < 5) {
        const c = l.findIndex((g) => g.i === a.value);
        if (c !== -1) {
          const g = l[0];
          l[0] = l[c], l[c] = g;
          for (let f = 0; f < l.length; f++) {
            const p = l[f];
            f === 0 ? (p.w += 6, p.h += 6, p.x = 0, p.y = 0) : (p.x = (f - 1) * 4, p.y = 12, p.w = 4, p.h = 4);
          }
        }
      } else {
        let c;
        i === 0 ? (l[a.value + 1] && (l[a.value + 1].x += 4), l[a.value + 2] && (l[a.value + 2].y += 4), c = {
          i: a.value,
          x: 0,
          y: r * 4,
          w: 8,
          h: 8
        }) : i === 2 ? (c = {
          i: a.value,
          x: 4,
          y: r * 4,
          w: 8,
          h: 8
        }, l[a.value - 1].x = 0, l[a.value - 1].y += 4) : i === 1 && (c = {
          i: a.value,
          x: 4,
          y: r * 4,
          w: 8,
          h: 8
        }, l[a.value + 1] && (l[a.value + 1].x = 0, l[a.value + 1].y += 4));
        const g = 3 - i;
        for (let f = a.value + g; f < l.length; f++) {
          const p = l[f];
          p.y += 4;
        }
        l[a.value] = c;
      }
      A.value = l;
    }
  }, {
    immediate: !0
  }), A;
}
function Ie() {
  return dA(la);
}
const ro = {
  calling: [[
    { name: "switchCamera", customStyle: { justifyContent: "flex-start" }, props: {} },
    { name: "virtualBackground" },
    { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
  ], [
    {},
    { name: "hangup", customStyle: { paddingTop: "6vh" } },
    {}
  ]],
  accept: [[
    { name: "switchCamera", customStyle: { justifyContent: "flex-start" }, props: {} },
    { name: "virtualBackground" },
    { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
  ], [
    { name: "reject", customStyle: { paddingTop: "6vh", justifyContent: "center" } },
    {},
    { name: "accept", customStyle: { paddingTop: "6vh", justifyContent: "center" } }
  ]],
  connected: [[
    { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
    { name: "speaker" },
    { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
  ], [
    { name: "virtualBackground", customStyle: { justifyContent: "center", paddingTop: "6vh" }, props: {} },
    { name: "hangup", customStyle: { paddingTop: "6vh" } },
    { name: "switchCamera", customStyle: { justifyContent: "center", paddingTop: "6vh" }, props: {} }
  ]]
}, co = {
  singleCall: {
    video: {
      calling: [
        [],
        [
          {},
          { name: "hangup" },
          {}
        ]
      ],
      accept: [
        [],
        [
          { name: "reject", customStyle: { justifyContent: "flex-end" } },
          {},
          { name: "accept", customStyle: { justifyContent: "flex-start" } }
        ]
      ],
      connected: [
        [
          { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
          { name: "speaker" },
          { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
        ],
        [
          {},
          { name: "hangup", customStyle: { paddingTop: "6vh" } },
          { name: "switchCamera", customStyle: { justifyContent: "center", paddingTop: "6vh" }, props: {} }
        ]
      ]
    },
    audio: {
      calling: [
        [],
        [
          {},
          { name: "hangup" },
          {}
        ]
      ],
      accept: [
        [],
        [
          { name: "reject", customStyle: { justifyContent: "flex-end" } },
          {},
          { name: "accept", customStyle: { justifyContent: "flex-start" } }
        ]
      ],
      connected: [
        [],
        [
          { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
          { name: "hangup" },
          { name: "speaker", customStyle: { justifyContent: "flex-end" } }
        ]
      ]
    }
  },
  groupCall: {
    video: {
      calling: [[
        { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
        { name: "speaker" },
        { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
      ], [
        {},
        { name: "hangup", customStyle: { paddingTop: "6vh" } },
        {}
      ]],
      accept: [
        [],
        [
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
          { name: "speaker" },
          { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
        ],
        [
          {},
          { name: "hangup", customStyle: { paddingTop: "6vh" } },
          {}
        ]
      ],
      close_calling: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "camera", props: {} },
          { name: "hangup" }
        ]
      ],
      close_connected: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "camera", props: {} },
          { name: "hangup" }
        ]
      ]
    },
    audio: {
      calling: [[
        { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
        { name: "speaker" },
        { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
      ], [
        {},
        { name: "hangup", customStyle: { paddingTop: "6vh" } },
        {}
      ]],
      accept: [
        [],
        [
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "microphone", customStyle: { justifyContent: "flex-start" }, props: {} },
          { name: "speaker" },
          { name: "camera", customStyle: { justifyContent: "flex-end" }, props: {} }
        ],
        [
          {},
          { name: "hangup", customStyle: { paddingTop: "6vh" } },
          {}
        ]
      ],
      close_calling: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "camera", props: {} },
          { name: "hangup" }
        ]
      ],
      close_connected: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "camera", props: {} },
          { name: "hangup" }
        ]
      ]
    }
  }
}, go = {
  singleCall: {
    video: {
      calling: [
        [
          { name: "camera", props: {} },
          { name: "microphone", props: {} },
          { name: "hangup" }
        ]
      ],
      accept: [
        [
          { name: "camera", props: {} },
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "camera", props: {} },
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "hangup" }
        ]
      ]
    },
    audio: {
      calling: [
        [
          { name: "microphone", props: {} },
          { name: "hangup" }
        ]
      ],
      accept: [
        [
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "hangup" }
        ]
      ]
    }
  },
  groupCall: {
    video: {
      calling: [[
        { name: "camera", props: {} },
        { name: "microphone", props: {} },
        { name: "inviteUser", props: {} },
        { name: "hangup" }
      ]],
      accept: [
        [
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "camera", props: {} },
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "inviteUser", props: {} },
          { name: "hangup" }
        ]
      ]
    },
    audio: {
      calling: [
        [
          { name: "microphone", props: {} },
          { name: "hangup" }
        ]
      ],
      accept: [
        [
          { name: "reject" },
          { name: "accept" }
        ]
      ],
      connected: [
        [
          { name: "microphone", props: {} },
          { name: "speaker" },
          { name: "inviteUser", props: {} },
          { name: "hangup" }
        ]
      ]
    }
  }
}, Vt = {
  pc: go,
  mobile: co
};
function pa(a) {
  const e = xe(a);
  return RA(e, "mobile.singleCall.video", ro), oe(e, "pc.singleCall.video.calling[0][2]", { name: "virtualBackground", props: {} }), oe(e, "pc.singleCall.video.accept[0][1]", { name: "virtualBackground", props: {} }), oe(e, "pc.singleCall.video.connected[0][3]", { name: "virtualBackground", props: {} }), oe(e, "pc.groupCall.video.calling[0][3]", { name: "virtualBackground", props: {} }), oe(e, "pc.groupCall.video.connected[0][4]", { name: "virtualBackground", props: {} }), e;
}
function Io(a, e, t) {
  let A = xe(a);
  return e ? (RA(A, "mobile.singleCall.video.connected[1][2].props.show", !0), t && pa(A)) : (RA(A, "mobile.singleCall.video.connected[1][2].props.show", !1), t && (RA(A, "mobile.singleCall.video.connected[1][0].props.show", !1), RA(A, "pc.singleCall.video.connected[0][3].props.show", !1), RA(A, "pc.groupCall.video.connected[0][4].props.show", !1))), A;
}
function po() {
  const { isShowEnableVirtualBackground: a, callStatus: e } = b(Y()), t = Ie(), { localUserInfoExcludeVolume: A } = b(pA()), n = M(() => (A == null ? void 0 : A.value.isVideoAvailable) || !1), s = M(() => a.value && !y.isH5), l = I([]);
  return K([t, a, n], () => {
    let r = xe(Vt);
    s.value && (r = pa(Vt)), e.value === w.CONNECTED && (r = Io(r, n.value, s.value));
    const { button: i } = t.value, c = [];
    function g(p) {
      return Object.keys(i).includes(p);
    }
    function f({ key: p, value: C }) {
      var z;
      const N = C.split(".");
      let h = N.slice(0, N.length - 1);
      const L = N.slice(0, N.length - 2);
      if (L[L.length - 1] === "0")
        for (let T = 0; T < 3; T++) {
          let j = L.slice();
          j.push(T), j.push("customStyle"), j.push("justifyContent"), j = j.join("."), c.push({ path: j, value: "center" });
        }
      return h.push("props"), h.push("show"), h = h.join("."), {
        path: h,
        value: (z = i == null ? void 0 : i[p]) == null ? void 0 : z.show
      };
    }
    Ke(r, g, "", c, f), c == null || c.forEach((p) => {
      RA(r, p.path, p.value);
    }), l.value = r;
  }, {
    immediate: !0
  }), l;
}
function Ca() {
  const a = Ie(), e = I(a.value.viewBackground);
  return K(a, () => {
    e.value = a.value.viewBackground;
  }), e;
}
function Co() {
  const a = I(d.getData(u.CALL, o.ROOM_ID)), e = I(d.getData(u.CALL, o.ROOM_ID_TYPE)), t = I(d.getData(u.CALL, o.GROUP_ID)), A = I(d.getData(u.CALL, o.CALL_MEDIA_TYPE)), n = I(d.getData(u.CALL, o.GROUP_CALL_MEMBERS)), s = I(d.getData(u.CALL, o.CALL_STATUS)), l = (C) => {
    a.value = C;
  }, r = (C) => {
    t.value = C;
  }, i = (C) => {
    A.value = C;
  }, c = (C) => {
    n.value = C;
  }, g = (C) => {
    s.value = C;
  }, f = (C) => {
    e.value = C;
  }, p = {
    [o.ROOM_ID]: l,
    [o.GROUP_ID]: r,
    [o.CALL_MEDIA_TYPE]: i,
    [o.GROUP_CALL_MEMBERS]: c,
    [o.CALL_STATUS]: g,
    [o.ROOM_ID_TYPE]: f
  };
  return sA(() => {
    d.watch(
      u.CALL,
      p,
      {
        notifyRangeWhenWatch: o.MYSELF
      }
    );
  }), aA(() => {
    d.unwatch(u.CALL, p);
  }), {
    roomId: a,
    roomIdType: e,
    groupId: t,
    callMediaType: A,
    groupCallMembers: n,
    callStatus: s
  };
}
function $() {
  return dA(ia, I(En));
}
const Mo = {
  customStyle: {
    type: Object
  },
  customClass: {
    type: String
  }
}, fo = ["start", "center", "end", "space-around", "space-between", "space-evenly"], No = ["top", "middle", "bottom"], Lo = {
  gutter: {
    type: Number,
    default: 0
  },
  justify: {
    type: String,
    values: fo,
    default: "start"
  },
  align: {
    type: String,
    values: No,
    default: "top"
  },
  customStyle: {
    type: Object,
    default: () => {
    }
  }
};
function Q(...a) {
  const e = [], t = {}.hasOwnProperty;
  for (let A = 0; A < a.length; A++) {
    const n = a[A];
    if (!n)
      continue;
    const s = typeof n;
    if (s === "string" || s === "number")
      e.push(n);
    else if (Array.isArray(n)) {
      if (n.length) {
        const l = Q.apply(null, n);
        l && e.push(l);
      }
    } else if (s === "object") {
      if (n.toString !== Object.prototype.toString && !n.toString.toString().includes("[native code]")) {
        e.push(n.toString());
        continue;
      }
      for (const l in n)
        t.call(n, l) && n[l] && e.push(l);
    }
  }
  return e.join(" ");
}
const de = typeof wx < "u" && typeof wx.getSystemInfoSync == "function" && Boolean(wx.getSystemInfoSync().fontSizeSetting), vo = typeof uni < "u" && typeof uni > "u", He = de || vo, So = typeof uni < "u", Ma = function() {
  return (typeof uni < "u" || typeof window < "u") && !He;
}();
(function() {
  return de ? wx : So ? uni : window;
})();
const VA = Ma && window && window.navigator && window.navigator.userAgent || "", Do = /Android/i.test(VA), ho = /(?:Windows Phone)/.test(VA), mo = /(?:SymbianOS)/.test(VA), yo = /iPad/i.test(VA) || /iPhone/i.test(VA) || /iPod/i.test(VA), fa = Do || ho || mo || yo, XA = Ma && !fa;
XA && VA.includes("Windows NT");
XA && VA.includes("Mac");
function ze(a) {
  const e = {};
  for (const t in a)
    Object.prototype.hasOwnProperty.call(a, t) && a[t] !== void 0 && (e[t] = a[t]);
  return e;
}
function To() {
  let a = "unknow";
  return typeof Xe == "string" && (a = Xe.split(".")[0]), { version: Xe, majorVersion: a };
}
function _t(a, e) {
  const { key: t, value: A } = e;
  return a.find((n) => n[t] === A);
}
const Na = "RowContextKey", U = "tk", ie = {
  SUCCESS: "success",
  INFO: "info",
  WARNING: "warning",
  ERROR: "error"
}, Eo = {
  options: {
    virtualHost: !0
  }
}, wo = /* @__PURE__ */ S({
  ...Eo,
  __name: "Row",
  props: Lo,
  setup(a) {
    const e = a, t = M(() => e.gutter);
    rA(Na, {
      gutter: t
    });
    const A = Q([
      `${U}-row`,
      `${U}-justify-${e.justify}`,
      `${U}-align-${e.align}`
    ]);
    return { __sfc: !0, props: e, gutter: t, classname: A };
  }
});
function v(a, e, t, A, n, s, l, r) {
  var i = typeof a == "function" ? a.options : a;
  e && (i.render = e, i.staticRenderFns = t, i._compiled = !0), A && (i.functional = !0), s && (i._scopeId = "data-v-" + s);
  var c;
  if (l ? (c = function(p) {
    p = p || this.$vnode && this.$vnode.ssrContext || this.parent && this.parent.$vnode && this.parent.$vnode.ssrContext, !p && typeof __VUE_SSR_CONTEXT__ < "u" && (p = __VUE_SSR_CONTEXT__), n && n.call(this, p), p && p._registeredComponents && p._registeredComponents.add(l);
  }, i._ssrRegister = c) : n && (c = r ? function() {
    n.call(
      this,
      (i.functional ? this.parent : this).$root.$options.shadowRoot
    );
  } : n), c)
    if (i.functional) {
      i._injectStyles = c;
      var g = i.render;
      i.render = function(C, N) {
        return c.call(N), g(C, N);
      };
    } else {
      var f = i.beforeCreate;
      i.beforeCreate = f ? [].concat(f, c) : [c];
    }
  return {
    exports: a,
    options: i
  };
}
var jo = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.classname, style: [e.customStyle] }, [e._t("default")], 2);
}, bo = [], zo = /* @__PURE__ */ v(
  wo,
  jo,
  bo,
  !1,
  null,
  null,
  null,
  null
);
const st = zo.exports, Uo = ["start", "center", "end", "space-around", "space-between", "space-evenly"], Oo = ["top", "middle", "bottom"], xo = {
  span: {
    type: Number,
    default: 24
  },
  justify: {
    type: String,
    values: Uo,
    default: "start"
  },
  align: {
    type: String,
    values: Oo,
    default: "middle"
  },
  offset: {
    type: Number,
    default: 0
  }
}, Bo = {
  options: {
    virtualHost: !0
  }
}, Po = /* @__PURE__ */ S({
  ...Bo,
  __name: "Col",
  props: xo,
  setup(a) {
    const e = a, { gutter: t } = dA(Na, { gutter: M(() => 0) }), A = Q([
      `${U}-col`,
      `${U}-justify-${e.justify}`,
      `${U}-align-${e.align}`
    ]), n = M(() => `${e.span / 24 * 100}%`), s = M(() => `${e.offset / 24 * 100}%`), l = M(() => `${t.value / 2}px`);
    return { __sfc: !0, props: e, gutter: t, classname: A, width: n, marginLeft: s, paddingLeft: l, paddingRight: l };
  }
});
var Vo = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.classname, style: {
    width: A.width,
    marginLeft: A.marginLeft,
    paddingLeft: A.paddingLeft,
    paddingRight: A.paddingRight
  } }, [e._t("default")], 2);
}, _o = [], Fo = /* @__PURE__ */ v(
  Po,
  Vo,
  _o,
  !1,
  null,
  null,
  null,
  null
);
const lt = Fo.exports, Ro = {
  width: {
    type: String,
    default: "40px"
  },
  height: {
    type: String,
    default: "40px"
  },
  color: {
    type: String
  }
}, Go = {
  options: {
    virtualHost: !0
  }
}, ko = /* @__PURE__ */ S({
  ...Go,
  __name: "Dot",
  props: Ro,
  setup(a) {
    const e = a, t = `${U}-loading_dot-container`, A = `${U}-loading_dot`, n = M(() => ({
      width: e.width,
      height: e.height
    })), s = M(() => {
      const l = {};
      return e.color && (l.backgroundColor = e.color), l;
    });
    return { __sfc: !0, props: e, dotContainerClassName: t, dotClassName: A, dotContainerStyle: n, dotStyle: s };
  }
});
var Wo = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.dotContainerClassName, style: [A.dotContainerStyle] }, [t("div", { class: A.dotClassName, style: [A.dotStyle] }), t("div", { class: A.dotClassName, style: [A.dotStyle] }), t("div", { class: A.dotClassName, style: [A.dotStyle] })]);
}, qo = [], Xo = /* @__PURE__ */ v(
  ko,
  Wo,
  qo,
  !1,
  null,
  null,
  null,
  null
);
const Qo = Xo.exports, Zo = {
  width: {
    type: String,
    default: "40px"
  },
  height: {
    type: String,
    default: "40px"
  },
  color: {
    type: String
  }
}, Yo = {
  options: {
    virtualHost: !0
  }
}, Ko = /* @__PURE__ */ S({
  ...Yo,
  __name: "Circle",
  props: Zo,
  setup(a) {
    const e = a, t = M(() => {
      const n = {
        width: e.width,
        height: e.height
      };
      return e.color && (n["--tk-loading-primary-color"] = e.color), n;
    }), A = `${U}-loading_circle-container`;
    return { __sfc: !0, props: e, circleContainerStyle: t, circleContainerClassName: A };
  }
});
var Ho = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.circleContainerClassName, style: [A.circleContainerStyle] });
}, Jo = [], $o = /* @__PURE__ */ v(
  Ko,
  Ho,
  Jo,
  !1,
  null,
  null,
  null,
  null
);
const As = $o.exports, es = ["circle", "dot"], ts = ["row", "column"], as = {
  mode: {
    type: String,
    values: es,
    default: "circle"
  },
  loadingWidth: {
    type: String,
    default: "40px"
  },
  loadingHeight: {
    type: String,
    default: "40px"
  },
  color: {
    type: String
  },
  text: {
    type: String
  },
  layout: {
    type: String,
    values: ts,
    default: "column"
  }
}, ns = {
  options: {
    virtualHost: !0
  }
}, os = /* @__PURE__ */ S({
  ...ns,
  __name: "Loading",
  props: as,
  setup(a) {
    const e = a, t = `${U}-loading ${U}-loading--${e.mode}`, A = `${U}-loading_text`, n = M(() => ({
      flexDirection: e.layout
    }));
    return { __sfc: !0, props: e, loadingContainerClassName: t, loadingTextClassName: A, loadingContainerStyle: n, LoadingDot: Qo, LoadingCircle: As };
  }
});
var ss = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.loadingContainerClassName, style: [A.loadingContainerStyle] }, [e.mode === "dot" ? t(A.LoadingDot, { attrs: { width: e.loadingWidth, height: e.loadingHeight, color: e.color } }) : t(A.LoadingCircle, { attrs: { width: e.loadingWidth, height: e.loadingHeight, color: e.color } }), e.text ? t("div", { class: A.loadingTextClassName }, [e._v(e._s(e.text))]) : e._e()], 1);
}, ls = [], is = /* @__PURE__ */ v(
  os,
  ss,
  ls,
  !1,
  null,
  null,
  null,
  null
);
const it = is.exports, us = {
  src: {
    type: String
  },
  size: {
    type: Number,
    default: 20
  }
}, rs = ["fill", "contain", "cover"], Ft = {
  fill: "scaleToFill",
  contain: "aspectFit",
  cover: "aspectFill"
}, cs = {
  width: {
    type: String,
    default: "320px"
  },
  height: {
    type: String,
    default: "240px"
  },
  src: {
    type: String
  },
  fit: {
    type: String,
    values: rs,
    default: "fill"
  },
  customStyle: {
    type: Object
  },
  defaultSrc: {
    type: String
  }
}, ds = {
  error: (a) => a
}, gs = {
  options: {
    virtualHost: !0
  }
}, Is = /* @__PURE__ */ S({
  ...gs,
  __name: "TKImage",
  props: cs,
  emits: ds,
  setup(a, { emit: e }) {
    const t = a, A = I(!1), n = I(t.src);
    K(() => t.src, () => {
      A.value = !1, n.value = t.src;
    });
    const s = M(() => Q([`${U}-image`])), l = M(() => ({
      width: t.width,
      height: t.height,
      ...t.customStyle
    })), r = M(() => Q([
      `${U}-image_inner`
    ])), i = M(() => de ? Ft[t.fit] || Ft.cover : t.fit);
    function c(g) {
      A.value = !0, n.value = t.defaultSrc, e("error", g);
    }
    return { __sfc: !0, props: t, emit: e, hasLoadError: A, imgSrc: n, classname: s, imgStyle: l, imageClassName: r, objectFit: i, handleError: c, IN_WX_MINI_APP: de };
  }
});
var ps = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.classname, style: [A.imgStyle] }, [A.hasLoadError && !e.defaultSrc ? [e._t("error")] : [A.IN_WX_MINI_APP ? e._e() : t("img", { class: A.imageClassName, style: { objectFit: A.objectFit }, attrs: { src: A.imgSrc }, on: { error: A.handleError } }), A.IN_WX_MINI_APP ? t("image", { class: A.imageClassName, attrs: { src: A.imgSrc, mode: A.objectFit }, on: { error: A.handleError } }) : e._e()]], 2);
}, Cs = [], Ms = /* @__PURE__ */ v(
  Is,
  ps,
  Cs,
  !1,
  null,
  null,
  null,
  null
);
const hA = Ms.exports, fs = {
  options: {
    virtualHost: !0
  }
}, Ns = /* @__PURE__ */ S({
  ...fs,
  __name: "Icon",
  props: us,
  setup(a) {
    const e = a, t = M(() => `${e.size}px`), A = M(() => `${e.size}px`);
    return { __sfc: !0, props: e, width: t, height: A, TKImage: hA };
  }
});
var Ls = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t(A.TKImage, { attrs: { src: e.src, width: A.width, height: A.height } });
}, vs = [], Ss = /* @__PURE__ */ v(
  Ns,
  Ls,
  vs,
  !1,
  null,
  null,
  null,
  null
);
const Pe = Ss.exports, Ds = ["small", "middle", "large"], hs = ["row", "column"], ms = ["circle", "round"], ys = {
  iconSrc: {
    type: String
  },
  iconSize: {
    type: Number
  },
  text: {
    type: String
  },
  loading: {
    type: Boolean,
    default: !1
  },
  loadingColor: {
    type: String,
    default: "#fff"
  },
  loadingWidth: {
    type: String,
    default: "40px"
  },
  loadingHeight: {
    type: String,
    default: "40px"
  },
  size: {
    type: String,
    values: Ds,
    default: "middle"
  },
  width: {
    type: String
  },
  height: {
    type: String
  },
  color: {
    type: String
  },
  direction: {
    type: String,
    values: hs,
    default: "row"
  },
  shape: {
    type: String,
    values: ms
  },
  buttonStyle: {
    type: Object
  },
  buttonTextStyle: {
    type: Object
  }
}, Ts = {
  options: {
    virtualHost: !0
  }
}, Es = /* @__PURE__ */ S({
  ...Ts,
  __name: "Button",
  props: ys,
  emits: ["click"],
  setup(a, { emit: e }) {
    const t = a, A = `${U}-button--content`, n = M(() => ({
      width: t.width,
      height: t.height,
      backgroundColor: t.color,
      flexDirection: t.direction,
      cursor: XA ? "pointer" : "auto",
      ...t.buttonStyle
    })), s = Q([
      `${U}-button`,
      { [`${U}-${t.shape}`]: t.shape },
      `${U}-button--${t.size}`
    ]);
    return { __sfc: !0, props: t, buttonTextClassName: A, style: n, buttonClassName: s, emit: e, handleClick: (r) => {
      !t.loading && e("click", r);
    }, Loading: it, Icon: Pe };
  }
});
var ws = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.buttonClassName, style: [A.style], on: { click: A.handleClick } }, [e.loading ? t(A.Loading, { attrs: { loadingWidth: e.loadingWidth, loadingHeight: e.loadingHeight, color: e.loadingColor } }) : e._e(), e.iconSrc && !e.loading ? t(A.Icon, { attrs: { size: e.iconSize, src: e.iconSrc } }) : e._e(), e.text ? t("div", { class: A.buttonTextClassName, style: [e.buttonTextStyle] }, [e._v(" " + e._s(e.text) + " ")]) : e._e()], 1);
}, js = [], bs = /* @__PURE__ */ v(
  Es,
  ws,
  js,
  !1,
  null,
  null,
  null,
  null
);
const MA = bs.exports, Je = "data:image/svg+xml;base64,", La = "data:image/svg+xml;base64,", va = "data:image/svg+xml;base64,", Sa = "data:image/svg+xml;base64,", Da = "data:image/svg+xml;base64,", ha = "data:image/svg+xml;base64,", ma = "data:image/svg+xml;base64,", zs = "data:image/svg+xml;base64,", Us = "data:image/svg+xml;base64,", BA = "data:image/svg+xml;base64,", $e = "data:image/svg+xml;base64,", Os = "data:image/svg+xml;base64,", xs = "data:image/svg+xml;base64,", Bs = "data:image/svg+xml;base64,", Ps = "data:image/svg+xml;base64,", OA = "data:image/svg+xml;base64,", Vs = "data:image/svg+xml;base64,", _s = "data:image/svg+xml;base64,", eA = !y.isPC;
let ya = Ps, Ta = Vs;
eA || (ya = xs, Ta = Bs);
const R = {
  width: eA ? "60px" : "40px",
  height: eA ? "60px" : "40px",
  shape: "circle",
  iconSize: eA ? 30 : 20,
  showText: !0,
  textColor: "#D5E0F2",
  textSize: "12px",
  textStyle: {
    marginTop: "5px"
  }
}, m = {
  accept: {
    basicConfig: {
      ...R,
      color: "#51C271",
      iconSrc: Us
    },
    loadingConfig: {
      ...R,
      color: "#51C271",
      loadingWidth: eA ? "30px" : "20px",
      loadingHeight: eA ? "30px" : "20px"
    }
  },
  hangup: {
    basicConfig: {
      ...R,
      color: "#ED4651",
      iconSrc: Je
    },
    loadingConfig: {
      ...R,
      color: "#ED4651",
      loadingWidth: eA ? "30px" : "20px",
      loadingHeight: eA ? "30px" : "20px"
    }
  },
  reject: {
    basicConfig: {
      ...R,
      color: "#ED4651",
      iconSrc: Je
    }
  },
  camera: {
    basicConfig: {
      ...R,
      color: "#FFFFFF",
      iconSrc: La,
      shape: "circle"
    },
    closedConfig: {
      ...R,
      color: "#6b758a4d",
      iconSrc: va
    },
    loadingConfig: {
      ...R,
      color: "#6b758a4d",
      loadingWidth: eA ? "30px" : "20px",
      loadingHeight: eA ? "30px" : "20px"
    }
  },
  microphone: {
    basicConfig: {
      ...R,
      color: "#FFFFFF",
      iconSrc: Sa
    },
    closedConfig: {
      ...R,
      color: "#6b758a4d",
      iconSrc: Da
    }
  },
  speaker: {
    basicConfig: {
      ...R,
      color: "#FFFFFF",
      iconSrc: ha
    },
    closedConfig: {
      ...R,
      color: "#6b758a4d",
      iconSrc: ma
    }
  },
  fullScreen: {
    basicConfig: {
      iconSize: 20,
      iconSrc: Os
    }
  },
  minimize: {
    basicConfig: {
      iconSize: eA ? 24 : 20,
      iconSrc: ya
    }
  },
  switchCamera: {
    basicConfig: {
      ...R,
      color: "transparent",
      iconSrc: OA,
      shape: "circle"
    }
  },
  inviteUser: {
    basicConfig: {
      ...R,
      color: eA ? "" : "#6b758a4d",
      width: eA ? "24px" : "40px",
      height: eA ? "24px" : "40px",
      shape: eA ? "" : "circle",
      iconSize: eA ? 24 : 20,
      iconSrc: Ta
    }
  },
  toggleButtonPanel: {
    basicConfig: {
      color: "transparent",
      width: "40px",
      height: "40px",
      shape: "circle",
      iconSize: 40,
      iconSrc: _s
    }
  },
  virtualBackground: {
    basicConfig: {
      ...R,
      color: "#6b758a4d",
      iconSrc: BA,
      shape: "circle"
    },
    closedConfig: {
      ...R,
      color: "#FFFFFF",
      iconSrc: $e
    },
    loadingConfig: {
      ...R,
      color: "#6b758a4d",
      loadingWidth: eA ? "30px" : "20px",
      loadingHeight: eA ? "30px" : "20px"
    },
    disableConfig: {
      ...R,
      color: "#6b758a4d",
      iconSrc: BA,
      buttonStyle: {
        opacity: 0.6
      }
    }
  }
}, UA = {
  singleCall: {
    video: {
      calling: {
        ...m,
        switchCamera: {
          basicConfig: {
            ...R,
            color: "#6b758a4d",
            iconSrc: OA,
            shape: "circle",
            showText: !0
          },
          disableConfig: {
            ...R,
            color: "#6b758a4d",
            iconSrc: OA,
            shape: "circle",
            showText: !0,
            buttonStyle: {
              opacity: 0.6
            }
          }
        },
        hangup: {
          basicConfig: {
            ...m.hangup.basicConfig
          },
          loadingConfig: {
            ...m.hangup.loadingConfig
          }
        },
        reject: {
          basicConfig: {
            ...m.reject.basicConfig,
            showText: !1
          }
        }
      },
      accept: {
        ...m,
        switchCamera: {
          basicConfig: {
            ...R,
            color: "#6b758a4d",
            iconSrc: OA,
            shape: "circle"
          },
          disableConfig: {
            ...R,
            color: "#6b758a4d",
            iconSrc: OA,
            shape: "circle",
            showText: !0,
            buttonStyle: {
              opacity: 0.6
            }
          }
        },
        accept: {
          basicConfig: {
            ...m.accept.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.accept.loadingConfig,
            showText: !1
          }
        },
        reject: {
          basicConfig: {
            ...m.reject.basicConfig,
            showText: !1
          }
        }
      },
      connected: {
        ...m,
        virtualBackground: {
          basicConfig: {
            ...R,
            color: "transparent",
            iconSrc: BA,
            shape: "circle",
            showText: !1
          },
          closedConfig: {
            ...R,
            color: "transparent",
            iconSrc: BA,
            showText: !1
          },
          disableConfig: {
            ...R,
            color: "transparent",
            iconSrc: BA,
            showText: !1,
            buttonStyle: {
              opacity: 0.6
            }
          }
        },
        hangup: {
          basicConfig: {
            ...m.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.hangup.loadingConfig,
            showText: !1
          }
        },
        switchCamera: {
          basicConfig: {
            ...m.switchCamera.basicConfig,
            showText: !1
          },
          disableConfig: {
            ...m.switchCamera.basicConfig,
            showText: !1,
            buttonStyle: {
              opacity: 0.6
            }
          }
        }
      }
    },
    audio: {
      calling: m,
      accept: m,
      connected: m
    }
  },
  groupCall: {
    video: {
      calling: {
        ...m,
        switchCamera: {
          basicConfig: {
            ...R,
            color: "#6b758a4d",
            iconSrc: OA,
            shape: "circle"
          }
        },
        hangup: {
          basicConfig: {
            ...m.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.hangup.loadingConfig,
            showText: !1
          }
        },
        accept: {
          basicConfig: {
            ...m.accept.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.accept.loadingConfig,
            showText: !1
          }
        },
        reject: {
          basicConfig: {
            ...m.reject.basicConfig,
            showText: !1
          }
        }
      },
      accept: {
        ...m,
        hangup: {
          basicConfig: {
            ...m.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.hangup.loadingConfig,
            showText: !1
          }
        },
        accept: {
          basicConfig: {
            ...m.accept.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.accept.loadingConfig,
            showText: !1
          }
        },
        reject: {
          basicConfig: {
            ...m.reject.basicConfig,
            showText: !1
          }
        }
      },
      connected: {
        ...m,
        virtualBackground: {
          basicConfig: {
            ...R,
            color: "transparent",
            iconSrc: BA,
            shape: "circle",
            showText: !1
          },
          closedConfig: {
            ...R,
            color: "transparent",
            iconSrc: $e,
            showText: !1
          }
        },
        hangup: {
          basicConfig: {
            ...m.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.hangup.loadingConfig,
            showText: !1
          }
        }
      }
    },
    audio: {
      calling: {
        ...m,
        switchCamera: {
          basicConfig: {
            ...R,
            color: "#6b758a4d",
            iconSrc: OA,
            shape: "circle"
          }
        },
        hangup: {
          basicConfig: {
            ...m.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.hangup.loadingConfig,
            showText: !1
          }
        },
        accept: {
          basicConfig: {
            ...m.accept.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.accept.loadingConfig,
            showText: !1
          }
        },
        reject: {
          basicConfig: {
            ...m.reject.basicConfig,
            showText: !1
          }
        }
      },
      accept: {
        ...m,
        hangup: {
          basicConfig: {
            ...m.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.hangup.loadingConfig,
            showText: !1
          }
        },
        accept: {
          basicConfig: {
            ...m.accept.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.accept.loadingConfig,
            showText: !1
          }
        },
        reject: {
          basicConfig: {
            ...m.reject.basicConfig,
            showText: !1
          }
        }
      },
      connected: {
        ...m,
        hangup: {
          basicConfig: {
            ...m.hangup.basicConfig,
            showText: !1
          },
          loadingConfig: {
            ...m.hangup.loadingConfig,
            showText: !1
          }
        },
        virtualBackground: {
          basicConfig: {
            ...R,
            color: "transparent",
            iconSrc: BA,
            shape: "circle"
          },
          closedConfig: {
            ...R,
            color: "transparent",
            iconSrc: $e
          }
        }
      }
    }
  }
}, Fs = {
  singleCall: {
    video: {
      calling: m,
      accept: m,
      connected: m
    },
    audio: {
      calling: m,
      accept: m,
      connected: m
    }
  },
  groupCall: {
    video: {
      calling: m,
      accept: m,
      connected: m
    },
    audio: {
      calling: m,
      accept: m,
      connected: m
    }
  }
}, Rs = {
  pc: Fs,
  mobile: UA
}, bA = {
  width: "40px",
  height: "40px",
  shape: "circle",
  iconSize: 20,
  textColor: "#D5E0F2"
}, Se = {
  microphone: {
    basicConfig: {
      ...bA,
      color: "#FFFFFF",
      iconSrc: Sa
    },
    closedConfig: {
      ...bA,
      color: "#6b758a4d",
      iconSrc: Da
    }
  },
  speaker: {
    basicConfig: {
      ...bA,
      color: "#FFFFFF",
      iconSrc: ha
    },
    closedConfig: {
      ...bA,
      color: "#6b758a4d",
      iconSrc: ma
    }
  },
  camera: {
    basicConfig: {
      ...bA,
      color: "#FFFFFF",
      iconSrc: La
    },
    closedConfig: {
      ...bA,
      color: "#6b758a4d",
      iconSrc: va
    }
  },
  hangup: {
    basicConfig: {
      ...bA,
      color: "#ED4651",
      iconSrc: Je
    },
    loadingConfig: {
      ...bA,
      color: "#ED4651",
      loadingWidth: "20px",
      loadingHeight: "20px"
    }
  },
  toggleButtonPanel: {
    basicConfig: {
      color: "transparent",
      width: "40px",
      height: "40px",
      shape: "circle",
      iconSize: 40,
      iconSrc: zs
    }
  }
}, Gs = {
  mobile: {
    ...UA,
    groupCall: {
      video: {
        ...UA.groupCall.video,
        calling: {
          ...UA.groupCall.video.calling,
          ...Se
        },
        connected: {
          ...UA.groupCall.video.connected,
          ...Se
        }
      },
      audio: {
        ...UA.groupCall.audio,
        calling: {
          ...UA.groupCall.audio.calling,
          ...Se
        },
        connected: {
          ...UA.groupCall.audio.connected,
          ...Se
        }
      }
    }
  }
}, cA = (a, e) => {
  var c;
  const t = y.isPC ? "pc" : "mobile", { status: A } = ge(), { callStatus: n, callRole: s, isGroupCall: l, callType: r } = b(Y()), i = I((c = m == null ? void 0 : m[a]) == null ? void 0 : c[e.value]);
  return K(
    [e, A, n, l, r],
    () => {
      var N, h, L, D, z;
      const g = l.value ? "groupCall" : "singleCall", f = r.value === _.AUDIO ? "audio" : "video";
      let p = "";
      n.value === w.CALLING ? p = s.value === J.CALLER ? "calling" : "accept" : n.value === w.CONNECTED && (p = "connected");
      let C = Rs;
      A.value === "close" && (C = Gs), i.value = ((z = (D = (L = (h = (N = C == null ? void 0 : C[t]) == null ? void 0 : N[g]) == null ? void 0 : h[f]) == null ? void 0 : L[p]) == null ? void 0 : D[a]) == null ? void 0 : z[e.value]) || {};
    },
    { immediate: !0 }
  ), i;
}, ks = {
  options: {
    virtualHost: !0
  }
}, Ws = /* @__PURE__ */ S({
  ...ks,
  __name: "Minimize",
  setup(a) {
    const e = cA("minimize", I("basicConfig"));
    return { __sfc: !0, config: e, handleClick: async () => {
      if (document != null && document.fullscreenElement)
        try {
          document == null || document.exitFullscreen();
        } catch (A) {
          console.debug(A);
        }
      await x.toggleMinimize();
    }, Button: MA };
  }
});
var qs = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t(A.Button, { attrs: { iconSrc: A.config.iconSrc, iconSize: A.config.iconSize }, on: { click: A.handleClick } });
}, Xs = [], Qs = /* @__PURE__ */ v(
  Ws,
  qs,
  Xs,
  !1,
  null,
  "135d060f",
  null,
  null
);
const Zs = Qs.exports, Ys = {
  options: {
    virtualHost: !0
  }
}, Ks = /* @__PURE__ */ S({
  ...Ys,
  __name: "FullScreen",
  setup(a) {
    const e = cA("fullScreen", I("basicConfig"));
    return { __sfc: !0, config: e, handleClick: async () => {
      await $n("tuicallkit-id");
    }, Button: MA };
  }
});
var Hs = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t(A.Button, { attrs: { iconSrc: A.config.iconSrc, iconSize: A.config.iconSize }, on: { click: A.handleClick } });
}, Js = [], $s = /* @__PURE__ */ v(
  Ks,
  Hs,
  Js,
  !1,
  null,
  "f92abf93",
  null,
  null
);
const Al = $s.exports, el = {
  width: {
    type: String
  },
  color: {
    type: String
  },
  size: {
    type: String
  },
  weight: {
    type: Number
  },
  truncated: {
    type: Boolean,
    default: !1
  },
  lineClamp: {
    type: Number
  },
  textStyle: {
    type: Object
  }
}, tl = {
  options: {
    virtualHost: !0
  }
}, al = /* @__PURE__ */ S({
  ...tl,
  __name: "TKText",
  props: el,
  emits: ["click"],
  setup(a, { emit: e }) {
    const t = a, A = Q([
      `${U}-text`,
      { [`${U}-text--line-clamp`]: t.lineClamp }
    ]), n = M(() => ze({
      maxWidth: t.width,
      fontSize: t.size,
      fontWeight: t.weight,
      color: t.color,
      textOverflow: t.truncated ? "ellipsis" : "auto",
      "-webkit-line-clamp": t.lineClamp,
      ...t.textStyle
    }));
    return { __sfc: !0, props: t, emit: e, textClassName: A, style: n, handleClick: () => {
      e("click");
    } };
  }
});
var nl = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("span", { class: A.textClassName, style: [A.style], on: { click: A.handleClick } }, [e._t("default")], 2);
}, ol = [], sl = /* @__PURE__ */ v(
  al,
  nl,
  ol,
  !1,
  null,
  null,
  null,
  null
);
const lA = sl.exports, mA = {
  width: {
    type: String
  },
  height: {
    type: String
  },
  showText: {
    type: Boolean,
    default: !0
  },
  iconSize: {
    type: String
  }
}, ll = {
  options: {
    virtualHost: !0
  }
}, il = /* @__PURE__ */ S({
  ...ll,
  __name: "InviteUser",
  props: mA,
  setup(a) {
    const e = a, t = cA("inviteUser", I("basicConfig")), A = $();
    return { __sfc: !0, props: e, config: t, t: A, handleClick: async () => {
      d.update(u.CALL, o.SHOW_SELECT_USER, !0);
    }, Button: MA, TKText: lA };
  }
});
var ul = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "btn-content" }, [t(A.Button, { attrs: { iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.config.iconSize, width: A.props.width || A.config.width, height: A.props.height || A.config.height, shape: A.config.shape }, on: { click: A.handleClick } }), e.showText ? t(A.TKText, { attrs: { textStyle: A.config.textStyle, color: A.config.textColor, size: A.config.textSize } }, [e._v(" " + e._s(A.t("invite member")) + " ")]) : e._e()], 1);
}, rl = [], cl = /* @__PURE__ */ v(
  il,
  ul,
  rl,
  !1,
  null,
  "7c062dcf",
  null,
  null
);
const Ea = cl.exports, dl = {
  color: {
    type: String,
    default: "#FFF"
  },
  callDuration: {
    type: String,
    default: ""
  },
  fontSize: {
    type: String,
    default: "16px"
  }
}, gl = {
  options: {
    virtualHost: !0
  }
}, Il = /* @__PURE__ */ S({
  ...gl,
  __name: "Timer",
  props: dl,
  setup(a) {
    const e = a, t = M(() => ({
      color: e.color,
      fontSize: e.fontSize
    }));
    return { __sfc: !0, props: e, timerStyle: t };
  }
});
var pl = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("span", { style: [A.timerStyle] }, [e._v(e._s(e.callDuration))]);
}, Cl = [], Ml = /* @__PURE__ */ v(
  Il,
  pl,
  Cl,
  !1,
  null,
  null,
  null,
  null
);
const ut = Ml.exports, fl = {
  options: {
    virtualHost: !0
  }
}, Nl = /* @__PURE__ */ S({
  ...fl,
  __name: "TopBar",
  props: Mo,
  setup(a) {
    const e = y.isPC, { callStatus: t, isGroupCall: A, callRole: n, allowedFullScreen: s } = b(Y()), { callDuration: l } = ot(), r = Ie(), i = M(() => t.value === w.CONNECTED), c = I(d.getData(u.CALL, o.ENABLE_FLOAT_WINDOW)), g = M(() => {
      var C, N;
      return !A.value || ((N = (C = r.value.button) == null ? void 0 : C[SA.InviteUser]) == null ? void 0 : N.show) === !1 ? !1 : t.value === w.CALLING ? n.value === J.CALLER : !0;
    });
    function f(C) {
      c.value = C;
    }
    const p = {
      [o.ENABLE_FLOAT_WINDOW]: f
    };
    return sA(() => {
      d.watch(
        u.CALL,
        p,
        {
          notifyRangeWhenWatch: o.MYSELF
        }
      );
    }), aA(() => {
      d.unwatch(u.CALL, {
        ...p
      });
    }), { __sfc: !0, isPC: e, callStatus: t, isGroupCall: A, callRole: n, allowedFullScreen: s, callDuration: l, customUIConfig: r, showTimer: i, showMinimize: c, showInviteUser: g, handleEnableFloatWindowChange: f, watchOptions: p, Row: st, Col: lt, Minimize: Zs, FullScreen: Al, InviteUser: Ea, Timer: ut };
  }
});
var Ll = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "top-bar-container", style: e.customStyle }, [A.isPC ? t(A.Row, [t(A.Col, { attrs: { span: 8 } }), t(A.Col, { attrs: { span: 8, justify: "center", align: "center" } }, [A.showTimer ? t(A.Timer, { attrs: { "call-duration": A.callDuration } }) : e._e()], 1), t(A.Col, { attrs: { span: 8, justify: "end", align: "center" } }, [t(A.Row, [t(A.Col, { attrs: { span: 18 } }), t(A.Col, { attrs: { span: 3, justify: "center" } }, [A.showMinimize ? t(A.Minimize) : e._e()], 1), t(A.Col, { attrs: { span: 3, justify: "center" } }, [A.allowedFullScreen ? t(A.FullScreen) : e._e()], 1)], 1)], 1)], 1) : e._e(), A.isPC ? e._e() : t(A.Row, [t(A.Col, { attrs: { span: 8, align: "center" } }, [t(A.Row, [t(A.Col, { attrs: { span: 8, justify: "center" } }, [!A.isPC && A.showMinimize ? t(A.Minimize) : e._e()], 1), t(A.Col, { attrs: { span: 16 } })], 1)], 1), t(A.Col, { attrs: { span: 8, justify: "center", align: "center" } }, [A.showTimer ? t(A.Timer, { attrs: { "call-duration": A.callDuration } }) : e._e()], 1), t(A.Col, { attrs: { span: 8, justify: "end", align: "center" } }, [t(A.Row, [t(A.Col, { attrs: { span: 16 } }), t(A.Col, { attrs: { span: 8, justify: "center" } }, [A.showInviteUser ? t(A.InviteUser, { attrs: { "show-text": A.isPC } }) : e._e()], 1)], 1)], 1)], 1)], 1);
}, vl = [], Sl = /* @__PURE__ */ v(
  Nl,
  Ll,
  vl,
  !1,
  null,
  "977481d5",
  null,
  null
);
const wa = Sl.exports, Dl = {
  show: {
    type: Boolean,
    default: !0
  },
  showMask: {
    type: Boolean,
    default: !0
  },
  showBackgroundImage: {
    type: Boolean,
    default: !0
  },
  blur: {
    type: Boolean,
    default: !0
  },
  bgColor: {
    type: String
  },
  bgImage: {
    type: String
  },
  zIndex: {
    type: Number,
    default: 11e3
  },
  customClass: {
    type: String
  },
  customStyle: {
    type: Object
  },
  customMaskStyle: {
    type: Object
  },
  fit: {
    type: String,
    default: "cover"
  },
  defaultSrc: {
    type: String
  }
}, hl = ["click", "error"], ml = {
  options: {
    virtualHost: !0
  }
}, yl = /* @__PURE__ */ S({
  ...ml,
  __name: "Overlay",
  props: Dl,
  emits: hl,
  setup(a, { emit: e }) {
    const t = a, A = Q([
      `${U}-overlay`,
      t.customClass
    ]), n = `${U}-overlay_mask-container`, s = M(() => Q([
      `${U}-overlay_mask`,
      { [`${U}-blur`]: t.blur }
    ])), l = `${U}-overlay_slot`, r = M(() => ({ zIndex: t.zIndex, ...t.customStyle })), i = M(() => ({
      backgroundColor: t.bgColor,
      ...t.customMaskStyle
    }));
    function c() {
      e("click");
    }
    return { __sfc: !0, props: t, emit: e, overlayContainerClassName: A, maskContainerClassName: n, maskClassName: s, slotClassName: l, overlayStyle: r, maskStyle: i, handleClick: c, handleError: (f) => {
      e("error", f);
    }, TKImage: hA };
  }
});
var Tl = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return e.show ? t("div", { class: A.overlayContainerClassName, style: [A.overlayStyle], on: { click: A.handleClick } }, [t("div", { class: A.maskContainerClassName }, [e.showMask ? t("div", { class: A.maskClassName, style: [A.maskStyle] }) : e._e(), e.showBackgroundImage ? t(A.TKImage, { attrs: { fit: e.fit, src: e.bgImage, width: "100%", height: "100%", defaultSrc: e.defaultSrc }, on: { error: A.handleError } }) : e._e()], 1), t("div", { class: A.slotClassName }, [e._t("default")], 2)]) : e._e();
}, El = [], wl = /* @__PURE__ */ v(
  yl,
  Tl,
  El,
  !1,
  null,
  null,
  null,
  null
);
const ja = wl.exports, jl = ["fill", "contain", "cover"], bl = ["circle", "square"], zl = {
  icon: {
    type: String
  },
  size: {
    type: [Number, String],
    default: 100
  },
  shape: {
    type: String,
    values: bl,
    default: "square"
  },
  src: {
    type: String
  },
  defaultSrc: {
    type: String
  },
  text: {
    type: String
  },
  fit: {
    type: String,
    values: jl,
    default: "cover"
  },
  customClass: {
    type: String
  }
}, Ul = {
  options: {
    virtualHost: !0
  }
}, Ol = /* @__PURE__ */ S({
  ...Ul,
  __name: "Avatar",
  props: zl,
  setup(a) {
    const e = a, t = I("");
    K(() => e.src, () => {
      t.value = e.src;
    }, {
      immediate: !0
    });
    const A = M(() => Q([
      `${U}-avatar`,
      `${U}-avatar--${e.shape}`,
      `${U}-avatar--${e.size}`,
      e.customClass
    ])), n = M(() => typeof e.size == "number" ? `${e.size}px` : e.size), s = M(() => typeof e.size == "number" ? `${e.size}px` : e.size), l = M(() => ({ width: n.value, height: s.value }));
    function r(i) {
      console.error(i), e.defaultSrc && (t.value = e.defaultSrc);
    }
    return { __sfc: !0, props: e, avatarSrc: t, classname: A, width: n, height: s, avatarStyle: l, handleError: r, TKImage: hA };
  }
});
var xl = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.classname, style: [A.avatarStyle] }, [A.avatarSrc ? t(A.TKImage, { attrs: { fit: e.fit, width: A.width, height: A.height, src: A.avatarSrc }, on: { error: A.handleError } }) : e._t("default")], 2);
}, Bl = [], Pl = /* @__PURE__ */ v(
  Ol,
  xl,
  Bl,
  !1,
  null,
  null,
  null,
  null
);
const ba = Pl.exports, Vl = "data:image/svg+xml;base64,", _l = "data:image/svg+xml;base64,", Fl = {
  isMuted: {
    type: Boolean,
    default: !1
  },
  volume: {
    type: Number,
    default: 0
  }
}, Rl = {
  options: {
    virtualHost: !0
  }
}, Gl = /* @__PURE__ */ S({
  ...Rl,
  __name: "MicrophoneVolume",
  props: Fl,
  setup(a) {
    const e = a, t = M(() => e.isMuted ? _l : Vl), A = M(() => e.isMuted || !e.volume ? "" : `height: ${e.volume * 4}%`);
    return { __sfc: !0, props: e, iconSrc: t, volumeLevelStyle: A, Icon: Pe };
  }
});
var kl = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "mic-container" }, [t("div", { staticClass: "mic-level-container" }, [t("div", { staticClass: "mic-level", style: A.volumeLevelStyle })]), t(A.Icon, { attrs: { src: A.iconSrc } })], 1);
}, Wl = [], ql = /* @__PURE__ */ v(
  Gl,
  kl,
  Wl,
  !1,
  null,
  "ea062939",
  null,
  null
);
const za = ql.exports, QA = "data:image/svg+xml;base64,", Xl = {
  showOverlayStream: {
    type: Boolean,
    default: !0
  },
  customClass: {
    type: String
  },
  customStyle: {
    type: Object
  },
  isSmallWindow: {
    type: Boolean,
    default: !1
  },
  tip: {
    type: String,
    default: null
  },
  showOverlay: {
    type: Boolean,
    default: !0
  },
  showMask: {
    type: Boolean,
    default: !0
  },
  showBackgroundImage: {
    type: Boolean,
    default: !0
  },
  blur: {
    type: Boolean,
    default: !0
  },
  bgColor: {
    type: String
  },
  bgImage: {
    type: String,
    default: QA
  },
  overlayZIndex: {
    type: Number
  },
  customOverlayClass: {
    type: String
  },
  fit: {
    type: String
  },
  showLoading: {
    type: Boolean,
    default: !1
  },
  showAvatar: {
    type: Boolean,
    default: !0
  },
  avatar: {
    type: String,
    default: QA
  },
  showUserName: {
    type: Boolean,
    default: !0
  },
  username: {
    type: String
  },
  color: {
    type: String,
    default: "#FFF"
  },
  showMicVolume: {
    type: Boolean,
    default: !1
  },
  isMuted: {
    type: Boolean,
    default: !1
  },
  volume: {
    type: Number,
    default: 0
  },
  showTip: {
    type: Boolean,
    default: !0
  }
}, Ql = ["error"], Zl = {
  options: {
    virtualHost: !0
  }
}, Yl = /* @__PURE__ */ S({
  ...Zl,
  __name: "OverlayStream",
  props: Xl,
  emits: Ql,
  setup(a, { emit: e }) {
    const t = a, A = I(100), { isFloatWindow: n } = b(LA()), s = y.isPC ? "40px" : "20px";
    K([() => t.isSmallWindow, n], () => {
      n.value ? A.value = t.isSmallWindow ? 20 : 40 : A.value = t.isSmallWindow ? 40 : 100;
    }, {
      immediate: !0
    });
    const l = M(() => Q([
      "overlay-stream-container",
      {
        pc: y.isPC,
        mobile: !y.isPC,
        float: n.value
      }
    ]));
    return { __sfc: !0, emit: e, props: t, avatarSize: A, isFloatWindow: n, fontSize: s, overlayStreamClassName: l, handleError: (i) => {
      e("error", i);
    }, Overlay: ja, Loading: it, TKText: lA, Avatar: ba, MicrophoneVolume: za, defaultAvatarSrc: QA };
  }
});
var Kl = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return e.showOverlayStream ? t("div", { class: A.overlayStreamClassName, style: [e.customStyle] }, [t(A.Overlay, { attrs: { show: e.showOverlay, "show-background-image": e.showBackgroundImage, "show-mask": e.showMask, blur: e.blur, zIndex: e.overlayZIndex, bgColor: e.bgColor, bgImage: e.bgImage || A.defaultAvatarSrc, fit: e.fit, defaultSrc: A.defaultAvatarSrc, customStyle: { position: "absolute", width: "100%", height: "100%" }, "custom-mask-style": { "backdrop-filter": "blur(12px)", "-webkit-backdrop-filter": "blur(12px)" }, customClass: e.customOverlayClass }, on: { error: A.handleError } }, [t("div", { staticClass: "overlay-stream-content-container" }, [t("div", { staticClass: "overlay-stream-content" }, [e.showLoading ? t(A.Loading, { attrs: { mode: "dot" } }) : e._e(), t("div", { staticClass: "overlay-stream-avatar" }, [e.showAvatar ? t(A.Avatar, { attrs: { src: e.avatar || A.defaultAvatarSrc, size: A.avatarSize } }) : e._e()], 1), t("div", { staticClass: "overlay-stream-info" }, [e.showUserName ? t(A.TKText, { attrs: { truncated: !0, size: A.fontSize, color: e.color, width: "200px", weight: 500 } }, [e._v(" " + e._s(e.username) + " ")]) : e._e(), e.showMicVolume ? t(A.MicrophoneVolume, { attrs: { isMuted: e.isMuted, volume: e.volume } }) : e._e()], 1), t("div", { staticClass: "overlay-stream-tip" }, [e.showTip && e.tip ? t("div", [e._v(e._s(e.tip))]) : e._e()])], 1), e._t("default")], 2)])], 1) : e._e();
}, Hl = [], Jl = /* @__PURE__ */ v(
  Yl,
  Kl,
  Hl,
  !1,
  null,
  "98c0998b",
  null,
  null
);
const Ua = Jl.exports, $l = /* @__PURE__ */ S({
  __name: "Waiting",
  setup(a) {
    const { localUserInfoExcludeVolume: e, remoteUserListExcludeVolume: t } = b(pA()), { callerUserInfo: A } = b(Jn()), { isGroupCall: n } = b(Y()), s = Ca(), l = $(), r = M(() => {
      var L, D;
      return n.value ? A.value.displayUserInfo : (D = (L = t.value) == null ? void 0 : L[0]) == null ? void 0 : D.displayUserInfo;
    }), i = M(() => {
      var L, D;
      return n.value ? A.value.avatar : (D = (L = t.value) == null ? void 0 : L[0]) == null ? void 0 : D.avatar;
    }), c = M(() => n.value ? !0 : !e.value.isVideoAvailable), g = M(() => n.value ? l.value("Invited group call") : null), f = M(() => {
      var L, D;
      return n.value ? s.value[A.value.userId] || i.value : s.value[(D = (L = t.value) == null ? void 0 : L[0]) == null ? void 0 : D.userId] || i.value;
    }), p = I(!1), C = I("fill");
    let N = I(!1);
    return $A(() => {
      var L, D, z, T;
      if (N.value) {
        p.value = n.value ? !0 : !e.value.isVideoAvailable, C.value = "cover";
        return;
      }
      n.value ? (p.value = !s.value[A.value.userId], C.value = s.value[A.value.userId] ? "fill" : "cover") : (p.value = !e.value.isVideoAvailable && !s.value[(D = (L = t.value) == null ? void 0 : L[0]) == null ? void 0 : D.userId], C.value = s.value[(T = (z = t.value) == null ? void 0 : z[0]) == null ? void 0 : T.userId] ? "fill" : "cover");
    }), { __sfc: !0, localUserInfoExcludeVolume: e, remoteUserListExcludeVolume: t, callerUserInfo: A, isGroupCall: n, viewBackgroundConfigObj: s, t: l, username: r, avatar: i, showBackgroundImage: c, tip: g, bgImage: f, showMask: p, fit: C, triggerError: N, handleError: () => {
      N.value = !0;
    }, TUIGlobal: y, OverlayStream: Ua, Avatar: ba, TKText: lA, defaultAvatarSrc: QA };
  }
});
var Ai = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "waiting-container" }, [t(A.OverlayStream, { attrs: { username: A.username, avatar: A.avatar, "bg-image": A.bgImage, "show-avatar": !A.TUIGlobal.isPC, "show-loading": !1, "bg-color": "#22262ed9", "show-mask": A.showMask, "show-background-image": A.showBackgroundImage, tip: A.tip, fit: A.fit }, on: { error: A.handleError } }, [!A.TUIGlobal.isPC && A.isGroupCall ? t("div", { staticClass: "groupcall-info" }, [t("div", { staticClass: "tip" }, [t(A.TKText, { attrs: { color: "#FFF" } }, [e._v(e._s(A.remoteUserListExcludeVolume.length) + e._s(A.t("people in the call")))])], 1), t("div", { staticClass: "avatar-group" }, e._l(A.remoteUserListExcludeVolume, function(n) {
    return t("div", { staticClass: "avatar-item" }, [t(A.Avatar, { attrs: { size: "100%", src: n.avatar || A.defaultAvatarSrc } })], 1);
  }), 0)]) : e._e()])], 1);
}, ei = [], ti = /* @__PURE__ */ v(
  $l,
  Ai,
  ei,
  !1,
  null,
  "8dc3669f",
  null,
  null
);
const Oa = ti.exports, ai = {
  bigWindow: {
    type: String
  },
  showSmallWindow: {
    type: Boolean,
    default: !0
  },
  smallWindowWidth: {
    type: String,
    default: "30%"
  },
  smallWindowHeight: {
    type: String,
    default: "30%"
  }
}, ni = ["toggle"], xa = "ToggleWindowContextKey", oi = {
  options: {
    virtualHost: !0
  }
}, si = /* @__PURE__ */ S({
  ...oi,
  __name: "ToggleWindow",
  props: ai,
  emits: ni,
  setup(a, { emit: e }) {
    const t = a, A = I(t.bigWindow), n = `${U}-toggle-window`, s = (l) => {
      A.value = l, e("toggle", l);
    };
    return K(() => t.bigWindow, () => {
      A.value = t.bigWindow;
    }), rA(xa, {
      bigWindow: A,
      toggleWindow: s,
      smallWindowWidth: Re(t, "smallWindowWidth"),
      smallWindowHeight: Re(t, "smallWindowHeight"),
      showSmallWindow: Re(t, "showSmallWindow")
    }), { __sfc: !0, props: t, emit: e, currentBigWindow: A, toggleWindowContainerClassName: n, toggleWindow: s };
  }
});
var li = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.toggleWindowContainerClassName }, [e._t("default")], 2);
}, ii = [], ui = /* @__PURE__ */ v(
  si,
  li,
  ii,
  !1,
  null,
  null,
  null,
  null
);
const ri = ui.exports, ci = {
  options: {
    virtualHost: !0
  }
}, di = /* @__PURE__ */ S({
  ...ci,
  __name: "Portal",
  props: ["disabled"],
  setup(a) {
    return { __sfc: !0, props: a };
  }
});
var gi = function() {
  var e = this, t = e._self._c;
  return e._self._setupProxy, t("root-portal", { attrs: { enable: !e.disabled } }, [e._t("default")], 2);
}, Ii = [], pi = /* @__PURE__ */ v(
  di,
  gi,
  Ii,
  !1,
  null,
  "1d3981ac",
  null,
  null
);
const Ci = pi.exports, Mi = {
  name: "teleport",
  props: {
    to: {
      type: String,
      required: !0
    },
    source: {
      type: String,
      required: !0
    },
    disabled: {
      type: Boolean,
      required: !0
    }
  },
  mounted() {
    if (this.disabled)
      return;
    const a = document.querySelector(this.to);
    a && a.appendChild(this.$el);
  },
  watch: {
    disabled: {
      immediate: !0,
      handler() {
        var a, e;
        this.disabled ? (e = document.querySelector(this.source)) == null || e.appendChild(this.$el) : (a = document.querySelector(this.to)) == null || a.appendChild(this.$el);
      }
    }
  },
  destroyed() {
    const a = document.querySelector(this.to);
    if (a)
      try {
        a.removeChild(this.$el);
      } catch (e) {
        console.debug(e);
      }
  }
};
var fi = function() {
  var e = this, t = e._self._c;
  return t("div", [e._t("default")], 2);
}, Ni = [], Li = /* @__PURE__ */ v(
  Mi,
  fi,
  Ni,
  !1,
  null,
  null,
  null,
  null
);
const vi = Li.exports, Si = {
  options: {
    virtualHost: !0
  }
}, Di = /* @__PURE__ */ S({
  ...Si,
  __name: "Portal",
  props: ["disabled", "to"],
  setup(a) {
    return { __sfc: !0 };
  }
});
var hi = function() {
  var e = this, t = e._self._c;
  return e._self._setupProxy, t("Teleport", { attrs: { disabled: e.disabled, to: e.to } }, [e._t("default")], 2);
}, mi = [], yi = /* @__PURE__ */ v(
  Di,
  hi,
  mi,
  !1,
  null,
  null,
  null,
  null
);
const Ti = yi.exports, Ei = {
  disabled: {
    type: Boolean,
    default: !1
  },
  to: {
    type: String,
    default: "body"
  },
  source: {
    type: String,
    default: "body"
  }
}, wi = /* @__PURE__ */ S({
  __name: "Portal",
  props: Ei,
  setup(a) {
    const { majorVersion: e } = To(), t = M(() => e === "3"), A = M(() => e === "2");
    return { __sfc: !0, majorVersion: e, isVue3: t, isVue2: A, PortalMini: Ci, PortalVue2: vi, PortalVue3: Ti, IN_MINI_APP: He };
  }
});
var ji = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", [A.IN_MINI_APP ? t(A.PortalMini, { attrs: { disabled: e.disabled } }, [e._t("default")], 2) : A.isVue2 ? t(A.PortalVue2, { attrs: { disabled: e.disabled, to: e.to, source: e.source } }, [e._t("default")], 2) : t(A.PortalVue3, { attrs: { disabled: e.disabled, to: e.to } }, [e._t("default")], 2)], 1);
}, bi = [], zi = /* @__PURE__ */ v(
  wi,
  ji,
  bi,
  !1,
  null,
  null,
  null,
  null
);
const Ba = zi.exports, Ui = {
  value: {
    type: String
  }
}, Oi = {
  options: {
    virtualHost: !0
  }
}, xi = /* @__PURE__ */ S({
  ...Oi,
  __name: "ToggleWindowItem",
  props: Ui,
  setup(a) {
    const e = a, {
      bigWindow: t,
      toggleWindow: A,
      smallWindowWidth: n,
      smallWindowHeight: s,
      showSmallWindow: l
    } = dA(xa), r = M(() => Q([
      `${U}-toggle-window-item`,
      `${U}-toggle-window-item--${t.value === e.value ? "big" : "small"}`,
      {
        pc: XA,
        mobile: !XA
      }
    ])), i = M(() => {
      let c = {};
      return t.value !== e.value && (c = { width: n.value, height: s.value }, l.value ? c.visibility = "" : c.visibility = "hidden"), c;
    });
    return { __sfc: !0, props: e, bigWindow: t, toggleWindow: A, smallWindowWidth: n, smallWindowHeight: s, showSmallWindow: l, classname: r, windowItemStyle: i };
  }
});
var Bi = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.classname, style: [A.windowItemStyle], on: { click: () => A.toggleWindow(e.value) } }, [e._t("default")], 2);
}, Pi = [], Vi = /* @__PURE__ */ v(
  xi,
  Bi,
  Pi,
  !1,
  null,
  null,
  null,
  null
);
const _i = Vi.exports, rt = {
  nickName: {
    type: String,
    default: ""
  },
  isSelf: {
    type: Boolean,
    default: !1
  },
  isMuted: {
    type: Boolean,
    default: !1
  },
  volume: {
    type: Number,
    default: 0
  },
  showNickName: {
    type: Boolean,
    default: !1
  },
  showSwitchCameraButton: {
    type: Boolean,
    default: !1
  },
  showVirtualBackgroundButton: {
    type: Boolean,
    default: !1
  },
  showNetWorkStatus: {
    type: Boolean,
    default: !1
  }
}, Pa = "data:image/png;base64,", Fi = {
  options: {
    virtualHost: !0
  }
}, Ri = /* @__PURE__ */ S({
  ...Fi,
  __name: "StreamInfoPC",
  props: rt,
  setup(a) {
    const e = $();
    return { __sfc: !0, t: e, MicrophoneVolume: za, TKText: lA, TKImage: hA, Row: st, Col: lt, networkSrc: Pa };
  }
});
var Gi = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t(A.Row, [t(A.Col, { attrs: { span: 12, justify: "start", align: "center" } }, [t("div", { staticClass: "stream-userInfo" }, [t(A.MicrophoneVolume, { staticClass: "mic-volume", attrs: { "is-muted": e.isMuted, volume: e.volume } }), t("div", { staticClass: "nickname" }, [t(A.TKText, { attrs: { color: "#fff", truncated: !0, width: "80px", size: "14px" } }, [e._v(e._s(e.nickName))]), t(A.TKText, { directives: [{ name: "show", rawName: "v-show", value: e.isSelf, expression: "isSelf" }], attrs: { color: "#fff", size: "14px" } }, [e._v(e._s(A.t("me")))])], 1)], 1)]), t(A.Col, { attrs: { span: 12, justify: "center" } }, [e.showNetWorkStatus ? t("div", [t(A.TKImage, { attrs: { width: "24px", height: "24px", src: A.networkSrc } })], 1) : e._e()])], 1);
}, ki = [], Wi = /* @__PURE__ */ v(
  Ri,
  Gi,
  ki,
  !1,
  null,
  "73beaeef",
  null,
  null
);
const qi = Wi.exports, Xi = "data:image/svg+xml;base64,", Qi = "data:image/svg+xml;base64,", Zi = {
  options: {
    virtualHost: !0
  }
}, Yi = /* @__PURE__ */ S({
  ...Zi,
  __name: "StreamInfoMobile",
  props: rt,
  setup(a) {
    const { enableVirtualBackground: e } = b(Y());
    return { __sfc: !0, enableVirtualBackground: e, switchCamera: async () => {
      await x.switchCamera();
    }, setBlurBackground: async () => {
      await x.setBlurBackground(!e.value);
    }, Row: st, Col: lt, Icon: Pe, TKImage: hA, TKText: lA, MicOnH5: Xi, MicOffH5: Qi, SwitchCameraSrc: OA, VirtualBackgroundOpenSrc: BA, networkSrc: Pa };
  }
});
var Ki = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t(A.Row, { attrs: { "custom-style": { padding: "2px 5px" } } }, [t(A.Col, { attrs: { span: 12, justify: "start", align: "center" } }, [e.showNickName ? t(A.TKText, { attrs: { width: "100px", truncated: !0, color: "#FFF" } }, [e._v(" " + e._s(e.nickName) + " ")]) : e._e(), t("div", { directives: [{ name: "show", rawName: "v-show", value: !e.isMuted && e.volume, expression: "!isMuted && volume" }] }, [t(A.Icon, { attrs: { size: 24, src: A.MicOnH5 } })], 1), t("div", { directives: [{ name: "show", rawName: "v-show", value: e.isMuted, expression: "isMuted" }] }, [t(A.Icon, { attrs: { size: 24, src: A.MicOffH5 } })], 1)], 1), t(A.Col, { attrs: { span: 12, justify: "end", align: "center" } }, [e.showNetWorkStatus ? t("div", [t(A.TKImage, { attrs: { width: "24px", height: "24px", src: A.networkSrc } })], 1) : e._e(), e.showSwitchCameraButton ? t("div", { staticClass: "switch-camera stream-icon", on: { click: function(n) {
    return n.stopPropagation(), A.switchCamera.apply(null, arguments);
  } } }, [t(A.TKImage, { attrs: { width: "15px", height: "15px", src: A.SwitchCameraSrc } })], 1) : e._e(), e.showVirtualBackgroundButton ? t("div", { staticClass: "stream-icon", on: { click: function(n) {
    return n.stopPropagation(), A.setBlurBackground.apply(null, arguments);
  } } }, [t(A.TKImage, { attrs: { width: "15px", height: "15px", src: A.VirtualBackgroundOpenSrc } })], 1) : e._e()])], 1);
}, Hi = [], Ji = /* @__PURE__ */ v(
  Yi,
  Ki,
  Hi,
  !1,
  null,
  "b2b7ff75",
  null,
  null
);
const $i = Ji.exports, Au = {
  options: {
    virtualHost: !0
  }
}, eu = /* @__PURE__ */ S({
  ...Au,
  __name: "TKStreamInfo",
  props: rt,
  setup(a) {
    const { isGroupCall: e } = Y(), t = y.isPC;
    return { __sfc: !0, isGroupCall: e, isPC: t, StreamInfoPC: qi, StreamInfoMobile: $i };
  }
});
var tu = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", [A.isPC ? t(A.StreamInfoPC, { attrs: { nickName: e.nickName, isSelf: e.isSelf, isMuted: e.isMuted, volume: e.volume, showNetWorkStatus: e.showNetWorkStatus } }) : e._e(), A.isGroupCall && !A.isPC ? t(A.StreamInfoMobile, { attrs: { showSwitchCameraButton: e.showSwitchCameraButton, showVirtualBackgroundButton: e.showVirtualBackgroundButton, showNetWorkStatus: e.showNetWorkStatus, nickName: e.nickName, showNickName: e.showNickName, isSelf: e.isSelf, isMuted: e.isMuted, volume: e.volume } }) : e._e()], 1);
}, au = [], nu = /* @__PURE__ */ v(
  eu,
  tu,
  au,
  !1,
  null,
  null,
  null,
  null
);
const Va = nu.exports, ou = {
  show: {
    type: Boolean,
    default: !0
  },
  domId: {
    type: String
  },
  loading: {
    type: Boolean
  },
  showStreamInfo: {
    type: Boolean
  },
  showAudioStream: {
    type: Boolean
  }
}, su = {
  options: {
    virtualHost: !0
  }
}, lu = /* @__PURE__ */ S({
  ...su,
  __name: "Pusher",
  props: ou,
  setup(a) {
    const e = a, t = I(d.getData(u.CALL, o.PUSHER_ID)), A = Q([
      "stream-info-container",
      { mobile: !y.isPC }
    ]), n = (l) => {
      t.value = l;
    }, s = {
      [o.PUSHER_ID]: n
    };
    return sA(() => {
      d.watch(u.CALL, s, { notifyRangeWhenWatch: o.MYSELF });
    }), aA(() => {
      d.unwatch(u.CALL, s);
    }), { __sfc: !0, pusherId: t, props: e, streamInfoContainerClassName: A, handleReloadPusher: n, watchOptions: s };
  }
});
var iu = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { directives: [{ name: "show", rawName: "v-show", value: e.show, expression: "show" }], staticClass: "pusher-container", attrs: { id: e.domId } }, [t("div", { directives: [{ name: "show", rawName: "v-show", value: e.showAudioStream, expression: "showAudioStream" }], staticClass: "audio-stream-container" }, [e._t("audio-stream")], 2), e._t("loading"), t("div", { class: A.streamInfoContainerClassName }, [e._t("stream-info")], 2)], 2);
}, uu = [], ru = /* @__PURE__ */ v(
  lu,
  iu,
  uu,
  !1,
  null,
  "8cf5d7b3",
  null,
  null
);
const _a = ru.exports, cu = {
  show: {
    type: Boolean,
    default: !0
  },
  domId: {
    type: String
  },
  loading: {
    type: Boolean
  },
  showStreamInfo: {
    type: Boolean
  },
  showAudioStream: {
    type: Boolean
  }
}, du = {
  options: {
    virtualHost: !0
  }
}, gu = /* @__PURE__ */ S({
  ...du,
  __name: "Player",
  props: cu,
  setup(a) {
    const e = Q([
      "stream-info-container",
      { mobile: !y.isPC }
    ]);
    return { __sfc: !0, streamInfoContainerClassName: e };
  }
});
var Iu = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { directives: [{ name: "show", rawName: "v-show", value: e.show, expression: "show" }], staticClass: "player-container", attrs: { id: e.domId } }, [t("div", { directives: [{ name: "show", rawName: "v-show", value: e.showAudioStream, expression: "showAudioStream" }], staticClass: "audio-stream-container" }, [e._t("audio-stream")], 2), e._t("loading"), t("div", { class: A.streamInfoContainerClassName }, [e._t("stream-info")], 2)], 2);
}, pu = [], Cu = /* @__PURE__ */ v(
  gu,
  Iu,
  pu,
  !1,
  null,
  "351781b9",
  null,
  null
);
const Fa = Cu.exports, Mu = {
  avatar: {
    type: String
  },
  username: {
    type: String
  },
  isVideoAvailable: {
    type: Boolean
  },
  showStreamInfo: {
    type: Boolean
  },
  isSmallWindow: {
    type: Boolean,
    default: !1
  },
  isMuted: {
    type: Boolean,
    default: !0
  },
  volume: {
    type: Number,
    default: 0
  },
  userId: {
    type: String
  }
}, fu = {
  options: {
    virtualHost: !0
  }
}, Nu = /* @__PURE__ */ S({
  ...fu,
  __name: "AudioStream",
  props: Mu,
  setup(a) {
    const e = a, t = Ca(), { callType: A, isGroupCall: n } = b(Y()), { isFloatWindow: s } = b(LA()), l = M(() => !n.value && !y.isPC), r = M(() => n.value ? y.isPC : A.value === _.AUDIO), i = M(() => !n.value && y.isPC && A.value === _.AUDIO), c = M(() => ({ zIndex: 1 })), g = M(() => t.value[e.userId] || e.avatar), f = I(!t.value[e.userId]), p = I(t.value[e.userId] ? "fill" : "cover");
    return K([() => e.userId, t], () => {
      t.value[e.userId] ? (p.value = "fill", f.value = !1) : (p.value = "cover", f.value = !0);
    }, {
      immediate: !0
    }), { __sfc: !0, props: e, viewBackgroundConfigObj: t, callType: A, isGroupCall: n, isFloatWindow: s, showAvatar: l, showUserName: r, showMicVolume: i, audioStreamStyle: c, bgImage: g, showViewMask: f, fit: p, handleError: () => {
      f.value = !0, p.value = "cover";
    }, OverlayStream: Ua };
  }
});
var Lu = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t(A.OverlayStream, { attrs: { avatar: e.avatar, "bg-image": A.bgImage, "show-avatar": A.showAvatar, username: e.username, "show-user-name": A.showUserName, "show-mask": !A.isGroupCall && A.showViewMask, fit: A.fit, blur: !A.isGroupCall, "show-mic-volume": A.showMicVolume, "show-tip": !A.isGroupCall && !A.isFloatWindow, "custom-style": A.audioStreamStyle, "is-small-window": e.isSmallWindow, "is-muted": e.isMuted, volume: e.volume, "bg-color": "rgba(0, 0, 0, 0.5)" }, on: { error: A.handleError } });
}, vu = [], Su = /* @__PURE__ */ v(
  Nu,
  Lu,
  vu,
  !1,
  null,
  null,
  null,
  null
);
const Ra = Su.exports, Du = "data:image/svg+xml;base64,", hu = "data:image/svg+xml;base64,", mu = "data:image/svg+xml;base64,", yu = "data:image/svg+xml;base64,", Tu = {
  options: {
    virtualHost: !0
  }
}, Eu = /* @__PURE__ */ S({
  ...Tu,
  __name: "FloatWindow",
  setup(a) {
    const { localUserInfoExcludeVolume: e } = b(pA()), { callRole: t, callStatus: A } = b(Y()), { isFloatWindow: n } = b(LA());
    function s() {
      x.toggleMinimize();
    }
    async function l() {
      t.value === J.CALLEE && A.value === w.CALLING ? await x.reject() : await x.hangup();
    }
    async function r() {
      e.value.isAudioAvailable ? await x.closeMicrophone() : await x.openMicrophone();
    }
    return { __sfc: !0, localUserInfo: e, callRole: t, callStatus: A, isFloatWindow: n, toggleMinimize: s, hangup: l, toggleMicrophone: r, FloatCallEnd: Du, FloatMicrophoneSVG: hu, FloatMicrophoneClosedSVG: mu, FloatFullScreenSVG: yu };
  }
});
var wu = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "float-window-container" }, [t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isFloatWindow, expression: "isFloatWindow" }], staticClass: "float-control-panel" }, [t("div", { staticClass: "float-control-item-icon" }, [t("div", { staticClass: "float-control-item-icon-container", on: { click: A.hangup } }, [t("img", { attrs: { src: A.FloatCallEnd } })])]), t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isFloatWindow, expression: "isFloatWindow" }], staticClass: "float-control-item-icon" }, [t("div", { staticClass: "float-control-item-icon-container", on: { click: A.toggleMicrophone } }, [A.localUserInfo.isAudioAvailable ? t("img", { attrs: { src: A.FloatMicrophoneSVG } }) : t("img", { attrs: { src: A.FloatMicrophoneClosedSVG } })])]), t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isFloatWindow, expression: "isFloatWindow" }], staticClass: "float-control-item-icon" }, [t("div", { staticClass: "float-control-item-icon-container", on: { click: A.toggleMinimize } }, [t("img", { attrs: { src: A.FloatFullScreenSVG } })])])]), t("div", { directives: [{ name: "show", rawName: "v-show", value: !A.isFloatWindow, expression: "!isFloatWindow" }] }, [e._t("default")], 2)]);
}, ju = [], bu = /* @__PURE__ */ v(
  Eu,
  wu,
  ju,
  !1,
  null,
  "63373c5d",
  null,
  null
);
const zu = bu.exports, Ga = "data:image/svg+xml;base64,", Uu = {
  options: {
    virtualHost: !0
  }
}, Ou = /* @__PURE__ */ S({
  ...Uu,
  __name: "FloatWindowSingleCall",
  setup(a) {
    const { callType: e, callStatus: t } = b(Y()), { isFloatWindow: A } = b(LA()), { callDuration: n } = ot(), s = $(), l = M(() => Q([
      "float-window-container",
      {
        "singlecall-video-float": e.value === _.VIDEO && A.value,
        "singlecall-audio-float": e.value === _.AUDIO && A.value
      }
    ]));
    function r() {
      A.value && x.toggleMinimize();
    }
    return { __sfc: !0, callType: e, callStatus: t, isFloatWindow: A, callDuration: n, t: s, floatWindowClassName: l, closeFloatWindow: r, CallMediaType: _, CallStatus: w, TKImage: hA, Timer: ut, TKText: lA, earphoneSrc: Ga };
  }
});
var xu = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.floatWindowClassName }, [t("div", { directives: [{ name: "show", rawName: "v-show", value: !(A.isFloatWindow && A.callType === A.CallMediaType.AUDIO), expression: "!(isFloatWindow && callType === CallMediaType.AUDIO)" }], staticClass: "singlecall-video-float-content", on: { click: A.closeFloatWindow } }, [e._t("default"), t("div", { staticClass: "float-window-tip-container" }, [A.callStatus === A.CallStatus.CALLING && A.isFloatWindow ? t(A.TKText, { attrs: { color: "#FFF", size: "12px" } }, [e._v(e._s(A.t("wait to be called")))]) : e._e()], 1)], 2), t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isFloatWindow && A.callType === A.CallMediaType.AUDIO, expression: "isFloatWindow && callType === CallMediaType.AUDIO" }], staticClass: "singlecall-audio-float-content", on: { click: A.closeFloatWindow } }, [t(A.TKImage, { attrs: { width: "36px", height: "36px", src: A.earphoneSrc } }), A.callStatus === A.CallStatus.CONNECTED ? t(A.Timer, { attrs: { fontSize: "12px", callDuration: A.callDuration, color: "#12b969" } }) : e._e(), A.callStatus === A.CallStatus.CALLING && A.isFloatWindow ? t(A.TKText, { attrs: { color: "#12b969", size: "12px" } }, [e._v(e._s(A.t("wait to be called")))]) : e._e()], 1)]);
}, Bu = [], Pu = /* @__PURE__ */ v(
  Ou,
  xu,
  Bu,
  !1,
  null,
  "ea53b7e0",
  null,
  null
);
const Vu = Pu.exports, _u = "data:image/svg+xml;base64,", Fu = "data:image/svg+xml;base64,", Ru = "data:image/svg+xml;base64,", Gu = "data:image/svg+xml;base64,", ku = {
  options: {
    virtualHost: !0
  }
}, Wu = /* @__PURE__ */ S({
  ...ku,
  __name: "FloatWindowGroupCall",
  setup(a) {
    const { isFloatWindow: e } = b(LA()), { callDuration: t } = ot(), { localUserInfoExcludeVolume: A } = b(pA()), { callStatus: n } = b(Y()), s = $(), l = M(() => A.value.isAudioAvailable ? _u : Ru), r = M(() => A.value.isVideoAvailable ? Fu : Gu), i = M(() => Q([
      "groupcall-video-float",
      {
        "not-float": !e.value,
        float: e.value
      }
    ]));
    function c() {
      e.value && x.toggleMinimize();
    }
    return { __sfc: !0, isFloatWindow: e, callDuration: t, localUserInfoExcludeVolume: A, callStatus: n, t: s, microphoneSrc: l, cameraSrc: r, floatWindowClassName: i, closeFloatWindow: c, TKImage: hA, Timer: ut, TKText: lA, CallStatus: w, earphoneSrc: Ga };
  }
});
var qu = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.floatWindowClassName }, [A.isFloatWindow ? t("div", { staticClass: "click-container", on: { "!click": function(n) {
    return n.stopPropagation(), A.closeFloatWindow.apply(null, arguments);
  } } }) : e._e(), t("div", { staticClass: "stream-container" }, [t("div", { staticClass: "video" }, [e._t("default")], 2), t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isFloatWindow, expression: "isFloatWindow" }], staticClass: "audio" }, [t(A.TKImage, { attrs: { width: "36px", height: "36px", src: A.earphoneSrc } }), A.callStatus === A.CallStatus.CONNECTED ? t(A.Timer, { attrs: { fontSize: "12px", callDuration: A.callDuration, color: "#12b969" } }) : e._e(), A.callStatus === A.CallStatus.CALLING ? t(A.TKText, { attrs: { size: "12px", color: "#12b969" } }, [e._v(e._s(A.t("wait to be called")))]) : e._e()], 1)]), t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isFloatWindow, expression: "isFloatWindow" }], staticClass: "device-status" }, [t(A.TKImage, { attrs: { width: "16px", height: "16px", src: A.microphoneSrc } }), t(A.TKImage, { attrs: { width: "16px", height: "16px", src: A.cameraSrc } })], 1)]);
}, Xu = [], Qu = /* @__PURE__ */ v(
  Wu,
  qu,
  Xu,
  !1,
  null,
  "0dfe3ae0",
  null,
  null
);
const Zu = Qu.exports, Yu = {
  options: {
    virtualHost: !0
  }
}, Ku = /* @__PURE__ */ S({
  ...Yu,
  __name: "FloatWindow",
  setup(a) {
    const { isGroupCall: e } = b(Y());
    return { __sfc: !0, isGroupCall: e, FloatWindowSingleCall: Vu, FloatWindowGroupCall: Zu };
  }
});
var Hu = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { attrs: { id: "float-window-id" } }, [A.isGroupCall ? t(A.FloatWindowGroupCall, [e._t("default")], 2) : t(A.FloatWindowSingleCall, [e._t("default")], 2)], 1);
}, Ju = [], $u = /* @__PURE__ */ v(
  Ku,
  Hu,
  Ju,
  !1,
  null,
  null,
  null,
  null
);
const Ar = $u.exports, er = {
  options: {
    virtualHost: !0
  }
}, tr = /* @__PURE__ */ S({
  ...er,
  __name: "FloatWindow",
  setup(a) {
    const { isFloatWindow: e } = b(LA()), t = M(() => Q([
      "float-window-container",
      {
        float: e.value,
        "not-float": !e.value,
        pc: y.isPC,
        mobile: !y.isPC
      }
    ]));
    return { __sfc: !0, isFloatWindow: e, floatWindowContainerClassName: t, FloatWindowPC: zu, FloatWindowMobile: Ar, TUIGlobal: y };
  }
});
var ar = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.floatWindowContainerClassName }, [A.TUIGlobal.isPC ? t(A.FloatWindowPC, [e._t("default")], 2) : e._e(), A.TUIGlobal.isPC ? e._e() : t(A.FloatWindowMobile, [e._t("default")], 2)], 1);
}, nr = [], or = /* @__PURE__ */ v(
  tr,
  ar,
  nr,
  !1,
  null,
  "0d17419d",
  null,
  null
);
const ka = or.exports;
function sr() {
  const a = Ie(), { callStatus: e } = b(Y()), t = I(ye.LOCAL), { remoteUserListExcludeVolume: A } = b(pA());
  return K([A, a, e], () => {
    var l, r, i, c;
    if (e.value === w.CALLING)
      return;
    const n = [Te.RemoteInLargeView, Te.LocalInLargeView], s = (l = a.value) == null ? void 0 : l.layoutMode;
    if (n.includes(s)) {
      t.value = (r = a.value) == null ? void 0 : r.layoutMode;
      return;
    }
    (c = (i = A.value) == null ? void 0 : i[0]) != null && c.isEnter && (t.value = ye.REMOTE);
  }, {
    immediate: !0
  }), t;
}
const lr = {
  options: {
    virtualHost: !0
  }
}, ir = /* @__PURE__ */ S({
  ...lr,
  __name: "MediaContainer",
  setup(a) {
    const e = sr(), t = I(!0), { isFloatWindow: A } = b(LA());
    function n() {
      A.value = !A.value;
    }
    const { localUserInfoExcludeVolume: s, remoteUserListExcludeVolume: l } = b(pA()), r = da(), { callType: i, callStatus: c } = b(Y()), g = ga(), f = M(
      () => {
        var L, D, z, T;
        return y.isWeChat ? (D = (L = g.value) == null ? void 0 : L.find((j) => {
          var F, Z;
          return (j == null ? void 0 : j.userID) === ((Z = (F = l.value) == null ? void 0 : F[0]) == null ? void 0 : Z.userId);
        })) == null ? void 0 : D.hasVideo : (T = (z = l.value) == null ? void 0 : z[0]) == null ? void 0 : T.isVideoAvailable;
      }
    ), p = M(() => {
      var L, D;
      return (D = (L = l.value) == null ? void 0 : L[0]) == null ? void 0 : D.domId;
    }), C = M(() => {
      var L, D, z;
      return (z = r.value) == null ? void 0 : z[(D = (L = l.value) == null ? void 0 : L[0]) == null ? void 0 : D.domId];
    });
    K([i, c], () => {
      i.value === _.AUDIO || c.value === w.CALLING ? t.value = !1 : t.value = !0;
    }, {
      immediate: !0
    });
    const N = M(() => Q([
      "singlecall-media-container",
      {
        mobile: !y.isPC,
        pc: y.isPC,
        float: A.value
      }
    ]));
    function h(L) {
      e.value = L;
    }
    return { __sfc: !0, largeViewName: e, showSmallWindow: t, isFloatWindow: A, toggle: n, localUserInfoExcludeVolume: s, remoteUserListExcludeVolume: l, volumeMap: r, callType: i, callStatus: c, wxPlayer: g, isPusherVideoAvailable: f, playerDomId: p, remoteStreamVolume: C, singleMediaContainerClassName: N, handleToggle: h, CallMediaType: _, ToggleWindow: ri, Portal: Ba, ToggleWindowItem: _i, TKStreamInfo: Va, Pusher: _a, Player: Fa, AudioStream: Ra, FloatWindow: ka, ViewName: ye };
  }
});
var ur = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t(A.Portal, { attrs: { id: "source", disabled: !A.isFloatWindow, source: "#source", to: "body" } }, [t(A.FloatWindow, [t("div", { class: A.singleMediaContainerClassName }, [t(A.ToggleWindow, { attrs: { "big-window": A.largeViewName, "show-small-window": A.showSmallWindow }, on: { toggle: A.handleToggle } }, [t(A.ToggleWindowItem, { key: A.ViewName.LOCAL, attrs: { value: A.ViewName.LOCAL } }, [t(A.Pusher, { attrs: { domId: A.localUserInfoExcludeVolume.domId, "show-audio-stream": !A.localUserInfoExcludeVolume.isVideoAvailable }, scopedSlots: e._u([{ key: "audio-stream", fn: function() {
    return [t(A.AudioStream, { attrs: { userId: A.localUserInfoExcludeVolume.userId, username: A.localUserInfoExcludeVolume.displayUserInfo, avatar: A.localUserInfoExcludeVolume.avatar, "is-video-available": A.localUserInfoExcludeVolume.isVideoAvailable, "is-small-window": A.largeViewName !== A.ViewName.LOCAL, "is-muted": !A.localUserInfoExcludeVolume.isAudioAvailable, volume: A.volumeMap && A.volumeMap[A.localUserInfoExcludeVolume.domId] } })];
  }, proxy: !0 }, { key: "stream-info", fn: function() {
    return [A.callType === A.CallMediaType.VIDEO ? t(A.TKStreamInfo, { attrs: { "nick-name": A.localUserInfoExcludeVolume.displayUserInfo, "is-self": !0, "is-muted": !A.localUserInfoExcludeVolume.isAudioAvailable, volume: A.volumeMap && A.volumeMap[A.localUserInfoExcludeVolume.domId] } }) : e._e()];
  }, proxy: !0 }]) })], 1), t(A.ToggleWindowItem, { key: A.ViewName.REMOTE, attrs: { value: A.ViewName.REMOTE } }, [t(A.Player, { attrs: { "dom-id": A.playerDomId, "show-audio-stream": !A.isPusherVideoAvailable }, scopedSlots: e._u([{ key: "audio-stream", fn: function() {
    return [t(A.AudioStream, { attrs: { userId: A.remoteUserListExcludeVolume[0] && A.remoteUserListExcludeVolume[0].userId, username: A.remoteUserListExcludeVolume[0] && A.remoteUserListExcludeVolume[0].displayUserInfo, avatar: A.remoteUserListExcludeVolume[0] && A.remoteUserListExcludeVolume[0].avatar, "is-video-available": A.isPusherVideoAvailable, "is-small-window": A.largeViewName !== A.ViewName.REMOTE, "is-muted": A.remoteUserListExcludeVolume[0] && !A.remoteUserListExcludeVolume[0].isAudioAvailable, volume: A.remoteStreamVolume } })];
  }, proxy: !0 }, { key: "stream-info", fn: function() {
    return [A.callType === A.CallMediaType.VIDEO ? t(A.TKStreamInfo, { attrs: { "nick-name": A.remoteUserListExcludeVolume[0] && A.remoteUserListExcludeVolume[0].displayUserInfo, "is-muted": A.remoteUserListExcludeVolume[0] && !A.remoteUserListExcludeVolume[0].isAudioAvailable, volume: A.remoteStreamVolume } }) : e._e()];
  }, proxy: !0 }]) })], 1)], 1)], 1)])], 1);
}, rr = [], cr = /* @__PURE__ */ v(
  ir,
  ur,
  rr,
  !1,
  null,
  "670a78d8",
  null,
  null
);
const dr = cr.exports, gr = [ie.SUCCESS, ie.INFO, ie.WARNING, ie.ERROR], HA = {
  isShow: {
    type: Boolean,
    default: !1
  },
  message: {
    type: String,
    default: ""
  },
  type: {
    type: String,
    values: gr,
    default: ie.INFO
  },
  duration: {
    type: Number,
    default: 3e3
  },
  offset: {
    type: Number,
    default: 16
  },
  showClose: {
    type: Boolean,
    default: !1
  },
  showIcon: {
    type: Boolean,
    default: !0
  },
  customClass: {
    type: String
  },
  customStyle: {
    type: Object
  }
}, Ir = {
  onClose: null
};
function pr(a, e) {
  const t = I((a == null ? void 0 : a.message) || HA.message.default), A = I((a == null ? void 0 : a.duration) || HA.duration.default), n = I((a == null ? void 0 : a.type) || HA.type.default), s = I((a == null ? void 0 : a.offset) || HA.offset.default), l = I((a == null ? void 0 : a.showClose) || HA.showClose.default), r = I(!1);
  let i = -1;
  const c = (p) => {
    i > -1 && (clearTimeout(i), i = -1), r.value = !0, f(p || {}), A.value && (i = setTimeout(() => {
      g();
    }, A.value));
  }, g = () => {
    r.value = !1, i > -1 && (clearTimeout(i), i = -1);
  }, f = (p) => {
    const {
      message: C = t.value,
      type: N = n.value,
      offset: h = s.value,
      duration: L = A.value,
      showClose: D = l.value
    } = p;
    A.value = (a == null ? void 0 : a.duration) === 0 ? a == null ? void 0 : a.duration : L, t.value = C, n.value = N, s.value = h, l.value = D;
  };
  return K(
    () => a == null ? void 0 : a.isShow,
    (p) => {
      p && c();
    },
    { immediate: !0 }
  ), K(r, (p) => {
    p || e("onClose");
  }), {
    messageContent: t,
    messageDuration: A,
    messageType: n,
    messageOffset: s,
    isShowCloseIcon: l,
    visible: r,
    show: c,
    close: g
  };
}
const Cr = "data:image/svg+xml;base64,", Mr = "data:image/svg+xml;base64,", fr = "data:image/svg+xml;base64,", Nr = "data:image/svg+xml;base64,", Lr = "data:image/svg+xml;base64,", vr = {
  options: {
    virtualHost: !0
  }
}, Sr = /* @__PURE__ */ S({
  ...vr,
  __name: "Message",
  props: HA,
  emits: Ir,
  setup(a, { expose: e, emit: t }) {
    const A = a, { messageContent: n, messageType: s, messageOffset: l, isShowCloseIcon: r, visible: i, show: c, close: g } = pr(A, t), f = M(() => Q([
      `${U}-message`,
      `${U}-message--${s.value}`,
      A.customClass
    ])), p = M(() => Q([`${U}-message_icon`])), C = M(() => Q([`${U}-message_close`])), N = M(() => `${l.value}px`), h = M(() => ({ top: N.value, ...A.customStyle })), L = {
      info: Nr,
      waring: fr,
      success: Mr,
      error: Cr
    };
    return e({
      show: c,
      close: g
    }), { __sfc: !0, props: A, emits: t, messageContent: n, messageType: s, messageOffset: l, isShowCloseIcon: r, visible: i, show: c, close: g, messageClassName: f, typeIconClassName: p, closeIconClassName: C, top: N, messageStyle: h, IconSrcMap: L, Icon: Pe, CloseSrc: Lr };
  }
});
var Dr = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return A.visible ? t("div", { class: A.messageClassName, style: [A.messageStyle] }, [t("div", { directives: [{ name: "show", rawName: "v-show", value: e.showIcon, expression: "showIcon" }], class: A.typeIconClassName }, [t(A.Icon, { attrs: { src: A.IconSrcMap[A.messageType] } })], 1), t("span", [e._v(e._s(A.messageContent))]), t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isShowCloseIcon, expression: "isShowCloseIcon" }], class: A.closeIconClassName, on: { click: A.close } }, [t(A.Icon, { attrs: { src: A.CloseSrc } })], 1)]) : e._e();
}, hr = [], mr = /* @__PURE__ */ v(
  Sr,
  Dr,
  hr,
  !1,
  null,
  null,
  null,
  null
);
const Wa = mr.exports, yr = {
  customClass: {
    type: String
  }
}, Tr = {
  options: {
    virtualHost: !0,
    styleIsolation: "shared"
  }
}, Er = /* @__PURE__ */ S({
  ...Tr,
  __name: "Tip",
  props: yr,
  setup(a) {
    const e = a, { isFloatWindow: t } = b(LA()), { tip: A, duration: n } = oo(), s = $(), { isGroupCall: l } = b(Y()), r = I(null), i = y.isPC, c = M(() => [e.customClass]), g = M(
      () => ze({
        color: "white",
        background: "none",
        position: "absolute",
        "z-index": 4,
        "align-items": "center",
        left: "50%",
        top: `${i ? "60%" : "70%"}`,
        transform: "translate(-50%, -50%)",
        "font-size": t.value ? "12px" : void 0
      })
    ), f = M(
      () => ze({
        color: "white",
        background: "none",
        position: "static",
        transform: "none",
        "font-size": t.value ? "12px" : void 0
      })
    ), p = M(() => l.value ? f.value : g.value);
    return K([A, s, n], () => {
      var C;
      (C = r.value) == null || C.show({
        message: s.value(A.value),
        duration: n.value,
        offset: 0
      });
    }), { __sfc: !0, isFloatWindow: t, tip: A, duration: n, t: s, isGroupCall: l, message: r, props: e, isPC: i, tipClassName: c, singleCustomStyle: g, groupCustomStyle: f, customStyle: p, Message: Wa };
  }
});
var wr = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", [t(A.Message, { ref: "message", class: A.tipClassName, attrs: { showIcon: !1, customStyle: A.customStyle } })], 1);
}, jr = [], br = /* @__PURE__ */ v(
  Er,
  wr,
  jr,
  !1,
  null,
  null,
  null,
  null
);
const qa = br.exports, zr = ["%", "vw"], Ur = {
  length: {
    type: Number,
    default: 0
  },
  unit: {
    type: String,
    values: zr,
    default: "%"
  },
  enableFocus: {
    type: Boolean,
    default: !1
  },
  layout: {
    type: Array
  },
  focus: {
    type: [String, Number]
  }
}, Or = ["change", "toggle"], Xa = "GridContextKey", xr = {
  options: {
    virtualHost: !0
  }
}, Br = /* @__PURE__ */ S({
  ...xr,
  __name: "Grid",
  props: Ur,
  emits: Or,
  setup(a, { emit: e }) {
    const t = a, A = I(t.focus), n = I(t.layout), s = I(t.unit);
    function l(r) {
      e("toggle", r);
    }
    return K(() => t.focus, () => A.value = t.focus), K(() => t.layout, () => n.value = t.layout), K(() => t.unit, () => s.value = t.unit), rA(Xa, {
      layout: n,
      enableFocus: t.enableFocus,
      handleFocusChange: l,
      focus: A,
      unit: s
    }), { __sfc: !0, props: t, focus: A, layout: n, unit: s, emit: e, handleFocusChange: l };
  }
});
var Pr = function() {
  var e = this, t = e._self._c;
  return e._self._setupProxy, t("div", { style: { height: "100%" } }, [e._t("default")], 2);
}, Vr = [], _r = /* @__PURE__ */ v(
  Br,
  Pr,
  Vr,
  !1,
  null,
  null,
  null,
  null
);
const Qa = _r.exports, Fr = {
  options: {
    virtualHost: !0
  }
}, Rr = /* @__PURE__ */ S({
  ...Fr,
  __name: "GridItem",
  props: ["index", "height", "customStyle"],
  setup(a) {
    const e = a, {
      layout: t,
      enableFocus: A,
      handleFocusChange: n,
      focus: s,
      unit: l
    } = dA(Xa), r = 100 / 12, i = r, c = I({}), g = Q([
      `${U}-grid-item`,
      {
        pc: XA,
        mobile: !XA,
        h5: fa
      }
    ]), f = () => {
      const C = String(e.index) === String(s.value) ? null : e.index;
      A && n(C);
    }, p = M(() => !!_t(t.value, { key: "i", value: e.index }));
    return $A(() => {
      const C = _t(t.value, { key: "i", value: e.index });
      if (!C)
        return;
      const { x: N, y: h, w: L, h: D, customStyle: z, customProps: T } = C;
      c.value = {
        width: L * r + l.value,
        height: e.height || D * i + l.value,
        left: N * r + l.value,
        top: h * i + l.value,
        position: "absolute",
        visibility: (T == null ? void 0 : T.show) === !1 ? "hidden" : "",
        ...e.customStyle,
        ...z
      };
    }), aA(() => {
      String(e.index) === String(s.value) && A && n(null);
    }), { __sfc: !0, props: e, layout: t, enableFocus: A, handleFocusChange: n, focus: s, unit: l, colWidth: r, rowHeight: i, style: c, gridItemClassName: g, handleClick: f, show: p };
  }
});
var Gr = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return A.show ? t("div", { class: A.gridItemClassName, style: [A.style], on: { click: A.handleClick } }, [e._t("default")], 2) : e._e();
}, kr = [], Wr = /* @__PURE__ */ v(
  Rr,
  Gr,
  kr,
  !1,
  null,
  null,
  null,
  null
);
const Za = Wr.exports;
function qr() {
  const a = I([]), e = I([]), { callStatus: t, isGroupCall: A, callType: n, callRole: s } = b(Y()), { status: l } = ge() || {}, r = po();
  return K(
    [t, A, n, s, l, r],
    () => {
      var D, z, T;
      let i = r.value;
      const c = y.isPC ? "pc" : "mobile", g = A.value ? "groupCall" : "singleCall", f = n.value === _.AUDIO ? "audio" : "video";
      let p = t.value === w.CALLING ? s.value === J.CALLER ? "calling" : "accept" : t.value;
      A && (l == null ? void 0 : l.value) === "close" && (p = "close_" + p);
      const C = ((T = (z = (D = i == null ? void 0 : i[c]) == null ? void 0 : D[g]) == null ? void 0 : z[f]) == null ? void 0 : T[p]) || [];
      e.value = C;
      const N = [];
      let h = 0;
      for (let j = 0; j < C.length; j++) {
        const F = j === 0 ? C[j].filter((B) => {
          var W;
          return ((W = B == null ? void 0 : B.props) == null ? void 0 : W.show) !== !1;
        }) : C[j];
        C[j] = F;
        const Z = 12 / F.length, V = 3;
        for (let B = 0; B < F.length; B++)
          N[h++] = {
            i: F[B].name,
            x: B * Z,
            y: j * Z,
            w: Z,
            h: V,
            customStyle: F[B].customStyle,
            customProps: F[B].props
          };
      }
      h = 0;
      let L = [];
      for (let j = 0; j < C.flat().length; j++)
        L[j] = N[h++];
      L = L.filter((j) => j.i), a.value = L;
    },
    {
      immediate: !0
    }
  ), { layout: a, config: e };
}
function Xr(...a) {
  let e = window == null ? void 0 : window.document, t = "", A = () => {
  }, n = {};
  if (typeof a[0] == "string" ? [t, A, n] = a : [e, t, A, n] = a, !!e)
    return e == null || e.addEventListener(t, A, n), () => {
      e == null || e.removeEventListener(t, A, n);
    };
}
function Qr(a, e) {
  let t = () => {
  };
  sA(() => {
    t = Xr("click", (n) => {
      const s = a.map((i) => an(i)), l = n.composedPath(), r = [];
      for (let i = 0; i < s.length; i++)
        r[i] = l.includes(s[i]);
      r.every((i) => !i) && e();
    }, { passive: !0 });
  }), aA(() => {
    t == null || t();
  });
}
const Zr = (a, e, t, A, n) => {
  const s = t.isShowArrow ? t.arrowSize : 0, l = `${a.width / 2 - e.width / 2}px`, r = `${a.height / 2 - e.height / 2}px`, i = `${s}px solid ${t.color}`, c = { ...A }, g = { ...n };
  switch (t.placement) {
    case "top":
      c.left = l, c.top = `-${e.height + s + t.arrowDistance}px`, g.borderTop = i, g.left = `${a.width / 2 - s / 2}px`, g.top = `-${s + t.arrowDistance}px`;
      break;
    case "bottom":
      c.left = l, c.top = `${a.height + s + t.arrowDistance}px`, g.borderBottom = i, g.left = `${a.width / 2 - s / 2}px`, g.top = `${a.height + t.arrowDistance - s}px`;
      break;
    case "left":
      c.left = `-${e.width + s + t.arrowDistance}px`, c.top = r, g.borderLeft = i, g.left = `-${s * 2 - t.arrowDistance}px`, g.top = `${a.height / 2 - s / 2}px`;
      break;
    case "right":
      c.left = `${a.width + s + t.arrowDistance}px`, c.top = r, g.borderRight = i, g.left = `${a.width + s * 2 - t.arrowDistance}px`, g.top = `${a.height / 2 - s / 2}px`;
      break;
  }
  return {
    finalContentStyle: c,
    finalArrowStyle: g
  };
}, Yr = ["click", "hover"], Kr = ["top", "bottom", "left", "right"], Ya = {
  trigger: {
    type: String,
    values: Yr,
    default: "click"
  },
  placement: {
    type: String,
    values: Kr,
    default: "top"
  },
  color: {
    type: String
  },
  isShowArrow: {
    type: Boolean,
    default: !0
  },
  arrowSize: {
    type: Number,
    default: 5
  },
  arrowDistance: {
    type: Number,
    default: 5
  },
  show: {
    type: Boolean
  },
  autoClose: {
    type: Number,
    default: 300
  }
}, Hr = {
  options: {
    virtualHost: !0
  }
}, Jr = /* @__PURE__ */ S({
  ...Hr,
  __name: "PopoverWeb",
  props: Ya,
  emits: ["hover"],
  setup(a, { emit: e }) {
    const t = a, A = `${U}-popover`, n = `${U}-popover_content`, s = `${U}-popover_arrow`, l = `${U}-popover_trigger`, r = I(), i = I(), c = I(), g = I(!1), f = I(null), p = M(() => g.value && t.isShowArrow && t.show), C = ze({ left: "0px", top: "0px", backgroundColor: t.color }), N = I(C), h = `${t.arrowSize}px solid transparent`, L = {
      borderLeft: h,
      borderRight: h,
      borderTop: h,
      borderBottom: h,
      top: "0px",
      left: "0px"
    }, D = I(L), z = () => {
      nn(() => {
        var _A, yA, TA, EA;
        if (!g.value || !t.show)
          return;
        const V = (yA = (_A = i.value).getBoundingClientRect) == null ? void 0 : yA.call(_A), B = (EA = (TA = c.value).getBoundingClientRect) == null ? void 0 : EA.call(TA), { finalContentStyle: W, finalArrowStyle: vA } = Zr(V, B, t, C, L);
        N.value = { ...C, ...W }, D.value = { ...L, ...vA };
      });
    }, T = () => {
      clearTimeout(f.value);
    }, j = () => {
      t.trigger === "click" && (g.value = !g.value, z());
    }, F = () => {
      t.trigger === "hover" && (e("hover"), T(), g.value = !0, z());
    }, Z = () => {
      t.trigger === "hover" && (f.value = setTimeout(() => {
        g.value = !1, z();
      }, 1e3));
    };
    return Qr([r], () => {
      g.value = !1;
    }), { __sfc: !0, props: t, popoverClassName: A, popoverContentClassName: n, popoverArrowClassName: s, popoverTriggerClassName: l, popoverRef: r, triggerRef: i, contentRef: c, visible: g, hidePopoverTimeout: f, showArrow: p, initContentStyle: C, contentStyle: N, commonBorder: h, initArrowStyle: L, arrowStyle: D, emit: e, togglePopover: z, clearTimer: T, handleClick: j, handleMouseEnter: F, handleMouseLeave: Z };
  }
});
var $r = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { ref: "popoverRef", class: A.popoverClassName }, [A.visible && e.show ? t("div", { ref: "contentRef", class: A.popoverContentClassName, style: [A.contentStyle], on: { mouseenter: A.clearTimer, mouseleave: A.handleMouseLeave } }, [e._t("content")], 2) : e._e(), A.showArrow ? t("div", { class: A.popoverArrowClassName, style: [A.arrowStyle] }) : e._e(), t("div", { ref: "triggerRef", class: A.popoverTriggerClassName, on: { click: A.handleClick, mouseenter: A.handleMouseEnter, mouseleave: A.handleMouseLeave } }, [e._t("trigger")], 2)]);
}, Ac = [], ec = /* @__PURE__ */ v(
  Jr,
  $r,
  Ac,
  !1,
  null,
  null,
  null,
  null
);
const tc = ec.exports, ac = {
  options: {
    virtualHost: !0
  }
}, nc = /* @__PURE__ */ S({
  ...ac,
  __name: "PopoverWx",
  setup(a) {
    return { __sfc: !0 };
  }
});
var oc = function() {
  var e = this, t = e._self._c;
  return e._self._setupProxy, t("div");
}, sc = [], lc = /* @__PURE__ */ v(
  nc,
  oc,
  sc,
  !1,
  null,
  null,
  null,
  null
);
const ic = lc.exports, uc = {
  options: {
    virtualHost: !0
  }
}, rc = /* @__PURE__ */ S({
  ...uc,
  __name: "Popover",
  props: Ya,
  emits: ["hover"],
  setup(a, { emit: e }) {
    return { __sfc: !0, emit: e, handleHover: () => {
      e("hover");
    }, PopoverWeb: tc, PopoverWx: ic, IN_WX_MINI_APP: de };
  }
});
var cc = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return A.IN_WX_MINI_APP ? e._e() : t(A.PopoverWeb, e._b({ on: { hover: A.handleHover }, scopedSlots: e._u([{ key: "trigger", fn: function() {
    return [e._t("trigger")];
  }, proxy: !0 }, { key: "content", fn: function() {
    return [e._t("content")];
  }, proxy: !0 }], null, !0) }, "PopoverWeb", e.$props, !1));
}, dc = [], gc = /* @__PURE__ */ v(
  rc,
  cc,
  dc,
  !1,
  null,
  null,
  null,
  null
);
const ct = gc.exports, Ic = {
  deviceType: {
    type: String
  },
  isShowControlBtn: {
    type: Boolean,
    default: !1
  }
}, pc = {
  options: {
    virtualHost: !0
  }
}, Cc = /* @__PURE__ */ S({
  ...pc,
  __name: "DeviceSelect",
  props: Ic,
  setup(a) {
    const e = a, [{ deviceList: t, currentDeviceId: A }, { updateCurrentDeviceId: n }] = Oe(e.deviceType), { localUserInfoExcludeVolume: s } = b(pA()), { isMuteSpeaker: l } = b(Y()), r = Be(), i = $(), c = async (C) => {
      try {
        p(), await x.switchDevice({
          deviceType: e.deviceType,
          deviceId: C
        }), n(C);
      } catch (N) {
        console.debug(N);
      }
    }, g = async () => {
      var C, N;
      p(), e.deviceType === oA.CAMERA && ((C = s.value) != null && C.isVideoAvailable ? await x.closeCamera() : await x.openCamera("localVideo")), e.deviceType === oA.MICROPHONE && ((N = s.value) != null && N.isAudioAvailable ? await x.closeMicrophone() : await x.openMicrophone()), e.deviceType === oA.SPEAKER && (l.value ? await x.unMuteSpeaker() : await x.muteSpeaker());
    }, f = M(() => {
      var C;
      if (e.deviceType === oA.CAMERA)
        return s != null && s.value.isVideoAvailable ? i.value("close camera") : i.value("open camera");
      if (e.deviceType === oA.MICROPHONE)
        return (C = s == null ? void 0 : s.value) != null && C.isAudioAvailable ? i.value("close microphone") : i.value("open microphone");
      if (e.deviceType === oA.SPEAKER)
        return l.value ? i.value("open speaker") : i.value("close speaker");
    }), p = () => {
      r.value = "close";
    };
    return { __sfc: !0, props: e, deviceList: t, currentDeviceId: A, updateCurrentDeviceId: n, localUserInfo: s, isMuteSpeaker: l, popoverValue: r, t: i, handleClickDeviceItem: c, switchStatus: g, deviceText: f, updatePopover: p, TKText: lA };
  }
});
var Mc = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "device-selector-container" }, [t("div", { staticClass: "scroll-container" }, [t("div", { staticClass: "scroll-content" }, e._l(A.deviceList, function(n) {
    return t("div", { key: n.deviceId }, [t(A.TKText, { staticClass: "device-item", class: { select: A.currentDeviceId === n.deviceId }, attrs: { width: "100%", color: "#ffff", truncated: !0, size: "14px" }, on: { click: () => A.handleClickDeviceItem(n.deviceId) } }, [e._v(" " + e._s(n.label) + " ")])], 1);
  }), 0)]), t("div", { staticClass: "control-item" }), e.isShowControlBtn ? t(A.TKText, { staticClass: "device-item", attrs: { width: "100%", color: "#ffff", truncated: !0, size: "14px" }, on: { click: A.switchStatus } }, [e._v(" " + e._s(A.deviceText) + " ")]) : e._e()], 1);
}, fc = [], Nc = /* @__PURE__ */ v(
  Cc,
  Mc,
  fc,
  !1,
  null,
  "406bc3d4",
  null,
  null
);
const dt = Nc.exports, Lc = {
  options: {
    virtualHost: !0
  }
}, vc = /* @__PURE__ */ S({
  ...Lc,
  __name: "Camera",
  props: mA,
  setup(a) {
    const e = a, t = $(), A = I(!0), n = y.isPC, { localUserInfoExcludeVolume: s } = b(pA()), l = I(s == null ? void 0 : s.value.isVideoAvailable), r = M(() => s == null ? void 0 : s.value.isVideoAvailable), i = Be(), [{ deviceList: c }] = Oe(oA.CAMERA), g = M(() => {
      var L;
      return ((L = c.value) == null ? void 0 : L.length) > 0 && i.value === "camera";
    }), f = M(() => A.value ? r.value ? "basicConfig" : "closedConfig" : "loadingConfig"), p = M(() => s != null && s.value.isVideoAvailable ? t.value("camera enabled") : t.value("camera disabled")), C = cA("camera", f);
    return { __sfc: !0, t, props: e, clickAble: A, isPC: n, localUserInfo: s, targetVideoAvailable: l, isVideoAvailable: r, popoverValue: i, deviceList: c, isShowPopover: g, configState: f, btnText: p, config: C, handleClick: async () => {
      A.value = !1, l.value = !r.value, r.value ? await x.closeCamera() : await x.openCamera("localVideo"), A.value = !0;
    }, handleMouseEnter: () => {
      i.value = "camera";
    }, DeviceType: oA, Button: MA, TKText: lA, Popover: ct, DeviceSelect: dt };
  }
});
var Sc = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "btn-content" }, [A.isPC ? t(A.Popover, { attrs: { color: "rgba(107, 117, 138, 0.5)", show: A.isShowPopover, trigger: "hover" }, on: { hover: A.handleMouseEnter }, scopedSlots: e._u([{ key: "content", fn: function() {
    return [t(A.DeviceSelect, { attrs: { isShowControlBtn: !0, deviceType: A.DeviceType.CAMERA } })];
  }, proxy: !0 }, { key: "trigger", fn: function() {
    return [t(A.Button, { attrs: { loading: !A.clickAble, iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.config.iconSize, width: A.props.width || A.config.width, height: A.props.height || A.config.height, loadingWidth: A.config.loadingWidth, loadingHeight: A.config.loadingHeight, shape: A.config.shape }, on: { click: A.handleClick } })];
  }, proxy: !0 }], null, !1, 1817826708) }) : e._e(), A.isPC ? e._e() : t(A.Button, { attrs: { loading: !A.clickAble, iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.props.iconSize || A.config.iconSize, width: A.props.width || A.config.width, height: A.props.height || A.config.height, loadingWidth: A.config.loadingWidth, loadingHeight: A.config.loadingHeight, shape: "circle" }, on: { click: A.handleClick } }), A.config.showText ? t(A.TKText, { attrs: { textStyle: A.config.textStyle, color: A.config.textColor, size: A.config.textSize } }, [e._v(" " + e._s(A.btnText) + " ")]) : e._e()], 1);
}, Dc = [], hc = /* @__PURE__ */ v(
  vc,
  Sc,
  Dc,
  !1,
  null,
  "24aa342a",
  null,
  null
);
const mc = hc.exports, yc = {
  options: {
    virtualHost: !0
  }
}, Tc = /* @__PURE__ */ S({
  ...yc,
  __name: "Hangup",
  props: mA,
  setup(a) {
    const e = a, t = lo(), A = M(() => t.value ? "basicConfig" : "loadingConfig"), n = cA("hangup", A), s = $();
    return { __sfc: !0, props: e, isClickable: t, hangupState: A, config: n, t: s, handleClick: async () => {
      await x.hangup();
    }, Button: MA, TKText: lA };
  }
});
var Ec = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "btn-content" }, [t(A.Button, { attrs: { loading: !A.isClickable, loadingWidth: A.config.loadingWidth, loadingHeight: A.config.loadingHeight, iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.props.iconSize || A.config.iconSize, width: A.props.width || A.config.width, height: A.props.height || A.config.height, shape: A.config.shape }, on: { click: A.handleClick } }), A.config.showText ? t(A.TKText, { attrs: { textStyle: A.config.textStyle, color: A.config.textColor, size: A.config.textSize } }, [e._v(" " + e._s(A.t("hangup")) + " ")]) : e._e()], 1);
}, wc = [], jc = /* @__PURE__ */ v(
  Tc,
  Ec,
  wc,
  !1,
  null,
  "e75a102a",
  null,
  null
);
const bc = jc.exports, zc = {
  options: {
    virtualHost: !0
  }
}, Uc = /* @__PURE__ */ S({
  ...zc,
  __name: "Accept",
  props: mA,
  setup(a) {
    const e = a, t = I(!0), A = M(() => t.value ? "basicConfig" : "loadingConfig"), n = cA("accept", A), s = $();
    return { __sfc: !0, clickAble: t, props: e, configState: A, config: n, t: s, handleClick: async () => {
      t.value = !1, await x.accept(), t.value = !0;
    }, Button: MA, TKText: lA };
  }
});
var Oc = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "btn-content" }, [t(A.Button, { attrs: { iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.props.iconSize || A.config.iconSize, width: A.props.width || A.config.width, height: A.props.height || A.config.height, shape: A.config.shape, loading: !A.clickAble, loadingWidth: A.config.loadingWidth, loadingHeight: A.config.loadingHeight }, on: { click: A.handleClick } }), A.config.showText ? t(A.TKText, { attrs: { textStyle: A.config.textStyle, color: A.config.textColor, size: A.config.textSize } }, [e._v(" " + e._s(A.t("accept")) + " ")]) : e._e()], 1);
}, xc = [], Bc = /* @__PURE__ */ v(
  Uc,
  Oc,
  xc,
  !1,
  null,
  "e1377967",
  null,
  null
);
const Pc = Bc.exports, Vc = {
  options: {
    virtualHost: !0
  }
}, _c = /* @__PURE__ */ S({
  ...Vc,
  __name: "Microphone",
  props: mA,
  setup(a) {
    const e = y.isPC, t = I(!0), { localUserInfoExcludeVolume: A } = b(pA()), n = Be(), [{ deviceList: s }] = Oe(oA.MICROPHONE), l = M(() => {
      var C;
      return ((C = s.value) == null ? void 0 : C.length) > 0 && n.value === "microphone";
    }), r = M(() => t.value ? A != null && A.value.isAudioAvailable ? "basicConfig" : "closedConfig" : "loadingConfig"), i = cA("microphone", r), c = $(), g = M(() => A != null && A.value.isAudioAvailable ? c.value("microphone enabled") : c.value("microphone disabled"));
    return { __sfc: !0, isPC: e, clickAble: t, localUserInfo: A, popoverValue: n, deviceList: s, isShowPopover: l, configState: r, config: i, t: c, btnText: g, handleClick: async () => {
      A != null && A.value.isAudioAvailable ? await x.closeMicrophone() : await x.openMicrophone();
    }, handleMouseEnter: () => {
      n.value = "microphone";
    }, DeviceType: oA, Button: MA, TKText: lA, Popover: ct, DeviceSelect: dt };
  }
});
var Fc = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "btn-content" }, [A.isPC ? t(A.Popover, { attrs: { color: "rgba(107, 117, 138, 0.5)", show: A.isShowPopover, trigger: "hover" }, on: { hover: A.handleMouseEnter }, scopedSlots: e._u([{ key: "content", fn: function() {
    return [t(A.DeviceSelect, { attrs: { isShowControlBtn: !0, deviceType: A.DeviceType.MICROPHONE } })];
  }, proxy: !0 }, { key: "trigger", fn: function() {
    return [t(A.Button, { attrs: { loading: !A.clickAble, iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.config.iconSize, width: A.config.width, height: A.config.height, loadingWidth: A.config.loadingWidth, loadingHeight: A.config.loadingHeight, shape: A.config.shape }, on: { click: A.handleClick } })];
  }, proxy: !0 }], null, !1, 3223001746) }) : e._e(), A.isPC ? e._e() : t(A.Button, { attrs: { loading: !A.clickAble, iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.config.iconSize, width: A.config.width, height: A.config.height, loadingWidth: A.config.loadingWidth, loadingHeight: A.config.loadingHeight, shape: A.config.shape }, on: { click: A.handleClick } }), A.config.showText ? t(A.TKText, { attrs: { textStyle: A.config.textStyle, color: A.config.textColor, size: A.config.textSize } }, [e._v(" " + e._s(A.btnText) + " ")]) : e._e()], 1);
}, Rc = [], Gc = /* @__PURE__ */ v(
  _c,
  Fc,
  Rc,
  !1,
  null,
  "1a5a5781",
  null,
  null
);
const kc = Gc.exports, Wc = {
  options: {
    virtualHost: !0
  }
}, qc = /* @__PURE__ */ S({
  ...Wc,
  __name: "Reject",
  props: mA,
  setup(a) {
    const e = a, t = cA("reject", I("basicConfig")), A = $();
    return { __sfc: !0, props: e, config: t, t: A, handleClick: async () => {
      await x.reject();
    }, Button: MA, TKText: lA };
  }
});
var Xc = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "btn-content" }, [t(A.Button, { attrs: { iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.config.iconSize, width: A.props.width || A.config.width, height: A.props.height || A.config.height, shape: A.config.shape }, on: { click: A.handleClick } }), A.config.showText ? t(A.TKText, { attrs: { textStyle: A.config.textStyle, color: A.config.textColor, size: A.config.textSize } }, [e._v(" " + e._s(A.t("reject")) + " ")]) : e._e()], 1);
}, Qc = [], Zc = /* @__PURE__ */ v(
  qc,
  Xc,
  Qc,
  !1,
  null,
  "cc016de6",
  null,
  null
);
const Yc = Zc.exports, Kc = {
  options: {
    virtualHost: !0
  }
}, Hc = /* @__PURE__ */ S({
  ...Kc,
  __name: "Speaker",
  props: mA,
  setup(a) {
    const e = a, t = y.isWeChat, A = y.isPC, { isEarPhone: n, isMuteSpeaker: s } = b(Y()), l = Be(), r = $(), i = M(() => n.value ? "closedConfig" : "basicConfig"), c = M(() => s.value ? "closedConfig" : "basicConfig"), [{ deviceList: g, currentDeviceId: f }] = Oe(oA.SPEAKER), p = M(() => {
      var z;
      return ((z = g.value) == null ? void 0 : z.length) > 0 && l.value === "speaker" && A;
    }), C = t ? cA("speaker", i) : cA("speaker", c), N = M(() => n.value ? r.value("speaker disabled") : r.value("speaker enabled"));
    return { __sfc: !0, isWeChat: t, isPC: A, props: e, isEarPhone: n, isMuteSpeaker: s, popoverValue: l, t: r, wxConfigState: i, webConfigState: c, deviceList: g, currentDeviceId: f, isShowPopover: p, config: C, btnText: N, handleClick: async () => {
      s.value ? await x.unMuteSpeaker() : await x.muteSpeaker();
    }, getButtonText: () => {
      if (A) {
        const z = g.value.find((T) => T.deviceId === f.value);
        return z ? z.label : r.value("speaker enabled");
      } else
        return s.value ? r.value("speaker disabled") : r.value("speaker enabled");
    }, handleMouseEnter: () => {
      l.value = "speaker";
    }, DeviceType: oA, Button: MA, TKText: lA, Popover: ct, DeviceSelect: dt };
  }
});
var Jc = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "btn-content" }, [t(A.Popover, { attrs: { color: "rgba(107, 117, 138, 0.5)", show: A.isShowPopover, trigger: "hover" }, on: { hover: A.handleMouseEnter }, scopedSlots: e._u([{ key: "content", fn: function() {
    return [t(A.DeviceSelect, { attrs: { isShowControlBtn: !0, deviceType: A.DeviceType.SPEAKER } })];
  }, proxy: !0 }, { key: "trigger", fn: function() {
    return [t(A.Button, { attrs: { iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.props.iconSize || A.config.iconSize, width: A.props.width || A.config.width, height: A.props.height || A.config.height, shape: A.config.shape }, on: { click: A.handleClick } })];
  }, proxy: !0 }]) }), A.config.showText ? t(A.TKText, { attrs: { width: "60px", truncated: !0, textStyle: A.config.textStyle, color: A.config.textColor, size: A.config.textSize, lineClamp: 2 } }, [e._v(" " + e._s(A.getButtonText()) + " ")]) : e._e()], 1);
}, $c = [], Ad = /* @__PURE__ */ v(
  Hc,
  Jc,
  $c,
  !1,
  null,
  "44a3def7",
  null,
  null
);
const ed = Ad.exports, td = {
  options: {
    virtualHost: !0
  }
}, ad = /* @__PURE__ */ S({
  ...td,
  __name: "SwitchCamera",
  setup(a) {
    const { localUserInfoExcludeVolume: e } = b(pA()), t = M(() => e == null ? void 0 : e.value.isVideoAvailable), A = M(() => t.value ? "basicConfig" : "disableConfig"), n = cA("switchCamera", A), s = $();
    return { __sfc: !0, localUserInfo: e, isVideoAvailable: t, configState: A, config: n, t: s, handleClick: async () => {
      !t.value || await x.switchCamera();
    }, Button: MA, TKText: lA };
  }
});
var nd = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "btn-content" }, [t(A.Button, { attrs: { iconSrc: A.config.iconSrc, iconSize: A.config.iconSize, color: A.config.color, width: A.config.width, height: A.config.height, buttonStyle: A.config.buttonStyle, shape: "circle" }, on: { click: A.handleClick } }), A.config.showText ? t(A.TKText, { attrs: { textStyle: A.config.textStyle, color: A.config.textColor, size: A.config.textSize } }, [e._v(" " + e._s(A.t("switch camera")) + " ")]) : e._e()], 1);
}, od = [], sd = /* @__PURE__ */ v(
  ad,
  nd,
  od,
  !1,
  null,
  "df69c038",
  null,
  null
);
const ld = sd.exports, id = {
  options: {
    virtualHost: !0
  }
}, ud = /* @__PURE__ */ S({
  ...id,
  __name: "VirtualBackground",
  props: mA,
  setup(a) {
    const e = a, t = I(!0), { localUserInfoExcludeVolume: A } = b(pA()), n = M(() => A == null ? void 0 : A.value.isVideoAvailable), { enableVirtualBackground: s } = b(Y()), l = M(() => t.value ? s.value ? "closedConfig" : n.value ? "basicConfig" : "disableConfig" : "loadingConfig"), r = cA("virtualBackground", l), i = $();
    return { __sfc: !0, props: e, clickAble: t, localUserInfo: A, isVideoAvailable: n, enableVirtualBackground: s, configState: l, config: r, t: i, handleClick: async () => {
      !n.value || (t.value = !1, await x.setBlurBackground(!s.value), t.value = !0);
    }, Button: MA, TKText: lA };
  }
});
var rd = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "btn-content" }, [t(A.Button, { attrs: { loading: !A.clickAble, iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.props.iconSize || A.config.iconSize, width: A.props.width || A.config.width, height: A.props.height || A.config.height, shape: A.config.shape, loadingWidth: A.config.loadingWidth, loadingHeight: A.config.loadingHeight, buttonStyle: A.config.buttonStyle }, on: { click: A.handleClick } }), A.config.showText ? t(A.TKText, { attrs: { width: "70px", lineClamp: 2, textStyle: A.config.textStyle, color: A.config.textColor, size: A.config.textSize } }, [e._v(" " + e._s(A.t("virtual-background")) + " ")]) : e._e()], 1);
}, cd = [], dd = /* @__PURE__ */ v(
  ud,
  rd,
  cd,
  !1,
  null,
  "f29c2e7a",
  null,
  null
);
const gd = dd.exports, Id = {
  options: {
    virtualHost: !0
  }
}, pd = /* @__PURE__ */ S({
  ...Id,
  __name: "ToggleButtonPanel",
  props: mA,
  setup(a) {
    const e = a, { status: t } = ge(), A = cA("toggleButtonPanel", I("basicConfig"));
    return { __sfc: !0, panelStatus: t, props: e, config: A, handleClick: async () => {
      t.value = t.value === "close" ? "open" : "close";
    }, Button: MA };
  }
});
var Cd = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "btn-content" }, [t(A.Button, { attrs: { iconSrc: A.config.iconSrc, color: A.config.color, iconSize: A.props.iconSize || A.config.iconSize, width: A.props.width || A.config.width, height: A.props.height || A.config.height, shape: A.config.shape }, on: { click: A.handleClick } })], 1);
}, Md = [], fd = /* @__PURE__ */ v(
  pd,
  Cd,
  Md,
  !1,
  null,
  "f7839a20",
  null,
  null
);
const Nd = fd.exports, Ld = {
  options: {
    virtualHost: !0
  }
}, vd = /* @__PURE__ */ S({
  ...Ld,
  __name: "ButtonPanel",
  setup(a) {
    const e = I(null), t = I({}), { isGroupCall: A } = b(Y()), { status: n } = ge() || {}, s = I(!1), l = I(!1), r = I(""), i = Ia();
    $A(() => {
      s.value || (A.value && !y.isPC && i.value !== null ? s.value = !0 : s.value = !1);
    });
    const c = M(() => Q([
      "button-panel-container",
      {
        pc: y.isPC,
        mobile: !y.isPC,
        h5: y.isH5,
        groupCall: A.value,
        singleCall: !A.value,
        close: (n == null ? void 0 : n.value) === "close",
        open: (n == null ? void 0 : n.value) === "open",
        showBackGround: s.value
      }
    ])), g = Q([
      "toggle-button-container",
      { h5: y.isH5 }
    ]), f = y.isH5 ? {
      transitionProperty: "width,height,left,top",
      transitionDuration: "0.3s",
      transitionTimingFunction: "ease-in"
    } : {};
    function p(D) {
      e.value = D;
    }
    const { layout: C, config: N } = qr();
    K(N, () => {
      const D = {}, z = N.value.flat();
      for (let T of z) {
        const { name: j, props: F = {} } = T;
        F.showText !== !1 && (F.showText = !0), D[j] = F;
      }
      t.value = D;
    }, { immediate: !0 });
    function h(D) {
      l.value = D;
    }
    const L = {
      [o.IS_CLICKABLE]: h
    };
    return sA(() => {
      d.watch(
        u.CALL,
        L,
        {
          notifyRangeWhenWatch: o.MYSELF
        }
      );
    }), aA(() => {
      d.unwatch(u.CALL, L);
    }), rA(oa, l), rA(sa, r), { __sfc: !0, focus: e, buttonProps: t, isGroupCall: A, panelStatus: n, showButtonPanelBackground: s, IsClickableContextValue: l, PopoverContextValue: r, focusElement: i, buttonPanelClassName: c, toggleButtonContainerClassName: g, customStyle: f, handleToggle: p, layout: C, config: N, handleIsClickableChange: h, watchOptions: L, Grid: Qa, GridItem: Za, TKCamera: mc, Hangup: bc, Accept: Pc, Microphone: kc, Reject: Yc, Speaker: ed, SwitchCamera: ld, VirtualBackground: gd, ToggleButtonPanel: Nd, InviteUser: Ea };
  }
});
var Sd = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { class: A.buttonPanelClassName }, [t("div", { staticClass: "button-group" }, [t(A.Grid, { attrs: { unit: "%", layout: A.layout, focus: A.focus }, on: { toggle: A.handleToggle } }, [t(A.GridItem, { attrs: { index: "accept", height: "auto", customStyle: A.customStyle } }, [t(A.Accept)], 1), t(A.GridItem, { attrs: { index: "reject", height: "auto", customStyle: A.customStyle } }, [t(A.Reject)], 1), t(A.GridItem, { attrs: { index: "microphone", height: "auto", customStyle: A.customStyle } }, [t(A.Microphone)], 1), t(A.GridItem, { attrs: { index: "hangup", height: "auto", customStyle: A.customStyle } }, [t(A.Hangup)], 1), t(A.GridItem, { attrs: { index: "speaker", height: "auto", customStyle: A.customStyle } }, [t(A.Speaker)], 1), t(A.GridItem, { attrs: { index: "camera", height: "auto", customStyle: A.customStyle } }, [t(A.TKCamera)], 1), t(A.GridItem, { attrs: { index: "virtualBackground", height: "auto", customStyle: A.customStyle } }, [t(A.VirtualBackground)], 1), t(A.GridItem, { attrs: { index: "switchCamera", height: "auto", customStyle: A.customStyle } }, [t(A.SwitchCamera)], 1), t(A.GridItem, { attrs: { index: "inviteUser", height: "auto", customStyle: A.customStyle } }, [t(A.InviteUser)], 1)], 1)], 1), A.showButtonPanelBackground ? t("div", { class: A.toggleButtonContainerClassName }, [t(A.ToggleButtonPanel)], 1) : e._e()]);
}, Dd = [], hd = /* @__PURE__ */ v(
  vd,
  Sd,
  Dd,
  !1,
  null,
  "6582c17d",
  null,
  null
);
const Ka = hd.exports, md = /* @__PURE__ */ S({
  __name: "SingleCall",
  setup(a) {
    const { callStatus: e } = b(Y());
    return { __sfc: !0, callStatus: e, CallStatus: w, TopBar: wa, Waiting: Oa, MediaContainer: dr, Tip: qa, ButtonPanel: Ka };
  }
});
var yd = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "singlecall-container" }, [t(A.TopBar), A.callStatus === A.CallStatus.CALLING ? t(A.Waiting) : e._e(), t(A.MediaContainer), t(A.Tip), t(A.ButtonPanel)], 1);
}, Td = [], Ed = /* @__PURE__ */ v(
  md,
  yd,
  Td,
  !1,
  null,
  "317c1d41",
  null,
  null
);
const wd = Ed.exports, jd = /* @__PURE__ */ S({
  __name: "StreamLoading",
  setup(a) {
    return { __sfc: !0, Loading: it };
  }
});
var bd = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "stream-loading-container" }, [t(A.Loading, { attrs: { mode: "dot", color: "#FFF", loadingWidth: "50px", loadingHeight: "50px" } })], 1);
}, zd = [], Ud = /* @__PURE__ */ v(
  jd,
  bd,
  zd,
  !1,
  null,
  "56b8bc56",
  null,
  null
);
const Od = Ud.exports, xd = {
  options: {
    virtualHost: !0
  }
}, Bd = /* @__PURE__ */ S({
  ...xd,
  __name: "MediaContainer",
  setup(a) {
    const e = I("local"), t = I(null), A = I(!1), n = Ie(), { localUserInfoExcludeVolume: s, remoteUserListExcludeVolume: l } = b(pA()), { callStatus: r, callType: i, isShowEnableVirtualBackground: c, callRole: g } = b(Y()), { netWorkQualityList: f } = so(), p = M(() => l.value.length + 1), C = ga(), N = uo(t, p), h = da(), { isFloatWindow: L } = b(LA()), D = Ia(), { status: z } = ge() || {}, T = !y.isPC, j = M(() => String(t.value) === "0"), F = M(() => !y.isPC && j.value && s.value.isVideoAvailable), Z = M(() => F.value), V = M(() => F.value && c.value && y.isWeChat), B = M(() => y.isPC || j.value), W = M(() => y.isPC || L.value ? "%" : "vw"), vA = M(() => [s.value, ...l.value].map((X) => {
      var tA;
      if (L.value) {
        if (((tA = h.value) == null ? void 0 : tA[X.domId]) >= 10)
          return X.domId;
      } else
        return X.domId;
    })), _A = M(() => !(g.value === J.CALLEE && r.value === w.CALLING && !L.value)), yA = M(() => {
      let X = "";
      return g.value === J.CALLEE && r.value === w.CALLING && !L.value && (X = "hidden"), {
        visibility: X
      };
    });
    function TA(X) {
      t.value = X, D.value = X, z.value = X !== null ? "close" : "open";
    }
    function EA(X) {
      var tA, uA;
      return y.isWeChat ? !((uA = (tA = C.value) == null ? void 0 : tA.find((wA) => wA.userID === X.userId)) != null && uA.hasVideo) : !X.isVideoAvailable;
    }
    function ee(X) {
      var tA, uA;
      return y.isWeChat ? !((uA = (tA = C.value) == null ? void 0 : tA.find((wA) => wA.userID === X.userId)) != null && uA.hasAudio) : !X.isAudioAvailable;
    }
    function pe(X) {
      if (!f.value)
        return;
      const tA = X !== s.value.userId;
      if (!y.isWeChat && tA)
        return;
      const uA = f.value.find((wA) => wA.userId === X);
      return uA && (uA == null ? void 0 : uA.quality) >= 4;
    }
    K([l, i], () => {
      var X, tA;
      (tA = (X = l.value) == null ? void 0 : X[0]) != null && tA.isEnter && (e.value = "remote"), i.value === _.AUDIO ? A.value = !1 : A.value = !0;
    });
    const Ce = M(() => Q([
      "groupcall-media-container",
      {
        mobile: !y.isPC,
        pc: y.isPC,
        "two-layout": p.value === 2 && t.value === null,
        float: L.value
      }
    ])), te = M(() => r.value === w.CALLING && (i.value === _.AUDIO && !s.value.isAudioAvailable || i.value === _.VIDEO && !s.value.isVideoAvailable));
    return { __sfc: !0, bigWindow: e, focus: t, showSmallWindow: A, customUI: n, localUserInfoExcludeVolume: s, remoteUserListExcludeVolume: l, callStatus: r, callType: i, isShowEnableVirtualBackground: c, callRole: g, netWorkQualityList: f, streamLength: p, wxPlayer: C, layout: N, volumeMap: h, isFloatWindow: L, focusElement: D, panelStatus: z, enableFocus: T, isCurrentUserStreamZoomIn: j, showControlButton: F, showSwitchCameraButton: Z, showVirtualBackgroundButton: V, showNickName: B, unit: W, visibleStreamIdList: vA, showStream: _A, mediaContainerStyle: yA, changeFocus: TA, isShowAudioStream: EA, isMute: ee, isShowNetWork: pe, groupMediaContainerClassName: Ce, isPusherLoading: te, TUIGlobal: y, Pusher: _a, Player: Fa, Grid: Qa, GridItem: Za, AudioStream: Ra, TKStreamInfo: Va, FloatWindow: ka, Portal: Ba, StreamLoading: Od };
  }
});
var Pd = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t(A.Portal, { attrs: { id: "source", disabled: !A.isFloatWindow, source: "#source", to: "body" } }, [t(A.FloatWindow, [t("div", { class: A.groupMediaContainerClassName, style: [A.mediaContainerStyle] }, [t(A.Grid, { attrs: { unit: A.unit, "enable-focus": A.enableFocus, focus: A.focus, length: A.streamLength, layout: A.layout }, on: { toggle: A.changeFocus } }, [t(A.GridItem, { key: A.localUserInfoExcludeVolume.userId, attrs: { index: 0 } }, [t(A.Pusher, { attrs: { domId: A.localUserInfoExcludeVolume.domId, "show-audio-stream": !A.localUserInfoExcludeVolume.isVideoAvailable, show: A.visibleStreamIdList.includes(A.localUserInfoExcludeVolume.domId) && A.showStream }, scopedSlots: e._u([{ key: "audio-stream", fn: function() {
    return [t(A.AudioStream, { attrs: { "user-id": A.localUserInfoExcludeVolume.userId, username: A.localUserInfoExcludeVolume.displayUserInfo, avatar: A.localUserInfoExcludeVolume.avatar, "is-video-available": A.localUserInfoExcludeVolume.isVideoAvailable } })];
  }, proxy: !0 }, { key: "loading", fn: function() {
    return [A.isPusherLoading ? t(A.StreamLoading) : e._e()];
  }, proxy: !0 }, { key: "stream-info", fn: function() {
    return [A.isFloatWindow ? e._e() : t(A.TKStreamInfo, { attrs: { "is-self": !0, "show-nick-name": A.showNickName, showSwitchCameraButton: A.showSwitchCameraButton, showVirtualBackgroundButton: A.showVirtualBackgroundButton, showNetWorkStatus: A.isShowNetWork(A.localUserInfoExcludeVolume.userId), nickName: A.localUserInfoExcludeVolume.displayUserInfo, isMuted: !A.localUserInfoExcludeVolume.isAudioAvailable, volume: A.volumeMap && A.volumeMap[A.localUserInfoExcludeVolume.domId] } })];
  }, proxy: !0 }]) })], 1), e._l(A.remoteUserListExcludeVolume, function(n, s) {
    return t(A.GridItem, { key: n.userId, attrs: { index: s + 1 } }, [t(A.Player, { attrs: { domId: n.domId, "show-audio-stream": A.isShowAudioStream(n), show: A.visibleStreamIdList.includes(n.domId) && A.showStream }, scopedSlots: e._u([{ key: "audio-stream", fn: function() {
      return [t(A.AudioStream, { attrs: { "user-id": n.userId, username: n.displayUserInfo, avatar: n.avatar } })];
    }, proxy: !0 }, { key: "loading", fn: function() {
      return [n.isEnter ? e._e() : t(A.StreamLoading)];
    }, proxy: !0 }, { key: "stream-info", fn: function() {
      return [A.isFloatWindow ? e._e() : t(A.TKStreamInfo, { attrs: { "show-nick-name": A.TUIGlobal.isPC || String(A.focus) === String(s + 1), "show-control-button": !1, showNetWorkStatus: A.isShowNetWork(n.userId), nickName: n.displayUserInfo, "is-muted": A.isMute(n), volume: A.volumeMap && A.volumeMap[n.domId] } })];
    }, proxy: !0 }], null, !0) })], 1);
  })], 2)], 1)])], 1);
}, Vd = [], _d = /* @__PURE__ */ v(
  Bd,
  Pd,
  Vd,
  !1,
  null,
  "0629f0d8",
  null,
  null
);
const Fd = _d.exports, Rd = /* @__PURE__ */ S({
  __name: "TDialog",
  props: {
    isHeaderShow: {
      type: Boolean,
      default: !0
    },
    isFooterShow: {
      type: Boolean,
      default: !0
    },
    background: {
      type: Boolean,
      default: !0
    },
    title: {
      type: String,
      default: ""
    },
    center: {
      type: Boolean,
      default: !1
    },
    isH5: {
      type: Boolean,
      default: !1
    }
  },
  emits: ["cancel", "submit"],
  setup(a, { emit: e }) {
    const t = a, A = {
      OUTSIDE: "outside",
      INSIDE: "inside"
    }, n = I(!0), s = I(!0), l = I(!0), r = I(""), i = $();
    $A(() => {
      r.value = t.title, n.value = t.isHeaderShow, s.value = t.isFooterShow, l.value = t.background;
    });
    const c = (p) => {
      p === A.OUTSIDE && g();
    }, g = () => {
      e("cancel");
    };
    return { __sfc: !0, clickType: A, props: t, isHeaderShowDialog: n, isFooterShowDialog: s, backgroundDialog: l, showTitle: r, t: i, emits: e, toggleView: c, close: g, submit: () => {
      e("submit");
    } };
  }
});
var Gd = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "dialog", class: [A.props.isH5 ? "dialog-h5" : "", e.center ? "center" : ""], on: { click: function(n) {
    return n.stopPropagation(), n.preventDefault(), A.toggleView(A.clickType.OUTSIDE);
  } } }, [t("main", { staticClass: "dialog-main", class: [A.backgroundDialog ? "" : "dialog-main-back"], on: { click: function(n) {
    return n.stopPropagation(), n.preventDefault(), A.toggleView(A.clickType.INSIDE);
  } } }, [A.isHeaderShowDialog ? t("header", { staticClass: "dialog-main-header" }, [t("h1", { staticClass: "dialog-main-title" }, [e._v(e._s(A.showTitle))]), t("i", { staticClass: "icon icon-close", on: { click: A.close } })]) : e._e(), t("div", { staticClass: "dialog-main-content", class: [A.props.isH5 ? "dialog-main-content-uniapp" : ""] }, [e._t("default")], 2), A.isFooterShowDialog ? t("footer", { staticClass: "dialog-main-footer" }, [t("button", { staticClass: "btn btn-cancel", on: { click: A.close } }, [e._v(e._s(A.t("Cancel")))]), t("button", { staticClass: "btn btn-default", on: { click: A.submit } }, [e._v(e._s(A.t("Done")))])]) : e._e()])]);
}, kd = [], Wd = /* @__PURE__ */ v(
  Rd,
  Gd,
  kd,
  !1,
  null,
  "26735e82",
  null,
  null
);
const qd = Wd.exports, Xd = /* @__PURE__ */ S({
  __name: "TIcon",
  props: {
    file: {
      type: String,
      default: ""
    },
    width: {
      type: String,
      default: "20px"
    },
    height: {
      type: String,
      default: "20px"
    },
    isApp: {
      type: Boolean,
      default: !1
    }
  },
  emits: ["click"],
  setup(a, { emit: e }) {
    return { __sfc: !0, emits: e, props: a, handlerImgClick: () => {
      e("click");
    } };
  }
});
var Qd = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "icon", on: { click: A.handlerImgClick } }, [A.props.isApp ? t("image", { staticClass: "icon", style: { width: A.props.width, height: A.props.height }, attrs: { src: A.props.file } }) : t("img", { staticClass: "icon", style: { width: A.props.width, height: A.props.height }, attrs: { src: A.props.file } })]);
}, Zd = [], Yd = /* @__PURE__ */ v(
  Xd,
  Qd,
  Zd,
  !1,
  null,
  "b989a330",
  null,
  null
);
const Kd = Yd.exports, Hd = "data:image/svg+xml;base64,", Jd = "data:image/svg+xml;base64,", $d = "data:image/svg+xml;base64,", Ag = /* @__PURE__ */ S({
  __name: "TTransfer",
  props: {
    list: {
      type: Array,
      default: () => []
    },
    selectedList: {
      type: Array,
      default: () => []
    },
    isSearch: {
      type: Boolean,
      default: !0
    },
    isRadio: {
      type: Boolean,
      default: !1
    },
    isCustomItem: {
      type: Boolean,
      default: !1
    },
    title: {
      type: String,
      default: ""
    },
    type: {
      type: String,
      default: ""
    },
    resultShow: {
      type: Boolean,
      default: !0
    },
    total: {
      type: Number,
      default: 0
    },
    isH5: {
      type: Boolean,
      default: !1
    }
  },
  emits: ["search", "submit", "cancel", "getMore"],
  setup(a, { emit: e }) {
    const t = a, A = I(""), n = I([]), s = I(0), l = I([]), r = I(!0), i = I(!1), c = I(""), g = I(""), f = $();
    $A(() => {
      const { list: T, isCustomItem: j, isSearch: F, title: Z, total: V, selectedList: B } = t;
      if (j)
        for (let W = 0; W < T.length; W++)
          T[W].conversationID.indexOf("@TIM#SYSTEM") > -1 && T.splice(W, 1), n.value = T;
      else
        n.value = T;
      s.value = V || T.length, l.value = B && B.length > 0 ? B : l.value, r.value = F, i.value = j, c.value = Z, A.value = t.type;
    });
    const p = M(
      () => n.value.filter((T) => !T.isDisabled)
    );
    return { __sfc: !0, props: t, emit: e, type: A, transferList: n, transferTotal: s, transferSelectedList: l, isTransferSearch: r, isTransferCustomItem: i, transferTitle: c, searchValue: g, t: f, optional: p, handleInput: (T) => {
      g.value = T.target.value, e("search", T.target.value);
    }, selected: (T) => {
      if (T.isDisabled)
        return;
      let j = l.value;
      const F = j.indexOf(T);
      if (F > -1)
        return l.value.splice(F, 1);
      t.isRadio && (j = []), j.push(T), l.value = j;
    }, selectedAll: () => {
      l.value.length === p.value.length ? l.value = [] : l.value = [...p.value];
    }, submit: () => {
      e("submit", l.value), g.value = "", l.value = [];
    }, cancel: () => {
      e("cancel"), g.value = "", l.value = [];
    }, getMore: () => {
      e("getMore");
    }, TIcon: Kd, selectedIcon: Hd, backIcon: Jd, cancelIcon: $d };
  }
});
var eg = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "transfer", class: [A.props.isH5 ? "transfer-h5" : ""] }, [A.props.isH5 ? t("header", { staticClass: "transfer-header transfer-h5-header", on: { click: A.cancel } }, [t(A.TIcon, { staticClass: "icon", attrs: { file: A.backIcon, width: "18px", height: "18px" } }), t("span", { staticClass: "title" }, [e._v(e._s(A.transferTitle))]), t("span", { staticClass: "space" })], 1) : e._e(), t("main", { staticClass: "main" }, [t("div", { staticClass: "left" }, [t("header", { staticClass: "transfer-header" }, [!A.props.isH5 && A.isTransferSearch ? t("input", { class: [A.props.isH5 ? "left-uniapp-input" : ""], attrs: { type: "text", placeholder: A.t("Please enter userID"), enterkeyhint: "search" }, domProps: { value: A.searchValue }, on: { keyup: function(n) {
    return !n.type.indexOf("key") && e._k(n.keyCode, "enter", 13, n.key, "Enter") ? null : A.handleInput.apply(null, arguments);
  } } }) : e._e(), A.props.isH5 && A.isTransferSearch ? t("input", { class: [A.props.isH5 ? "left-uniapp-input" : ""], attrs: { type: "text", placeholder: A.t("Please enter userID"), enterkeyhint: "search" }, domProps: { value: A.searchValue }, on: { blur: A.handleInput, confirm: A.handleInput } }) : e._e()]), t("main", { staticClass: "transfer-left-main" }, [t("ul", { staticClass: "transfer-list" }, [A.optional.length > 1 && !e.isRadio ? t("li", { staticClass: "transfer-list-item", on: { click: A.selectedAll } }, [A.transferSelectedList.length === A.optional.length ? t(A.TIcon, { attrs: { file: A.selectedIcon, width: "18px", height: "18px" } }) : t("i", { staticClass: "icon-unselected" }), t("span", { staticClass: "select-all" }, [e._v(e._s(A.t("Select all")))])], 1) : e._e(), e._l(A.transferList, function(n) {
    return t("li", { key: n.userID, staticClass: "transfer-list-item", on: { click: function(s) {
      return A.selected(n);
    } } }, [A.transferSelectedList.indexOf(n) > -1 ? t(A.TIcon, { class: [n.isDisabled && "disabled"], attrs: { file: A.selectedIcon, width: "18px", height: "18px" } }) : t("i", { class: [n.isDisabled && "disabled", "icon-unselected"] }), A.isTransferCustomItem ? [e._t("left", null, { data: n })] : [t("img", { staticClass: "avatar", attrs: { src: n.avatar || "https://web.sdk.qcloud.com/component/TUIKit/assets/avatar_21.png", onerror: "this.src='https://web.sdk.qcloud.com/component/TUIKit/assets/avatar_21.png'" } }), t("span", { staticClass: "name" }, [e._v(e._s(n.nick || n.userID))]), n.isDisabled ? t("span") : e._e()]], 2);
  }), A.transferTotal > A.transferList.length ? t("li", { staticClass: "transfer-list-item more", on: { click: A.getMore } }, [e._v(" " + e._s(A.t("View more")) + " ")]) : e._e()], 2)])]), t("div", { staticClass: "right" }, [A.props.isH5 ? e._e() : t("header", { staticClass: "transfer-header" }, [e._v(" " + e._s(A.transferTitle) + " ")]), e.resultShow ? t("ul", { staticClass: "transfer-list" }, [A.transferSelectedList.length > 0 && !A.props.isH5 ? t("p", { staticClass: "transfer-text" }, [e._v(" " + e._s(A.transferSelectedList.length) + " " + e._s(A.t("people selected")) + " ")]) : e._e(), e._l(A.transferSelectedList, function(n, s) {
    return t("li", { key: s, staticClass: "transfer-list-item space-between" }, [t("aside", { staticClass: "transfer-list-item-content" }, [A.isTransferCustomItem ? [e._t("right", null, { data: n })] : [t("img", { staticClass: "avatar", attrs: { src: n.avatar || "https://web.sdk.qcloud.com/component/TUIKit/assets/avatar_21.png", onerror: "this.src='https://web.sdk.qcloud.com/component/TUIKit/assets/avatar_21.png'" } }), A.props.isH5 ? e._e() : t("span", { staticClass: "name" }, [e._v(e._s(n.nick || n.userID))])]], 2), A.props.isH5 ? e._e() : t("span", { on: { click: function(l) {
      return A.selected(n);
    } } }, [t(A.TIcon, { attrs: { file: A.cancelIcon, width: "18px", height: "18px" } })], 1)]);
  })], 2) : e._e(), t("footer", { staticClass: "transfer-right-footer" }, [t("button", { staticClass: "btn btn-cancel", on: { click: A.cancel } }, [e._v(e._s(A.t("Cancel")))]), A.transferSelectedList.length > 0 ? t("button", { staticClass: "btn", on: { click: A.submit } }, [e._v(" " + e._s(A.t("Done")) + " ")]) : t("button", { staticClass: "btn btn-no", on: { click: A.submit } }, [e._v(e._s(A.t("Done")))])])])])]);
}, tg = [], ag = /* @__PURE__ */ v(
  Ag,
  eg,
  tg,
  !1,
  null,
  "cc0325fc",
  null,
  null
);
const ng = ag.exports, og = /* @__PURE__ */ S({
  __name: "SelectUser",
  props: {
    isRadio: {
      type: Boolean,
      default: !1
    },
    isNeedSearch: {
      type: Boolean,
      default: !1
    },
    title: {
      type: String,
      default: ""
    },
    userList: {
      type: Array,
      default: () => []
    },
    total: {
      type: Number,
      default: 0
    },
    isPC: {
      type: Boolean,
      default: !0
    }
  },
  emits: ["confirm", "cancel", "search", "getMore"],
  setup(a, { emit: e }) {
    return { __sfc: !0, emits: e, props: a, handleSelectUserCancel: () => {
      e("cancel");
    }, handleTransferSubmit: (r) => {
      e("confirm", r);
    }, handleTransferSearchUser: (r) => {
      e("search", r);
    }, handleTransferGetMore: () => {
      e("getMore");
    }, TDialog: qd, TTransfer: ng };
  }
});
var sg = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t(A.TDialog, { attrs: { isH5: !e.isPC, isHeaderShow: !1, isFooterShow: !1, background: !1 }, on: { cancel: A.handleSelectUserCancel } }, [t(A.TTransfer, { attrs: { isSearch: A.props.isNeedSearch, title: A.props.title, list: A.props.userList, isH5: !e.isPC, isRadio: A.props.isRadio, total: A.props.total }, on: { getMore: A.handleTransferGetMore, search: A.handleTransferSearchUser, submit: A.handleTransferSubmit, cancel: A.handleSelectUserCancel } })], 1);
}, lg = [], ig = /* @__PURE__ */ v(
  og,
  sg,
  lg,
  !1,
  null,
  null,
  null,
  null
);
const ug = ig.exports, rg = {
  options: {
    virtualHost: !0
  }
}, cg = /* @__PURE__ */ S({
  ...rg,
  __name: "BackGround",
  setup(a) {
    return { __sfc: !0, Overlay: ja, defaultAvatarSrc: QA };
  }
});
var dg = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t(A.Overlay, { attrs: { bgColor: "#22262ed9", bgImage: A.defaultAvatarSrc, customStyle: { position: "absolute", zIndex: 0 } } });
}, gg = [], Ig = /* @__PURE__ */ v(
  cg,
  dg,
  gg,
  !1,
  null,
  null,
  null,
  null
);
const pg = Ig.exports, Cg = /* @__PURE__ */ S({
  __name: "GroupCall",
  setup(a) {
    const e = I(null), t = I("open"), A = I(!1), n = I(0), s = I([]), l = I([]), r = I(0), i = 30, c = e, g = { status: t }, { callRole: f, callStatus: p } = b(Y()), { isFloatWindow: C } = b(LA()), { localUserInfoExcludeVolume: N, remoteUserListExcludeVolume: h } = b(pA()), L = async (V) => {
      if (A.value = V, A.value) {
        await T();
        const B = await x.getGroupProfile();
        n.value = B.memberCount;
      }
    }, D = () => {
      A.value = !1, d.update(u.CALL, o.SHOW_SELECT_USER, !1), r.value = 0, s.value = [];
    }, z = async () => {
      r.value += i, await T();
    }, T = async () => {
      const V = await x.getGroupMemberList(i, r.value), B = [...h.value, N.value].map((W) => W.userId);
      s.value.push(...V), s.value = s.value.map((W) => (B.includes(W.userID) && (W = { ...W, isDisabled: !0 }), W)), l.value = s.value;
    }, j = async (V) => {
      try {
        if (V.length <= 0)
          return;
        A.value = !1, d.update(u.CALL, o.SHOW_SELECT_USER, !1), r.value = 0;
        const B = V.map((W) => W.userID);
        await x.inviteUser({ userIDList: B }), s.value = [];
      } catch (B) {
        console.debug(B);
      }
    }, F = (V) => {
      V ? s.value = s.value.filter((B) => B.userID.includes(V) || B.nick.includes(V)) : s.value = l.value;
    }, Z = {
      [o.SHOW_SELECT_USER]: L
    };
    return sA(() => {
      d.watch(u.CALL, Z, { notifyRangeWhenWatch: o.MYSELF });
    }), aA(() => {
      d.unwatch(u.CALL, Z);
    }), g.status = I("open"), c.value = null, rA(aa, g), rA(ta, c), { __sfc: !0, focusElement: e, buttonPanelStatus: t, showSelectUser: A, memberCount: n, groupMemberList: s, backupGroupMemberList: l, offset: r, count: i, FocusContextValue: c, ButtonPanelContextValue: g, callRole: f, callStatus: p, isFloatWindow: C, localUserInfoExcludeVolume: N, remoteUserListExcludeVolume: h, handleShowSelectUser: L, handleCancel: D, getMoreMemberList: z, getGroupMemberList: T, handleSelectedResult: j, handleSearch: F, watchOptions: Z, TopBar: wa, Waiting: Oa, MediaContainer: Fd, Tip: qa, ButtonPanel: Ka, SelectUser: ug, BackGround: pg, CallRole: J, CallStatus: w, TUIGlobal: y };
  }
});
var Mg = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "groupcall-container" }, [t(A.TopBar), A.callStatus === A.CallStatus.CALLING && A.callRole === A.CallRole.CALLEE && !A.isFloatWindow ? t(A.Waiting) : e._e(), t(A.MediaContainer), t(A.Tip), t(A.ButtonPanel), t(A.BackGround), A.showSelectUser ? t(A.SelectUser, { attrs: { isNeedSearch: !0, userList: A.groupMemberList, isPC: A.TUIGlobal.isPC, total: A.memberCount }, on: { confirm: A.handleSelectedResult, cancel: A.handleCancel, getMore: A.getMoreMemberList, search: A.handleSearch } }) : e._e()], 1);
}, fg = [], Ng = /* @__PURE__ */ v(
  Cg,
  Mg,
  fg,
  !1,
  null,
  "aa4e300f",
  null,
  null
);
const Lg = Ng.exports, vg = {};
var Sg = function() {
  var e = this, t = e._self._c;
  return t("svg", { attrs: { viewBox: "0 0 1024 1024", xmlns: "http://www.w3.org/2000/svg", "data-v-ea893728": "" } }, [t("path", { attrs: { fill: "currentColor", d: "M764.288 214.592 512 466.88 259.712 214.592a31.936 31.936 0 0 0-45.12 45.12L466.752 512 214.528 764.224a31.936 31.936 0 1 0 45.12 45.184L512 557.184l252.288 252.288a31.936 31.936 0 0 0 45.12-45.12L557.12 512.064l252.288-252.352a31.936 31.936 0 1 0-45.12-45.184z" } })]);
}, Dg = [], hg = /* @__PURE__ */ v(
  vg,
  Sg,
  Dg,
  !1,
  null,
  null,
  null,
  null
);
const mg = hg.exports, Rt = "data:image/jpeg;base64,", yg = "data:image/jpeg;base64,", Tg = "data:image/png;base64,", Eg = "data:image/png;base64,", wg = /* @__PURE__ */ S({
  __name: "SystemPermitTip",
  props: {
    platform: null,
    language: null
  },
  setup(a) {
    const e = a, t = {
      mac: {
        "zh-cn": Rt,
        en: yg
      },
      win: {
        "zh-cn": Tg,
        en: Eg
      }
    }, A = M(() => e.platform === "mac" ? "mac-privacy" : "win-privacy"), n = M(() => e.platform === "mac" ? "mac-preferences" : "win-preferences"), s = I(Rt), l = I(!0), r = $(), i = (g) => {
      g && y.isPC && (l.value = !0);
    }, c = () => {
      l.value = !1;
    };
    return K(() => e.language, () => {
      s.value = t[e.platform][e.language];
    }, {
      immediate: !0
    }), sA(() => {
      d.watch(u.CALL, {
        [o.SHOW_PERMISSION_TIP]: i
      }, {
        notifyRangeWhenWatch: o.MYSELF
      });
    }), aA(() => {
      d.unwatch(u.CALL, {
        [o.SHOW_PERMISSION_TIP]: i
      });
    }), { __sfc: !0, permitPngMap: t, props: e, privacy: A, preferences: n, permitPng: s, showModal: l, t: r, handlePermissionErrorTipChange: i, handleClose: c, CloseIcon: mg };
  }
});
var jg = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return A.showModal ? t("div", { staticClass: "card" }, [t("div", { staticClass: "card-body" }, [t("div", { staticClass: "close", on: { click: A.handleClose } }, [t(A.CloseIcon)], 1), t("div", { staticClass: "card-wrapper" }, [t("div", { staticClass: "title" }, [e._v(e._s(A.t("browser-authorization")))]), t("div", { staticClass: "button" }, [e._v(e._s(A.t(A.preferences)))]), t("div", { staticClass: "allow" }, [e._v(e._s(A.t(A.privacy)))]), t("div", { staticClass: "image-wrapper" }, [t("img", { staticClass: "image", attrs: { src: A.permitPng, alt: "system permit" } })])])])]) : e._e();
}, bg = [], zg = /* @__PURE__ */ v(
  wg,
  jg,
  bg,
  !1,
  null,
  "9982fa76",
  null,
  null
);
const Ug = zg.exports, Og = /* @__PURE__ */ S({
  __name: "PermitTip",
  setup(a) {
    const e = M(() => y.isMAC ? Qe.MAC : Qe.WIN), t = I(d.getData(u.CALL, o.LANGUAGE)), A = (n) => t.value = n;
    return sA(() => {
      d.watch(u.CALL, {
        [o.LANGUAGE]: A
      }, {
        notifyRangeWhenWatch: o.MYSELF
      });
    }), aA(() => {
      d.unwatch(u.CALL, {
        [o.LANGUAGE]: A
      });
    }), { __sfc: !0, platform: e, language: t, updateLanguage: A, SystemPermitTip: Ug };
  }
});
var xg = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "tip" }, [t("div", { staticClass: "card" }, [t(A.SystemPermitTip, { attrs: { platform: A.platform, language: A.language } })], 1)]);
}, Bg = [], Pg = /* @__PURE__ */ v(
  Og,
  xg,
  Bg,
  !1,
  null,
  "cd63c32e",
  null,
  null
);
const Vg = Pg.exports, Ha = {};
let De = null, _g = 1;
const Fg = () => {
  const a = document.body;
  De && (a.removeChild(De.$el), De.$destroy());
  const e = At.extend(Vg), t = new e({
    propsData: {
      index: `toast-item-${_g++}`,
      destroy() {
        a.removeChild(t.$el), t.$destroy();
      }
    }
  });
  t.$mount(), a.appendChild(t.$el), De = t;
};
Ha.show = () => Fg();
const Rg = /* @__PURE__ */ S({
  __name: "Toast",
  props: {
    type: null,
    duration: null,
    message: null,
    index: null,
    showClose: { type: Boolean },
    destroy: null,
    close: null
  },
  setup(a) {
    const e = a, t = I(!1), A = I(null), n = M(() => `toast-container ${e.type}`), s = () => {
      e.destroy();
    }, l = () => {
      e.close(), t.value = !1;
    }, r = () => {
      A.value && clearTimeout(A.value), e.duration && (A.value = setTimeout(() => {
        l();
      }, e.duration));
    };
    return sA(() => {
      t.value = !0, r();
    }), aA(() => {
      A.value && clearTimeout(A.value);
    }), { __sfc: !0, props: e, isShow: t, timer: A, toastClass: n, handleAfterLeave: s, handleClose: l, startTimer: r };
  }
});
var Gg = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("Transition", { attrs: { name: "slide-up" }, on: { "after-leave": A.handleAfterLeave } }, [t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isShow, expression: "isShow" }], key: e.index, class: A.toastClass }, [t("div", { staticClass: "toast-content" }, [e._v(e._s(e.message))]), t("i", { directives: [{ name: "show", rawName: "v-show", value: e.showClose, expression: "showClose" }], staticClass: "toast-close-icon", on: { click: A.handleClose } })])]);
}, kg = [], Wg = /* @__PURE__ */ v(
  Rg,
  Gg,
  kg,
  !1,
  null,
  "27fc22c5",
  null,
  null
);
const qg = Wg.exports, xA = {}, CA = [];
let Xg = 1;
function Qg() {
  var e, t, A;
  const a = CA.length;
  return a > 0 ? (A = (t = (e = CA[a - 1]) == null ? void 0 : e.$el) == null ? void 0 : t.style) == null ? void 0 : A.top : "30px";
}
function Zg() {
  var e, t;
  const a = CA.length;
  return a > 0 ? (t = (e = CA[a - 1]) == null ? void 0 : e.$el) == null ? void 0 : t.offsetHeight : 0;
}
const Ja = (a) => {
  const e = document.body, { type: t, duration: A = 3e3, message: n, showClose: s = !1, onClose: l } = a, r = At.extend(qg), i = new r({
    propsData: {
      type: t,
      duration: A,
      message: n,
      showClose: s,
      index: `toast-item-${Xg++}`,
      close() {
        var C, N, h, L;
        const f = CA.findIndex((D) => D === i);
        console.log((N = (C = CA[f]) == null ? void 0 : C.$el) == null ? void 0 : N.offsetHeight);
        const p = parseInt((L = (h = CA[f]) == null ? void 0 : h.$el) == null ? void 0 : L.offsetHeight) + 20;
        for (let D = 0; D < CA.length; D++)
          D < f || (CA[D].$el.style.top = parseInt(CA[D].$el.style.top) - p + "px");
        CA.splice(f, 1);
      },
      destroy() {
        e.removeChild(i.$el), i.$destroy();
      }
    }
  });
  i.$mount(), e.appendChild(i.$el);
  const c = Qg(), g = Zg();
  i.$el.style.top = parseInt(c) + parseInt(g) + 20 + "px", CA.push(i);
};
["info", "waring", "error", "success"].forEach((a) => {
  let e = {};
  xA[a] = (t) => {
    typeof t == "string" ? e.message = t : e = t, Ja({ type: a, ...e });
  };
});
xA.show = (a) => {
  let e = {};
  return typeof a == "string" ? (e.message = a, e.type = "info") : e = a, Ja({ type: "info", ...e });
};
const Yg = /* @__PURE__ */ S({
  __name: "TUICallKit",
  props: {
    beforeCalling: null,
    afterCalling: null,
    onMinimized: null,
    onMessageSentByMe: null,
    kickedOut: null,
    statusChanged: null,
    allowedMinimized: { type: Boolean, default: !1 },
    allowedFullScreen: { type: Boolean, default: !0 },
    videoDisplayMode: { default: "cover" },
    videoResolution: { default: "480p" }
  },
  setup(a) {
    const e = a, t = !y.isPC, A = t ? "TUICallKit-mobile transition-animation" : "TUICallKit-desktop", n = I(""), s = I(""), l = I(""), r = I(!1), i = I(d.getData(u.CALL, o.CALL_STATUS)), c = I(d.getData(u.CALL, o.CALL_ROLE)), g = I(d.getData(u.CALL, o.CALL_MEDIA_TYPE)), f = I(d.getData(u.CALL, o.IS_GROUP)), p = I(d.getData(u.CALL, o.IS_EAR_PHONE)), C = I(null), N = I(d.getData(u.CALL, o.LOCAL_USER_INFO_EXCLUDE_VOLUMN)), h = I(d.getData(u.CALL, o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST)), L = I(d.getData(u.CALL, o.CALLER_USER_INFO)), D = I(d.getData(u.CALL, o.IS_MINIMIZED)), z = I(d.getData(u.CALL, o.ENABLE_VIRTUAL_BACKGROUND)), T = I(d.getData(u.CALL, o.IS_SHOW_ENABLE_VIRTUAL_BACKGROUND)), j = I(d.getData(u.CALL, o.CUSTOM_UI_CONFIG)), F = I(d.getData(u.CALL, o.IS_MUTE_SPEAKER)), Z = I(d.getData(u.CALL, o.TRANSLATE)), V = Me({
      callStatus: i,
      callRole: c,
      callType: g,
      isGroupCall: f,
      isEarPhone: p,
      focusElement: C,
      allowedFullScreen: e.allowedFullScreen,
      enableVirtualBackground: z,
      isShowEnableVirtualBackground: T,
      isMuteSpeaker: F
    }), B = Me({ callerUserInfo: L }), W = Me({
      localUserInfoExcludeVolume: N,
      remoteUserListExcludeVolume: h
    }), vA = Me({ isFloatWindow: D }), _A = I(Z), {
      beforeCalling: yA,
      afterCalling: TA,
      onMinimized: EA,
      onMessageSentByMe: ee,
      videoDisplayMode: pe,
      videoResolution: Ce,
      kickedOut: te,
      statusChanged: X,
      allowedMinimized: tA
    } = b(e), uA = (P) => {
      V.callStatus = P;
    }, wA = (P) => {
      V.isGroupCall = P;
    }, gt = (P) => {
      if (typeof P == "object") {
        const { content: ZA, type: An = "info" } = P;
        !ca(ZA) && yt(Z.value(ZA), An);
      }
    }, It = (P) => {
      V.callType = P, t && r.value && (l.value = "miniMized-mobile-audio", s.value = ""), t && !r.value && (s.value = "mobile-audio");
    }, _e = (P) => {
      vA.isFloatWindow = P, P ? t ? V.callType === _.AUDIO ? s.value = "miniMized-mobile-audio" : l.value = "miniMized-mobile-video" : n.value = "miniMized" : (s.value = "mobile-audio", n.value = "", l.value = "");
    }, pt = (P) => {
      if (P && y.isPC)
        try {
          Ha.show();
        } catch (ZA) {
          console.debug(ZA);
        }
    }, Ct = (P) => {
      V.callRole = P;
    }, Mt = (P) => {
      W.localUserInfoExcludeVolume = P;
    }, ft = (P) => {
      W.remoteUserListExcludeVolume = P;
    }, Nt = (P) => {
      B.callerUserInfo = P;
    }, Lt = (P) => {
      V.isEarPhone = P;
    }, vt = (P) => {
      V.enableVirtualBackground = P;
    }, St = (P) => {
      V.isShowEnableVirtualBackground = P;
    }, Dt = (P) => {
      j.value = P;
    }, ht = (P) => {
      V.isMuteSpeaker = P;
    }, mt = (P) => {
      Z.value = P;
    };
    $A(() => {
      x.setCallback({
        beforeCalling: yA && yA.value,
        afterCalling: TA && TA.value,
        onMinimized: EA && EA.value,
        onMessageSentByMe: ee && ee.value,
        kickedOut: te && te.value,
        statusChanged: X && X.value
      });
    });
    const Fe = {
      [o.CALL_STATUS]: uA,
      [o.IS_GROUP]: wA,
      [o.TOAST_INFO]: gt,
      [o.CALL_MEDIA_TYPE]: It,
      [o.SHOW_PERMISSION_TIP]: pt,
      [o.CALL_ROLE]: Ct,
      [o.LOCAL_USER_INFO_EXCLUDE_VOLUMN]: Mt,
      [o.REMOTE_USER_INFO_EXCLUDE_VOLUMN_LIST]: ft,
      [o.CALLER_USER_INFO]: Nt,
      [o.IS_EAR_PHONE]: Lt,
      [o.ENABLE_VIRTUAL_BACKGROUND]: vt,
      [o.IS_SHOW_ENABLE_VIRTUAL_BACKGROUND]: St,
      [o.CUSTOM_UI_CONFIG]: Dt,
      [o.IS_MUTE_SPEAKER]: ht,
      [o.TRANSLATE]: mt
    };
    sA(() => {
      tA.value && x.enableFloatWindow(tA.value), x.setVideoDisplayMode(pe.value), x.setVideoResolution(Ce.value), d.watch(
        u.CALL,
        Fe,
        {
          notifyRangeWhenWatch: o.MYSELF
        }
      ), d.watch(u.CALL, {
        [o.IS_MINIMIZED]: _e
      });
    });
    function yt(P, ZA) {
      switch (ZA) {
        case "info":
          xA == null || xA.info(P);
          break;
        case "error":
          xA == null || xA.error(P);
          break;
      }
    }
    return aA(async () => {
      d.unwatch(u.CALL, {
        ...Fe,
        [o.IS_MINIMIZED]: _e
      });
    }), rA($t, V), rA(Aa, B), rA(ea, W), rA(na, vA), rA(la, j), rA(ia, _A), { __sfc: !0, isMobile: t, bodyStyle: A, miniMizedDeskStyle: n, mobileAudioStyle: s, mobileVideoStyle: l, isShowFloatWindow: r, props: e, callStatus: i, callRole: c, callType: g, isGroupCall: f, isEarPhone: p, focusElement: C, localUserInfoExcludeVolume: N, remoteUserListExcludeVolume: h, callerUserInfo: L, isFloatWindow: D, enableVirtualBackground: z, isShowEnableVirtualBackground: T, customUIConfigContextValue: j, isMuteSpeaker: F, translate: Z, callInfoContextValue: V, callerUserInfoValue: B, userInfoExcludeVolumeContextValue: W, floatWindowContextValue: vA, translateContextValue: _A, beforeCalling: yA, afterCalling: TA, onMinimized: EA, onMessageSentByMe: ee, videoDisplayMode: pe, videoResolution: Ce, kickedOut: te, statusChanged: X, allowedMinimized: tA, handleCallStatusChange: uA, handleIsGroupChange: wA, handleToastInfoChange: gt, handleCallMediaTypeChange: It, handleIsMinimizedChange: _e, handlePermissionErrorTipChange: pt, handleCallRoleChange: Ct, handleLocalUserInfoChange: Mt, handleRemoteUserInfoListChange: ft, handleCallerUserInfoChange: Nt, handEarPhoneChange: Lt, handleEnableVirtualBackgroundChange: vt, handleIsShowEnableVirtualBackgroundChange: St, handleCustomUIConfigChange: Dt, handleMuteSpeakerChange: ht, handleTranslateChange: mt, watchOptions: Fe, showToast: yt, SingleCall: wd, GroupCall: Lg, CallStatus: w };
  }
});
var Kg = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return A.callInfoContextValue.callStatus !== A.CallStatus.IDLE ? t("div", { class: [A.bodyStyle, A.miniMizedDeskStyle, A.mobileVideoStyle, A.mobileAudioStyle], style: [{ visibility: A.floatWindowContextValue.isFloatWindow ? "hidden" : "" }], attrs: { id: "tuicallkit-id" } }, [A.callInfoContextValue.isGroupCall ? t(A.GroupCall, { staticClass: "singCall" }) : t(A.SingleCall, { staticClass: "singCall" })], 1) : e._e();
}, Hg = [], Jg = /* @__PURE__ */ v(
  Yg,
  Kg,
  Hg,
  !1,
  null,
  null,
  null,
  null
);
const Ve = Jg.exports, $a = "data:image/svg+xml;base64,", $g = /* @__PURE__ */ S({
  __name: "JoinGroupCard",
  props: {
    userListInfo: {
      type: Array,
      default: []
    }
  },
  emits: ["joinGroupCall"],
  setup(a, { emit: e }) {
    const t = I(!1), A = $();
    return { __sfc: !0, emit: e, isShowContent: t, t: A, handleJoinGroup: () => {
      e("joinGroupCall"), t.value = !1;
    }, handleClick: () => {
      t.value = !t.value;
    }, TKImage: hA, CallIcon: $a, defaultAvatarSrc: QA };
  }
});
var AI = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "banner-pc" }, [t("div", { staticClass: "top", on: { click: A.handleClick } }, [t(A.TKImage, { staticClass: "call-icon", attrs: { src: A.CallIcon, width: "16px", height: "16px" } }), t("span", { staticClass: "text" }, [e._v(" " + e._s(e.userListInfo.length) + " " + e._s(A.t("people on the call")) + " ")])], 1), t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isShowContent, expression: "isShowContent" }], staticClass: "content" }, [t("div", { staticClass: "content-arrows" }), t("div", { staticClass: "avatar-box" }, e._l(e.userListInfo, function(n) {
    return t(A.TKImage, { staticClass: "avatar", attrs: { src: n == null ? void 0 : n.avatar, defaultSrc: A.defaultAvatarSrc, fit: "cover", width: "32px", height: "32px" } });
  }), 1), t("div", { staticClass: "btn", on: { click: A.handleJoinGroup } }, [e._v(e._s(A.t("join")))])])]);
}, eI = [], tI = /* @__PURE__ */ v(
  $g,
  AI,
  eI,
  !1,
  null,
  null,
  null,
  null
);
const aI = tI.exports, nI = "data:image/svg+xml;base64,", oI = /* @__PURE__ */ S({
  __name: "JoinGroupCard",
  props: {
    userListInfo: {
      type: Array,
      default: []
    }
  },
  emits: ["joinGroupCall"],
  setup(a, { emit: e }) {
    const t = $(), A = I({ margin: "8px 10px 8px 0px" }), n = I({
      margin: "6px",
      borderRadius: "4px"
    }), s = I(!1);
    return { __sfc: !0, t, CallIconStyle: A, avatarImgStyle: n, isShowContent: s, emit: e, handleJoinGroup: () => {
      e("joinGroupCall"), s.value = !1;
    }, handleClick: () => {
      s.value = !s.value;
    }, TKImage: hA, CallIcon: $a, ArrowsIcon: nI, defaultAvatarSrc: QA };
  }
});
var sI = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { staticClass: "banner-h5" }, [t("div", { staticClass: "top", on: { click: A.handleClick } }, [t("div", { staticClass: "left" }, [t(A.TKImage, { staticClass: "call-icon", attrs: { src: A.CallIcon, width: "16px", height: "16px", customStyle: A.CallIconStyle } }), t("span", { staticClass: "text" }, [e._v(" " + e._s(e.userListInfo.length) + " " + e._s(A.t("people on the call")) + " ")])], 1), t(A.TKImage, { attrs: { src: A.ArrowsIcon, width: "9px", height: "9px" } })], 1), t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isShowContent, expression: "isShowContent" }], staticClass: "content-box" }, [t("div", { staticClass: "content" }, [t("div", { staticClass: "avatar-box" }, e._l(e.userListInfo, function(n) {
    return t(A.TKImage, { attrs: { src: n == null ? void 0 : n.avatar, customStyle: A.avatarImgStyle, defaultSrc: A.defaultAvatarSrc, fit: "cover", width: "50px", height: "50px" } });
  }), 1), t("div", { staticClass: "btn", on: { click: function(n) {
    return n.stopPropagation(), A.handleJoinGroup.apply(null, arguments);
  } } }, [e._v(e._s(A.t("join")))])])])]);
}, lI = [], iI = /* @__PURE__ */ v(
  oI,
  sI,
  lI,
  !1,
  null,
  null,
  null,
  null
);
const uI = iI.exports, rI = /* @__PURE__ */ S({
  __name: "JoinGroupCard",
  setup(a) {
    const e = I(y.isPC), t = I(null), { groupId: A, roomId: n, roomIdType: s, callMediaType: l, groupCallMembers: r, callStatus: i } = b(Co()), c = M(() => A && (i == null ? void 0 : i.value) === w.IDLE && r.value.length);
    return { __sfc: !0, isPC: e, message: t, groupId: A, roomId: n, roomIdType: s, callMediaType: l, groupCallMembers: r, callStatus: i, isShow: c, handleClick: async () => {
      if (r.value.length > 9) {
        t.value.show({
          message: "Supports a maximum of 9 people for simultaneous calls",
          type: "success"
        });
        return;
      }
      const f = {
        type: l.value,
        groupID: A.value,
        roomID: s.value === 1 ? Number(n.value) : 0,
        strRoomID: String(n.value)
      };
      try {
        await x.joinInGroupCall(f);
      } catch (p) {
        console.error(`${o.PREFIX}joinInGroupCall fail: ${p}`);
      }
    }, Message: Wa, JoinGroupCardPC: aI, JoinGroupCardMobile: uI };
  }
});
var cI = function() {
  var e = this, t = e._self._c, A = e._self._setupProxy;
  return t("div", { directives: [{ name: "show", rawName: "v-show", value: A.isShow, expression: "isShow" }] }, [A.isPC ? t(A.JoinGroupCardPC, { attrs: { userListInfo: A.groupCallMembers }, on: { joinGroupCall: A.handleClick } }) : t(A.JoinGroupCardMobile, { attrs: { userListInfo: A.groupCallMembers }, on: { joinGroupCall: A.handleClick } }), t(A.Message, { ref: "message" })], 1);
}, dI = [], gI = /* @__PURE__ */ v(
  rI,
  cI,
  dI,
  !1,
  null,
  null,
  null,
  null
);
const II = gI.exports, pI = (a) => (a.component("TUICallKit", { server: x }), x.bindTUICore(a), Ve), CI = (a) => {
  a.component("TUICallKit", Ve).component("JoinGroupCard", II), console.log("TUICallKit&mini installed", a);
};
Ve.plugin = pI;
Ve.install = CI;
const vI = {
  AUDIO_CALL: 1,
  VIDEO_CALL: 2
}, SI = "4.0.2";
export {
  _ as CallMediaType,
  J as CallRole,
  SA as FeatureButton,
  II as JoinGroupCard,
  Te as LayoutMode,
  o as NAME,
  nA as STATUS,
  u as StoreName,
  Ve as TUICallKit,
  x as TUICallKitServer,
  vI as TUICallType,
  y as TUIGlobal,
  d as TUIStore,
  SI as Version,
  ue as VideoDisplayMode,
  re as VideoResolution,
  CI as install,
  pI as plugin,
  qA as t,
  zA as uiDesign
};
