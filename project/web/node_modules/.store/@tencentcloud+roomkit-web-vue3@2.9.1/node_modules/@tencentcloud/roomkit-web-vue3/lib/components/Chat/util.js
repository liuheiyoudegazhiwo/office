...
function decodeMessageText(payload) {
  const renderDom = [];
    ...
    switch (left) {
      case 0:
        if (right === -1) {
          ...
          const emojiSrc = emojiUrlMapping[emojiKey] ? emojiBaseUrl + emojiUrlMapping[emojiKey] : deprecatedEmojiMap[emojiKey] ? deprecatedEmojiBaseUrl + deprecatedEmojiMap[emojiKey] : null;
          if (emojiSrc) {
...
function decodeSendTextMsg(payload) {
  let temp = payload;
  let left = -1;
  let right = -1;
  let result = "";
...
exports.decodeSendTextMsg = decodeSendTextMsg;
exports.emojiBaseUrl = emojiBaseUrl;
